...
pub(crate) fn bench_display(c: &mut Criterion) {
    let history_commits = env_usize("WORKTREE_BENCH_DISPLAY_HISTORY_COMMITS", 10_000);
    let local_branches = env_usize("WORKTREE_BENCH_DISPLAY_LOCAL_BRANCHES", 100);
    let remote_branches = env_usize("WORKTREE_BENCH_DISPLAY_REMOTE_BRANCHES", 400);
    let diff_lines = env_usize("WORKTREE_BENCH_DISPLAY_DIFF_LINES", 5_000);
    let history_window = env_usize("WORKTREE_BENCH_DISPLAY_HISTORY_WINDOW", 120);
    let diff_window = env_usize("WORKTREE_BENCH_DISPLAY_DIFF_WINDOW", 200);
