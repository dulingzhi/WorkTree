...
pub(crate) fn bench_worktree_preview_render(c: &mut Criterion) {
    let lines = env_usize("WORKTREE_BENCH_WORKTREE_PREVIEW_LINES", 4_000);
    let window = env_usize("WORKTREE_BENCH_WORKTREE_PREVIEW_WINDOW", 200);
    let line_bytes = env_usize("WORKTREE_BENCH_WORKTREE_PREVIEW_LINE_BYTES", 128);
