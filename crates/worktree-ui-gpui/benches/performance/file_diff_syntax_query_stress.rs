...
pub(crate) fn bench_file_diff_syntax_query_stress(c: &mut Criterion) {
    let lines = env_usize("WORKTREE_BENCH_FILE_DIFF_SYNTAX_STRESS_LINES", 256);
    let line_bytes = env_usize("WORKTREE_BENCH_FILE_DIFF_SYNTAX_STRESS_LINE_BYTES", 4_096);
    let nesting_depth = env_usize("WORKTREE_BENCH_FILE_DIFF_SYNTAX_STRESS_NESTING", 128);
    let fixture =
