...
pub(crate) fn bench_keyboard(c: &mut Criterion) {
    let history_commits = env_usize("WORKTREE_BENCH_KEYBOARD_HISTORY_COMMITS", 50_000);
    let history_local_branches = env_usize("WORKTREE_BENCH_KEYBOARD_HISTORY_LOCAL_BRANCHES", 400);
    let history_remote_branches =
        env_usize("WORKTREE_BENCH_KEYBOARD_HISTORY_REMOTE_BRANCHES", 1_200);
    let history_window = env_usize("WORKTREE_BENCH_KEYBOARD_HISTORY_WINDOW", 120);
    let history_scroll_step = env_usize("WORKTREE_BENCH_KEYBOARD_HISTORY_SCROLL_STEP", 1);
    let history_repeat_events = env_usize("WORKTREE_BENCH_KEYBOARD_HISTORY_REPEAT_EVENTS", 240);
    let diff_lines = env_usize("WORKTREE_BENCH_KEYBOARD_DIFF_LINES", 100_000);
    let diff_window = env_usize("WORKTREE_BENCH_KEYBOARD_DIFF_WINDOW", 200);
    let diff_line_bytes = env_usize("WORKTREE_BENCH_KEYBOARD_DIFF_LINE_BYTES", 96);
    let diff_scroll_step = env_usize("WORKTREE_BENCH_KEYBOARD_DIFF_SCROLL_STEP", 1);
    let diff_repeat_events = env_usize("WORKTREE_BENCH_KEYBOARD_DIFF_REPEAT_EVENTS", 240);
    let tab_focus_repo_tabs = env_usize("WORKTREE_BENCH_KEYBOARD_TAB_FOCUS_REPO_TABS", 20);
    let tab_focus_cycle_events = env_usize("WORKTREE_BENCH_KEYBOARD_TAB_FOCUS_CYCLE_EVENTS", 240);
    let stage_toggle_paths = env_usize("WORKTREE_BENCH_KEYBOARD_STAGE_TOGGLE_PATHS", 128);
    let stage_toggle_events = env_usize("WORKTREE_BENCH_KEYBOARD_STAGE_TOGGLE_EVENTS", 240);
    let frame_budget_ns = u64::try_from(env_usize(
        "WORKTREE_BENCH_KEYBOARD_FRAME_BUDGET_NS",
        16_666_667,
    ))
    .unwrap_or(u64::MAX);
