...
pub(crate) fn bench_diff_open_conflict_compare_first_window(c: &mut Criterion) {
    let lines = env_usize("WORKTREE_BENCH_CONFLICT_COMPARE_LINES", 10_000);
    let conflict_blocks = env_usize("WORKTREE_BENCH_CONFLICT_COMPARE_BLOCKS", 300);
    let window = env_usize("WORKTREE_BENCH_CONFLICT_COMPARE_WINDOW", 200);
