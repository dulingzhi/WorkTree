...
pub(crate) fn bench_text_input_long_line_cap(c: &mut Criterion) {
    let long_line_bytes = env_usize("WORKTREE_BENCH_TEXT_INPUT_LONG_LINE_BYTES", 256 * 1024);
    let max_shape_bytes = env_usize("WORKTREE_BENCH_TEXT_INPUT_MAX_LINE_SHAPE_BYTES", 4 * 1024);
