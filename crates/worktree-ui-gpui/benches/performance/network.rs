...
pub(crate) fn bench_network(c: &mut Criterion) {
    let history_commits = env_usize("WORKTREE_BENCH_NETWORK_HISTORY_COMMITS", 50_000);
    let history_local_branches = env_usize("WORKTREE_BENCH_NETWORK_HISTORY_LOCAL_BRANCHES", 400);
    let history_remote_branches =
        env_usize("WORKTREE_BENCH_NETWORK_HISTORY_REMOTE_BRANCHES", 1_200);
    let history_window = env_usize("WORKTREE_BENCH_NETWORK_HISTORY_WINDOW", 120);
    let history_scroll_step = env_usize("WORKTREE_BENCH_NETWORK_HISTORY_SCROLL_STEP", 24);
    let ui_frames = env_usize("WORKTREE_BENCH_NETWORK_UI_FRAMES", 240);
    let progress_updates = env_usize("WORKTREE_BENCH_NETWORK_PROGRESS_UPDATES", 360);
    let cancel_after_updates = env_usize("WORKTREE_BENCH_NETWORK_CANCEL_AFTER_UPDATES", 64);
    let cancel_drain_events = env_usize("WORKTREE_BENCH_NETWORK_CANCEL_DRAIN_EVENTS", 4);
    let cancel_total_updates = env_usize("WORKTREE_BENCH_NETWORK_CANCEL_TOTAL_UPDATES", 160);
    let line_bytes = env_usize("WORKTREE_BENCH_NETWORK_PROGRESS_LINE_BYTES", 72);
    let bar_width = env_usize("WORKTREE_BENCH_NETWORK_BAR_WIDTH", 32);
    let frame_budget_ns = u64::try_from(env_usize(
        "WORKTREE_BENCH_NETWORK_FRAME_BUDGET_NS",
        16_666_667,
    ))
    .unwrap_or(u64::MAX);
