...
pub(crate) fn bench_large_html_syntax(c: &mut Criterion) {
    let fixture_path = env_string("WORKTREE_BENCH_HTML_FIXTURE_PATH");
    let synthetic_lines = env_usize("WORKTREE_BENCH_HTML_LINES", 20_000);
    let synthetic_line_bytes = env_usize("WORKTREE_BENCH_HTML_LINE_BYTES", 192);
    let window_lines = env_usize("WORKTREE_BENCH_HTML_WINDOW_LINES", 160);
