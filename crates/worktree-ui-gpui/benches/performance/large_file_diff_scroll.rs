...
pub(crate) fn bench_large_file_diff_scroll(c: &mut Criterion) {
    let lines = env_usize("WORKTREE_BENCH_DIFF_LINES", 10_000);
    let window = env_usize("WORKTREE_BENCH_DIFF_WINDOW", 200);
    let line_bytes = env_usize("WORKTREE_BENCH_DIFF_LINE_BYTES", 96);
    let long_line_bytes = env_usize("WORKTREE_BENCH_DIFF_LONG_LINE_BYTES", 4_096);
