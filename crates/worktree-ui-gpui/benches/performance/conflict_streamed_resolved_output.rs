...
pub(crate) fn bench_conflict_streamed_resolved_output(c: &mut Criterion) {
    let lines = env_usize("WORKTREE_BENCH_STREAMED_LINES", 50_000);
    let conflict_blocks = env_usize("WORKTREE_BENCH_CONFLICT_BLOCKS", 500);
    let window = env_usize("WORKTREE_BENCH_STREAMED_WINDOW", 200);
