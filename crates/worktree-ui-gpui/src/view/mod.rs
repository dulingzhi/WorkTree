...
#[cfg(test)]
use worktree_core::domain::RepoStatus;
...
pub(crate) const WEBSITE_URL: &str = "https://worktree.dev";
pub(crate) const EDITIONS_URL: &str = "https://worktree.dev/#editions";
pub(crate) const RELEASES_URL: &str = "https://github.com/dulingzhi/WorkTree/releases";
...
impl WorkTreeView {
    ...
    pub fn new(
        ...
        let config = match initial_path {
            Some(path) => WorkTreeViewConfig::normal_with_initial_repository(path, None),
            None => WorkTreeViewConfig::normal(None),
        };
    ...
    pub fn new_with_config(
        store: AppStore,
        events: smol::channel::Receiver<StoreEvent>,
        config: WorkTreeViewConfig,
        window: &mut Window,
        cx: &mut gpui::Context<Self>,
    ) -> Self {
        let WorkTreeViewConfig {
            mut initial_path,
        ...
        let focused_mergetool_bootstrap = if view_mode == WorkTreeViewMode::FocusedMergetool {
            focused_mergetool
                .clone()
        ...
        let main_pane = cx.new(|cx| {
            MainPaneView::new(
                ...
                matches!(
                    ...
                    worktree_state::model::GitLogTagFetchMode::OnRepositoryActivation
                ),
                view_mode,
    ...
    pub(in crate::view) fn set_history_tag_preferences(
        &mut self,
        show_tags: bool,
        tag_fetch_mode: worktree_state::model::GitLogTagFetchMode,
        cx: &mut gpui::Context<Self>,
    ) {
    ...
    fn animate_sidebar_render_width_to(&mut self, target: Pixels, cx: &mut gpui::Context<Self>) {
        ...
        cx.spawn(
            async move |view: WeakEntity<WorkTreeView>, cx: &mut gpui::AsyncApp| loop {
...
impl Render for WorkTreeView {
    fn render(&mut self, window: &mut Window, cx: &mut gpui::Context<Self>) -> impl IntoElement {
        ...
        if let Some(report) = self.startup_crash_report.clone()
            && self.view_mode == WorkTreeViewMode::Normal
        {
