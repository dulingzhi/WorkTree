...
pub(in super::super) struct WorktreeFileListInputs {
    pub(in super::super) files: Vec<worktree_core::domain::CommitFileChange>,
    pub(in super::super) entries: Vec<worktree_state::model::InlineSubmoduleDiffEntry>,
}
...
pub(in super::super) struct DetailsPaneView {
    ...
    pub(in super::super) ai_commit_test_last_context:
        Option<Arc<worktree_state::model::AiCommitContext>>,
...
impl DetailsPaneView {
    ...
    fn spawn_ai_commit_generation(
        &mut self,
        repo_id: RepoId,
        context: Arc<worktree_state::model::AiCommitContext>,
        cx: &mut gpui::Context<Self>,
    ) {
    ...
    pub(in super::super) fn selected_worktree_summary(
        &self,
    ) -> Option<&worktree_core::domain::WorktreeDirtySummary> {
    ...
    pub(in super::super) fn cached_worktree_file_inputs(
        ...
        let entries = worktree_state::model::worktree_inline_diff_entries(summary);
        let files = entries
            .iter()
            .map(|entry| worktree_core::domain::CommitFileChange {
    ...
    pub(in super::super) fn cached_working_tree_review_inputs(
        ...
        let mut push = |list: &Loadable<Arc<Vec<worktree_core::domain::FileStatus>>>,
                        area: DiffArea| {
            let Loadable::Ready(entries) = list else {
                return;
            };
            for file in entries.iter() {
                files.push(worktree_core::domain::CommitFileChange {
