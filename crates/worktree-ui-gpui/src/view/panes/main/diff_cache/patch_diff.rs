...
pub(in crate::view) struct PagedPatchDiffRows {
    diff: Arc<worktree_core::domain::Diff>,
    page_size: usize,
...
impl PagedPatchDiffRows {
    pub(in crate::view) fn new(diff: Arc<worktree_core::domain::Diff>, page_size: usize) -> Self {
    ...
    fn advance_state(
        ...
        line: &worktree_core::domain::DiffLine,
    ) -> DiffLineNumberState {
        match line.kind {
    ...
    fn build_page(&self, page_ix: usize) -> Option<Arc<[AnnotatedDiffLine]>> {
        ...
        for line in &self.diff.lines[start..end] {
            let (old_line, new_line) = match line.kind {
                worktree_core::domain::DiffLineKind::Context => (state.old_line, state.new_line),
                worktree_core::domain::DiffLineKind::Remove => (state.old_line, None),
                worktree_core::domain::DiffLineKind::Add => (None, state.new_line),
                worktree_core::domain::DiffLineKind::Header
                | worktree_core::domain::DiffLineKind::Hunk => (None, None),
            };
...
impl worktree_core::domain::DiffRowProvider for PagedPatchDiffRows {
...
impl PagedPatchSplitRows {
    ...
    #[cfg(test)]
    fn count_rows(lines: &[worktree_core::domain::DiffLine]) -> usize {
    ...
    fn scan_to_block_boundary(
        lines: &[worktree_core::domain::DiffLine],
    ...
    fn flush_pending(state: &mut PatchSplitMaterializationState) {
        ...
        for i in 0..pairs {
            ...
            let kind = match (left.is_some(), right.is_some()) {
                (true, true) => worktree_core::file_diff::FileDiffRowKind::Modify,
                (true, false) => worktree_core::file_diff::FileDiffRowKind::Remove,
                (false, true) => worktree_core::file_diff::FileDiffRowKind::Add,
                (false, false) => worktree_core::file_diff::FileDiffRowKind::Context,
            };
...
impl worktree_core::domain::DiffRowProvider for PagedPatchSplitRows {
...
pub(super) fn should_hide_unified_diff_header_raw(
    ...
    matches!(kind, worktree_core::domain::DiffLineKind::Header)
        && (text.starts_with("index ") || text.starts_with("--- ") || text.starts_with("+++ "))
}

pub(super) fn build_patch_split_visible_meta_from_src(
    line_kinds: &[worktree_core::domain::DiffLineKind],
    visual_line_kinds: &[worktree_core::domain::DiffLineKind],
