...
use worktree_core::domain::{Remote, RemoteBranch};

/// The forge URL shapes WorkTree knows how to generate.
...
/// Parsed web root of a remote, e.g. `https://github.com/dulingzhi/WorkTree`.
...
/// `https://github.com/dulingzhi/WorkTree/commit/<sha>`.
...
/// e.g. `https://github.com/dulingzhi/WorkTree/blob/<ref>/src/main.rs` or
...
mod tests {
    ...
    fn origin_is_preferred_over_other_remotes() {
        let remotes = [
            ...
            remote("origin", "git@github.com:dulingzhi/WorkTree.git"),
        ];
        assert_eq!(
    ...
    fn remote_branch(remote: &str, name: &str) -> RemoteBranch {
        RemoteBranch {
            ...
            name: name.to_string(),
            target: worktree_core::domain::CommitId("abc123".into()),
        }
    }
    ...
    fn commit_permalink_for_ssh_github_remote() {
        let remotes = [remote("origin", "git@github.com:dulingzhi/WorkTree.git")];
        assert_eq!(
            commit_permalink(&remotes, "abc123").as_deref(),
            Some("https://github.com/dulingzhi/WorkTree/commit/abc123")
        );
    }

    #[test]
    fn commit_permalink_for_https_github_remote_without_git_suffix() {
        let remotes = [remote("origin", "https://github.com/dulingzhi/WorkTree")];
        assert_eq!(
            commit_permalink(&remotes, "abc123").as_deref(),
            Some("https://github.com/dulingzhi/WorkTree/commit/abc123")
        );
    ...
    fn file_permalink_encodes_path_and_uses_blob_path() {
        ...
        assert_eq!(
            ...
            Some("https://github.com/dulingzhi/WorkTree/blob/main/src/my%20file%231.rs")
        );
    }
    ...
    fn file_permalink_normalizes_backslashes_to_forward_slashes() {
        let remotes = [remote("origin", "git@github.com:org/repo.git")];
        assert_eq!(
            file_permalink(&remotes, "abc123", r"crates\worktree\src\lib.rs").as_deref(),
            Some("https://github.com/org/repo/blob/abc123/crates/worktree/src/lib.rs")
        );
    }
