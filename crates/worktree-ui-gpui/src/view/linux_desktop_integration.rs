...
impl WorkTreeView {
    ...
    pub(in crate::view) fn maybe_auto_install_linux_desktop_integration(
        ...
        if !should_auto_install_linux_desktop_integration(
            std::env::var_os("WORKTREE_NO_DESKTOP_INSTALL").is_some(),
            desktop.as_deref(),
        ) {
            return;
        }
        ...
        let desktop_path = data_home.join("applications/worktree.desktop");
        let all_icons_exist = ICON_SIZES.iter().all(|size| {
            data_home
                .join(format!("icons/hicolor/{size}x{size}/apps/worktree.png"))
                .exists()
        });
                ...
                let result: Result<(std::path::PathBuf, std::path::PathBuf), String> =
                    smol::unblock(|| {
                        ...
                        const DESKTOP_TEMPLATE: &str = include_str!(concat!(
                            ...
                            "/../../assets/linux/worktree.desktop"
                        ));
                        const ICON_32_PNG: &[u8] = include_bytes!(concat!(
                            env!("CARGO_MANIFEST_DIR"),
                            "/../../assets/linux/hicolor/32x32/apps/worktree.png"
                        ));
                        const ICON_48_PNG: &[u8] = include_bytes!(concat!(
                            env!("CARGO_MANIFEST_DIR"),
                            "/../../assets/linux/hicolor/48x48/apps/worktree.png"
                        ));
                        const ICON_128_PNG: &[u8] = include_bytes!(concat!(
                            env!("CARGO_MANIFEST_DIR"),
                            "/../../assets/linux/hicolor/128x128/apps/worktree.png"
                        ));
                        const ICON_256_PNG: &[u8] = include_bytes!(concat!(
                            env!("CARGO_MANIFEST_DIR"),
                            "/../../assets/linux/hicolor/256x256/apps/worktree.png"
                        ));
                        const ICON_512_PNG: &[u8] = include_bytes!(concat!(
                            env!("CARGO_MANIFEST_DIR"),
                            "/../../assets/linux/hicolor/512x512/apps/worktree.png"
                        ));
                        ...
                        let desktop_path = applications_dir.join("worktree.desktop");
                        let icon_path = icons_root.join("512x512/apps/worktree.png");
                        ...
                        for (size, icon_bytes) in ICON_ASSETS {
                            ...
                            let icon_file = icon_dir.join("worktree.png");
...
mod tests {
    ...
    fn desktop_exec_path_is_quoted() {
        let escaped =
            desktop_entry_exec_path_arg(Path::new("/usr/local/bin/worktree")).expect("path");
        assert_eq!(escaped, "\"/usr/local/bin/worktree\"");
    }
