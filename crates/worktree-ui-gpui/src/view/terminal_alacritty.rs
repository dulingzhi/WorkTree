...
pub(super) type AlacrittyTermLock = Arc<FairMutex<Term<WorkTreeListener>>>;
...
#[derive(Clone)]
pub(super) struct WorkTreeListener {
    pub(super) events_tx: smol::channel::Sender<TerminalBackendEvent>,
}

impl EventListener for WorkTreeListener {
...
fn spawn_alacritty_terminal_inner(
    ...
    let env: Vec<(String, String)> = vec![
        ...
        ("TERM_PROGRAM".to_string(), "WorkTree".to_string()),
        (
...
pub(super) fn new_term(
    ...
    let term = Term::new(config.clone(), bounds, WorkTreeListener { events_tx });
...
pub(super) fn terminal_live_modes(term: &Term<WorkTreeListener>) -> TerminalModes {
    TerminalModes::from_term_mode(*term.mode())
}

pub(super) fn make_terminal_content(term: &Term<WorkTreeListener>) -> TerminalContent {
...
mod tests {
    ...
    fn build_row_accumulates_text_for_same_style_cells() {
        ...
        let (text, runs, _bg_rects) =
            build_alacritty_row(&cells, 0, 5, &base, AppTheme::worktree_dark());
    ...
    fn build_row_handles_wide_char_spacer() {
        ...
        let (text, _runs, _bg_rects) =
            build_alacritty_row(&cells, 0, 3, &base, AppTheme::worktree_dark());
    ...
    fn build_row_keeps_reading_cells_after_a_wide_char() {
        ...
        let (text, _runs, _bg_rects) =
            build_alacritty_row(&cells, 0, 4, &base, AppTheme::worktree_dark());
    ...
    fn build_row_does_not_reserve_per_column_for_runs_and_backgrounds() {
        ...
        let (_text, runs, backgrounds) =
            build_alacritty_row(&cells, 0, cols, &base, AppTheme::worktree_dark());
    ...
    fn color_to_rgba_resolves_named_ansi_colors() {
        let palette = TerminalAnsiPalette::from_theme(AppTheme::worktree_dark());
    ...
    fn terminal_palette_dark_vs_light_differs() {
        let dark = TerminalAnsiPalette::from_theme(AppTheme::worktree_dark());
        let light = TerminalAnsiPalette::from_theme(AppTheme::worktree_light());
        assert_ne!(
    ...
    fn build_row_handles_zero_width_chars() {
        ...
        let (text, runs, _bg_rects) =
            build_alacritty_row(&[cell], 0, 2, &base, AppTheme::worktree_dark());
    ...
    fn build_row_with_negative_grid_row() {
        ...
        let (text, runs, _bg) =
            build_alacritty_row(&cells, -3, 5, &base, AppTheme::worktree_dark());
        assert_eq!(
            text, "S",
    ...
    fn build_row_empty_when_no_cells_at_row() {
        ...
        let (text, runs, _bg) = build_alacritty_row(&cells, 5, 3, &base, AppTheme::worktree_dark());
