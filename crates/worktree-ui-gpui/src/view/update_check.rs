...
const UPDATE_CHECK_DISABLE_ENV: &str = "WORKTREE_NO_UPDATE_CHECK";
#[cfg(not(test))]
const UPDATE_CHECK_REPO_ENV: &str = "WORKTREE_UPDATE_REPO";
#[cfg(not(test))]
const DEFAULT_UPDATE_REPO: &str = "dulingzhi/WorkTree";
...
impl WorkTreeView {
    pub(in crate::view) fn maybe_check_for_updates_on_startup(
        ...
        if self.view_mode != WorkTreeViewMode::Normal
            ...
            async move |view: WeakEntity<WorkTreeView>, cx: &mut gpui::AsyncApp| {
                ...
                let _ = view.update(cx, |this, cx| {
                    this.push_toast_with_link(
                        ...
                        format!(
                            "A newer WorkTree version is available: {} (current {}).",
...
mod tests {
    ...
    fn build_update_notice_returns_none_when_release_is_not_newer() {
        let repo = GitHubRepo::from_slug("dulingzhi/WorkTree");
    ...
    fn parse_github_repo_from_url_supports_https_and_ssh_forms() {
        assert_eq!(
            parse_github_repo_from_url("https://github.com/dulingzhi/WorkTree.git"),
            Some(GitHubRepo {
                owner: "dulingzhi".to_string(),
                repo: "WorkTree".to_string(),
            })
        );
        assert_eq!(
            parse_github_repo_from_url("git@github.com:dulingzhi/WorkTree.git"),
            Some(GitHubRepo {
                owner: "dulingzhi".to_string(),
                repo: "WorkTree".to_string(),
            })
