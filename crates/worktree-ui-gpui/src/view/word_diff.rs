...
pub(super) fn capped_word_diff_ranges_for_file_diff_texts(
    old: &worktree_core::file_diff::FileDiffLineText,
    new: &worktree_core::file_diff::FileDiffLineText,
) -> (Vec<Range<usize>>, Vec<Range<usize>>) {
...
mod tests {
    ...
    fn capped_word_diff_ranges_for_file_diff_texts_checks_len_before_loading() {
        let old = worktree_core::file_diff::FileDiffLineText::file_slice(
            std::sync::Arc::new(std::path::PathBuf::from("/definitely/missing/worktree-old")),
            0..WORD_DIFF_MAX_BYTES_PER_SIDE.saturating_add(1),
            true,
            false,
        );
        let new = worktree_core::file_diff::FileDiffLineText::from("small");
