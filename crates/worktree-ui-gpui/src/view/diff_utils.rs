...
pub(super) fn diff_content_line_text(
    ...
    let content_start = match line.kind {
        worktree_core::domain::DiffLineKind::Add
        | worktree_core::domain::DiffLineKind::Remove
        | worktree_core::domain::DiffLineKind::Context => 1,
        worktree_core::domain::DiffLineKind::Header | worktree_core::domain::DiffLineKind::Hunk => {
            0
        }
    };
...
pub(super) fn compute_diff_file_stats(
    ...
    for (ix, line) in diff.iter().enumerate() {
        let is_file_header = matches!(line.kind(), worktree_core::domain::DiffLineKind::Header)
        ...
        match line.kind() {
            worktree_core::domain::DiffLineKind::Add => adds += 1,
            worktree_core::domain::DiffLineKind::Remove => removes += 1,
            _ => {}
        }
    }
...
pub(super) fn compute_diff_file_for_src_ix(diff: &[impl UnifiedDiffLine]) -> Vec<Option<Arc<str>>> {
    ...
    for (ix, line) in diff.iter().enumerate() {
        ...
        if is_file_header {
            ...
            current_file =
                worktree_core::diff::unified_diff_file_path(diff[ix..].iter().map(|l| l.text()))
                    .map(Arc::<str>::from);
        }
...
fn diff_line_content_text(kind: worktree_core::domain::DiffLineKind, text: &str) -> &str {
    match kind {
        worktree_core::domain::DiffLineKind::Add => text.strip_prefix('+').unwrap_or(text),
        worktree_core::domain::DiffLineKind::Remove => text.strip_prefix('-').unwrap_or(text),
        worktree_core::domain::DiffLineKind::Context => text.strip_prefix(' ').unwrap_or(text),
        worktree_core::domain::DiffLineKind::Header | worktree_core::domain::DiffLineKind::Hunk => {
            text
        }
    }
...
pub(super) trait UnifiedDiffLine {
    fn kind(&self) -> worktree_core::domain::DiffLineKind;
    fn text(&self) -> &str;
}
...
impl UnifiedDiffLine for worktree_core::domain::DiffLine {
    fn kind(&self) -> worktree_core::domain::DiffLineKind {
        self.kind
    }
...
fn unified_patch_file_and_hunk_bounds<T: UnifiedDiffLine>(
    ...
    if !matches!(hunk.kind(), worktree_core::domain::DiffLineKind::Hunk) {
        return None;
    }
    ...
    let first_hunk = (file_start + 1..lines.len())
        .find(|&ix| {
            ...
            matches!(line.kind(), worktree_core::domain::DiffLineKind::Hunk)
                || line.text().starts_with("diff --git ")
        })
        .unwrap_or(lines.len());
