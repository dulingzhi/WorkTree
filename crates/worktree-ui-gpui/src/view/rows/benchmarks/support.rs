...
pub(crate) fn hash_file_diff_plan(plan: &worktree_core::file_diff::FileDiffPlan) -> u64 {
    let mut h = FxHasher::default();
    plan.row_count.hash(&mut h);
    plan.inline_row_count.hash(&mut h);
    match plan.eof_newline {
        Some(worktree_core::file_diff::FileDiffEofNewline::MissingInOld) => 1u8,
        Some(worktree_core::file_diff::FileDiffEofNewline::MissingInNew) => 2u8,
        None => 0u8,
    }
    .hash(&mut h);
