...
pub struct ConflictTwoWaySplitScrollFixture {
    diff_rows: Vec<worktree_core::file_diff::FileDiffRow>,
...
pub struct ConflictTwoWayDiffBuildFixture {
    ...
    ours_text: String,
    theirs_text: String,
    full_diff_rows: Vec<worktree_core::file_diff::FileDiffRow>,
    block_local_diff_rows: Vec<worktree_core::file_diff::FileDiffRow>,
    conflict_count: usize,
}
...
impl ConflictTwoWayDiffBuildFixture {
    pub fn new(lines: usize, conflict_blocks: usize) -> Self {
        ...
        let full_diff_rows = worktree_core::file_diff::side_by_side_rows(&ours_text, &theirs_text);
    ...
    pub fn run_full_diff_build_step(&self) -> u64 {
        let diff_rows =
            worktree_core::file_diff::side_by_side_rows(&self.ours_text, &self.theirs_text);
...
fn hash_file_diff_rows(rows: &[worktree_core::file_diff::FileDiffRow]) -> u64 {
    let mut h = FxHasher::default();
    rows.len().hash(&mut h);
