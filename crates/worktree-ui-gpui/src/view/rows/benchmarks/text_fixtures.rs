...
impl UndoRedoFixture {
    ...
    pub fn run_with_state(&self, state: &mut AppState) -> (u64, UndoRedoMetrics) {
        ...
        let choices = [
            worktree_state::msg::ConflictRegionChoice::Ours,
            worktree_state::msg::ConflictRegionChoice::Theirs,
            worktree_state::msg::ConflictRegionChoice::Both,
            worktree_state::msg::ConflictRegionChoice::Base,
        ];
...
impl ReplacementAlignmentFixture {
    ...
    pub fn run_plan_step(&self) -> u64 {
        let plan = worktree_core::file_diff::side_by_side_plan(&self.old_text, &self.new_text);
        hash_file_diff_plan(&plan)
    }

    pub fn run_plan_step_with_backend(
        &self,
        backend: worktree_core::file_diff::BenchmarkReplacementDistanceBackend,
    ) -> u64 {
        let plan = worktree_core::file_diff::benchmark_side_by_side_plan_with_replacement_backend(
            &self.old_text,
            &self.new_text,
            backend,
        );
