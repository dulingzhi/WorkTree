...
mod tests {
    ...
    fn runs_are_sorted_disjoint_and_clipped() {
        ...
        let snapshot = doc.snapshot(AppTheme::worktree_dark());
    ...
    fn masked_placeholder_does_not_poison_following_lines() {
        ...
        let masked = masked.snapshot(AppTheme::worktree_dark());
        let spaced = document(with_spaces, Vec::new());
        let spaced = spaced.snapshot(AppTheme::worktree_dark());
    ...
    fn unmasked_placeholder_is_what_masking_protects_against() {
        ...
        let text = "fn a() {}\n<Merge Conflict>\nfn b() -> u32 { 7 }\n";
        let masked =
            document(text, Vec::from([10..26usize; 1])).snapshot(AppTheme::worktree_dark());
        let unmasked = document(text, Vec::new()).snapshot(AppTheme::worktree_dark());
    ...
    fn edit_keeps_the_document_exact() {
        ...
        let incremental = doc.snapshot(AppTheme::worktree_dark());
        let scratch = document(after, Vec::new()).snapshot(AppTheme::worktree_dark());
        assert_eq!(
    ...
    fn an_edit_that_outruns_the_budget_defers_and_the_background_pass_catches_up() {
        ...
        let deferred = doc.snapshot(AppTheme::worktree_dark());
        assert!(
        ...
        let caught_up = doc.snapshot(AppTheme::worktree_dark());
        let scratch = document(&after, Vec::new()).snapshot(AppTheme::worktree_dark());
        assert_eq!(
    ...
    fn a_wholesale_replacement_that_outruns_the_budget_is_abandoned() {
        ...
        assert!(
            doc.snapshot(AppTheme::worktree_dark())
                .0
                .tree
                .root_node()
                .end_byte()
    ...
    fn a_background_tree_for_a_superseded_version_is_rejected() {
        ...
        let tree = doc.snapshot(AppTheme::worktree_dark()).0.tree.clone();
    ...
    fn wholesale_replacement_reparses_from_scratch() {
        ...
        let replaced = doc.snapshot(AppTheme::worktree_dark());
        let scratch = document(after, Vec::new()).snapshot(AppTheme::worktree_dark());
        assert_eq!(
    ...
    fn the_two_engines_agree_on_toml_keys() {
        ...
        assert_engines_agree(
            ...
            &[
                ...
                ("\"worktree\"", SyntaxTokenKind::String),
    ...
    fn bracket_pair_text<'a>(
        ...
        document
            .snapshot(AppTheme::worktree_dark())
...
mod injection_tests {
    ...
    fn combined_layer_captures_are_clipped_to_its_ranges() {
        ...
        let snapshot = document.snapshot(AppTheme::worktree_dark());
        let highlights = snapshot.highlights_for_byte_range(0..text.len());
