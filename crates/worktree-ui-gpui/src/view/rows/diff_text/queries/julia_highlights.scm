; Verbatim copy of `queries/highlights.scm` from tree-sitter-julia 0.23.1 (MIT),
; vendored because the crate ships the file but exports no query constant.
;
; No local changes. If the grammar is bumped, re-copy rather than hand-edit: the
; only thing holding this file to the grammar is the
; `assert_capture_names_are_supported` call for Julia in
; `vendored_capture_names_are_supported_or_ignored`, which checks that it compiles
; and that every capture name resolves -- not that it is current.
