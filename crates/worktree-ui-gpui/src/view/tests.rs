...
fn sync_view_snapshot(cx: &mut gpui::VisualTestContext, view: &gpui::Entity<WorkTreeView>) {
    cx.update(|_window, app| {
        view.update(app, |this, cx| test_support::sync_store_snapshot(this, cx));
    });
    test_support::redraw(cx);
...
fn startup_crash_report_is_visible_after_relaunch(cx: &mut gpui::TestAppContext) {
    ...
    let config = WorkTreeViewConfig::normal(Some(StartupCrashReport {
        issue_url: "https://example.invalid/crash-report".to_string(),
        summary: "WSLg clipboard copy terminated unexpectedly".to_string(),
        crash_log_path: PathBuf::from("/tmp/worktree-crash.log"),
    }));
    let (view, cx) = cx.add_window_view(|window, cx| {
        WorkTreeView::new_with_config(store, events, config, window, cx)
    });
...
fn command_palette_stage_all_asks_before_staging_unresolved_conflicts(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_command_stage_all_conflict",
        std::process::id()
    ));
...
fn remote_rows_groups_and_sorts() {
    ...
    let rows = WorkTreeView::remote_rows(&repo);
    assert_eq!(
        rows,
        vec![
...
fn remote_headers_include_remotes_with_no_branches() {
    ...
    let rows = WorkTreeView::branch_sidebar_rows(&repo);
...
fn worktree_tooltip_includes_branch_name() {
    ...
    let rows = WorkTreeView::branch_sidebar_rows_with_collapsed(&repo, &[expanded_key.as_str()]);
    let row = rows
        .iter()
...
fn branch_sidebar_lists_tags_below_stash_newest_first() {
    ...
    let rows = WorkTreeView::branch_sidebar_rows_with_collapsed(&repo, &[expanded.as_str()]);
...
fn branch_sidebar_collapses_branch_sections_without_hiding_other_sections() {
    ...
    let rows = WorkTreeView::branch_sidebar_rows_with_collapsed(
        &repo,
        &[
...
fn branch_sidebar_collapses_local_branch_groups() {
    ...
    let rows =
        WorkTreeView::branch_sidebar_rows_with_collapsed(&repo, &[feature_group_key.as_str()]);
...
fn branch_sidebar_collapses_remote_section_and_remote_groups() {
    ...
    let rows = WorkTreeView::branch_sidebar_rows_with_collapsed(&repo, &[origin_key.as_str()]);
    assert!(rows.iter().any(|row| {
        matches!(
            row,
...
fn focused_mergetool_mode_hides_full_chrome() {
    assert!(renders_full_chrome(WorkTreeViewMode::Normal));
    assert!(!renders_full_chrome(WorkTreeViewMode::FocusedMergetool));
}
...
fn state_with_active_diff(path: &str, kind: FileStatusKind) -> AppState {
    ...
    repo.worktree_status = Loadable::Ready(Arc::new(vec![FileStatus {
        ...
        conflict: (kind == FileStatusKind::Conflicted)
            .then_some(worktree_core::domain::FileConflictKind::BothModified),
    }]));
...
fn merge_view_temporarily_collapses_and_restores_sidebar(cx: &mut gpui::TestAppContext) {
    let (store, events) = AppStore::new(Arc::new(TestBackend));
    let (view, cx) =
        cx.add_window_view(|window, cx| WorkTreeView::new(store.clone(), events, None, window, cx));
...
fn focused_mergetool_keeps_titlebar_actions_without_repo_tabs_or_command_palette() {
    ...
    assert!(!show_titlebar_repo_tabs(WorkTreeViewMode::FocusedMergetool));
    assert!(!command_palette_available(
        WorkTreeViewMode::FocusedMergetool
    ));

    assert!(show_titlebar_repo_tabs(WorkTreeViewMode::Normal));
    assert!(command_palette_available(WorkTreeViewMode::Normal));
}
...
fn quit_or_warn_stores_other_window_views(cx: &mut gpui::TestAppContext) {
    ...
    let fake_views: Vec<gpui::WeakEntity<WorkTreeView>> = vec![
...
fn confirm_quit_app_terminates_other_window_terminals(cx: &mut gpui::TestAppContext) {
    ...
    let fake_views: Vec<gpui::WeakEntity<WorkTreeView>> = vec![gpui::WeakEntity::new_invalid()];
...
fn generic_error_banner_is_hidden_when_auth_prompt_is_active() {
    assert!(WorkTreeView::should_render_generic_error_banner(false));
    assert!(!WorkTreeView::should_render_generic_error_banner(true));
}

#[test]
fn error_banner_overflow_hint_is_hidden_for_short_errors() {
    assert!(!WorkTreeView::should_show_error_banner_overflow_hint(
...
fn error_banner_overflow_hint_is_shown_for_long_command_failures() {
    ...
    .join("\n");
    assert!(WorkTreeView::should_show_error_banner_overflow_hint(&error));
}

#[test]
fn auth_prompt_banner_colors_use_accent_palette() {
    let theme = AppTheme::worktree_light();
    let (bg, border) = WorkTreeView::auth_prompt_banner_colors(theme);
...
fn apply_state_snapshot_routes_command_errors_into_store_backed_banner(
    ...
    repo.command_log
        .push(worktree_state::model::CommandLogEntry {
...
fn apply_state_snapshot_routes_clone_progress_errors_into_global_banner(
    ...
    next.clone = Some(worktree_state::model::CloneOpState {
        url: Arc::<str>::from("git@github.com:private/repo.git"),
        dest: Arc::new(PathBuf::from("/tmp/private-repo")),
        status: worktree_state::model::CloneOpStatus::FinishedErr(
...
fn pane_collapse_ease_is_a_well_formed_easing_curve() {
    ...
    assert_eq!(WorkTreeView::pane_collapse_ease(0.0), 0.0);
    assert_eq!(WorkTreeView::pane_collapse_ease(1.0), 1.0);

    // Out-of-range inputs clamp to the endpoints.
    assert_eq!(WorkTreeView::pane_collapse_ease(-0.5), 0.0);
    assert_eq!(WorkTreeView::pane_collapse_ease(1.5), 1.0);

    // Monotonically non-decreasing across the domain.
    let mut prev = 0.0;
    for i in 0..=100 {
        let t = i as f32 / 100.0;
        let y = WorkTreeView::pane_collapse_ease(t);
        assert!(
            y >= prev - 1e-4,
    ...
    assert!(WorkTreeView::pane_collapse_ease(0.5) > 0.5);
}
...
fn cubic_bezier_matches_a_linear_curve_for_the_identity_control_points() {
    ...
    for i in 0..=20 {
        let t = i as f32 / 20.0;
        let y = WorkTreeView::cubic_bezier(1.0 / 3.0, 1.0 / 3.0, 2.0 / 3.0, 2.0 / 3.0, t);
...
fn locate_open_file_switches_to_files_and_expands_its_folders(cx: &mut gpui::TestAppContext) {
    ...
    let mut state = view_state_with_active_ready_repo(RepoId(1));
    state.sidebar_mode = worktree_state::model::SidebarMode::Branches;
    ...
    state.repos[0].file_browser.bump_rev();
    state.repos[0].diff_state.diff_target = Some(worktree_core::domain::DiffTarget::WorkingTree {
        path: nested.clone(),
    ...
    cx.update(|_window, app| {
        ...
        assert_eq!(
            state.sidebar_mode,
            worktree_state::model::SidebarMode::Files,
...
fn the_locate_button_is_present_whenever_the_files_tab_is(cx: &mut gpui::TestAppContext) {
    ...
    state.sidebar_mode = worktree_state::model::SidebarMode::Files;
    store.replace_snapshot_for_test(Arc::new(state.clone()));
    sync_view_snapshot(cx, &view);
    assert!(
...
fn sidebar_worktree_badges_share_one_right_edge_near_the_pane_edge(cx: &mut gpui::TestAppContext) {
    ...
    let mut state = view_state_with_active_ready_repo(RepoId(1));
    let branch = |name: &str| worktree_core::domain::Branch {
        name: name.to_string(),
        target: CommitId("1111111111111111".into()),
        upstream: None,
        divergence: None,
    };
    let worktree = |path: &str, branch: &str| worktree_core::domain::Worktree {
...
fn right_clicking_a_branch_group_row_opens_the_group_context_menu(cx: &mut gpui::TestAppContext) {
    ...
    let branch = |name: &str| worktree_core::domain::Branch {
        name: name.to_string(),
        target: worktree_core::domain::CommitId("aaaaaaaaaaaa".into()),
        upstream: None,
        divergence: None,
    };
