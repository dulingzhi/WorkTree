...
use worktree_core::domain::{
    CommitId, PullRequest, PullRequestChecksState, PullRequestState, Remote,
};
use worktree_core::error::{Error, ErrorKind};

use super::permalink::{ForgeKind, origin_remote, parse_remote_url};
use crate::ai_commit_sources::{EnvAccess, read_gh_token};
use crate::http;
...
/// Pull requests per load. Fifty comfortably covers what an interactive
/// client cares about while keeping the response small. The listing covers
/// every state — open, merged, closed — so a repository whose PRs have all
/// settled still lists them instead of presenting an empty section.
...
pub(super) fn pulls_api_url(slug: &RepoSlug) -> String {
    format!(
        "https://api.github.com/repos/{}/{}/pulls?state=all&per_page={PULL_REQUESTS_PER_PAGE}",
...
/// List the repository's pull requests — open, merged, and closed. Runs from
/// UI-driven async context (the sidebar pane spawns it); results return to
...
pub(super) async fn fetch_pull_requests(slug: &RepoSlug) -> Result<Vec<PullRequest>, Error> {
    ...
    .await
    .map_err(|err| {
        worktree_core::applog_warn!("github api: pulls request failed: {err}");
        Error::new(ErrorKind::Backend(err.to_string()))
    })?;
    let status = response.status.into();
    let pull_requests = parse_pull_requests(status, &response.body).map_err(|err| {
        worktree_core::applog_warn!("github api: pulls parse failed (HTTP {status}): {err}");
        Error::new(ErrorKind::Backend(err))
    })?;
    worktree_core::applog_info!(
        "github api: pulls for {}/{} returned {} pull requests (HTTP {status})",
        slug.owner,
        slug.repo,
        pull_requests.len()
    );
...
pub(super) async fn fetch_pull_request_checks(
    ...
    .await
    .map_err(|err| {
        worktree_core::applog_warn!("github api: status request failed for {sha}: {err}");
        Error::new(ErrorKind::Backend(err.to_string()))
    })?;
    parse_combined_status(response.status.into(), &response.body).map_err(|err| {
        worktree_core::applog_warn!("github api: status parse failed for {sha}: {err}");
        Error::new(ErrorKind::Backend(err))
    })
}

/// Parse a PR listing. Entries the API shape no longer matches (no number)
/// are dropped rather than failing the whole list; `head` can be `null` when
/// the fork was deleted, which leaves empty head fields — the row still
/// lists, and checkout re-resolves the ref server-side anyway.
pub(super) fn parse_pull_requests(status: u16, body: &[u8]) -> Result<Vec<PullRequest>, String> {
...
fn pull_request_from_json(entry: &Value) -> Option<PullRequest> {
    ...
    Some(PullRequest {
        ...
        state: pull_request_state_from_json(entry),
        draft: entry.get("draft").and_then(Value::as_bool).unwrap_or(false),
        checks: None,
    })
}

/// `state` is "open" or "closed"; a `closed` PR with a non-null `merged_at`
/// is merged. A missing state reads as open so the row still lists.
fn pull_request_state_from_json(entry: &Value) -> PullRequestState {
...
mod tests {
    ...
    fn sample_pulls_body() -> String {
        r#"[
            ...
            {
                "number": 5,
                "title": "Landed: faster graph",
                "user": {"login": "jai"},
                "head": {"ref": "fast-graph", "sha": "bb2222222222222222222222222222222222222222"},
                "base": {"ref": "main"},
                "state": "closed",
            ...
            },
            {
                ...
                "title": "Dropped idea",
                "user": {"login": "mira"},
                "head": {"ref": "idea", "sha": "cc3333333333333333333333333333333333333333"},
                "base": {"ref": "main"},
                "state": "closed",
                "merged_at": null
            }
    ...
    fn slug_resolves_from_https_and_ssh_github_remotes() {
        let https = github_slug_from_remotes(&[remote(
            "origin",
            Some("https://github.com/dulingzhi/WorkTree.git"),
        )]);
        assert_eq!(
            https,
            Some(RepoSlug {
        ...
        let ssh =
            github_slug_from_remotes(&[remote("origin", Some("git@github.com:acme/widgets.git"))]);
        assert_eq!(
            ssh,
            Some(RepoSlug {
                owner: "acme".into(),
    ...
    fn slug_rejects_other_forges_and_missing_urls() {
        assert!(
            github_slug_from_remotes(&[remote(
                "origin",
    ...
    fn urls_and_headers_follow_the_documented_shapes() {
        ...
        assert_eq!(
            pulls_api_url(&slug),
            "https://api.github.com/repos/acme/widgets/pulls?state=all&per_page=50"
        );
        assert_eq!(
        ...
        assert_eq!(authorized[2], ("Authorization", "Bearer tok".to_string()));
    }
    ...
    fn pull_list_parses_entries_and_tolerates_a_deleted_head() {
        ...
        // A closed PR with a merge timestamp is merged; without one, closed.
        assert_eq!(pulls[2].state, PullRequestState::Merged);
        assert_eq!(pulls[3].state, PullRequestState::Closed);
    }

    #[test]
    fn a_missing_state_reads_as_open_so_the_row_still_lists() {
        let body = r#"[{"number": 12, "title": "Legacy shape", "user": {"login": "jai"}}]"#;
        let pulls = parse_pull_requests(200, body.as_bytes()).unwrap();
    ...
    fn combined_status_maps_states_and_treats_zero_statuses_as_absent() {
        let state =
            |total: u64, state: &str| format!(r#"{{"total_count": {total}, "state": "{state}"}}"#);
        assert_eq!(
