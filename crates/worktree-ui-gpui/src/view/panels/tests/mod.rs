...
fn shows_external_mergetool_actions_only_in_normal_mode() {
    assert!(show_external_mergetool_actions(WorkTreeViewMode::Normal));
...
fn shows_save_stage_action_only_in_normal_mode() {
    assert!(show_conflict_save_stage_action(WorkTreeViewMode::Normal));
...
fn shows_diff_file_navigation_only_in_normal_mode() {
    assert!(show_diff_file_navigation(WorkTreeViewMode::Normal));
...
fn conflict_side_output_bytes_prefers_original_bytes_and_falls_back_to_text() {
    let file = worktree_state::model::ConflictFile {
...
fn conflict_resolver_strategy_maps_conflict_kinds() {
    use worktree_core::conflict_session::ConflictResolverStrategy as S;
    use worktree_core::domain::FileConflictKind as K;
...
pub(super) fn seed_file_image_diff_state_with_rev(
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.diff_state.diff_file_rev = diff_file_rev;
            repo.diff_state.diff_file_image = worktree_state::model::Loadable::Ready(Some(
                Arc::new(worktree_core::domain::FileDiffImage {
...
pub(super) fn conflict_compare_repo_state(
    ...
    set_test_file_status(
        ...
        worktree_core::domain::FileStatusKind::Conflicted,
        worktree_core::domain::DiffArea::Unstaged,
    );
...
pub(super) fn assert_file_preview_ctrl_a_ctrl_c_copies_all(
    ...
    status_kind: worktree_core::domain::FileStatusKind,
    lines: Arc<Vec<String>>,
) {
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            if let Some(source_path) = deleted_preview_source_path.as_ref() {
                repo.diff_state.diff_preview_text_file = worktree_state::model::Loadable::Ready(
                    Some(Arc::new(worktree_core::domain::DiffPreviewTextFile {
        ...
        view.update(app, |this, cx| {
            ...
            if let Some(side) = preview_side {
                let preview_source_path = workdir.join(format!(
                    ...
                    repo_id.0,
                    match side {
                        worktree_core::domain::DiffPreviewTextSide::New => "new",
                        worktree_core::domain::DiffPreviewTextSide::Old => "old",
                    }
                ));
                let contents = match side {
                    worktree_core::domain::DiffPreviewTextSide::New => new_text.unwrap_or_default(),
                    worktree_core::domain::DiffPreviewTextSide::Old => old_text.unwrap_or_default(),
                };
...
pub(super) fn app_state_with_repo(
    repo: worktree_state::model::RepoState,
    repo_id: worktree_state::model::RepoId,
) -> Arc<AppState> {
    Arc::new(AppState {
        repos: vec![repo],
...
pub(super) fn opening_repo_state(
    ...
    worktree_state::model::RepoState::new_opening(
        repo_id,
...
pub(super) fn set_test_file_status(
    repo: &mut worktree_state::model::RepoState,
...
pub(super) fn set_test_file_status_with_conflict(
    ...
    path: impl Into<std::path::PathBuf>,
    kind: worktree_core::domain::FileStatusKind,
    conflict: Option<worktree_core::domain::FileConflictKind>,
    area: worktree_core::domain::DiffArea,
) {
    let path = path.into();
    let file_status = worktree_core::domain::FileStatus {
        path: path.clone(),
        kind,
        conflict,
    };
    let (staged, unstaged) = match area {
        worktree_core::domain::DiffArea::Staged => (vec![file_status], vec![]),
        worktree_core::domain::DiffArea::Unstaged => (vec![], vec![file_status]),
    };
    repo.status = worktree_state::model::Loadable::Ready(
        worktree_core::domain::RepoStatus { staged, unstaged }.into(),
    );
    ...
    repo.diff_state.diff_target =
        Some(worktree_core::domain::DiffTarget::WorkingTree { path, area });
    repo.diff_state.diff_state_rev = repo.diff_state.diff_state_rev.wrapping_add(1);
}
