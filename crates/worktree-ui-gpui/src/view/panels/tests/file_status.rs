...
fn patch_diff_search_query_keeps_stable_style_cache_entries(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(22);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_patch_search",
        std::process::id()
    ));
...
fn worktree_preview_search_query_clears_row_cache_without_dropping_source_path(
    ...
    let repo_id = worktree_state::model::RepoId(23);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_preview_search",
        std::process::id()
    ));
...
fn worktree_preview_identical_refresh_preserves_row_cache(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(24);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_preview_refresh_preserves_cache",
        std::process::id()
    ));
...
fn staged_deleted_file_preview_uses_old_contents(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(3);
    let workdir =
        std::env::temp_dir().join(format!("worktree_ui_test_{}_deleted", std::process::id()));
        ...
        |pane| {
            pane.worktree_preview_path.as_ref() == Some(&workdir.join(&file_rel))
                ...
                && matches!(
                    ...
                    worktree_state::model::Loadable::Ready(3)
                )
...
fn committed_deleted_file_preview_uses_preview_text_file_without_patch_fallback(
    ...
    let repo_id = worktree_state::model::RepoId(303);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_committed_deleted",
        std::process::id()
    ));
    ...
    wait_for_main_pane_condition(
        ...
        |pane| {
            pane.worktree_preview_path.as_ref() == Some(&workdir.join(&file_rel))
                ...
                && matches!(
                    pane.worktree_preview,
                    worktree_state::model::Loadable::Ready(2)
                )
...
fn untracked_markdown_file_preview_defaults_to_preview_mode_and_renders_container(
    ...
    let repo_id = worktree_state::model::RepoId(59);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_markdown_untracked_default_preview",
        std::process::id()
    ));
...
fn staged_added_markdown_file_preview_shows_preview_text_toggle(cx: &mut gpui::TestAppContext) {
    let repo_id = worktree_state::model::RepoId(57);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_markdown_added_toggle",
        std::process::id()
    ));
    let file_rel = std::path::PathBuf::from("notes.md");
...
fn staged_deleted_markdown_file_preview_shows_preview_text_toggle(cx: &mut gpui::TestAppContext) {
    let repo_id = worktree_state::model::RepoId(58);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_markdown_deleted_toggle",
        std::process::id()
    ));
    let file_rel = std::path::PathBuf::from("notes.md");
...
fn unstaged_deleted_gitlink_preview_does_not_stay_loading(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(44);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_unstaged_gitlink",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.diff_state.diff_file = worktree_state::model::Loadable::Ready(None);
...
fn unstaged_modified_gitlink_target_uses_unified_diff_mode(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(45);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_unstaged_gitlink_mod",
        std::process::id()
    ));
    let file_rel = std::path::PathBuf::from("chess3");
    let _ = std::fs::remove_dir_all(&workdir);
...
fn ensure_preview_loading_does_not_reenter_loading_from_error_for_same_path(
    ...
    let temp = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_preview_loading_error",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            this.main_pane.update(cx, |pane, _cx| {
                ...
                pane.worktree_preview = worktree_state::model::Loadable::Error("boom".into());
...
fn switching_diff_target_clears_stale_worktree_preview_loading(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(36);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_switch_preview_target",
        std::process::id()
    ));
        ...
        Arc::new(AppState {
            repos: vec![{
                ...
                repo.diff_state.diff_target =
                    Some(worktree_core::domain::DiffTarget::WorkingTree {
                        path: target_path,
                        area: worktree_core::domain::DiffArea::Unstaged,
                    });
    ...
    cx.update(|_window, app| {
        ...
        let is_stale_loading =
            matches!(pane.worktree_preview, worktree_state::model::Loadable::Loading)
...
fn staged_directory_target_uses_unified_diff_mode(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(34);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_staged_dir",
        std::process::id()
    ));
...
fn staged_added_missing_target_uses_unified_diff_mode(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(43);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_staged_added_missing",
        std::process::id()
    ));
    let file_rel = std::path::PathBuf::from("subproject");
    let _ = std::fs::remove_dir_all(&workdir);
...
fn untracked_directory_target_uses_unified_diff_mode(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(35);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_unstaged_dir",
        std::process::id()
    ));
    let file_rel = std::path::PathBuf::from("subproject");
    let _ = std::fs::remove_dir_all(&workdir);
...
fn untracked_directory_target_clears_stale_file_loading_state(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_unstaged_dir_stale_loading",
        std::process::id()
    ));
    let file_rel = std::path::PathBuf::from("chess3");
    let _ = std::fs::remove_dir_all(&workdir);
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.diff_state.diff = worktree_state::model::Loadable::Ready(Arc::new(
                worktree_core::domain::Diff::from_unified(
                    worktree_core::domain::DiffTarget::WorkingTree {
                        path: file_rel.clone(),
                        area: worktree_core::domain::DiffArea::Unstaged,
                    },
                    "",
...
fn directory_target_with_loading_status_clears_stale_file_loading_state(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_directory_loading_status",
        std::process::id()
    ));
    let file_rel = std::path::PathBuf::from("chess3");
    let _ = std::fs::remove_dir_all(&workdir);
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.status = worktree_state::model::Loadable::Loading;
            repo.diff_state.diff_target = Some(worktree_core::domain::DiffTarget::WorkingTree {
                path: file_rel.clone(),
                area: worktree_core::domain::DiffArea::Unstaged,
            });
            repo.diff_state.diff = worktree_state::model::Loadable::Loading;
...
fn added_file_preview_ctrl_a_ctrl_c_copies_all_content(cx: &mut gpui::TestAppContext) {
    let repo_id = worktree_state::model::RepoId(31);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_preview_added_copy",
        std::process::id()
    ));
...
fn deleted_file_preview_ctrl_a_ctrl_c_copies_all_content(cx: &mut gpui::TestAppContext) {
    let repo_id = worktree_state::model::RepoId(32);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_preview_deleted_copy",
        std::process::id()
    ));
    let file_rel = std::path::PathBuf::from("deleted.rs");
...
fn commit_details_metadata_fields_are_selectable(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(33);
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.history_state.selected_commit =
                Some(worktree_core::domain::CommitId(commit_sha.clone().into()));
            repo.history_state.commit_details = worktree_state::model::Loadable::Ready(Arc::new(
                worktree_core::domain::CommitDetails {
                    id: worktree_core::domain::CommitId(commit_sha.clone().into()),
                    ...
                    committed_at_unix: 0,
                    parent_ids: vec![worktree_core::domain::CommitId(parent_sha.clone().into())],
                    files: vec![],
                signed: false,},
            ));
...
fn click_commit_details_link(
    ...
    view: &gpui::Entity<crate::view::WorkTreeView>,
        ...
        view.update(app, |this, cx| {
            ...
            repo.log = Loadable::Ready(Arc::new(worktree_core::domain::LogPage {
                commits: vec![worktree_core::domain::Commit {
                    signed: false,
                    id: worktree_core::domain::CommitId(current_sha.into()),
            ...
            repo.log_rev = 1;
            repo.history_state.selected_commit =
                Some(worktree_core::domain::CommitId(current_sha.into()));
...
fn commit_details_message_url_click_opens_the_web_link_menu(cx: &mut gpui::TestAppContext) {
    ...
    show_commit_details_message(
        cx,
        &view,
        worktree_state::model::RepoId(41),
...
fn commit_details_message_mailto_click_opens_the_web_link_menu(cx: &mut gpui::TestAppContext) {
    ...
    show_commit_details_message(
        cx,
        &view,
        worktree_state::model::RepoId(42),
...
fn commit_details_message_trailers_do_not_linkify_as_commits(cx: &mut gpui::TestAppContext) {
    ...
    show_commit_details_message(
        cx,
        &view,
        worktree_state::model::RepoId(43),
...
fn commit_details_message_filename_hashes_do_not_linkify_as_commits(cx: &mut gpui::TestAppContext) {
    ...
    show_commit_details_message(
        cx,
        &view,
        worktree_state::model::RepoId(44),
...
fn commit_details_message_sha_click_menu_navigate_reveals_referenced_commit(
        ...
        view.update(app, |this, cx| {
            ...
            repo.log = Loadable::Ready(Arc::new(worktree_core::domain::LogPage {
                commits: vec![
                    ...
                    worktree_core::domain::Commit {
                        signed: false,
                        id: worktree_core::domain::CommitId(target_sha.into()),
                        parent_ids: worktree_core::domain::CommitParentIds::new(),
                        summary: "target".into(),
                        author: "Alice".into(),
                        time: std::time::SystemTime::UNIX_EPOCH,
    ...
    cx.update(|_window, app| {
        ...
        let expected = worktree_core::domain::CommitId(target_sha.into());
...
fn commit_details_message_sha_retained_details_are_inert_after_selection_changes(
    ...
    let repo_id = worktree_state::model::RepoId(38);
        ...
        view.update(app, |this, cx| {
            ...
            repo.log = Loadable::Ready(Arc::new(worktree_core::domain::LogPage {
                commits: vec![
                    worktree_core::domain::Commit {
                        signed: false,
                        id: worktree_core::domain::CommitId(retained_sha.into()),
                        parent_ids: worktree_core::domain::CommitParentIds::new(),
                    ...
                    worktree_core::domain::Commit {
                        signed: false,
                        id: worktree_core::domain::CommitId(selected_sha.into()),
                        parent_ids: worktree_core::domain::CommitParentIds::new(),
            ...
            repo.log_rev = 1;
            repo.history_state.selected_commit =
                Some(worktree_core::domain::CommitId(selected_sha.into()));
            repo.history_state.commit_details = worktree_state::model::Loadable::Ready(Arc::new(
                worktree_core::domain::CommitDetails {
                    id: worktree_core::domain::CommitId(retained_sha.into()),
    ...
    cx.update(|_window, app| {
        let selected = view
            ...
            .find(|repo| repo.id == repo_id)
            .and_then(|repo| repo.history_state.selected_commit.as_ref());
        let expected = worktree_core::domain::CommitId(selected_sha.into());
        assert_eq!(selected, Some(&expected));
    });
...
fn commit_details_message_sha_click_opens_the_menu_without_navigating(
    ...
    cx.update(|_window, app| {
        ...
        let selected = view
            ...
            .find(|repo| repo.id == repo_id)
            .and_then(|repo| repo.history_state.selected_commit.as_ref());
        let expected = worktree_core::domain::CommitId(current_sha.into());
        assert_eq!(selected, Some(&expected));
    });
}
...
fn commit_details_message_sha_double_click_selects_instead_of_opening_the_menu(
    ...
    let repo_id = worktree_state::model::RepoId(41);
    let current_sha = "0123456789abcdef0123456789abcdef01234567";
    let target_sha = "89abcdef0123456789abcdef0123456789abcdef";
...
fn commit_details_parent_sha_click_menu_navigate_reveals_referenced_commit(
    ...
    let repo_id = worktree_state::model::RepoId(39);
    let current_sha = "0123456789abcdef0123456789abcdef01234567";
        ...
        view.update(app, |this, cx| {
            ...
            repo.log = Loadable::Ready(Arc::new(worktree_core::domain::LogPage {
                commits: vec![
                    ...
                    worktree_core::domain::Commit {
                        signed: false,
                        id: worktree_core::domain::CommitId(parent_sha.into()),
                        parent_ids: worktree_core::domain::CommitParentIds::new(),
                        summary: "parent".into(),
                        author: "Alice".into(),
                        time: std::time::SystemTime::UNIX_EPOCH,
            ...
            repo.history_state.commit_details =
                Loadable::Ready(Arc::new(worktree_core::domain::CommitDetails {
                    ...
                    committed_at: "2026-03-08 12:34:56 +0200".into(),
                    committed_at_unix: 0,
                    parent_ids: vec![worktree_core::domain::CommitId(parent_sha.into())],
                    files: vec![],
                signed: false,}));
    ...
    cx.update(|_window, app| {
        let selected = view
            ...
            .find(|repo| repo.id == repo_id)
            .and_then(|repo| repo.history_state.selected_commit.as_ref());
        let expected = worktree_core::domain::CommitId(parent_sha.into());
        assert_eq!(selected, Some(&expected));
    });
}
...
fn commit_details_parent_sha_dash_has_no_link_menu(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(40);
    let current_sha = "0123456789abcdef0123456789abcdef01234567";
...
fn commit_details_added_file_copy_path_works_after_left_clicking_menu_entry(
    ...
    let commit_sha = "0123456789abcdef0123456789abcdef01234567".to_string();
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_commit_added_copy_path",
        std::process::id()
    ));
        ...
        view.update(app, |this, cx| {
            ...
            repo.history_state.commit_details = worktree_state::model::Loadable::Ready(Arc::new(
                worktree_core::domain::CommitDetails {
                    ...
                    committed_at_unix: 0,
                    parent_ids: vec![worktree_core::domain::CommitId(
...
fn commit_details_file_right_click_only_opens_menu_for_added_modified_and_deleted(
    ...
    let repo_id = worktree_state::model::RepoId(61);
    let commit_sha = "0123456789abcdef0123456789abcdef01234567".to_string();
    let workdir = std::env::temp_dir().join(format!(
    ...
    let initial_target = worktree_core::domain::DiffTarget::WorkingTree {
        ...
        view.update(app, |this, cx| {
            ...
            repo.history_state.commit_details = worktree_state::model::Loadable::Ready(Arc::new(
                worktree_core::domain::CommitDetails {
                    ...
                    files: files
                        .iter()
                        .map(|(path, kind)| worktree_core::domain::CommitFileChange {
                            path: path.clone(),
                            kind: *kind,
                            is_submodule: false,
                            additions: None,
    ...
    for (ix, (path, _kind)) in files.iter().enumerate() {
        ...
        assert_eq!(
            ...
            Some(PopoverKind::CommitFileMenu {
                repo_id,
                commit_id: worktree_core::domain::CommitId(commit_sha.clone().into()),
                path: path.clone(),
            })
        );
...
fn status_file_right_click_opens_menu_without_opening_diff_or_changing_selection(
    ...
    let (view, cx) = cx.add_window_view(|window, cx| {
        super::super::WorkTreeView::new(store_for_view, events, None, window, cx)
    });

    let repo_id = worktree_state::model::RepoId(62);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_right_click_menu_only",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            let mut repo = opening_repo_state(repo_id, &workdir);
            repo.open = worktree_state::model::Loadable::Ready(());
            repo.status = worktree_state::model::Loadable::Ready(
                worktree_core::domain::RepoStatus {
                    staged: vec![worktree_core::domain::FileStatus {
                        path: staged.clone(),
    ...
    for (section_label, ix, area, path) in cases {
        ...
        let (popover_kind, multi_selection) = cx.update(|_window, app| {
            ...
            let multi_selection = view
                ...
                .get(&repo_id)
                .map(|sel| {
                    sel.selected_paths_for_area(worktree_core::domain::DiffArea::Unstaged)
                        .to_vec()
                });
...
fn commit_details_file_list_keeps_visible_viewport_when_overflowing(cx: &mut gpui::TestAppContext) {
    ...
    let commit_sha = "0123456789abcdef0123456789abcdef01234567".to_string();
    let files = (0..48)
        .map(|ix| worktree_core::domain::CommitFileChange {
...
fn details_row_renderers_begin_separate_alignment_groups_for_status_and_commit_files(
    ...
    let repo_id = worktree_state::model::RepoId(631);
    let commit_id = worktree_core::domain::CommitId("0123456789abcdef".into());
...
fn switching_active_repo_restores_commit_message_draft_per_repo(cx: &mut gpui::TestAppContext) {
    ...
    let repo_a = worktree_state::model::RepoId(41);
    let repo_b = worktree_state::model::RepoId(42);
    let make_state = |active_repo: worktree_state::model::RepoId| {
        Arc::new(AppState {
            repos: vec![
...
fn merge_start_prefills_default_commit_message(cx: &mut gpui::TestAppContext) {
    ...
    let make_state = |merge_message: Option<&str>| {
        ...
        repo.merge_commit_message = worktree_state::model::Loadable::Ready(
...
fn state_with_recent_commit_message(
    repo_id: worktree_state::model::RepoId,
    workdir: &str,
    recent: worktree_state::model::Loadable<Arc<Vec<worktree_core::domain::RecentCommitMessage>>>,
) -> Arc<AppState> {
    let mut repo = opening_repo_state(repo_id, Path::new(workdir));
...
fn recent_messages(
    messages: &[&str],
) -> worktree_state::model::Loadable<Arc<Vec<worktree_core::domain::RecentCommitMessage>>> {
    worktree_state::model::Loadable::Ready(Arc::new(
        messages
            .iter()
            .enumerate()
            .map(|(ix, message)| worktree_core::domain::RecentCommitMessage {
                id: worktree_core::domain::CommitId(format!("commit{ix}").into()),
        ...
        repo.status = worktree_state::model::Loadable::Ready(
            worktree_core::domain::RepoStatus {
                staged: (0..staged_count)
                    .map(|ix| worktree_core::domain::FileStatus {
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            this.main_pane.update(cx, |pane, cx| {
                ...
                let new_theme = crate::theme::AppTheme::worktree_light();
...
fn status_section_drag_updates_saved_height(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(46);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_resize_drag",
        std::process::id()
    ));
...
fn staged_section_remains_visible_after_window_resize_with_saved_split_height(
    ...
    let repo_id = worktree_state::model::RepoId(51);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_resize_window_shrink",
        std::process::id()
    ));
...
fn split_status_section_resize_moves_untracked_section(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(47);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_split_status_resize_drag",
        std::process::id()
    ));
...
fn unstaged_scroll_viewport_tracks_resized_section_height(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_unstaged_scroll_viewport",
        std::process::id()
    ));
...
fn split_unstaged_scroll_viewport_tracks_resized_section_height(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_split_unstaged_scroll_viewport",
        std::process::id()
    ));
...
fn split_unstaged_scroll_viewport_updates_after_outer_resize_shrink(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(50);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_split_unstaged_outer_resize",
        std::process::id()
    ));
...
fn stage_all_asks_before_staging_unresolved_conflicts(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(70615);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_stage_all_conflict_confirm",
        std::process::id()
    ));
...
fn stage_selected_label_shrinks_when_the_details_pane_is_narrow(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_header_shrink",
        std::process::id()
    ));
    let a = std::path::PathBuf::from("a.txt");
    let b = std::path::PathBuf::from("b.txt");
...
fn worktree_file_alignment_signatures_differ_per_worktree(cx: &mut gpui::TestAppContext) {
    ...
    cx.update(|_window, app| {
        let pane = view.read(app).details_pane.read(app);
        let repo_id = worktree_state::model::RepoId(4);
...
fn worktree_file_inputs_are_derived_once_per_scan_and_keyed_by_worktree(
    ...
    let summary = |path: &str, files: usize| worktree_core::domain::WorktreeDirtySummary {
        path: std::path::PathBuf::from(path),
        head: Some(worktree_core::domain::CommitId("tip".into())),
    ...
    cx.update(|_window, app| {
        let pane = view.read(app).details_pane.read(app);
        let repo_id = worktree_state::model::RepoId(9);
        ...
        for (ix, entry) in first.entries.iter().enumerate() {
            ...
            assert_eq!(
                entry.section,
                if staged {
                    worktree_state::model::InlineSubmoduleDiffSection::LiveStaged
                } else {
                    worktree_state::model::InlineSubmoduleDiffSection::LiveUnstaged
                },
