...
fn large_file_diff_keeps_prepared_syntax_documents_above_old_line_gate(
    ...
    let repo_id = worktree_state::model::RepoId(53);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_large_file_diff_syntax",
        std::process::id()
    ));
...
",
    ...
    let repo_id = worktree_state::model::RepoId(88);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_source_backed_file_diff_syntax",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.diff_state.diff = worktree_state::model::Loadable::Ready(Arc::new(
                worktree_core::domain::Diff::from_unified(target, &unified),
            ));
            repo.diff_state.diff_file_rev = 1;
            repo.diff_state.diff_file = worktree_state::model::Loadable::Ready(Some(Arc::new(
                worktree_core::domain::FileDiffText::new_sources(
                    path.clone(),
                    Some(worktree_core::domain::FileDiffTextSource::new(
...
fn oversized_json_file_diff_uses_visible_line_fallback_without_prepared_syntax_documents(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_oversized_json_file_diff_syntax",
        std::process::id()
    ));
...
fn minified_json_file_diff_streams_visible_slices_and_inline_search(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(92);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_minified_json_file_diff_streamed",
        std::process::id()
    ));
...
fn split_file_diff_scroll_sync_matrix_covers_all_modes_and_axes(cx: &mut gpui::TestAppContext) {
    ...
    let repo_id = worktree_state::model::RepoId(214);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_split_scroll_sync_none",
        std::process::id()
    ));
...
fn minified_json_file_diff_partial_copy_uses_streamed_inline_row_source(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_minified_json_file_diff_partial_copy",
        std::process::id()
    ));
...
fn minified_json_file_diff_context_menu_copy_uses_streamed_inline_row_source(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_minified_json_file_diff_context_menu_copy",
        std::process::id()
    ));
...
fn minified_json_file_diff_split_partial_copy_uses_streamed_row_source(
    ...
    let repo_id = worktree_state::model::RepoId(195);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_minified_json_file_diff_split_partial_copy",
        std::process::id()
    ));
...
fn large_file_diff_renders_plain_text_then_upgrades_after_background_syntax(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_large_file_diff_background_syntax",
        std::process::id()
    ));
...
fn edited_large_file_diff_reparses_incrementally_in_background_after_timeout(
    ...
    let repo_id = worktree_state::model::RepoId(64);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_edited_large_file_diff_background_syntax",
        std::process::id()
    ));
...
fn file_diff_background_left_syntax_upgrade_preserves_right_cached_rows(
    ...
    let repo_id = worktree_state::model::RepoId(65);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_one_sided_file_diff_background_syntax",
        std::process::id()
    ));
