...
fn ai_generate_fetches_context_then_runs_the_provider_request(cx: &mut gpui::TestAppContext) {
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.ai_commit_context = worktree_state::model::Loadable::Ready(Arc::new(
                worktree_state::model::AiCommitContext {
...
fn switching_repos_abandons_an_in_flight_generation(cx: &mut gpui::TestAppContext) {
    ...
    let (view, cx) =
        cx.add_window_view(|window, cx| WorkTreeView::new(store, events, None, window, cx));
    let repo_a = worktree_state::model::RepoId(63);
    let repo_b = worktree_state::model::RepoId(64);
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            let mut state = worktree_state::model::AppState::default();
