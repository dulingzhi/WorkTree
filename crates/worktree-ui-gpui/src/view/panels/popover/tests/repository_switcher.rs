...
fn repository_switcher_selecting_recent_repo_opens_it_wrapper() {
    ...
    worktree_state::session::persist_recent_repo_to_path(&repo_path, &session_file)
...
fn repository_switcher_selecting_recent_repo_opens_it_subprocess(cx: &mut gpui::TestAppContext) {
    ...
    let _visual_guard = crate::test_support::lock_visual_test();
    let expected_path = worktree_state::session::load()
        .recent_repos
        .into_iter()
        .next()
...
fn repository_switcher_removes_a_recent_repo_subprocess(cx: &mut gpui::TestAppContext) {
    ...
    let _visual_guard = crate::test_support::lock_visual_test();
    let seeded = worktree_state::session::load().recent_repos;
    ...
    assert_eq!(
        worktree_state::session::load().recent_repos,
        vec![kept],
        "expected the removal to persist to the session file"
    );
    assert!(
        !worktree_state::session::load()
            .recent_repos
            .contains(&removed),
