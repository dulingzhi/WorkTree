...
fn status_file_menu_uses_multi_selection_for_stage(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_menu",
        std::process::id()
    ));
...
fn status_file_menu_uses_multi_selection_for_unstage(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_menu_staged",
        std::process::id()
    ));
...
fn status_file_menu_offers_resolve_actions_for_conflicts(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_menu_conflict",
        std::process::id()
    ));
...
fn status_file_menu_hides_external_mergetool_for_staged_conflicts(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_menu_staged_conflict",
        std::process::id()
    ));
    let path = std::path::PathBuf::from("conflict.txt");
...
fn status_file_menu_hides_permalink_for_local_only_branch(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_menu_local_only_permalink",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.remotes = Loadable::Ready(Arc::new(vec![worktree_core::domain::Remote {
                ...
                url: Some("git@github.com:dulingzhi/WorkTree.git".to_string()),
            }]));
            repo.remote_branches =
                Loadable::Ready(Arc::new(vec![worktree_core::domain::RemoteBranch {
                    remote: "origin".to_string(),
                    name: "main".to_string(),
                    target: worktree_core::domain::CommitId("deadbeef".into()),
                }]));
...
fn status_file_menu_offers_permalink_for_pushed_branch(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_menu_pushed_permalink",
        std::process::id()
    ));
    let path = std::path::PathBuf::from("a.txt");
    ...
    cx.update(|_window, app| {
        ...
        match permalink {
            Some(ContextMenuAction::CopyText { text }) => {
                assert_eq!(
                    text,
                    "https://github.com/dulingzhi/WorkTree/blob/main/a.txt"
                );
            }
...
fn status_file_menu_open_from_details_pane_does_not_double_lease_panic(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_menu_reentrant",
        std::process::id()
    ));
    let path = std::path::PathBuf::from("conflict.txt");
...
fn status_menu_for(
    cx: &mut gpui::TestAppContext,
    entries: &[(&str, worktree_core::domain::FileStatusKind)],
    selection: &[&str],
    clicked: &str,
) -> ContextMenuModel {
    let (store, events) = AppStore::new(Arc::new(TestBackend));
    ...
    let repo_id = RepoId(7);
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_gitignore_menu",
        std::process::id()
    ));

    let unstaged: Vec<_> = entries
        .iter()
        .map(|(path, kind)| worktree_core::domain::FileStatus {
