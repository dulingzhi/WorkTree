...
pub(super) struct TrackingRepo {
    ...
    bisect: Arc<Mutex<Option<worktree_core::services::BisectState>>>,
...
impl GitRepository for TrackingRepo {
    ...
    fn log_head_page(
        &self,
        _limit: usize,
        _cursor: Option<&worktree_core::domain::LogCursor>,
    ) -> Result<LogPage> {
        Ok(LogPage {
            commits: Vec::new(),
            next_cursor: None,
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "worktree-ui-popover-{label}-{}-{suffix}",
        std::process::id()
    ));
...
fn rename_branch_prompt_cancel_button_and_escape_close(cx: &mut gpui::TestAppContext) {
    ...
    let open_prompt =
        |window: &mut gpui::Window, app: &mut gpui::App, view: &Entity<WorkTreeView>| {
            view.update(app, |this, cx| {
                this.popover_host.update(cx, |host, cx| {
                    host.open_popover_at(
...
mod checkout_picker {
    ...
    use worktree_core::domain::{RefMetadata, RemoteBranch};
    ...
    fn opening_the_picker_requests_ref_metadata_once(cx: &mut gpui::TestAppContext) {
        ...
        let ref_metadata_untouched = |snapshot: &Arc<worktree_state::model::AppState>| {
            snapshot
                .repos
                .iter()
                .find(|r| r.id == repo_id)
...
fn branch_group_menu_model_filtered(
    ...
    let remote_branch = |remote: &str, name: &str| worktree_core::domain::RemoteBranch {
        remote: remote.to_string(),
        name: name.to_string(),
        target: CommitId("aaaaaaaaaaaa".into()),
    };
            ...
            let mut repo = RepoState::new_opening(
                ...
                RepoSpec {
                    workdir: std::env::temp_dir().join(format!(
                        "worktree_ui_test_{}_branch_group_menu",
                        std::process::id()
                    )),
                },
            );
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            let mut repo = RepoState::new_opening(
                ...
                RepoSpec {
                    workdir: std::env::temp_dir().join(format!(
                        "worktree_ui_test_{}_group_delete_confirm",
                        std::process::id()
                    )),
                },
            );
...
fn pinned_section_menu_model(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_pinned_section_menu",
        std::process::id()
    ));
...
fn activate_sidebar_action_with(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_sidebar_action",
        std::process::id()
    ));
    let branch = |name: &str| Branch {
        name: name.to_string(),
...
fn pinned_section_menu_reports_expanded_while_a_filter_is_live(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_pinned_filter",
        std::process::id()
    ));
