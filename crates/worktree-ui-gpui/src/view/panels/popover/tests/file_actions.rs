...
fn commit_menu_test_repo(repo_id: RepoId, commit_id: &CommitId) -> RepoState {
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_commit_menu",
        std::process::id()
    ));
    ...
    repo.sequencer_state = Loadable::Ready(worktree_core::services::SequencerState::None);
...
fn status_file_menu_has_open_file_entries(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_menu_open_file",
        std::process::id()
    ));
...
fn unopened_submodule_menus_disable_open_in_code_editor(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_unopened_submodule_editor_menu",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.submodules = Loadable::Ready(
                vec![worktree_core::domain::Submodule {
                    path: path.clone(),
                    recorded_head: CommitId("1111111111111111".into()),
                    checked_out_head: None,
                    status: worktree_core::domain::SubmoduleStatus::NotInitialized,
                }]
                .into(),
            );
...
fn status_file_menu_copy_path_uses_os_native_separators(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_menu_copy_path_native",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        ...
        expected.push("worktree-ui-gpui");
        expected.push("src");
...
fn commit_file_menu_copy_path_uses_os_native_separators(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_commit_menu_copy_path_native",
        std::process::id()
    ));
    let path = std::path::PathBuf::from("crates/worktree-ui-gpui/src/smoke_tests.rs");
...
fn commit_file_menu_copy_path_supports_right_button_release(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_commit_menu_copy_path_right_release",
        std::process::id()
    ));
    let path = std::path::PathBuf::from("crates/worktree-ui-gpui/src/smoke_tests.rs");
...
fn status_file_menu_copy_path_supports_right_button_release(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_status_menu_copy_path_right_release",
        std::process::id()
    ));
    let path = std::path::PathBuf::from("crates/worktree-ui-gpui/src/smoke_tests.rs");
...
fn file_preview_context_menu_matches_diff_editor_actions(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_preview_context_menu",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.diff_state.diff_preview_text_file =
                Loadable::Ready(Some(Arc::new(worktree_core::domain::DiffPreviewTextFile {
...
fn file_browser_folder_menu_model(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_file_browser_folder_menu",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.file_browser.entries = Loadable::Ready(Arc::new(vec![
                worktree_core::domain::FileEntry {
                    name: "src".to_string(),
                ...
                worktree_core::domain::FileEntry {
                    ...
                    kind: worktree_core::domain::FileEntryKind::File,
                    depth: 1,
                },
            ]));
            configure(&mut repo);
...
fn file_browser_folder_menu_hides_os_actions_for_a_commit_source(cx: &mut gpui::TestAppContext) {
    let model = file_browser_folder_menu_model(cx, |repo| {
        repo.file_browser.source =
            worktree_core::domain::FileSource::Commit(CommitId("abc123".into()));
    });
...
fn copy_path_mnemonic_selects_the_relative_entry_in_every_menu(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "worktree_ui_test_{}_copy_mnemonic",
        std::process::id()
    ));
