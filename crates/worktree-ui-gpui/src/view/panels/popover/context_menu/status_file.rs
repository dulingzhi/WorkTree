...
pub(super) fn model(
    ...
    let is_deleted = this
        ...
        .is_some_and(|repo| {
            matches!(
                ...
                Some(worktree_core::domain::FileStatusKind::Deleted)
            )
        });
    ...
    let (
        ...
        .map(|repo| {
            ...
            (
                matches!(
                    unstaged_kind,
                    Some(worktree_core::domain::FileStatusKind::Conflicted)
                ) || matches!(
                    staged_kind,
                    Some(worktree_core::domain::FileStatusKind::Conflicted)
                ),
                ...
                matches!(
                    staged_kind,
                    Some(worktree_core::domain::FileStatusKind::Added)
                ),
                matches!(
                    unstaged_kind,
                    Some(worktree_core::domain::FileStatusKind::Untracked)
                ),
            )
        })
    ...
    let submodule_menu_state = this
        ...
        .and_then(|repo| {
            ...
            if status_entry.kind == worktree_core::domain::FileStatusKind::Untracked {
                return None;
            }
