...
impl WorkTreeView {
    ...
    pub(crate) fn blocks_non_repository_actions(&self) -> bool {
        repository_entry_interstitial_active(self.view_mode, self.has_repo_tabs())
            || matches!(self.view_mode, WorkTreeViewMode::Normal) && self.git_runtime_unavailable()
    }
    ...
    fn interstitial_logo(_theme: AppTheme, size: Pixels) -> AnyElement {
        div()
            ...
            .size(size)
            .child(gpui::svg().path("worktree_logo.svg").w(size).h(size))
            .into_any_element()
    }
    ...
    fn git_unavailable_open_settings_button(
        ...
        .worktree_tooltip(self.theme, settings_tooltip)
    ...
    pub(super) fn splash_screen(&mut self, cx: &mut gpui::Context<Self>) -> AnyElement {
        ...
        .worktree_tooltip(self.theme, open_tooltip)
        ...
        let clone_button = {
            ...
            .worktree_tooltip(self.theme, clone_tooltip)
