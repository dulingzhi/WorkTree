...
pub(crate) const DEFAULT_DARK_THEME_KEY: &str = "worktree_dark";
pub(crate) const DEFAULT_LIGHT_THEME_KEY: &str = "worktree_light";
...
impl AppTheme {
    ...
    /// WorkTree's default dark theme loaded from an embedded JSON definition.
    pub fn worktree_dark() -> Self {
        Self::from_key(DEFAULT_DARK_THEME_KEY)
            .unwrap_or_else(|| panic!("missing default dark theme `{DEFAULT_DARK_THEME_KEY}`"))
    }

    /// WorkTree's default light theme loaded from an embedded JSON definition.
    #[cfg(test)]
    pub fn worktree_light() -> Self {
...
fn resolved_runtime_themes_dir(runtime_dir: Option<&Path>) -> Option<PathBuf> {
    let dir = match runtime_dir {
        ...
        None => worktree_state::session::user_themes_dir()?,
    };
...
/// instead would hand every such theme worktree's hand-picked lane colours and
...
mod tests {
    ...
    /// wild with worktree's own lane colours.
    #[test]
    ...
    fn bundled_themes_keep_the_canvas_and_chrome_hierarchy_for_their_appearance() {
        assert_eq!(
            AppTheme::worktree_light().colors.surface.canvas,
            gpui::rgba(0xffffffff),
            "WorkTree Light should keep its pure-white canvas"
        );
        assert_eq!(
    ...
    fn embedded_theme_registry_exposes_default_keys() {
        ...
        assert_eq!(
            ...
            Some("WorkTree Dark".to_string())
        );
        assert_eq!(
            theme_label(DEFAULT_LIGHT_THEME_KEY),
            Some("WorkTree Light".to_string())
        );
    }
    ...
    fn runtime_theme_dir_ignores_reserved_system_theme_filenames() {
        let dir = tempdir().expect("temp dir should exist");
        fs::write(dir.path().join("worktree.json"), "not parsed")
    ...
    fn runtime_theme_dir_ignores_embedded_theme_key_collisions_but_keeps_custom_entries() {
        ...
        collision["name"] = json!("Fake WorkTree Dark");
    ...
    fn themes_markdown_documents_custom_theme_override_rules() {
        ...
        for snippet in [
            "WorkTree creates the user themes directory on startup",
