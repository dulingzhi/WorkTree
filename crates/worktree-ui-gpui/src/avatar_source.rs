...
fn avatar_cache_dir() -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        non_empty(std::env::var_os("HOME")).map(|home| home.join("Library/Caches/worktree/avatars"))
    }

    #[cfg(target_os = "linux")]
    {
        non_empty(std::env::var_os("XDG_CACHE_HOME"))
            .map(|dir| dir.join("worktree/avatars"))
            .or_else(|| {
                non_empty(std::env::var_os("HOME")).map(|home| home.join(".cache/worktree/avatars"))
            })
    }
    ...
    #[cfg(target_os = "windows")]
    {
        non_empty(std::env::var_os("LOCALAPPDATA").or_else(|| std::env::var_os("APPDATA")))
            .map(|dir| dir.join("worktree/cache/avatars"))
    }
