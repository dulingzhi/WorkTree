use worktree_core::process::background_command;
use worktree_state::session::{self, ExternalCodeEditorSetting};
...
pub(crate) fn detect_external_editors() -> Vec<DetectedExternalEditor> {
    ...
    // entries): set WORKTREE_LOG_EDITOR_DETECT=1 to time each pass.
    if env::var_os("WORKTREE_LOG_EDITOR_DETECT").is_some() {
        eprintln!(
...
fn spawn_detection_refresh() {
    ...
    let spawned = std::thread::Builder::new()
        .name("worktree-editor-detection".to_string())
        .spawn(|| {
            let editors = detect_external_editors();
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let dir = env::temp_dir().join(format!(
            "worktree-external-editor-{label}-{}-{}",
            std::process::id(),
            SystemTime::now()
    ...
    fn custom_launch_args_preserve_non_utf8_path_bytes_in_placeholder() {
        ...
        let target = PathBuf::from(OsString::from_vec(b"/tmp/worktree-\xff/repo".to_vec()));
        let setting = ExternalCodeEditorSetting::Custom {
            executable: PathBuf::from("/usr/bin/editor"),
        ...
        assert_eq!(
            ...
            b"--marker=before-/tmp/worktree-\xff/repo-after"
        );
    }
