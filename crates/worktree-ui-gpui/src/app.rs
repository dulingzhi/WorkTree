use crate::ai_commit;
use crate::assets::WorkTreeAssets;
...
struct WindowLaunchConfig {
    title: String,
    app_id: String,
    view_config: WorkTreeViewConfig,
}
...
/// Launch the unified focused mergetool window using the shared `WorkTreeView`.
...
///
/// [`open_worktree_window`] later initializes the `i18n::AppLanguage` gpui
...
fn normal_launch_config(
    ...
    let mut view_config = WorkTreeViewConfig::normal(startup_crash_report);
    view_config.initial_path = initial_path;
    WindowLaunchConfig {
        title: "WorkTree".to_string(),
        app_id: "worktree".to_string(),
        view_config,
    }
}
...
fn normal_launch_config_with_initial_repository(
    ...
    WindowLaunchConfig {
        title: "WorkTree".to_string(),
        app_id: "worktree".to_string(),
        view_config: WorkTreeViewConfig::normal_with_initial_repository(
            initial_path,
            startup_crash_report,
        ),
    }
...
pub(crate) fn begin_window_move(window: &Window) {
    ...
    if let Some(hwnd) = window_hwnd(window)
        && worktree_win32_window_utils::begin_window_move(hwnd)
    {
        return;
    }
...
fn restore_maximized_window(window: &Window) -> bool {
    ...
    worktree_win32_window_utils::restore_window(hwnd)
}
...
fn show_windows_window_system_menu(window: &Window, position: Point<Pixels>) -> bool {
    ...
    worktree_win32_window_utils::show_window_system_menu(request.hwnd, request.x, request.y);
    true
}
...
fn run_windowed_app(
    ...
    let application = application()
        .with_assets(WorkTreeAssets)
        .with_http_client(crate::http::client());

    #[cfg(target_os = "macos")]
    let open_urls_rx = if launch.view_config.view_mode == WorkTreeViewMode::Normal {
    ...
    #[cfg(target_os = "macos")]
    if launch.view_config.view_mode == WorkTreeViewMode::Normal {
        let reopen_backend = Arc::clone(&backend);
        let reopen_launch = launch.clone();
        application.on_reopen(move |cx: &mut App| {
            if cx.windows().is_empty() {
                open_worktree_window(cx, Arc::clone(&reopen_backend), &reopen_launch);
                cx.activate(true);
            }
        });
    }
    ...
    application.run(move |cx: &mut App| {
        ...
        open_worktree_window(cx, Arc::clone(&backend), &launch);
...
fn should_quit_when_all_windows_closed(launch: &WindowLaunchConfig) -> bool {
    launch.view_config.view_mode == WorkTreeViewMode::FocusedMergetool || !cfg!(target_os = "macos")
}

fn open_worktree_window(
    cx: &mut App,
    backend: Arc<dyn GitBackend>,
    launch: &WindowLaunchConfig,
) -> gpui::WindowHandle<WorkTreeView> {
    ...
    let intercept_native_close = view_config.view_mode == WorkTreeViewMode::Normal;
    ...
    .unwrap_or_else(|err| {
        panic!(
            "failed to open main WorkTree window: {err}\n\
             This is usually a GPU/display problem, not a WorkTree bug. \
...
fn apply_ui_scale_to_open_windows(cx: &mut App, percent: u32) {
    for handle in cx.windows() {
        let _ = handle.update(cx, |root_view, window, cx| {
            let root_view = match root_view.downcast::<WorkTreeView>() {
                Ok(view) => {
...
fn install_app_actions(cx: &mut App, backend: Arc<dyn GitBackend>) {
    ...
    cx.on_action(move |_: &NewWindow, cx| {
        ...
        cx.defer(move |cx| {
            ...
            open_worktree_window(cx, backend, &launch);
            cx.activate(true);
        });
    });
    ...
    cx.on_action(|_: &OpenInCodeEditor, cx| {
        cx.defer(|cx| {
            let _ = update_active_or_existing_normal_worktree_window(cx, |view, cx| {
    ...
    cx.on_action(move |_: &OpenRepository, cx| {
        ...
        cx.defer(move |cx| {
            if existing_normal_worktree_window_blocks_repository_management_actions(cx) {
                return;
            }
    ...
    cx.on_action(|_: &ShowReflog, cx| {
        cx.defer(|cx| {
            let _ = update_active_normal_worktree_window(cx, |view, cx| {
    ...
    cx.on_action(|_: &Close, cx| {
        cx.defer(|cx| {
            let handled =
                update_active_normal_worktree_window(cx, |view, cx| view.close_active_repo_tab(cx))
                    .unwrap_or(false);
            if !handled {
...
fn install_global_diff_shortcut_fallback(cx: &mut App) {
    cx.observe_keystrokes(|event, window, cx| {
        ...
        let Some(entry) = worktree_window_entries(cx).into_iter().find(|entry| {
            entry.handle.window_id() == window_id && entry.view_mode == WorkTreeViewMode::Normal
        }) else {
            return;
        };
...
#[derive(Clone)]
struct WorkTreeWindowEntry {
    handle: gpui::AnyWindowHandle,
    view: gpui::WeakEntity<WorkTreeView>,
...
#[derive(Default)]
struct WorkTreeWindowRegistry {
    windows: FxHashMap<gpui::WindowId, WorkTreeWindowEntry>,
}

impl gpui::Global for WorkTreeWindowRegistry {}

pub(crate) fn sync_worktree_window_state<C>(
    cx: &mut C,
    handle: gpui::AnyWindowHandle,
    view: gpui::WeakEntity<WorkTreeView>,
    main_pane: gpui::WeakEntity<MainPaneView>,
    ...
    C: BorrowAppContext,
{
    cx.update_default_global::<WorkTreeWindowRegistry, _>(|registry, _cx| {
...
fn worktree_window_entries(cx: &mut App) -> Vec<WorkTreeWindowEntry> {
...
fn active_worktree_window_entry(cx: &mut App) -> Option<WorkTreeWindowEntry> {
    let active_window_id = cx.active_window()?.window_id();
    worktree_window_entries(cx)
        .into_iter()
        .find(|entry| entry.handle.window_id() == active_window_id)
}

fn entry_contains_repo_path(entry: &WorkTreeWindowEntry, path: &Path) -> bool {
    entry.repo_paths.iter().any(|repo_path| repo_path == path)
}

fn active_normal_worktree_window(cx: &mut App) -> Option<WorkTreeWindowEntry> {
    let entry = active_worktree_window_entry(cx)?;
    (entry.view_mode == WorkTreeViewMode::Normal).then_some(entry)
}

fn normal_worktree_window_by_id(
    cx: &mut App,
    window_id: gpui::WindowId,
) -> Option<WorkTreeWindowEntry> {
    worktree_window_entries(cx).into_iter().find(|entry| {
        entry.handle.window_id() == window_id && entry.view_mode == WorkTreeViewMode::Normal
    })
}

fn update_active_normal_worktree_window<R>(
    cx: &mut App,
    f: impl FnOnce(&mut WorkTreeView, &mut gpui::Context<WorkTreeView>) -> R,
) -> Option<R> {
    let window = active_normal_worktree_window(cx)?;
    window.view.update(cx, f).ok()
}

fn update_active_or_existing_normal_worktree_window<R>(
    cx: &mut App,
    f: impl FnOnce(&mut WorkTreeView, &mut gpui::Context<WorkTreeView>) -> R,
) -> Option<R> {
    let window = find_normal_worktree_window(cx)?;
    window.view.update(cx, f).ok()
}

fn normal_worktree_window_blocks_repository_management_actions(
    cx: &mut App,
    window: &WorkTreeWindowEntry,
) -> bool {
    window
        .view
...
fn existing_normal_worktree_window_blocks_repository_management_actions(cx: &mut App) -> bool {
    let Some(window) = find_normal_worktree_window(cx) else {
        return false;
    };
    normal_worktree_window_blocks_repository_management_actions(cx, &window)
}
...
fn locate_file_in_active_or_existing_normal_window(cx: &mut App) {
    ...
    if cx.active_window().map(|active| active.window_id()) != Some(window.handle.window_id()) {
        activate_worktree_window(cx, window.handle);
    }
}
...
pub(crate) fn close_window_or_warn(window: &mut Window, cx: &mut App) {
    let window_id = window.window_handle().window_id();
    let handled = normal_worktree_window_by_id(cx, window_id)
        .and_then(|entry| {
            entry
                .view
                .update(cx, |view, cx| {
...
pub(crate) fn close_window_by_id_or_warn(cx: &mut App, window_id: gpui::WindowId) {
    ...
    mark_clean_shutdown_if_last_window(cx);
    if let Some(entry) = normal_worktree_window_by_id(cx, window_id) {
        let _ = entry
            .handle
...
pub(crate) fn close_active_window_or_warn(cx: &mut App) {
    ...
    let handled = active_window_id
        .and_then(|window_id| {
            update_active_normal_worktree_window(cx, move |view, cx| {
                view.request_close_window_or_warn(window_id, cx)
            })
        })
        .unwrap_or(false);
...
pub(crate) fn quit_app_or_warn(cx: &mut App) {
    let entries: Vec<_> = worktree_window_entries(cx)
        .into_iter()
        .filter(|entry| entry.view_mode == WorkTreeViewMode::Normal)
        .collect();
    if entries.is_empty() {
        mark_clean_shutdown(cx);
        cx.quit();
...
fn find_normal_worktree_window(cx: &mut App) -> Option<WorkTreeWindowEntry> {
    let entries = worktree_window_entries(cx);
    let active_window_id = cx.active_window().map(|window| window.window_id());
        ...
        && let Some(entry) = entries
            ...
            .find(|entry| {
                entry.handle.window_id() == active_window_id
                    && entry.view_mode == WorkTreeViewMode::Normal
            })
            .cloned()
    {
        return Some(entry);
    ...
    entries
        .into_iter()
        .find(|entry| entry.view_mode == WorkTreeViewMode::Normal)
}

fn find_normal_worktree_window_for_repo(cx: &mut App, path: &Path) -> Option<WorkTreeWindowEntry> {
    let entries = worktree_window_entries(cx);
    let active_window_id = cx.active_window().map(|window| window.window_id());
    if let Some(active_window_id) = active_window_id
        && let Some(entry) = entries
    ...
    entries.into_iter().find(|entry| {
        entry.view_mode == WorkTreeViewMode::Normal && entry_contains_repo_path(entry, path)
    })
}

fn activate_worktree_window(cx: &mut App, window: gpui::AnyWindowHandle) {
    let _ = window.update(cx, |_view, window, _cx| {
        window.activate_window();
    });
}

fn open_repository_in_window(cx: &mut App, window: &WorkTreeWindowEntry, path: PathBuf) {
...
fn focus_existing_repository_window(cx: &mut App, window: &WorkTreeWindowEntry, path: &Path) {
...
pub(crate) fn focus_existing_repository_window_for_path(cx: &mut App, path: &Path) -> bool {
    let Some(window) = find_normal_worktree_window_for_repo(cx, path) else {
        return false;
    };
...
fn open_repository_switcher_in_window(cx: &mut App, window: &WorkTreeWindowEntry) {
    let _ = window.handle.update(cx, |root_view, window, cx| {
        let Ok(view) = root_view.downcast::<WorkTreeView>() else {
            return;
        };
        view.update(cx, |view, cx| {
...
fn open_repository_switcher_in_existing_or_new_window(cx: &mut App, backend: Arc<dyn GitBackend>) {
    if let Some(window) = find_normal_worktree_window(cx) {
        open_repository_switcher_in_window(cx, &window);
        return;
    }

    let launch = normal_launch_config(None, None);
    let window = open_worktree_window(cx, backend, &launch);
    let _ = window.update(cx, |view, window, cx| {
        view.toggle_repository_switcher(window, cx);
    });
    activate_worktree_window(cx, window.into());
    cx.activate(true);
}

#[cfg(target_os = "macos")]
fn open_clone_repository_in_window(cx: &mut App, window: &WorkTreeWindowEntry) {
    let _ = window.handle.update(cx, |root_view, window, cx| {
        let Ok(view) = root_view.downcast::<WorkTreeView>() else {
            return;
        };
...
fn open_clone_repository_in_existing_or_new_window(cx: &mut App, backend: Arc<dyn GitBackend>) {
    if let Some(window) = find_normal_worktree_window(cx) {
        if normal_worktree_window_blocks_repository_management_actions(cx, &window) {
            return;
        }
...
#[cfg(target_os = "macos")]
fn prompt_initialize_repository_in_window(cx: &mut App, window: &WorkTreeWindowEntry) {
    let _ = window.handle.update(cx, |root_view, window, cx| {
        let Ok(view) = root_view.downcast::<WorkTreeView>() else {
            return;
        };
...
fn toggle_command_palette_in_window(cx: &mut App, window: &WorkTreeWindowEntry) {
    let _ = window.handle.update(cx, |root_view, window, cx| {
        let Ok(view) = root_view.downcast::<WorkTreeView>() else {
            return;
        };
...
fn toggle_command_palette_in_active_existing_or_new_window(
    ...
    if let Some(window) =
        active_normal_worktree_window(cx).or_else(|| find_normal_worktree_window(cx))
    {
...
fn open_repositories_in_existing_or_new_window(
    ...
    let mut target_window = find_normal_worktree_window(cx);

    for path in paths {
        if let Some(window) = find_normal_worktree_window_for_repo(cx, path.as_path()) {
        ...
        let window = open_worktree_window(cx, Arc::clone(&backend), &launch);
        activate_worktree_window(cx, window.into());
        target_window = find_normal_worktree_window(cx);
        cx.activate(true);
    }
}
...
fn prompt_apply_patch(cx: &mut App) {
    if find_normal_worktree_window(cx).is_none() {
        return;
    }
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn macos_app_menus_use_worktree_terminology_and_actions() {
        let menus = macos_app_menus_with_external_editor(true);
        let app_menu = menus
            .iter()
            .find(|menu| menu.name.as_ref() == "WorkTree")
    ...
    fn seed_workspace_repo(
        ...
        store.dispatch(Msg::OpenRepo(PathBuf::from("/tmp/worktree-app-test-repo")));
    ...
    fn focused_mergetool_title_uses_file_name_when_available() {
        ...
        assert_eq!(title, "WorkTree - Mergetool (conflict.txt)");
    }
    ...
    fn focused_mergetool_launch_config_sets_focused_view_mode_and_repo() {
        ...
        assert_eq!(launch.app_id, "worktree-mergetool");
        assert_eq!(launch.title, "WorkTree - Mergetool (conflict.txt)");
        assert_eq!(launch.view_config.initial_path, Some(config.repo_path));
        assert_eq!(
            launch.view_config.view_mode,
            WorkTreeViewMode::FocusedMergetool
        );
        assert_eq!(
    ...
    fn recent_repository_label_formats_repo_name_and_parent() {
        let label = recent_repository_label(Path::new("/Users/sampo/projects/worktree"));
        assert_eq!(label, "worktree - /Users/sampo/projects");
    }
    ...
    fn close_button_closes_only_the_clicked_window_after_opening_a_new_window(
        ...
        let second_window_id = cx.cx.update(|app| {
            assert_eq!(app.windows().len(), 2, "expected two WorkTree windows");
            app.windows()
                .into_iter()
                .map(|window| window.window_id())
    ...
    fn ctrl_tab_shortcuts_cycle_repository_tabs_in_the_main_window(cx: &mut gpui::TestAppContext) {
        ...
        let repos = vec![
            PathBuf::from("/tmp/worktree-app-test-repo-1"),
            PathBuf::from("/tmp/worktree-app-test-repo-2"),
            PathBuf::from("/tmp/worktree-app-test-repo-3"),
        ];
    ...
    fn repository_picker_fallback_opens_new_normal_window_when_none_exist(
        ...
        let panel_visible = cx.update(|app| {
            let entry = find_normal_worktree_window(app)
                .expect("expected a normal WorkTree window for manual repository entry");
            entry
                .view
                .update(app, |view, _cx| {
                    crate::view::test_support::open_repo_panel_visible(view)
                })
                .expect("expected to inspect the new WorkTree window")
        });
    ...
    fn command_palette_opens_new_normal_window_when_none_exist(cx: &mut gpui::TestAppContext) {
        ...
        let palette_open = cx.update(|app| {
            let entry = find_normal_worktree_window(app)
                .expect("expected a normal WorkTree window for Command Palette");
            entry
                .view
    ...
    fn macos_clone_repository_action_opens_native_clone_prompt(cx: &mut gpui::TestAppContext) {
        ...
        assert!(
            ...
            "Clone repository should open WorkTree's native clone prompt"
        );
    }
    ...
    fn locate_file_action_activates_background_normal_window(cx: &mut gpui::TestAppContext) {
        ...
        cx.cx.update(|app| {
            assert_eq!(
                app.active_window().map(|window| window.window_id()),
                Some(main_window_id),
                "locating a file should bring the main WorkTree window forward"
            );
        });
    }
    ...
    fn focus_existing_repository_window_for_path_avoids_reading_the_active_window_on_stack(
        ...
        let repo_path = PathBuf::from("/tmp/worktree-not-open");
        cx.cx.update(|app| {
    ...
    #[cfg(target_os = "macos")]
    #[gpui::test]
    fn update_active_normal_worktree_window_avoids_reading_the_active_window_on_stack(
        cx: &mut gpui::TestAppContext,
    ) {
        let _visual_guard = lock_visual_test();
        let backend: Arc<dyn GitBackend> = Arc::new(TestBackend);
        ...
        cx.cx.update(|app| {
            let result = window_handle.update(app, |_root_view, _window, app| {
                update_active_normal_worktree_window(app, |view, cx| view.close_active_repo_tab(cx))
            });
