...
//!
//! The default test runs against the repositorytree repository itself. An ignored
...
use repositorytree_core::merge::{MergeOptions, merge_file};
use repositorytree_core::merge_extraction::{
...
/// Find the root of the repositorytree repository (the repo we're building from).
fn find_repositorytree_repo() -> Option<PathBuf> {
...
/// Run extraction against repositorytree's own repository for real-world regression testing.
///
/// This test scans the most recent merge commits in the repositorytree repo, extracts
...
#[test]
fn extraction_regression_on_repositorytree_repo() {
    let Some(repo) = find_repositorytree_repo() else {
        eprintln!(
            "Skipping repositorytree extraction: could not find a host repository root from {}",
    ...
    let host_merges = match discover_merge_commits(&repo, 1) {
        ...
        Err(error) => {
            eprintln!(
                "Skipping repositorytree extraction: repository validation failed for {}: {}",
                repo.display(),
                error
            );
            return;
    ...
    if host_merges.is_empty() {
        eprintln!(
            "Skipping repositorytree extraction: no merge commits in {}",
            repo.display()
        );
        return;
    }
    ...
    eprintln!(
        "repositorytree extraction: {} total cases, {} clean, {} conflicts",
        total, clean, conflicts
    );
}

/// Run extraction against a large external repository for comprehensive
/// regression testing. Set the `REPOSITORYTREE_MERGE_EXTRACTION_REPO` environment
/// variable to the path of the repository to test against.
///
/// Example:
///   REPOSITORYTREE_MERGE_EXTRACTION_REPO=/home/user/git/linux cargo test \
///     --test merge_git_extraction extraction_regression_on_external_repo \
///     -- --ignored
#[test]
#[ignore]
fn extraction_regression_on_external_repo() {
    let repo_path = std::env::var("REPOSITORYTREE_MERGE_EXTRACTION_REPO").unwrap_or_else(|_| {
        panic!(
            "Set REPOSITORYTREE_MERGE_EXTRACTION_REPO to a git repo path to run this test.\n\
             Example: REPOSITORYTREE_MERGE_EXTRACTION_REPO=/home/user/git/linux"
        )
    });
...
///
/// Set `REPOSITORYTREE_MERGE_EXTRACTION_REPO` and `REPOSITORYTREE_MERGE_EXTRACTION_DEST`
/// to run. Generated fixtures are compatible with the existing fixture harness.
///
/// Example:
///   REPOSITORYTREE_MERGE_EXTRACTION_REPO=/home/user/git/linux \
///   REPOSITORYTREE_MERGE_EXTRACTION_DEST=crates/repositorytree-core/tests/fixtures/merge_extracted \
///   cargo test --test merge_git_extraction generate_fixtures_from_repo -- --ignored
#[test]
#[ignore]
fn generate_fixtures_from_repo() {
    let repo_path = std::env::var("REPOSITORYTREE_MERGE_EXTRACTION_REPO")
        .unwrap_or_else(|_| panic!("Set REPOSITORYTREE_MERGE_EXTRACTION_REPO to a git repo path"));
    let dest_path = std::env::var("REPOSITORYTREE_MERGE_EXTRACTION_DEST")
        .unwrap_or_else(|_| panic!("Set REPOSITORYTREE_MERGE_EXTRACTION_DEST to an output directory"));
