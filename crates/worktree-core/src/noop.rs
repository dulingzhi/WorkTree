//! No-op Git backend fallback for builds without a backend feature
//! (e.g. `gix`). Merged into `worktree-core` from the retired `worktree-git`
//! crate; `default_backend()` keeps its original signature and semantics.
...
#[cfg(test)]
pub(crate) fn open_repo(workdir: &Path) -> Result<Arc<dyn GitRepository>> {
...
#[cfg(test)]
mod tests {
    use super::{NoopBackend, NoopRepo, default_backend, open_repo};
    use crate::domain::{CommitId, DiffArea, DiffTarget, LogCursor, RepoSpec};
