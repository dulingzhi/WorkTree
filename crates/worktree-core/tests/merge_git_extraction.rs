...
//!
//! The default test runs against the worktree repository itself. An ignored
...
/// Find the root of the worktree repository (the repo we're building from).
fn find_worktree_repo() -> Option<PathBuf> {
...
/// Run extraction against worktree's own repository for real-world regression testing.
///
/// This test scans the most recent merge commits in the worktree repo, extracts
...
#[test]
fn extraction_regression_on_worktree_repo() {
    let Some(repo) = find_worktree_repo() else {
        eprintln!(
            "Skipping worktree extraction: could not find a host repository root from {}",
    ...
    let host_merges = match discover_merge_commits(&repo, 1) {
        ...
        Err(error) => {
            eprintln!(
                "Skipping worktree extraction: repository validation failed for {}: {}",
                repo.display(),
                error
            );
            return;
    ...
    if host_merges.is_empty() {
        eprintln!(
            "Skipping worktree extraction: no merge commits in {}",
            repo.display()
        );
        return;
    }
    ...
    eprintln!(
        "worktree extraction: {} total cases, {} clean, {} conflicts",
        total, clean, conflicts
    );
}

/// Run extraction against a large external repository for comprehensive
/// regression testing. Set the `WORKTREE_MERGE_EXTRACTION_REPO` environment
/// variable to the path of the repository to test against.
///
/// Example:
///   WORKTREE_MERGE_EXTRACTION_REPO=/home/user/git/linux cargo test \
///     --test merge_git_extraction extraction_regression_on_external_repo \
///     -- --ignored
#[test]
#[ignore]
fn extraction_regression_on_external_repo() {
    let repo_path = std::env::var("WORKTREE_MERGE_EXTRACTION_REPO").unwrap_or_else(|_| {
        panic!(
            "Set WORKTREE_MERGE_EXTRACTION_REPO to a git repo path to run this test.\n\
             Example: WORKTREE_MERGE_EXTRACTION_REPO=/home/user/git/linux"
        )
    });
...
///
/// Set `WORKTREE_MERGE_EXTRACTION_REPO` and `WORKTREE_MERGE_EXTRACTION_DEST`
/// to run. Generated fixtures are compatible with the existing fixture harness.
///
/// Example:
///   WORKTREE_MERGE_EXTRACTION_REPO=/home/user/git/linux \
///   WORKTREE_MERGE_EXTRACTION_DEST=crates/worktree-core/tests/fixtures/merge_extracted \
///   cargo test --test merge_git_extraction generate_fixtures_from_repo -- --ignored
#[test]
#[ignore]
fn generate_fixtures_from_repo() {
    let repo_path = std::env::var("WORKTREE_MERGE_EXTRACTION_REPO")
        .unwrap_or_else(|_| panic!("Set WORKTREE_MERGE_EXTRACTION_REPO to a git repo path"));
    let dest_path = std::env::var("WORKTREE_MERGE_EXTRACTION_DEST")
        .unwrap_or_else(|_| panic!("Set WORKTREE_MERGE_EXTRACTION_DEST to an output directory"));
