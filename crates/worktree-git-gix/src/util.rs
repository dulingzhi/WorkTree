    ...
    fn kind_env(&self) -> &'static str {
        match self {
            Self::Explicit(auth) => match auth.kind {
                GitAuthKind::UsernamePassword => WORKTREE_AUTH_KIND_USERNAME_PASSWORD,
                GitAuthKind::Passphrase => WORKTREE_AUTH_KIND_PASSPHRASE,
                GitAuthKind::HostVerification => WORKTREE_AUTH_KIND_HOST_VERIFICATION,
            },
            Self::CachedPassphrases(_) => WORKTREE_AUTH_KIND_PASSPHRASE_CACHED,
        }
    }
...
if [ -n "${WORKTREE_ASKPASS_PROMPT_LOG:-}" ]; then
  case "$lower_prompt" in
    *authenticity\ of\ host*|*continue\ connecting*|*yes/no*|*fingerprint*)
      printf '%s\n' "$prompt" >> "${WORKTREE_ASKPASS_PROMPT_LOG}" ;;
  esac
fi
if [ -n "${WORKTREE_ASKPASS_PASSPHRASE_PROMPT_LOG:-}" ]; then
  case "$lower_prompt" in
    *passphrase*)
      printf '%s\n' "$prompt" >> "${WORKTREE_ASKPASS_PASSPHRASE_PROMPT_LOG}" ;;
  esac
fi
kind="${WORKTREE_AUTH_KIND:-}"
if [ "$kind" = "username_password" ]; then
  case "$lower_prompt" in
    *username*) printf '%s\n' "${WORKTREE_AUTH_USERNAME:-}" ;;
    *) printf '%s\n' "${WORKTREE_AUTH_SECRET:-}" ;;
  esac
elif [ "$kind" = "passphrase_cached" ]; then
  cache_size="${WORKTREE_AUTH_CACHE_SIZE:-0}"
  i=0
  while [ "$i" -lt "$cache_size" ]; do
    cached_prompt=$(printenv "WORKTREE_AUTH_CACHE_PROMPT_$i")
    if [ "$prompt" = "$cached_prompt" ]; then
      printenv "WORKTREE_AUTH_CACHE_SECRET_$i"
      exit 0
    fi
    i=$((i + 1))
  done
...
set "prompt=%~1"
if not "%WORKTREE_ASKPASS_PROMPT_LOG%"=="" (
  echo %prompt% | findstr /I /C:"authenticity of host" /C:"continue connecting" /C:"yes/no" /C:"fingerprint" >nul
  if not errorlevel 1 (
    >>"%WORKTREE_ASKPASS_PROMPT_LOG%" echo %prompt%
  )
)
if not "%WORKTREE_ASKPASS_PASSPHRASE_PROMPT_LOG%"=="" (
  echo %prompt% | findstr /I "passphrase" >nul
  if not errorlevel 1 (
    >>"%WORKTREE_ASKPASS_PASSPHRASE_PROMPT_LOG%" echo %prompt%
  )
)
if /I "%WORKTREE_AUTH_KIND%"=="username_password" (
  echo %prompt% | findstr /I "username" >nul
  if not errorlevel 1 (
    echo %WORKTREE_AUTH_USERNAME%
    exit /b 0
  )
  echo %WORKTREE_AUTH_SECRET%
  exit /b 0
)
if /I "%WORKTREE_AUTH_KIND%"=="passphrase_cached" (
  set "cache_size=%WORKTREE_AUTH_CACHE_SIZE%"
  if "!cache_size!"=="" set "cache_size=0"
  set /a cache_last=!cache_size!-1
  if !cache_last! GEQ 0 (
    for /L %%i in (0,1,!cache_last!) do (
      call set "cached_prompt=%%WORKTREE_AUTH_CACHE_PROMPT_%%i%%"
      if "!prompt!"=="!cached_prompt!" (
        call set "cached_secret=%%WORKTREE_AUTH_CACHE_SECRET_%%i%%"
        echo !cached_secret!
        exit /b 0
      )
    )
...
if /I "%WORKTREE_AUTH_KIND%"=="host_verification" (
...
fn create_askpass_script() -> Result<AskPassScript> {
    ...
    #[cfg(windows)]
    let script_name = "worktree-askpass.cmd";
    #[cfg(not(windows))]
    let script_name = "worktree-askpass.sh";
    let path = dir.path().join(script_name);
    let host_prompt_log_path = dir.path().join("worktree-askpass-host-prompt.log");
    let passphrase_prompt_log_path = dir.path().join("worktree-askpass-passphrase-prompt.log");
...
fn configure_git_auth_prompt(
    ...
    cmd.env(
        WORKTREE_ASKPASS_PROMPT_LOG_ENV,
        &askpass.host_prompt_log_path,
    );
    cmd.env(
        WORKTREE_ASKPASS_PASSPHRASE_PROMPT_LOG_ENV,
        &askpass.passphrase_prompt_log_path,
    );
    if cfg!(all(unix, not(target_os = "macos"))) && std::env::var_os("DISPLAY").is_none() {
        cmd.env("DISPLAY", "worktree:0");
    }

    cmd.env(WORKTREE_AUTH_CACHE_SIZE_ENV, "0");
    if let Some(auth) = auth {
        match auth {
            PromptAuth::Explicit(_) => {
                cmd.env(WORKTREE_AUTH_KIND_ENV, auth.kind_env());
                if let Some(username) = auth.username() {
                    cmd.env(WORKTREE_AUTH_USERNAME_ENV, username);
                } else {
                    cmd.env_remove(WORKTREE_AUTH_USERNAME_ENV);
                }
                cmd.env(WORKTREE_AUTH_SECRET_ENV, auth.secret());
            }
            PromptAuth::CachedPassphrases(entries) => {
                cmd.env(WORKTREE_AUTH_KIND_ENV, auth.kind_env());
                cmd.env_remove(WORKTREE_AUTH_USERNAME_ENV);
                cmd.env_remove(WORKTREE_AUTH_SECRET_ENV);
                cmd.env(WORKTREE_AUTH_CACHE_SIZE_ENV, entries.len().to_string());
                for (idx, entry) in entries.iter().enumerate() {
                    cmd.env(
                        format!("{WORKTREE_AUTH_CACHE_PROMPT_ENV_PREFIX}{idx}"),
                        &entry.prompt,
                    );
                    cmd.env(
                        format!("{WORKTREE_AUTH_CACHE_SECRET_ENV_PREFIX}{idx}"),
                        &entry.secret,
                    );
                }
            }
        ...
        cmd.env_remove(WORKTREE_AUTH_KIND_ENV);
        cmd.env_remove(WORKTREE_AUTH_USERNAME_ENV);
        cmd.env_remove(WORKTREE_AUTH_SECRET_ENV);
    }
}
...
fn add_git_failure_hint(mut detail: String) -> String {
    ...
    {
        detail.push_str(
            "\n\nHint: Git could not complete GPG signing. WorkTree may be running with a GUI app PATH that differs from your shell PATH. If Git cannot find gpg, configure an absolute GPG path with `git config --global gpg.program /path/to/gpg`, or make gpg available on WorkTree's PATH.",
        );
    }
    detail
}
...
mod tests {
        ...
        match err.kind() {
            ErrorKind::Git(failure) => {
                ...
                assert!(failure.detail().is_some_and(|detail| {
                    detail.contains("set WORKTREE_GIT_COMMAND_TIMEOUT_SECS to override")
                }));
                assert!(
                    failure
                        .to_string()
    ...
    fn create_askpass_script_writes_expected_content_and_permissions() {
        ...
        assert!(contents.contains("WORKTREE_AUTH_SECRET"));
        assert!(contents.contains("WORKTREE_AUTH_KIND"));
    ...
    fn configure_git_auth_prompt_sets_username_password_env() {
        ...
        assert_eq!(
            command_env_value(&cmd, WORKTREE_ASKPASS_PROMPT_LOG_ENV).as_deref(),
            askpass.host_prompt_log_path.to_str()
        );
        assert_eq!(
            command_env_value(&cmd, WORKTREE_ASKPASS_PASSPHRASE_PROMPT_LOG_ENV).as_deref(),
            askpass.passphrase_prompt_log_path.to_str()
        );
        assert_eq!(
            command_env_value(&cmd, WORKTREE_AUTH_KIND_ENV).as_deref(),
            Some(WORKTREE_AUTH_KIND_USERNAME_PASSWORD)
        );
        assert_eq!(
            command_env_value(&cmd, WORKTREE_AUTH_USERNAME_ENV).as_deref(),
            Some("alice")
        );
        assert_eq!(
            command_env_value(&cmd, WORKTREE_AUTH_SECRET_ENV).as_deref(),
            Some("secret-token")
        );
        assert_eq!(
            command_env_value(&cmd, WORKTREE_AUTH_CACHE_SIZE_ENV).as_deref(),
            Some("0")
        );
    ...
    fn configure_git_auth_prompt_sets_passphrase_env_and_removes_username() {
        let askpass = create_askpass_script().expect("askpass script creation");
        let mut cmd = Command::new("git");
        cmd.env(WORKTREE_AUTH_USERNAME_ENV, "legacy-user");
        let auth = PromptAuth::Explicit(StagedGitAuth {
        ...
        assert_eq!(
            command_env_value(&cmd, WORKTREE_AUTH_KIND_ENV).as_deref(),
            Some(WORKTREE_AUTH_KIND_PASSPHRASE)
        );
        assert!(command_env_removed(&cmd, WORKTREE_AUTH_USERNAME_ENV));
        assert_eq!(
            command_env_value(&cmd, WORKTREE_AUTH_SECRET_ENV).as_deref(),
            Some("ssh-passphrase")
        );
    ...
    fn configure_git_auth_prompt_sets_cached_passphrase_env_and_removes_username() {
        ...
        assert_eq!(
            command_env_value(&cmd, WORKTREE_AUTH_KIND_ENV).as_deref(),
            Some(WORKTREE_AUTH_KIND_PASSPHRASE_CACHED)
        );
        assert!(command_env_removed(&cmd, WORKTREE_AUTH_USERNAME_ENV));
        assert!(command_env_removed(&cmd, WORKTREE_AUTH_SECRET_ENV));
        assert_eq!(
            command_env_value(&cmd, WORKTREE_AUTH_CACHE_SIZE_ENV).as_deref(),
            Some("2")
        );
        assert_eq!(
            command_env_value(&cmd, "WORKTREE_AUTH_CACHE_PROMPT_0").as_deref(),
            Some("Enter passphrase for key '/tmp/key-a':")
        );
        assert_eq!(
            command_env_value(&cmd, "WORKTREE_AUTH_CACHE_SECRET_0").as_deref(),
            Some("ssh-passphrase-a")
        );
        assert_eq!(
            command_env_value(&cmd, "WORKTREE_AUTH_CACHE_PROMPT_1").as_deref(),
            Some("Enter passphrase for key '/tmp/key-b':")
        );
        assert_eq!(
            command_env_value(&cmd, "WORKTREE_AUTH_CACHE_SECRET_1").as_deref(),
    ...
    fn configure_git_auth_prompt_sets_host_verification_env_and_removes_username() {
        ...
        assert_eq!(
            command_env_value(&cmd, WORKTREE_AUTH_KIND_ENV).as_deref(),
            Some(WORKTREE_AUTH_KIND_HOST_VERIFICATION)
        );
        assert!(command_env_removed(&cmd, WORKTREE_AUTH_USERNAME_ENV));
        assert_eq!(
            command_env_value(&cmd, WORKTREE_AUTH_SECRET_ENV).as_deref(),
    ...
    fn configure_git_auth_prompt_without_staged_auth_clears_auth_env() {
        let askpass = create_askpass_script().expect("askpass script creation");
        let mut cmd = Command::new("git");
        cmd.env(WORKTREE_AUTH_KIND_ENV, "legacy-kind");
        cmd.env(WORKTREE_AUTH_USERNAME_ENV, "legacy-user");
        cmd.env(WORKTREE_AUTH_SECRET_ENV, "legacy-secret");
        ...
        assert!(command_env_removed(&cmd, WORKTREE_AUTH_KIND_ENV));
        assert!(command_env_removed(&cmd, WORKTREE_AUTH_USERNAME_ENV));
        assert!(command_env_removed(&cmd, WORKTREE_AUTH_SECRET_ENV));
    }
