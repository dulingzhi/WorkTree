...
fn git_range_status_changes(
    ...
    while let Some(header) = next_non_empty_nul_field(&mut fields) {
        ...
        let kind = match status_code {
            b'A' | b'C' => worktree_core::domain::FileStatusKind::Added,
            b'D' => worktree_core::domain::FileStatusKind::Deleted,
            b'R' => worktree_core::domain::FileStatusKind::Renamed,
            b'U' => worktree_core::domain::FileStatusKind::Conflicted,
            _ => worktree_core::domain::FileStatusKind::Modified,
        };
    ...
    let clone_only_state = match failed_submodule_add_left_clone_only_state(workdir, path) {
        ...
        Err(probe_err) => {
            return append_failed_submodule_add_note(
                err,
                &format!("WorkTree could not inspect failed submodule add state: {probe_err}"),
            );
        }
    };
...
fn submodule_file_transport_consent_key(trust_root: &Path, source_path: &Path) -> String {
    ...
    format!(
        "worktree.submodule.allowfiletransport-{:016x}",
        fnv1a_64(&bytes)
    )
}
...
mod tests {
    ...
    fn git_config_failure(stderr: &str) -> Error {
        Error::new(ErrorKind::Git(GitFailure::new(
            "git config --global worktree.submodule.allowfiletransport-example true",
    ...
    fn git_config_contention_retry_does_not_retry_unrelated_failures() {
        ...
        assert_eq!(
            err.to_string(),
            "git config --global worktree.submodule.allowfiletransport-example true failed"
        );
    }
