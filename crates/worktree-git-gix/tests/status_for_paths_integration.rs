...
use worktree_core::services::{GitBackend, StatusForPaths};
...
fn fixture_repo(tag: &str) -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "worktree_gix_status_paths_{}_{}_{}",
        tag,
        std::process::id(),
...
fn kinds_for(status: &StatusForPaths, path: &str) -> Vec<FileStatusKind> {
    let (unstaged, staged): (&[worktree_core::domain::FileStatus], _) = match status {
...
fn status_for_paths_agrees_with_the_full_scan_on_covered_paths() {
    ...
    let full_for = |entries: &[worktree_core::domain::FileStatus]| {
