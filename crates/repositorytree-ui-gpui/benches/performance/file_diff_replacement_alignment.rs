...
pub(crate) fn bench_file_diff_replacement_alignment(c: &mut Criterion) {
    let blocks = env_usize("REPOSITORYTREE_BENCH_REPLACEMENT_BLOCKS", 12);
    let balanced_lines = env_usize("REPOSITORYTREE_BENCH_REPLACEMENT_BALANCED_LINES", 48);
    let skewed_old_lines = env_usize("REPOSITORYTREE_BENCH_REPLACEMENT_SKEW_OLD_LINES", 40);
    let skewed_new_lines = env_usize("REPOSITORYTREE_BENCH_REPLACEMENT_SKEW_NEW_LINES", 56);
    let context_lines = env_usize("REPOSITORYTREE_BENCH_REPLACEMENT_CONTEXT_LINES", 3);
    let line_bytes = env_usize("REPOSITORYTREE_BENCH_REPLACEMENT_LINE_BYTES", 128);
