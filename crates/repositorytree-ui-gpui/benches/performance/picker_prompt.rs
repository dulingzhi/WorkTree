...
pub(crate) fn bench_picker_prompt(c: &mut Criterion) {
    let scales = [
        env_usize("REPOSITORYTREE_BENCH_PICKER_REFS_SMALL", 20),
        env_usize("REPOSITORYTREE_BENCH_PICKER_REFS_TYPICAL", 200),
        env_usize("REPOSITORYTREE_BENCH_PICKER_REFS_LARGE", 1_200),
    ];
    let worktrees = env_usize("REPOSITORYTREE_BENCH_PICKER_WORKTREES", 8);
