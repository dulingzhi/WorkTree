...
pub(crate) fn bench_diff_open_svg_dual_path_first_window(c: &mut Criterion) {
    let shapes = env_usize("REPOSITORYTREE_BENCH_SVG_SHAPES", 200);
    let fallback_bytes = env_usize("REPOSITORYTREE_BENCH_SVG_FALLBACK_BYTES", 64 * 1024);
    let window = env_usize("REPOSITORYTREE_BENCH_SVG_WINDOW", 200);
