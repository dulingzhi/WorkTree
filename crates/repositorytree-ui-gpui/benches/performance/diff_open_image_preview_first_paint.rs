...
pub(crate) fn bench_diff_open_image_preview_first_paint(c: &mut Criterion) {
    let old_bytes = env_usize("REPOSITORYTREE_BENCH_IMAGE_PREVIEW_OLD_BYTES", 256 * 1024);
    let new_bytes = env_usize("REPOSITORYTREE_BENCH_IMAGE_PREVIEW_NEW_BYTES", 384 * 1024);
