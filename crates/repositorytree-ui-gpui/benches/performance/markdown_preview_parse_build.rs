...
pub(crate) fn bench_markdown_preview_parse_build(c: &mut Criterion) {
    {
        let medium_sections = env_usize("REPOSITORYTREE_BENCH_MARKDOWN_PREVIEW_MEDIUM_SECTIONS", 256);
        let large_sections = env_usize("REPOSITORYTREE_BENCH_MARKDOWN_PREVIEW_LARGE_SECTIONS", 768);
        let line_bytes = env_usize("REPOSITORYTREE_BENCH_MARKDOWN_PREVIEW_LINE_BYTES", 128);
