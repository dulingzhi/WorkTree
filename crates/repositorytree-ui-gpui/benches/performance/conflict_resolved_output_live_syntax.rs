...
pub(crate) fn bench_conflict_resolved_output_live_syntax(c: &mut Criterion) {
    let lines = env_usize("REPOSITORYTREE_BENCH_LIVE_SYNTAX_LINES", 20_000);
    let conflicts = env_usize("REPOSITORYTREE_BENCH_LIVE_SYNTAX_CONFLICTS", 8);
    let window = env_usize("REPOSITORYTREE_BENCH_LIVE_SYNTAX_WINDOW", 60);
