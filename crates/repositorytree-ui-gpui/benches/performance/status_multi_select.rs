...
pub(crate) fn bench_status_multi_select(c: &mut Criterion) {
    let entries = env_usize("REPOSITORYTREE_BENCH_STATUS_MULTI_SELECT_ENTRIES", 20_000);
    let anchor_index = env_usize("REPOSITORYTREE_BENCH_STATUS_MULTI_SELECT_ANCHOR", 4_096);
    let selected_paths = env_usize("REPOSITORYTREE_BENCH_STATUS_MULTI_SELECT_RANGE", 512);
