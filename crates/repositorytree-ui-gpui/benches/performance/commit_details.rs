...
pub(crate) fn bench_commit_details(c: &mut Criterion) {
    let files = env_usize("REPOSITORYTREE_BENCH_COMMIT_FILES", 5_000);
    let depth = env_usize("REPOSITORYTREE_BENCH_COMMIT_PATH_DEPTH", 4);
    let deep_depth = env_usize(
        "REPOSITORYTREE_BENCH_COMMIT_DEEP_PATH_DEPTH",
        depth.saturating_mul(4).max(12),
    );
    let huge_files = env_usize("REPOSITORYTREE_BENCH_COMMIT_HUGE_FILES", files.saturating_mul(2));
    let large_message_files = env_usize("REPOSITORYTREE_BENCH_COMMIT_LARGE_MESSAGE_FILES", files.max(1));
    let large_message_depth = env_usize("REPOSITORYTREE_BENCH_COMMIT_LARGE_MESSAGE_DEPTH", depth.max(1));
    let large_message_bytes = env_usize("REPOSITORYTREE_BENCH_COMMIT_LARGE_MESSAGE_BYTES", 96 * 1024);
    let large_message_line_bytes = env_usize("REPOSITORYTREE_BENCH_COMMIT_LARGE_MESSAGE_LINE_BYTES", 192);
    let large_message_visible_lines =
        env_usize("REPOSITORYTREE_BENCH_COMMIT_LARGE_MESSAGE_VISIBLE_LINES", 48);
    let large_message_wrap_width_px =
        env_usize("REPOSITORYTREE_BENCH_COMMIT_LARGE_MESSAGE_WRAP_WIDTH_PX", 560);
    let extreme_files = env_usize("REPOSITORYTREE_BENCH_COMMIT_EXTREME_FILES", 10_000);
    let extreme_depth = env_usize("REPOSITORYTREE_BENCH_COMMIT_EXTREME_DEPTH", 12);
    ...
    let churn_files = env_usize("REPOSITORYTREE_BENCH_PATH_CHURN_FILES", 10_000);
    let churn_depth = env_usize("REPOSITORYTREE_BENCH_PATH_CHURN_DEPTH", 6);
