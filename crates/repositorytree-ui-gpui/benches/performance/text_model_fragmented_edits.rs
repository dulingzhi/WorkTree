...
pub(crate) fn bench_text_model_fragmented_edits(c: &mut Criterion) {
    let bytes = env_usize("REPOSITORYTREE_BENCH_TEXT_MODEL_BYTES", 512 * 1024);
    let edits = env_usize("REPOSITORYTREE_BENCH_TEXT_MODEL_EDITS", 500);
    let reads = env_usize("REPOSITORYTREE_BENCH_TEXT_MODEL_READS_AFTER_EDIT", 64);
