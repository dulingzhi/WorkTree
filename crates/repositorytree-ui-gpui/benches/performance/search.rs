...
pub(crate) fn bench_search(c: &mut Criterion) {
    let commits = env_usize("REPOSITORYTREE_BENCH_SEARCH_COMMITS", 50_000);
    let diff_lines = env_usize("REPOSITORYTREE_BENCH_SEARCH_DIFF_LINES", 100_000);
    let file_preview_lines = env_usize("REPOSITORYTREE_BENCH_SEARCH_FILE_PREVIEW_LINES", 100_000);
    let file_diff_lines = env_usize("REPOSITORYTREE_BENCH_SEARCH_FILE_DIFF_LINES", 100_000);
    let file_diff_window = env_usize("REPOSITORYTREE_BENCH_SEARCH_FILE_DIFF_WINDOW", 200);
    let fuzzy_files = env_usize("REPOSITORYTREE_BENCH_SEARCH_FUZZY_FILES", 100_000);
    ...
    let author_query =
        env_string("REPOSITORYTREE_BENCH_SEARCH_AUTHOR_QUERY").unwrap_or_else(|| "Alice".to_string());
    // Message query: "fix" matches ~10% of commits (1 of 10 prefixes).
    let message_query =
        env_string("REPOSITORYTREE_BENCH_SEARCH_MESSAGE_QUERY").unwrap_or_else(|| "fix".to_string());
    ...
    let diff_query = env_string("REPOSITORYTREE_BENCH_SEARCH_DIFF_QUERY")
        .unwrap_or_else(|| "render_cache".to_string());
    let diff_refined_query = env_string("REPOSITORYTREE_BENCH_SEARCH_DIFF_REFINED_QUERY")
        .unwrap_or_else(|| "render_cache_hot_path".to_string());
    let diff_refinement_matches = diff_fixture.prepare_matches(&diff_query);
    let file_preview_query = env_string("REPOSITORYTREE_BENCH_SEARCH_FILE_PREVIEW_QUERY")
        .unwrap_or_else(|| "render_cache".to_string());
    let file_diff_query = env_string("REPOSITORYTREE_BENCH_SEARCH_FILE_DIFF_QUERY")
        .unwrap_or_else(|| "render_cache_hot_path".to_string());
    ...
    let fuzzy_query =
        env_string("REPOSITORYTREE_BENCH_SEARCH_FUZZY_QUERY").unwrap_or_else(|| "dcrs".to_string());
    let fuzzy_short_query =
        env_string("REPOSITORYTREE_BENCH_SEARCH_FUZZY_SHORT_QUERY").unwrap_or_else(|| "dc".to_string());
