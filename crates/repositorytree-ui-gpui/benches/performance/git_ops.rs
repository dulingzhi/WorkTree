...
pub(crate) fn bench_git_ops(c: &mut Criterion) {
    let status_files = env_usize("REPOSITORYTREE_BENCH_GIT_STATUS_FILES", 1_000);
    let status_dirty_files = env_usize("REPOSITORYTREE_BENCH_GIT_STATUS_DIRTY_FILES", 500);
    let log_commits = env_usize("REPOSITORYTREE_BENCH_GIT_LOG_COMMITS", 10_000);
    let log_shallow_total_commits =
        env_usize("REPOSITORYTREE_BENCH_GIT_LOG_SHALLOW_TOTAL_COMMITS", 100_000);
    let log_shallow_requested_commits =
        env_usize("REPOSITORYTREE_BENCH_GIT_LOG_SHALLOW_REQUESTED_COMMITS", 200);
    let status_clean_files = env_usize("REPOSITORYTREE_BENCH_GIT_STATUS_CLEAN_FILES", 10_000);
    let ref_count = env_usize("REPOSITORYTREE_BENCH_GIT_REF_COUNT", 10_000);
    let diff_rename_files = env_usize("REPOSITORYTREE_BENCH_GIT_DIFF_RENAME_FILES", 256);
    let diff_binary_files = env_usize("REPOSITORYTREE_BENCH_GIT_DIFF_BINARY_FILES", 128);
    let diff_binary_bytes = env_usize("REPOSITORYTREE_BENCH_GIT_DIFF_BINARY_BYTES", 4_096);
    let diff_large_file_lines = env_usize("REPOSITORYTREE_BENCH_GIT_DIFF_LARGE_FILE_LINES", 100_000);
    let diff_large_file_line_bytes = env_usize("REPOSITORYTREE_BENCH_GIT_DIFF_LARGE_FILE_LINE_BYTES", 48);
    let blame_large_file_lines = env_usize("REPOSITORYTREE_BENCH_GIT_BLAME_LINES", 100_000);
    let blame_large_file_commits = env_usize("REPOSITORYTREE_BENCH_GIT_BLAME_COMMITS", 16);
    let file_history_total_commits =
        env_usize("REPOSITORYTREE_BENCH_GIT_FILE_HISTORY_TOTAL_COMMITS", 100_000);
    let file_history_requested_commits =
        env_usize("REPOSITORYTREE_BENCH_GIT_FILE_HISTORY_REQUESTED_COMMITS", 200);
    let file_history_touch_every = env_usize("REPOSITORYTREE_BENCH_GIT_FILE_HISTORY_TOUCH_EVERY", 10);

    let log_author_filter_total_commits =
        env_usize("REPOSITORYTREE_BENCH_GIT_LOG_AUTHOR_FILTER_COMMITS", 100_000);
    let log_author_filter_requested_commits =
        env_usize("REPOSITORYTREE_BENCH_GIT_LOG_AUTHOR_FILTER_REQUESTED", 200);
    ...
    let log_author_filter_rare_every =
        env_usize("REPOSITORYTREE_BENCH_GIT_LOG_AUTHOR_FILTER_EVERY", 2_000);
