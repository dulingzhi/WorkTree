...
pub(crate) fn bench_markdown_preview_scroll(c: &mut Criterion) {
    let sections = env_usize("REPOSITORYTREE_BENCH_MARKDOWN_PREVIEW_SCROLL_SECTIONS", 768);
    let window = env_usize("REPOSITORYTREE_BENCH_MARKDOWN_PREVIEW_WINDOW", 200);
    let scroll_step_rows = env_usize("REPOSITORYTREE_BENCH_MARKDOWN_PREVIEW_SCROLL_STEP", 24).max(1);
