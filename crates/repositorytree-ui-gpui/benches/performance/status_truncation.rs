...
pub(crate) fn bench_status_truncation(c: &mut Criterion) {
    let entries_per_section = env_usize("REPOSITORYTREE_BENCH_STATUS_TRUNCATION_SECTION_ENTRIES", 240);
    let max_width_px = env_usize("REPOSITORYTREE_BENCH_STATUS_TRUNCATION_WIDTH_PX", 180) as f32;
