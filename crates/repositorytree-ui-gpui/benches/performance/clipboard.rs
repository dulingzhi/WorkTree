...
pub(crate) fn bench_clipboard(c: &mut Criterion) {
    let copy_lines = env_usize("REPOSITORYTREE_BENCH_CLIPBOARD_COPY_LINES", 10_000);
    let paste_lines = env_usize("REPOSITORYTREE_BENCH_CLIPBOARD_PASTE_LINES", 2_000);
    let paste_line_bytes = env_usize("REPOSITORYTREE_BENCH_CLIPBOARD_PASTE_LINE_BYTES", 96);
    let select_total_lines = env_usize("REPOSITORYTREE_BENCH_CLIPBOARD_SELECT_TOTAL_LINES", 10_000);
    let select_range_lines = env_usize("REPOSITORYTREE_BENCH_CLIPBOARD_SELECT_RANGE_LINES", 5_000);
