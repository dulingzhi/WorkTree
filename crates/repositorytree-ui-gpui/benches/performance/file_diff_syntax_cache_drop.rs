...
pub(crate) fn bench_file_diff_syntax_cache_drop(c: &mut Criterion) {
    let lines = env_usize("REPOSITORYTREE_BENCH_FILE_DIFF_SYNTAX_DROP_LINES", 2_048);
    let tokens_per_line = env_usize("REPOSITORYTREE_BENCH_FILE_DIFF_SYNTAX_DROP_TOKENS_PER_LINE", 8);
    let replacements = env_usize("REPOSITORYTREE_BENCH_FILE_DIFF_SYNTAX_DROP_REPLACEMENTS", 4);
