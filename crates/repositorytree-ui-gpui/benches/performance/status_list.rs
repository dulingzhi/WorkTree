...
pub(crate) fn bench_status_list(c: &mut Criterion) {
    let entries = env_usize("REPOSITORYTREE_BENCH_STATUS_ENTRIES", 10_000);
    let window = env_usize("REPOSITORYTREE_BENCH_STATUS_WINDOW", 200);
    let mixed_depth_entries = env_usize("REPOSITORYTREE_BENCH_STATUS_MIXED_DEPTH_ENTRIES", 20_000);
    let mixed_depth_prewarm = env_usize("REPOSITORYTREE_BENCH_STATUS_MIXED_DEPTH_PREWARM", 8_193);
