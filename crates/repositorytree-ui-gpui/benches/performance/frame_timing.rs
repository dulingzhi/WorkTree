...
pub(crate) fn bench_frame_timing(c: &mut Criterion) {
    let history_commits = env_usize("REPOSITORYTREE_BENCH_FRAME_HISTORY_COMMITS", 50_000);
    let history_local_branches = env_usize("REPOSITORYTREE_BENCH_FRAME_HISTORY_LOCAL_BRANCHES", 400);
    let history_remote_branches = env_usize("REPOSITORYTREE_BENCH_FRAME_HISTORY_REMOTE_BRANCHES", 1_200);
    let history_window = env_usize("REPOSITORYTREE_BENCH_FRAME_HISTORY_WINDOW", 120);
    let history_scroll_step = env_usize("REPOSITORYTREE_BENCH_FRAME_HISTORY_SCROLL_STEP", 24);
    let diff_lines = env_usize("REPOSITORYTREE_BENCH_FRAME_DIFF_LINES", 100_000);
    let diff_window = env_usize("REPOSITORYTREE_BENCH_FRAME_DIFF_WINDOW", 200);
    let diff_line_bytes = env_usize("REPOSITORYTREE_BENCH_FRAME_DIFF_LINE_BYTES", 96);
    let diff_scroll_step = env_usize("REPOSITORYTREE_BENCH_FRAME_DIFF_SCROLL_STEP", 40);
    let frames = env_usize("REPOSITORYTREE_BENCH_FRAME_TIMING_FRAMES", 240);
    let frame_budget_ns =
        u64::try_from(env_usize("REPOSITORYTREE_BENCH_FRAME_BUDGET_NS", 16_666_667)).unwrap_or(u64::MAX);
    ...
    let sidebar_drag_frames = env_usize("REPOSITORYTREE_BENCH_FRAME_SIDEBAR_DRAG_FRAMES", 240);
    group.bench_function(
    ...
    let rapid_commit_count = env_usize("REPOSITORYTREE_BENCH_FRAME_RAPID_COMMIT_COUNT", 120);
    let rapid_commit_files = env_usize("REPOSITORYTREE_BENCH_FRAME_RAPID_COMMIT_FILES", 200);
    ...
    let switch_every = env_usize("REPOSITORYTREE_BENCH_FRAME_SWITCH_EVERY_N_FRAMES", 30);
