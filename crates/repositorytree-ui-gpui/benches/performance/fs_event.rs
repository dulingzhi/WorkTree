...
pub(crate) fn bench_fs_event(c: &mut Criterion) {
    let tracked_files = env_usize("REPOSITORYTREE_BENCH_FS_EVENT_TRACKED_FILES", 1_000);
    let checkout_files = env_usize("REPOSITORYTREE_BENCH_FS_EVENT_CHECKOUT_FILES", 200);
    let rapid_save_count = env_usize("REPOSITORYTREE_BENCH_FS_EVENT_RAPID_SAVES", 50);
    let churn_files = env_usize("REPOSITORYTREE_BENCH_FS_EVENT_CHURN_FILES", 100);
