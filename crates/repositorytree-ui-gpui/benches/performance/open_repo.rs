...
pub(crate) fn bench_open_repo(c: &mut Criterion) {
    ...
    let branch_heavy_local_branches = env_usize(
        "REPOSITORYTREE_BENCH_BRANCH_HEAVY_LOCAL_BRANCHES",
        local_branches.saturating_mul(6),
    );
    let branch_heavy_remote_branches = env_usize(
        "REPOSITORYTREE_BENCH_BRANCH_HEAVY_REMOTE_BRANCHES",
        remote_branches.saturating_mul(4),
    );
    let branch_heavy_remotes = env_usize("REPOSITORYTREE_BENCH_BRANCH_HEAVY_REMOTES", remotes.max(8));
    let extreme_fanout_commits = env_usize("REPOSITORYTREE_BENCH_OPEN_REPO_EXTREME_COMMITS", 1_000);
    let extreme_fanout_local_branches =
        env_usize("REPOSITORYTREE_BENCH_OPEN_REPO_EXTREME_LOCAL_BRANCHES", 1_000);
    let extreme_fanout_remote_branches =
        env_usize("REPOSITORYTREE_BENCH_OPEN_REPO_EXTREME_REMOTE_BRANCHES", 10_000);
    let extreme_fanout_remotes = env_usize("REPOSITORYTREE_BENCH_OPEN_REPO_EXTREME_REMOTES", 1);
    let extreme_fanout_worktrees = env_usize("REPOSITORYTREE_BENCH_OPEN_REPO_EXTREME_WORKTREES", 1);
    let extreme_fanout_submodules = env_usize("REPOSITORYTREE_BENCH_OPEN_REPO_EXTREME_SUBMODULES", 259);
    let extreme_fanout_tags = env_usize("REPOSITORYTREE_BENCH_OPEN_REPO_EXTREME_TAGS", 37_610);
