...
pub(crate) fn bench_file_diff_inline_syntax_projection(c: &mut Criterion) {
    let lines = env_usize("REPOSITORYTREE_BENCH_FILE_DIFF_INLINE_LINES", 4_000);
    let line_bytes = env_usize("REPOSITORYTREE_BENCH_FILE_DIFF_INLINE_LINE_BYTES", 128);
    let window = env_usize("REPOSITORYTREE_BENCH_FILE_DIFF_INLINE_WINDOW", 200);
