...
pub(crate) const DEFAULT_DARK_THEME_KEY: &str = "repositorytree_dark";
pub(crate) const DEFAULT_LIGHT_THEME_KEY: &str = "repositorytree_light";
...
impl AppTheme {
    ...
    /// RepositoryTree's default dark theme loaded from an embedded JSON definition.
    pub fn repositorytree_dark() -> Self {
        Self::from_key(DEFAULT_DARK_THEME_KEY)
            .unwrap_or_else(|| panic!("missing default dark theme `{DEFAULT_DARK_THEME_KEY}`"))
    }

    /// RepositoryTree's default light theme loaded from an embedded JSON definition.
    #[cfg(test)]
    pub fn repositorytree_light() -> Self {
...
fn resolved_runtime_themes_dir(runtime_dir: Option<&Path>) -> Option<PathBuf> {
    let dir = match runtime_dir {
        ...
        None => repositorytree_state::session::user_themes_dir()?,
    };
...
/// instead would hand every such theme repositorytree's hand-picked lane colours and
...
mod tests {
    ...
    /// wild with repositorytree's own lane colours.
    #[test]
    ...
    fn bundled_themes_keep_the_canvas_and_chrome_hierarchy_for_their_appearance() {
        assert_eq!(
            AppTheme::repositorytree_light().colors.surface.canvas,
            gpui::rgba(0xffffffff),
            "RepositoryTree Light should keep its pure-white canvas"
        );
        assert_eq!(
        ...
        for key in ["repositorytree_light", "sunset_veil"] {
        ...
        for key in ["repositorytree_dark", "tokyo_night"] {
    ...
    fn embedded_theme_registry_exposes_default_keys() {
        ...
        assert_eq!(
            ...
            Some("RepositoryTree Dark".to_string())
        );
        assert_eq!(
            theme_label(DEFAULT_LIGHT_THEME_KEY),
            Some("RepositoryTree Light".to_string())
        );
    }
    ...
    fn runtime_theme_dir_ignores_reserved_system_theme_filenames() {
        let dir = tempdir().expect("temp dir should exist");
        fs::write(dir.path().join("repositorytree.json"), "not parsed")
    ...
    fn runtime_theme_dir_ignores_embedded_theme_key_collisions_but_keeps_custom_entries() {
        ...
        collision["name"] = json!("Fake RepositoryTree Dark");
    ...
    fn themes_markdown_documents_custom_theme_override_rules() {
        ...
        for snippet in [
            "RepositoryTree creates the user themes directory on startup",
