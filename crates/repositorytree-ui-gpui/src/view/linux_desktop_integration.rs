...
impl RepositoryTreeView {
    ...
    pub(in crate::view) fn maybe_auto_install_linux_desktop_integration(
        ...
        if !should_auto_install_linux_desktop_integration(
            std::env::var_os("REPOSITORYTREE_NO_DESKTOP_INSTALL").is_some(),
            desktop.as_deref(),
        ) {
            return;
        }
        ...
        let desktop_path = data_home.join("applications/repositorytree.desktop");
        let all_icons_exist = ICON_SIZES.iter().all(|size| {
            data_home
                .join(format!("icons/hicolor/{size}x{size}/apps/repositorytree.png"))
                .exists()
        });
                ...
                let result: Result<(std::path::PathBuf, std::path::PathBuf), String> =
                    smol::unblock(|| {
                        ...
                        const DESKTOP_TEMPLATE: &str = include_str!(concat!(
                            ...
                            "/../../assets/linux/repositorytree.desktop"
                        ));
                        const ICON_32_PNG: &[u8] = include_bytes!(concat!(
                            env!("CARGO_MANIFEST_DIR"),
                            "/../../assets/linux/hicolor/32x32/apps/repositorytree.png"
                        ));
                        const ICON_48_PNG: &[u8] = include_bytes!(concat!(
                            env!("CARGO_MANIFEST_DIR"),
                            "/../../assets/linux/hicolor/48x48/apps/repositorytree.png"
                        ));
                        const ICON_128_PNG: &[u8] = include_bytes!(concat!(
                            env!("CARGO_MANIFEST_DIR"),
                            "/../../assets/linux/hicolor/128x128/apps/repositorytree.png"
                        ));
                        const ICON_256_PNG: &[u8] = include_bytes!(concat!(
                            env!("CARGO_MANIFEST_DIR"),
                            "/../../assets/linux/hicolor/256x256/apps/repositorytree.png"
                        ));
                        const ICON_512_PNG: &[u8] = include_bytes!(concat!(
                            env!("CARGO_MANIFEST_DIR"),
                            "/../../assets/linux/hicolor/512x512/apps/repositorytree.png"
                        ));
                        ...
                        let desktop_path = applications_dir.join("repositorytree.desktop");
                        let icon_path = icons_root.join("512x512/apps/repositorytree.png");
                        ...
                        for (size, icon_bytes) in ICON_ASSETS {
                            ...
                            let icon_file = icon_dir.join("repositorytree.png");
...
mod tests {
    ...
    fn desktop_exec_path_is_quoted() {
        let escaped =
            desktop_entry_exec_path_arg(Path::new("/usr/local/bin/repositorytree")).expect("path");
        assert_eq!(escaped, "\"/usr/local/bin/repositorytree\"");
    }
