...
use crate::ui_scale;
use repositorytree_core::domain::HistoryMode;
use repositorytree_core::process::{
    GitExecutablePreference, GitRuntimeState, install_git_executable_path, refresh_git_runtime,
};
use repositorytree_state::model::{DefaultTagType, GitLogTagFetchMode};
use repositorytree_state::session::ExternalCodeEditorSetting;
use gpui::{
...
#[cfg(test)]
const SETTINGS_WINDOW_TITLE: &str = "Settings: RepositoryTree";
const SETTINGS_TRAFFIC_LIGHTS_SAFE_INSET_PX: f32 = 78.0;
const MIN_GIT_MAJOR: u32 = 2;
const MIN_GIT_MINOR: u32 = 50;
const GITHUB_URL: &str = "https://github.com/RepositoryTree/RepositoryTree";
const THEMES_GUIDE_URL: &str = "https://github.com/RepositoryTree/RepositoryTree/blob/main/docs/themes.md";
const LICENSE_URL: &str = "https://github.com/RepositoryTree/RepositoryTree/blob/main/LICENSE-AGPL-3.0";
...
fn settings_window_options_for_scale(
    ...
    WindowOptions {
        ...
        app_id: Some("repositorytree-settings".into()),
...
impl SettingsWindowView {
    ...
    fn preferred_terminal_launch_context(
        ...
        for handle in cx
            .windows()
            .into_iter()
            .filter_map(|window| window.downcast::<RepositoryTreeView>())
        {
    ...
    fn update_main_windows(
        &self,
        cx: &mut gpui::Context<Self>,
        f: impl FnMut(&mut RepositoryTreeView, &mut Window, &mut gpui::Context<RepositoryTreeView>) + 'static,
    ) {
            ...
            .child(
                self.card("settings_window_overflow_probe_card", "Overflow probe", theme)
                    ...
                    .child(self.link_row(
                        ...
                        "https://github.com/RepositoryTree/RepositoryTree/releases/tag/settings-overflow-regression"
                            .into(),
                        theme,
                    ))
                    ...
                    let links_card = self
                        ...
                        .child(
                            self.link_row(
                                ...
                                "GitHub",
                                "RepositoryTree/RepositoryTree".into(),
                                theme,
                            )
                            .on_click(|_, _, cx| {
...
impl SettingsRuntimeInfo {
    ...
    fn from_runtime(runtime: GitRuntimeState) -> Self {
        Self {
            ...
            app_version_display: format!("RepositoryTree v{}", env!("CARGO_PKG_VERSION")).into(),
...
mod tests {
    ...
    const DIFF_DEFAULTS_SESSION_SUBTEST_ENV: &str = "REPOSITORYTREE_DIFF_DEFAULTS_SESSION_SUBTEST";
    ...
    fn unique_session_file(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "repositorytree-settings-window-{label}-{}-{}",
            std::process::id(),
            SystemTime::now()
    ...
    fn settings_dropdown_background_is_darker_than_card_surface() {
        ...
        let dark = AppTheme::repositorytree_dark();
        assert!(
        ...
        let light = AppTheme::repositorytree_light();
        assert!(
    ...
    fn browsed_external_editor_path_updates_custom_setting_and_notifies(
        ...
        let editor_path = PathBuf::from("/tmp/repositorytree-custom-editor");
        let _ = settings_window.update(&mut settings_cx, |settings, _window, cx| {
    ...
    fn external_editor_setting_seeds_from_pending_override_and_can_clear(
        ...
        let pending_setting = ExternalCodeEditorSetting::Custom {
            executable: PathBuf::from("/tmp/repositorytree-pending-editor"),
    ...
    fn external_editor_preference_persist_queue_skips_stale_custom_draft_writes() {
        ...
        let loaded = repositorytree_state::session::load_from_path(&session_file);
    ...
    fn settings_window_rows_clamp_under_lilex_at_minimum_width(cx: &mut gpui::TestAppContext) {
        ...
        let _ = settings_window.update(&mut settings_cx, |settings, _window, cx| {
            ...
            settings.runtime_info.app_version_display =
                "RepositoryTree v0.0.0-overflow-regression-build".into();
    ...
    fn settings_window_containers_fill_available_width_when_content_wraps(
        ...
        let _ = settings_window.update(&mut settings_cx, |settings, _window, cx| {
            ...
            settings.git_executable_mode = GitExecutableMode::Custom;
            settings.runtime_info.app_version_display =
                "RepositoryTree v0.0.0-overflow-regression-build-with-extra-layout-metadata".into();
            settings.runtime_info.operating_system =
    ...
    fn show_timezone_toggle_defers_main_window_update(cx: &mut gpui::TestAppContext) {
        ...
        assert!(
            result.is_ok(),
            "settings window toggle should not re-enter RepositoryTreeView updates"
        );
    ...
    fn change_tracking_setting_defers_main_window_update(cx: &mut gpui::TestAppContext) {
        ...
        assert!(
            result.is_ok(),
            "change tracking update should not re-enter RepositoryTreeView updates"
        );
    ...
    fn action_bar_terminal_target_setting_defers_main_window_update(cx: &mut gpui::TestAppContext) {
        ...
        assert!(
            result.is_ok(),
            "action bar terminal target updates should not re-enter RepositoryTreeView updates"
        );
    ...
    fn diff_scroll_sync_setting_defers_main_window_update(cx: &mut gpui::TestAppContext) {
        ...
        assert!(
            result.is_ok(),
            "diff scroll sync update should not re-enter RepositoryTreeView updates"
        );
    ...
    fn diff_content_mode_setting_defers_main_window_update(cx: &mut gpui::TestAppContext) {
        ...
        assert!(
            result.is_ok(),
            "diff content mode update should not re-enter RepositoryTreeView updates"
        );
    ...
    fn diff_whitespace_mode_setting_defers_main_window_update(cx: &mut gpui::TestAppContext) {
        ...
        assert!(
            result.is_ok(),
            "diff whitespace mode update should not re-enter RepositoryTreeView updates"
        );
    ...
    fn auto_save_file_edits_toggle_reaches_the_main_window(cx: &mut gpui::TestAppContext) {
        ...
        // Nested inside a `RepositoryTreeView` update, as the deferral regression
        ...
        assert!(
            result.is_ok(),
            "the auto-save toggle should not re-enter RepositoryTreeView updates"
        );
    ...
    fn diff_render_settings_update_main_window(cx: &mut gpui::TestAppContext) {
        ...
        assert!(
            result.is_ok(),
            "diff render setting updates should not re-enter RepositoryTreeView updates"
        );
    ...
    fn diff_render_defaults_from_session_wrapper() {
        ...
        repositorytree_state::session::persist_ui_settings_to_path(
            repositorytree_state::session::UiSettings {
    ...
    fn external_terminal_mode_setting_defers_main_window_update(cx: &mut gpui::TestAppContext) {
        ...
        assert!(
            result.is_ok(),
            "external terminal mode updates should not re-enter RepositoryTreeView updates"
        );
