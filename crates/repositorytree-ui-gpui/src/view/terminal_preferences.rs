...
mod tests {
    ...
    fn temp_test_dir(label: &str) -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!(
            "repositorytree_terminal_preferences_{label}_{}_{}",
            std::process::id(),
            nonce
        ));
    ...
    fn resolve_embedded_shell_program_automatic_prefers_shell_env_and_falls_back_to_bin_sh() {
        let automatic = resolve_automatic_embedded_shell_program_from_shell(Some(OsString::from(
            "/tmp/repositorytree-shell",
        )))
        .expect("automatic shell should resolve");
        assert_eq!(automatic, PathBuf::from("/tmp/repositorytree-shell"));
    ...
    fn custom_launch_spec_substitutes_placeholders() {
        ...
        let context = ExternalTerminalLaunchContext {
            cwd: PathBuf::from("/tmp/repositorytree"),
            repo_name: Some("repositorytree".to_string()),
        };
        ...
        assert_eq!(
            ...
            vec![
                OsString::from("start"),
                OsString::from("--cwd"),
                OsString::from("/tmp/repositorytree"),
                OsString::from("--class"),
                OsString::from("repositorytree"),
            ]
        );
    }
    ...
    fn automatic_external_terminal_prefers_first_supported_candidate_on_path() {
        ...
        assert_eq!(spec.current_dir, Some(PathBuf::from("/tmp/repositorytree")));
    ...
    fn automatic_external_terminal_uses_xterm_shell_wrapper_when_needed() {
        ...
        let context = ExternalTerminalLaunchContext {
            cwd: PathBuf::from("/tmp/repositorytree path/it's here"),
            repo_name: Some("repositorytree".to_string()),
        };
        ...
        assert_eq!(
            ...
            vec![
                ...
                OsString::from("-lc"),
                OsString::from(
                    "cd '/tmp/repositorytree path/it'\"'\"'s here' && exec \"${SHELL:-/bin/sh}\""
                ),
            ]
        );
        assert_eq!(
            spec.current_dir,
            Some(PathBuf::from("/tmp/repositorytree path/it's here"))
        );
