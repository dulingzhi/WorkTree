...
fn stage_gutter_repo(
    ...
    repo.diff_state.diff = Loadable::Ready(Arc::new(repositorytree_core::domain::Diff::from_unified(
        target, &unified,
    )));
...
fn open_stage_gutter_view(
    ...
    gpui::Entity<super::super::RepositoryTreeView>,
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_stage_gutter",
        std::process::id()
    ));
