...
pub(super) use repositorytree_core::error::{Error, ErrorKind};
pub(super) use repositorytree_core::services::{GitBackend, GitRepository, Result};
pub(super) use repositorytree_state::store::AppStore;
...
fn shows_external_mergetool_actions_only_in_normal_mode() {
    assert!(show_external_mergetool_actions(RepositoryTreeViewMode::Normal));
...
fn shows_save_stage_action_only_in_normal_mode() {
    assert!(show_conflict_save_stage_action(RepositoryTreeViewMode::Normal));
...
fn shows_diff_file_navigation_only_in_normal_mode() {
    assert!(show_diff_file_navigation(RepositoryTreeViewMode::Normal));
...
fn conflict_side_output_bytes_prefers_original_bytes_and_falls_back_to_text() {
    let file = repositorytree_state::model::ConflictFile {
...
fn conflict_resolver_strategy_maps_conflict_kinds() {
    use repositorytree_core::conflict_session::ConflictResolverStrategy as S;
    use repositorytree_core::domain::FileConflictKind as K;
...
pub(super) fn seed_file_image_diff_state_with_rev(
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.diff_state.diff_file_rev = diff_file_rev;
            repo.diff_state.diff_file_image = repositorytree_state::model::Loadable::Ready(Some(
                Arc::new(repositorytree_core::domain::FileDiffImage {
...
pub(super) fn conflict_compare_repo_state(
    ...
    set_test_file_status(
        ...
        repositorytree_core::domain::FileStatusKind::Conflicted,
        repositorytree_core::domain::DiffArea::Unstaged,
    );
...
pub(super) fn assert_file_preview_ctrl_a_ctrl_c_copies_all(
    ...
    status_kind: repositorytree_core::domain::FileStatusKind,
    lines: Arc<Vec<String>>,
) {
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            if let Some(source_path) = deleted_preview_source_path.as_ref() {
                repo.diff_state.diff_preview_text_file = repositorytree_state::model::Loadable::Ready(
                    Some(Arc::new(repositorytree_core::domain::DiffPreviewTextFile {
        ...
        view.update(app, |this, cx| {
            ...
            let preview_side = match status_kind {
                ...
                | repositorytree_core::domain::FileStatusKind::Untracked => {
                    Some(repositorytree_core::domain::DiffPreviewTextSide::New)
                }
                repositorytree_core::domain::FileStatusKind::Deleted => {
                    Some(repositorytree_core::domain::DiffPreviewTextSide::Old)
                }
                _ => None,
            };
            if let Some(side) = preview_side {
                let preview_source_path = workdir.join(format!(
                    ...
                    repo_id.0,
                    match side {
                        repositorytree_core::domain::DiffPreviewTextSide::New => "new",
                        repositorytree_core::domain::DiffPreviewTextSide::Old => "old",
                    }
                ));
                let contents = match side {
                    repositorytree_core::domain::DiffPreviewTextSide::New => new_text.unwrap_or_default(),
                    repositorytree_core::domain::DiffPreviewTextSide::Old => old_text.unwrap_or_default(),
                };
...
pub(super) fn opening_repo_state(
    ...
    repositorytree_state::model::RepoState::new_opening(
        repo_id,
...
pub(super) fn push_test_state(
    this: &super::super::RepositoryTreeView,
    state: Arc<AppState>,
...
pub(super) fn set_test_file_status(
    repo: &mut repositorytree_state::model::RepoState,
    path: impl Into<std::path::PathBuf>,
    kind: repositorytree_core::domain::FileStatusKind,
...
pub(super) fn set_test_file_status_with_conflict(
    ...
    let path = path.into();
    let file_status = repositorytree_core::domain::FileStatus {
        path: path.clone(),
        kind,
        conflict,
    };
    let (staged, unstaged) = match area {
        repositorytree_core::domain::DiffArea::Staged => (vec![file_status], vec![]),
        repositorytree_core::domain::DiffArea::Unstaged => (vec![], vec![file_status]),
    };
    repo.status = repositorytree_state::model::Loadable::Ready(
        repositorytree_core::domain::RepoStatus { staged, unstaged }.into(),
    );
    ...
    repo.diff_state.diff_target =
        Some(repositorytree_core::domain::DiffTarget::WorkingTree { path, area });
    repo.diff_state.diff_state_rev = repo.diff_state.diff_state_rev.wrapping_add(1);
}
