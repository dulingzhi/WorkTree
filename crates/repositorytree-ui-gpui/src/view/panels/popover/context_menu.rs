...
fn gitignore_pattern_line(line: &str) -> Option<&str> {
    let line = repositorytree_core::gitignore::trim_trailing_spaces(line.trim_start());
...
impl PopoverHost {
    ...
    pub(in crate::view) fn context_menu_activate_action(
        ...
        match action {
            ...
            ContextMenuAction::RevealHistoryCommit { repo_id, commit_id } => {
                self.main_pane.update(cx, |main, cx| {
                    main.reveal_history_commit(
                        repo_id,
                        commit_id,
                        Some(repositorytree_core::domain::LogScope::AllBranches),
                        cx,
                    );
                });
            }
            ...
            ContextMenuAction::OpenInDetectedEditor {
                ...
                let setting = repositorytree_state::session::ExternalCodeEditorSetting::Detected {
                    id,
                    path: editor_path,
                };
                ...
                let reword_state = if was_reword {
                    self.main_pane.read_with(cx, |pane, _| {
                        pane.active_irebase().and_then(|st| {
                            ...
                            let msg = repositorytree_core::squash::reword_seed_message(&st.entries, ix);
                            Some((action, msg))
                        })
                    })
                } else {
    ...
    pub(super) fn add_to_gitignore_target(
        ...
        repositorytree_core::gitignore::GitignoreSuggestions,
    )> {
        ...
        let suggestions = repositorytree_core::gitignore::suggestions_for_paths(&paths)?;
    ...
    pub(super) fn prepare_add_to_gitignore(
        ...
        let scope = repositorytree_core::gitignore::GitignoreScope::File;
        let text = target
            .as_ref()
    ...
    pub(super) fn set_add_to_gitignore_scope(
        &mut self,
        scope: repositorytree_core::gitignore::GitignoreScope,
        cx: &mut gpui::Context<Self>,
    ) {
        let Some(text) = self
                            ...
                            ContextMenuItem::Segmented { label, segments } => {
                                ...
                                for (seg_ix, segment) in segments.into_iter().enumerate() {
                                    ...
                                    if let Some(tooltip) = tooltip {
                                        button = button.repositorytree_tooltip(
                                            theme,
