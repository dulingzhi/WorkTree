...
use super::*;
use repositorytree_core::domain::{Branch, CommitId};
...
fn repo_picker_lists_recently_closed_repositories_wrapper() {
    ...
    repositorytree_state::session::persist_recent_repo_to_path(&closed_repo, &session_file)
        .expect("seed recent repo session");

    let current_exe = std::env::current_exe().expect("locate current test binary");
    let output = repositorytree_core::process::background_command(current_exe)
...
fn repo_picker_lists_recently_closed_repositories_subprocess(cx: &mut gpui::TestAppContext) {
    ...
    let closed_repo = repositorytree_state::session::load()
        .recent_repos
        .into_iter()
        .next()
        .expect("seeded recent repo");
    let closed_repo = repositorytree_core::path_utils::canonicalize_or_original(closed_repo);

    let (store, events, _repo, workdir) = create_tracking_store("repo-picker-recently-closed");
    let open_workdir = repositorytree_core::path_utils::canonicalize_or_original(workdir);
    let store_for_view = store.clone();
    let (view, cx) = cx
        .add_window_view(|window, cx| RepositoryTreeView::new(store_for_view, events, None, window, cx));
...
fn repo_picker_sort_menu_reorders_rows_wrapper() {
    ...
    for repo in ["c-parent/mike", "a-parent/zulu", "b-parent/Alpha"] {
        ...
        repositorytree_state::session::persist_recent_repo_to_path(&path, &session_file)
            .expect("seed recent repo session");
    }
...
fn repo_picker_sort_menu_reorders_rows_subprocess(cx: &mut gpui::TestAppContext) {
    ...
    assert_eq!(
        repositorytree_state::session::load().repo_picker_sort.as_deref(),
        Some("path")
    );
}
...
fn repo_picker_pins_outlive_the_recents_list_wrapper() {
    ...
    repositorytree_state::session::persist_pinned_repo_to_path(&pinned, &session_file)
    ...
    repositorytree_state::session::persist_recent_repo_to_path(&recent, &session_file)
...
fn repo_picker_row_menu_takes_over_navigation_and_escape(cx: &mut gpui::TestAppContext) {
    ...
    assert_eq!(
        entry,
        repo_picker::RepoPickerEntry::Open(repositorytree_state::model::RepoId(1))
    );
...
fn repo_picker_close_row_action_promotes_the_recents_cache(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = repositorytree_core::path_utils::canonicalize_or_original(workdir);
    let store_for_view = store.clone();
    let (view, cx) = cx
        .add_window_view(|window, cx| RepositoryTreeView::new(store_for_view, events, None, window, cx));
    ...
    let entry = repo_picker::RepoPickerEntry::Open(repositorytree_state::model::RepoId(1));
    cx.update(|window, app| {
        popover_host.update(app, |host, cx| {
            ...
            picker_row_menu::activate(
                ...
                ContextMenuAction::CloseRepo {
                    repo_id: repositorytree_state::model::RepoId(1),
                },
                window,
                cx,
            );
...
fn repo_picker_row_action_says_so_when_the_repository_is_gone(cx: &mut gpui::TestAppContext) {
    ...
    let entry = repo_picker::RepoPickerEntry::Open(repositorytree_state::model::RepoId(999));

    cx.update(|_window, app| {
        popover_host.update(app, |host, cx| {
            assert!(
                host.workdir_for_repo(repositorytree_state::model::RepoId(999))
                    .is_none()
            );
            picker_row_menu::open(
                host,
    ...
    cx.update(|window, app| {
        popover_host.update(app, |host, cx| {
            picker_row_menu::activate(
                host,
                ContextMenuAction::CloseRepo {
                    repo_id: repositorytree_state::model::RepoId(999),
                },
                window,
                cx,
            );
...
fn open_repo_picker(view: &gpui::Entity<RepositoryTreeView>, cx: &mut gpui::VisualTestContext) {
    cx.update(|window, app| {
        let _ = window.draw(app);
        view.update(app, |this, cx| {
            this.popover_host.update(cx, |host, cx| {
