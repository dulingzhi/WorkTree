...
fn tag_menu_lists_delete_entries_for_commit_tags(cx: &mut gpui::TestAppContext) {
    ...
    let workdir =
        std::env::temp_dir().join(format!("repositorytree_ui_test_{}_tag_menu", std::process::id()));
...
fn tag_menu_lists_remote_push_and_delete_entries(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_tag_menu_remote",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.remotes = Loadable::Ready(Arc::new(vec![
                repositorytree_core::domain::Remote {
            ...
            repo.remote_tags = Loadable::Ready(Arc::new(vec![repositorytree_core::domain::RemoteTag {
...
fn tag_ref_menu_scopes_actions_to_clicked_tag(cx: &mut gpui::TestAppContext) {
    ...
    let commit_id = CommitId("0123456789abcdef".into());
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_tag_ref_menu",
        std::process::id()
    ));
...
fn tag_ref_menu_requests_remote_tags_when_not_loaded(cx: &mut gpui::TestAppContext) {
    ...
    let commit_id = CommitId("fedcba9876543210".into());
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_tag_ref_lazy_remote_tags",
        std::process::id()
    ));
    let mut repo = RepoState::new_opening(
        repo_id,
...
fn remote_menu_lists_fetch_and_prune_actions(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_remote_menu_prune",
        std::process::id()
    ));
...
fn local_branch_menu_has_pull_merge_and_squash_actions(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_local_branch_menu_merge",
        std::process::id()
    ));
...
fn branch_menu_pin_entry_toggles_and_relabels(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_branch_menu_pin",
        std::process::id()
    ));
    ...
    // `RepositoryTreeView` update would double-lease the root view, which the real
...
fn remote_branch_menu_has_pull_merge_and_squash_actions(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_remote_branch_menu_merge",
        std::process::id()
    ));
...
fn remote_branch_menu_renders_squash_entry_without_panic(cx: &mut gpui::TestAppContext) {
    ...
    let branch_name = "origin/feature/awesome".to_string();
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_remote_branch_menu_render",
        std::process::id()
    ));
...
fn remote_branch_menu_only_enables_unlink_for_active_branch_upstream(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_remote_branch_menu_unlink",
        std::process::id()
    ));
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.branches = Loadable::Ready(Arc::new(vec![repositorytree_core::domain::Branch {
                ...
                upstream: Some(repositorytree_core::domain::Upstream {
                    remote: "origin".to_string(),
...
fn remote_branch_menu_offers_set_tracking_upstream_only_without_current_upstream(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_remote_branch_menu_set_tracking_upstream",
        std::process::id()
    ));
...
fn pull_and_push_picker_headers_include_tracking_branch_name(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_pull_push_picker_header_branch_name",
        std::process::id()
    ));
...
fn local_branch_menu_excludes_pull_merge_and_squash_for_current_branch(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_local_branch_menu_current_branch",
        std::process::id()
    ));
...
fn local_branch_menu_offers_fast_forward_only_with_upstream(cx: &mut gpui::TestAppContext) {
    ...
    let branch_name = "feature/awesome".to_string();
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_local_branch_menu_fast_forward",
        std::process::id()
    ));

    let open_menu = |cx: &mut gpui::VisualTestContext,
                     upstream: Option<repositorytree_core::domain::Upstream>| {
        let mut repo = RepoState::new_opening(
            repo_id,
            repositorytree_core::domain::RepoSpec {
                workdir: workdir.clone(),
    ...
    let model = open_menu(
        cx,
        Some(repositorytree_core::domain::Upstream {
            remote: "origin".to_string(),
            branch: "main".to_string(),
        }),
    );
