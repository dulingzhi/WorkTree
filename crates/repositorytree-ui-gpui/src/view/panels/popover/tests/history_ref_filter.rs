...
use super::*;
use repositorytree_core::domain::{Branch, CommitId, RemoteBranch, Tag};
...
fn seed_refs(store: &AppStore) -> RepoId {
    ...
    store.dispatch(Msg::Internal(
        repositorytree_state::msg::InternalMsg::RemoteBranchesLoaded {
            repo_id,
            result: Ok(vec![remote_branch("origin", "main")]),
        },
    ));
    store.dispatch(Msg::Internal(
        repositorytree_state::msg::InternalMsg::TagsLoaded {
            repo_id,
    ...
    wait_until("seeded ref lists to land in the store", || {
        store.snapshot().repos.iter().any(|repo| {
            repo.id == repo_id
                && matches!(&repo.branches, repositorytree_state::model::Loadable::Ready(_))
                && matches!(
                    &repo.remote_branches,
                    repositorytree_state::model::Loadable::Ready(_)
                )
                && matches!(&repo.tags, repositorytree_state::model::Loadable::Ready(_))
        })
    });
    repo_id
}
...
fn sync_and_redraw(cx: &mut gpui::VisualTestContext, view: &gpui::Entity<RepositoryTreeView>) {
