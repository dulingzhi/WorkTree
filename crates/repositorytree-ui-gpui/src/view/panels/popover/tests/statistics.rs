...
use repositorytree_core::domain::ContributorCommit;
...
fn open_statistics(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_statistics",
        std::process::id()
    ));
...
fn click(cx: &mut gpui::VisualTestContext, _view: &Entity<RepositoryTreeView>, selector: &'static str) {
    let bounds = cx
