...
use repositorytree_core::services::{CommandOutput, PullMode};
...
impl GitRepository for TrackingRepo {
    ...
    fn log_head_page(
        &self,
        _limit: usize,
        _cursor: Option<&repositorytree_core::domain::LogCursor>,
    ) -> Result<LogPage> {
        Ok(LogPage {
            commits: Vec::new(),
            next_cursor: None,
    ...
    fn list_remotes(&self) -> Result<Vec<repositorytree_core::domain::Remote>> {
        Ok(Vec::new())
    }

    fn list_remote_branches(&self) -> Result<Vec<repositorytree_core::domain::RemoteBranch>> {
        Ok(Vec::new())
    }
    ...
    fn diff_unified(&self, _target: &repositorytree_core::domain::DiffTarget) -> Result<String> {
        Err(Error::new(ErrorKind::Unsupported(
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "repositorytree-ui-popover-{label}-{}-{suffix}",
        std::process::id()
    ));
...
fn rename_branch_prompt_cancel_button_and_escape_close(cx: &mut gpui::TestAppContext) {
    ...
    let open_prompt =
        |window: &mut gpui::Window, app: &mut gpui::App, view: &Entity<RepositoryTreeView>| {
            view.update(app, |this, cx| {
                this.popover_host.update(cx, |host, cx| {
                    host.open_popover_at(
...
mod checkout_picker {
    ...
    use repositorytree_core::domain::{RefMetadata, RemoteBranch};
    use repositorytree_state::model::{RepoId, RepoState};
    ...
    fn open_checkout_picker(
        ...
        repo: RepoState,
        repo_id: RepoId,
    ) -> (gpui::Entity<RepositoryTreeView>, &mut gpui::VisualTestContext) {
        let (store, events) = AppStore::new(Arc::new(TestBackend));
        let (view, cx) =
            cx.add_window_view(|window, cx| RepositoryTreeView::new(store, events, None, window, cx));
    ...
    fn opening_the_picker_requests_ref_metadata_once(cx: &mut gpui::TestAppContext) {
        ...
        let ref_metadata_untouched = |snapshot: &Arc<repositorytree_state::model::AppState>| {
            snapshot
                .repos
                .iter()
                .find(|r| r.id == repo_id)
...
fn branch_group_menu_model_filtered(
    ...
    let remote_branch = |remote: &str, name: &str| repositorytree_core::domain::RemoteBranch {
        remote: remote.to_string(),
        name: name.to_string(),
        target: CommitId("aaaaaaaaaaaa".into()),
    };
            ...
            let mut repo = RepoState::new_opening(
                ...
                RepoSpec {
                    workdir: std::env::temp_dir().join(format!(
                        "repositorytree_ui_test_{}_branch_group_menu",
                        std::process::id()
                    )),
                },
            );
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            let mut repo = RepoState::new_opening(
                ...
                RepoSpec {
                    workdir: std::env::temp_dir().join(format!(
                        "repositorytree_ui_test_{}_group_delete_confirm",
                        std::process::id()
                    )),
                },
            );
...
fn pinned_section_menu_model(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_pinned_section_menu",
        std::process::id()
    ));
...
fn activate_sidebar_action_with(
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_sidebar_action",
        std::process::id()
    ));
    let branch = |name: &str| Branch {
        name: name.to_string(),
...
fn pinned_section_menu_reports_expanded_while_a_filter_is_live(cx: &mut gpui::TestAppContext) {
    ...
    let workdir = std::env::temp_dir().join(format!(
        "repositorytree_ui_test_{}_pinned_filter",
        std::process::id()
    ));
