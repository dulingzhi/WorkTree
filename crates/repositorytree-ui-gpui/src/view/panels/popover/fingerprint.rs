...
use repositorytree_state::model::CloneProgressStage;
...
mod tests {
    use super::*;
    use repositorytree_core::domain::{Branch, CommitId, Upstream};
    use std::sync::Arc;
    ...
    fn grouped_repo_popover_resolves_explicit_repo_id() {
        ...
        let repo = RepoState::new_opening(
            ...
            repositorytree_core::domain::RepoSpec {
                workdir: std::env::temp_dir().join("repositorytree_repo_popover_repo_for_popover"),
            },
        );
        let state = AppState {
            repos: vec![repo],
    ...
    fn pull_picker_fingerprint_changes_when_branches_rev_changes() {
        ...
        let mut repo = RepoState::new_opening(
            repo_id,
            repositorytree_core::domain::RepoSpec {
                workdir: std::env::temp_dir().join("repositorytree_pull_picker_fingerprint"),
            },
        );
    ...
    fn commit_options_fingerprint_changes_when_amend_availability_revisions_change() {
        ...
        let repo = RepoState::new_opening(
            repo_id,
            repositorytree_core::domain::RepoSpec {
                workdir: std::env::temp_dir().join("repositorytree_commit_options_fingerprint"),
            },
        );
        let mut state = AppState {
            active_repo: Some(repo_id),
    ...
    fn force_push_confirm_fingerprint_changes_when_pending_lease_changes() {
        ...
        let repo = RepoState::new_opening(
            repo_id,
            repositorytree_core::domain::RepoSpec {
                workdir: std::env::temp_dir().join("repositorytree_force_push_fingerprint"),
            },
        );
        let mut state = AppState {
            active_repo: Some(repo_id),
    ...
    fn push_picker_fingerprint_changes_when_pending_lease_changes() {
        ...
        let repo = RepoState::new_opening(
            repo_id,
            repositorytree_core::domain::RepoSpec {
                workdir: std::env::temp_dir().join("repositorytree_push_picker_lease_fingerprint"),
            },
        );
        let mut state = AppState {
            active_repo: Some(repo_id),
