...
pub(super) fn model(this: &PopoverHost, repo_id: RepoId, commit_id: &CommitId) -> ContextMenuModel {
    ...
    let squash_plan = this
        ...
        .and_then(|repo| {
            ...
            repositorytree_core::squash::squash_eligibility(&page.commits, &selection.commits, &head)
        });
    ...
    if !is_head_commit {
        ...
        let children_count = this
            ...
            .and_then(|repo| {
                let head = repo.head_commit_id()?;
                match &repo.log {
                    Loadable::Ready(page) => repositorytree_core::squash::linear_first_parent_distance(
                        &page.commits,
                        &head,
                        commit_id,
                    ),
...
mod tests {
    use super::*;
    use repositorytree_core::domain::{CommitParentIds, LogPage, RepoSpec};
