...
fn detected_editor_entries(
    ...
    configured: Option<&repositorytree_state::session::ExternalCodeEditorSetting>,
) -> Vec<ContextMenuItem> {
    let configured_id = match configured {
        Some(repositorytree_state::session::ExternalCodeEditorSetting::Detected { id, .. }) => {
            Some(id.as_str())
        }
        _ => None,
    };
...
mod tests {
    ...
    fn detected_editor_entries_skip_the_configured_editor() {
        ...
        let configured = repositorytree_state::session::ExternalCodeEditorSetting::Detected {
            id: "vscode".to_string(),
    ...
    fn detected_editor_entries_list_all_tools_for_a_custom_configuration() {
        let workdir = PathBuf::from("/tmp/repo-tab-menu-2");
        let configured = repositorytree_state::session::ExternalCodeEditorSetting::Custom {
    ...
    fn workdir_fixture(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "repositorytree-repo-tab-vs-{label}-{}-{}",
            std::process::id(),
