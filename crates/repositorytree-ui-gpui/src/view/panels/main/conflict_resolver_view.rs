...
impl MainPaneView {
    ...
    pub(super) fn conflict_toolbar_full_controls(
            ...
            .when(!conflict_rendered_preview_active, |d| {
                ...
                d.child(
                    components::Button::new("conflict_first", "")
                        ...
                        .repositorytree_tooltip(theme, crate::i18n::tr("conflict.toolbar.first_delta")),
                )
                .child(
                ...
                .child(
                    components::Button::new("conflict_last", "")
                        ...
                        .repositorytree_tooltip(theme, crate::i18n::tr("conflict.toolbar.last_delta")),
                )
                .child(
        ...
        if let (Some(repo_id), Some(path)) = (repo_id, conflict_target_path.clone()) {
            ...
            let focused_mergetool_mode = self.view_mode == RepositoryTreeViewMode::FocusedMergetool;
            ...
            let save_button = components::Button::new("conflict_save", save_label)
                ...
                .on_click(theme, cx, move |this, _e, _w, cx| {
                    ...
                    if this.view_mode == RepositoryTreeViewMode::FocusedMergetool {
            ...
            controls = controls
                ...
                .when(show_conflict_save_stage_action(self.view_mode), |d| {
                    ...
                    if gate_unresolved > 0 {
                        ...
                        save_stage_btn =
                            save_stage_btn.repositorytree_tooltip(theme, disabled_tooltip.into());
                    }
                    ...
                    match renderable_conflict_file(repo, &self.conflict_resolver, &path) {
                            ...
                            RenderableConflictFile::File(file)
                                if matches!(
                                    ...
                                    Some(repositorytree_core::conflict_session::ConflictResolverStrategy::TwoWayKeepDelete)
                                ) =>
                            {
                                // Keep/delete resolver for modify/delete conflicts.
                                let kind = self.conflict_resolver.conflict_kind.unwrap_or(
                                    repositorytree_core::domain::FileConflictKind::DeletedByUs,
                                );
                            ...
                            RenderableConflictFile::File(file)
                                if matches!(
                                    self.conflict_resolver.strategy,
                                    Some(repositorytree_core::conflict_session::ConflictResolverStrategy::DecisionOnly)
                                ) =>
                            {
