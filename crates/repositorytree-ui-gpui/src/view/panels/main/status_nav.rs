...
pub(super) struct StatusNavigationContext<'a> {
    ...
    entries: Vec<&'a repositorytree_core::domain::FileStatus>,
    current_ix: usize,
}
    ...
    match area {
        ...
        DiffArea::Unstaged => match change_tracking_view {
            ...
            ChangeTrackingView::SplitUntracked => status
                ...
                .map(|entry| {
                    if entry.kind == repositorytree_core::domain::FileStatusKind::Untracked {
...
fn status_navigation_entries_for_section(
    status: &repositorytree_core::domain::RepoStatus,
    section: StatusSection,
) -> Vec<&repositorytree_core::domain::FileStatus> {
    match section {
        StatusSection::CombinedUnstaged => status.unstaged.iter().collect(),
        StatusSection::Untracked => status
            .unstaged
            .iter()
            .filter(|entry| entry.kind == repositorytree_core::domain::FileStatusKind::Untracked)
            .collect(),
...
pub(super) fn status_navigation_context<'a>(
    status: &'a repositorytree_core::domain::RepoStatus,
    ...
    match diff_target {
        DiffTarget::WorkingTree { .. } => {
            ...
            let is_conflicted = area == DiffArea::Unstaged
                && entry.kind == repositorytree_core::domain::FileStatusKind::Conflicted;
...
mod tests {
    ...
    fn repo_state(id: RepoId, path: &str) -> RepoState {
        RepoState::new_opening(id, repositorytree_core::domain::RepoSpec { workdir: pb(path) })
    }
    ...
    fn split_untracked_navigation_scopes_to_untracked_section() {
        let status = repositorytree_core::domain::RepoStatus {
            staged: Vec::new(),
            unstaged: vec![
                file_status(
                    "new-a.txt",
    ...
    fn commit_details_file_navigation_selects_adjacent_commit_files() {
        ...
        repo.history_state.commit_details =
            Loadable::Ready(std::sync::Arc::new(repositorytree_core::domain::CommitDetails {
                id: commit_id.clone(),
