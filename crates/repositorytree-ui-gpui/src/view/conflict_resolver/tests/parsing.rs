...
fn split_and_inline_views_consistent_for_mixed_mode_conflicts() {
    use repositorytree_core::file_diff::{FileDiffRow, FileDiffRowKind as RK};
...
fn inline_rows_expand_modify_into_remove_and_add() {
    ...
    assert_eq!(inline[1].kind, repositorytree_core::domain::DiffLineKind::Remove);
    assert_eq!(inline[2].kind, repositorytree_core::domain::DiffLineKind::Add);
}
