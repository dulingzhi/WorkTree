...
fn split_row_index_single_block_plain_rows() {
    ...
    assert_eq!(r0.kind, repositorytree_core::file_diff::FileDiffRowKind::Modify);
    ...
    assert_eq!(r1.kind, repositorytree_core::file_diff::FileDiffRowKind::Modify);

    // Row 2: only ours (theirs shorter)
    let r2 = index.row_at(&segments, 2).unwrap();
    assert_eq!(r2.old, Some("c".into()));
    assert_eq!(r2.new, None);
    assert_eq!(r2.kind, repositorytree_core::file_diff::FileDiffRowKind::Remove);
...
fn split_row_index_context_plus_block() {
    ...
    assert_eq!(c0.kind, repositorytree_core::file_diff::FileDiffRowKind::Context);
    ...
    assert_eq!(b0.kind, repositorytree_core::file_diff::FileDiffRowKind::Modify);
    ...
    assert_eq!(b1.kind, repositorytree_core::file_diff::FileDiffRowKind::Add);
}
...
fn split_row_index_equal_block_rows_share_old_new_arc() {
    ...
    assert_eq!(row.kind, repositorytree_core::file_diff::FileDiffRowKind::Context);
...
fn split_row_index_shared_slice_context_rows_reuse_conflict_backing() {
    ...
    let expected =
        repositorytree_core::file_diff::FileDiffLineText::shared_slice(Arc::clone(&shared), 4..10);
    assert_eq!(row.kind, repositorytree_core::file_diff::FileDiffRowKind::Context);
    assert_eq!(row.old, Some("shared".into()));
    assert_eq!(row.new, Some("shared".into()));
...
fn split_row_index_shared_slice_modify_rows_reuse_conflict_backing() {
    ...
    let row = index.row_at(&segments, 0).unwrap();
    let expected_old =
        repositorytree_core::file_diff::FileDiffLineText::shared_slice(Arc::clone(&ours), 0..3);
    let expected_new =
        repositorytree_core::file_diff::FileDiffLineText::shared_slice(Arc::clone(&theirs), 0..3);
    assert_eq!(row.kind, repositorytree_core::file_diff::FileDiffRowKind::Modify);
...
fn split_row_index_large_block_no_diff_computation() {
    ...
    assert_eq!(
        last_ours.kind,
        repositorytree_core::file_diff::FileDiffRowKind::Remove
    );
}
...
fn split_row_index_positional_alignment_handles_shifted_insertions() {
    ...
    assert_eq!(body.kind, repositorytree_core::file_diff::FileDiffRowKind::Modify);
    ...
    assert_eq!(tail.kind, repositorytree_core::file_diff::FileDiffRowKind::Add);
}
...
fn split_row_index_classifies_modify_vs_context_kinds() {
    ...
    let r0 = index.row_at(&segments, 0).unwrap();
    assert_eq!(r0.kind, repositorytree_core::file_diff::FileDiffRowKind::Context);
    ...
    let r2 = index.row_at(&segments, 2).unwrap();
    assert_eq!(r2.kind, repositorytree_core::file_diff::FileDiffRowKind::Context);
...
fn split_row_index_positional_alignment_keeps_inserted_block_reviewable() {
    ...
    let r2 = index.row_at(&segments, 2).unwrap();
    assert_eq!(r2.kind, repositorytree_core::file_diff::FileDiffRowKind::Modify);
    assert_eq!(r2.old.as_deref(), Some("gamma"));
    assert_eq!(r2.new.as_deref(), Some("new2"));

    let r3 = index.row_at(&segments, 3).unwrap();
    assert_eq!(r3.kind, repositorytree_core::file_diff::FileDiffRowKind::Add);
    assert_eq!(r3.old, None);
    assert_eq!(r3.new.as_deref(), Some("beta"));

    let r4 = index.row_at(&segments, 4).unwrap();
    assert_eq!(r4.kind, repositorytree_core::file_diff::FileDiffRowKind::Add);
...
fn split_row_index_gap_diff_no_common_lines_falls_back_to_positional() {
    ...
    let r2 = index.row_at(&segments, 2).unwrap();
    assert_eq!(r2.kind, repositorytree_core::file_diff::FileDiffRowKind::Add);
...
fn split_row_index_modify_rows_get_word_highlights() {
    ...
    let r1 = index.row_at(&segments, 1).unwrap();
    assert_eq!(r1.kind, repositorytree_core::file_diff::FileDiffRowKind::Context);
    assert!(
