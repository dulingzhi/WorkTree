use super::*;
use repositorytree_core::conflict_session::{ConflictPayload, ConflictRegionResolution, ConflictSession};
use repositorytree_core::domain::FileConflictKind;
use repositorytree_core::merge::{MergeBlockId, MergeSource};
...
fn conflict_navigation_reaches_rendered_blocks_the_plan_does_not_call_conflicts() {
    let mut displayed_but_not_a_plan_conflict = target(
        ConflictNavTargetId::PlanBlock(repositorytree_core::merge::MergeBlockId {
            fingerprint: 7,
            occurrence: 0,
        }),
        1,
