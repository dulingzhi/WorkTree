...
pub fn compute_three_way_word_highlights(
    ...
    fn apply_aligned_word_highlights(
        ...
        use repositorytree_core::file_diff::PlanRowView;

        repositorytree_core::file_diff::for_each_side_by_side_row(
            old_text,
            new_text,
            |view| match view {
                PlanRowView::Modify {
...
pub fn compute_two_way_word_highlights(
    ...
    let modify_rows = diff_rows
        .iter()
        .filter(|row| row.kind == repositorytree_core::file_diff::FileDiffRowKind::Modify)
        .count();
    ...
    for (row_ix, row) in diff_rows.iter().enumerate() {
        if row.kind != repositorytree_core::file_diff::FileDiffRowKind::Modify {
            continue;
        }
...
mod tests {
    use super::*;
    use repositorytree_core::file_diff::{FileDiffLineText, FileDiffRow, FileDiffRowKind};
    use std::sync::Arc;
