...
pub use repositorytree_core::conflict_output::ConflictOutputChoice as ConflictChoice;
...
pub(in crate::view) enum ConflictNavTargetId {
    PlanBlock(repositorytree_core::merge::MergeBlockId),
    Region(usize),
    DisplayBlock(usize),
}
...
pub struct ConflictInlineRow {
    ...
    pub kind: repositorytree_core::domain::DiffLineKind,
...
pub fn format_autosolve_trace_summary(
    ...
    stats: &repositorytree_state::msg::ConflictAutosolveStats,
) -> String {
...
pub fn conflict_session_summary_counts(
    session: &repositorytree_core::conflict_session::ConflictSession,
) -> ConflictSummaryCounts {
    use repositorytree_core::conflict_session::ConflictRegionResolution;
...
/// auto-resolved.
///
/// [`AutoResolved`]: repositorytree_core::conflict_session::ConflictRegionResolution::AutoResolved
pub fn on_open_autosolve_summary(
    session: &repositorytree_core::conflict_session::ConflictSession,
) -> Option<String> {
    use repositorytree_core::conflict_session::{AutosolveRule, ConflictRegionResolution};

    let mut stats = repositorytree_state::msg::ConflictAutosolveStats::default();
...
pub fn parse_conflict_markers_shared(text: Arc<str>) -> Vec<ConflictSegment> {
    repositorytree_core::conflict_session::parse_conflict_marker_ranges(text.as_ref())
        .into_iter()
        .map(|segment| match segment {
            repositorytree_core::conflict_session::ParsedConflictSegmentRanges::Text(range) => {
                ConflictSegment::Text(ConflictText::shared_slice(Arc::clone(&text), range))
            }
            repositorytree_core::conflict_session::ParsedConflictSegmentRanges::Conflict(block) => {
...
fn content_matches_block_choice(block: &ConflictBlock, content: &str) -> bool {
    use repositorytree_core::conflict_output::ConflictOutputSource;
...
fn resolution_for_choice(
    ...
    has_base: bool,
) -> repositorytree_core::conflict_session::ConflictRegionResolution {
    use repositorytree_core::conflict_output::ConflictOutputSource;
    use repositorytree_core::conflict_session::ConflictRegionResolution;
...
pub(in crate::view) fn choice_for_selection(
    selection: &repositorytree_core::merge::OrderedSelection,
    has_base: bool,
) -> Option<ConflictChoice> {
    use repositorytree_core::conflict_output::{ConflictOutputChoice, ConflictOutputSource};
    use repositorytree_core::merge::MergeSource;
...
> {
    use repositorytree_core::conflict_session::ConflictRegionResolution as R;
...
fn apply_region_resolution_to_block(
    ...
    resolution: &repositorytree_core::conflict_session::ConflictRegionResolution,
) -> Option<String> {
    use repositorytree_core::conflict_session::ConflictRegionResolution as R;
...
pub(in crate::view) fn apply_ordered_region_resolutions(
    segments: &mut Vec<ConflictSegment>,
    resolutions: &[repositorytree_core::conflict_session::ConflictRegionResolution],
) -> usize {
...
pub fn apply_session_region_resolutions(
    segments: &mut Vec<ConflictSegment>,
    regions: &[repositorytree_core::conflict_session::ConflictRegion],
) -> usize {
...
pub(in crate::view) fn build_conflict_nav_targets(
    session: Option<&repositorytree_core::conflict_session::ConflictSession>,
...
/// session-based `apply_autosolve_to_session` in repositorytree-state, and the
...
pub fn auto_resolve_segments_with_options(
    ...
    use repositorytree_core::conflict_session::{AutosolvePickSide, safe_auto_resolve_pick};
...
pub fn auto_resolve_segments_regex(
    segments: &mut [ConflictSegment],
    options: &repositorytree_core::conflict_session::RegexAutosolveOptions,
) -> usize {
    use repositorytree_core::conflict_session::{AutosolvePickSide, regex_assisted_auto_resolve_pick};
...
pub fn auto_resolve_segments_history(
    segments: &mut Vec<ConflictSegment>,
    options: &repositorytree_core::conflict_session::HistoryAutosolveOptions,
) -> usize {
...
pub fn auto_resolve_segments_history_with_region_indices(
    ...
    use repositorytree_core::conflict_session::history_merge_region;
...
pub fn auto_resolve_segments_pass2_with_region_indices(
    ...
    use repositorytree_core::conflict_session::{Subchunk, split_conflict_into_subchunks};
...
fn unresolved_merge_conflict_placeholder_text(block: &ConflictBlock) -> &'static str {
    use repositorytree_core::conflict_output::{
...
pub fn generate_resolved_text_with_options(
    segments: &[ConflictSegment],
    options: repositorytree_core::conflict_output::GenerateResolvedTextOptions<'_>,
) -> String {
    use repositorytree_core::conflict_output::{
...
fn resolved_output_block_source_fragment(
    segment_ix: usize,
    block: &ConflictBlock,
    source: repositorytree_core::conflict_output::ConflictOutputSource,
            ...
            .map(|segment| match segment {
                ...
                ConflictSegment::Block(block) => block
                    ...
                    .filter(|source| match source {
                        repositorytree_core::conflict_output::ConflictOutputSource::Base => {
                            block.base.as_ref().is_some_and(|base| !base.is_empty())
                        }
                        repositorytree_core::conflict_output::ConflictOutputSource::Ours => {
                            !block.ours.is_empty()
                        }
                        repositorytree_core::conflict_output::ConflictOutputSource::Theirs => {
...
#[cfg(any(test, feature = "benchmarks"))]
pub fn build_inline_rows(rows: &[repositorytree_core::file_diff::FileDiffRow]) -> Vec<ConflictInlineRow> {
    use repositorytree_core::domain::DiffLineKind as K;
...
pub fn three_way_alignment_is_practical(base: &str, ours: &str, theirs: &str) -> bool {
    repositorytree_core::merge::interactive_merge_plan_is_practical(
        Some(base),
        ours,
        theirs,
        repositorytree_core::merge::InteractiveMergePlanBudget::default(),
    )
}
...
fn push_renumbered_block_diff_rows(
    rows: &mut Vec<repositorytree_core::file_diff::FileDiffRow>,
    old_text: &str,
    new_text: &str,
    old_line_offset: u32,
    new_line_offset: u32,
    ...
    repositorytree_core::file_diff::append_side_by_side_rows_with_offsets(
        rows,
        old_text,
        new_text,
        old_line_offset,
...
fn push_tiny_block_context_row(
    ...
    use repositorytree_core::file_diff::{FileDiffRow, FileDiffRowKind};
...
fn push_large_conflict_block_preview_rows(
    ...
    if omitted_ours > 0 || omitted_theirs > 0 {
        ...
        .into();
        rows.push(repositorytree_core::file_diff::FileDiffRow {
...
pub fn block_local_two_way_diff_rows(
    segments: &[ConflictSegment],
) -> Vec<repositorytree_core::file_diff::FileDiffRow> {
...
pub fn map_two_way_rows_to_conflicts(
    segments: &[ConflictSegment],
    diff_rows: &[repositorytree_core::file_diff::FileDiffRow],
...
struct AlignedMapRun {
    ...
    starts: [usize; 3],
    lens: [usize; 3],
    kind: repositorytree_core::merge::AlignedRunKind,
}

impl ThreeWayAlignedMap {
    /// Build from the merge engine's alignment runs.
    pub fn from_alignment(alignment: &[repositorytree_core::merge::AlignedRun]) -> Self {
...
pub fn build_minimap_bands(
    ...
    conflict_resolved: &[bool],
    trailing_rows: usize,
) -> Vec<repositorytree_core::merge::MinimapRowKind> {
    use repositorytree_core::merge::MinimapRowKind;
    ...
    for (span_ix, span) in spans.iter().enumerate() {
        ...
        {
            run_ix += 1;
            let kind = repositorytree_core::merge::minimap_row_kind(run.kind);
