...
const UPDATE_CHECK_DISABLE_ENV: &str = "REPOSITORYTREE_NO_UPDATE_CHECK";
#[cfg(not(test))]
const UPDATE_CHECK_REPO_ENV: &str = "REPOSITORYTREE_UPDATE_REPO";
#[cfg(not(test))]
const DEFAULT_UPDATE_REPO: &str = "RepositoryTree/RepositoryTree";
...
impl RepositoryTreeView {
    pub(in crate::view) fn maybe_check_for_updates_on_startup(
        ...
        if self.view_mode != RepositoryTreeViewMode::Normal
            ...
            async move |view: WeakEntity<RepositoryTreeView>, cx: &mut gpui::AsyncApp| {
                ...
                let _ = view.update(cx, |this, cx| {
                    this.push_toast_with_link(
                        ...
                        format!(
                            "A newer RepositoryTree version is available: {} (current {}).",
...
mod tests {
    ...
    fn build_update_notice_returns_none_when_release_is_not_newer() {
        let repo = GitHubRepo::from_slug("RepositoryTree/RepositoryTree");
    ...
    fn parse_github_repo_from_url_supports_https_and_ssh_forms() {
        assert_eq!(
            parse_github_repo_from_url("https://github.com/RepositoryTree/RepositoryTree.git"),
            Some(GitHubRepo {
                owner: "RepositoryTree".to_string(),
                repo: "RepositoryTree".to_string(),
            })
        );
        assert_eq!(
            parse_github_repo_from_url("git@github.com:RepositoryTree/RepositoryTree.git"),
            Some(GitHubRepo {
                owner: "RepositoryTree".to_string(),
                repo: "RepositoryTree".to_string(),
            })
