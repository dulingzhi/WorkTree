...
use crate::ui_scale;
use repositorytree_core::diff::AnnotatedDiffLine;
#[cfg(test)]
use repositorytree_core::diff::annotate_unified;
#[cfg(test)]
use repositorytree_core::domain::RepoStatus;
use repositorytree_core::domain::{
    Branch, Commit, CommitId, DiffArea, DiffTarget, FileStatus, FileStatusKind, Tag,
    UpstreamDivergence,
};
use repositorytree_core::file_diff::FileDiffRow;
use repositorytree_core::process::refresh_git_runtime;
use repositorytree_core::services::{PullMode, RemoteUrlKind, ResetMode};
use repositorytree_state::model::{
...
use repositorytree_state::msg::{Msg, StoreEvent};
...
pub use mod_helpers::{
    ...
    RepositoryTreeViewMode, InitialRepositoryLaunchMode, StartupCrashReport,
};
...
use tooltip::RepositoryTreeTooltipExt;
#[cfg(test)]
...
pub(crate) const WEBSITE_URL: &str = "https://repositorytree.dev";
pub(crate) const EDITIONS_URL: &str = "https://repositorytree.dev/#editions";
pub(crate) const RELEASES_URL: &str = "https://github.com/RepositoryTree/RepositoryTree/releases";
...
struct UiScaleScrollCapture {
    view: Entity<RepositoryTreeView>,
}
...
impl RepositoryTreeView {
    ...
    pub fn new(
        ...
        let config = match initial_path {
            Some(path) => RepositoryTreeViewConfig::normal_with_initial_repository(path, None),
            None => RepositoryTreeViewConfig::normal(None),
        };
    ...
    pub fn new_with_config(
        store: AppStore,
        events: smol::channel::Receiver<StoreEvent>,
        config: RepositoryTreeViewConfig,
        window: &mut Window,
        cx: &mut gpui::Context<Self>,
    ) -> Self {
        let RepositoryTreeViewConfig {
            mut initial_path,
        ...
        let focused_mergetool_bootstrap = if view_mode == RepositoryTreeViewMode::FocusedMergetool {
            focused_mergetool
                .clone()
        ...
        let main_pane = cx.new(|cx| {
            MainPaneView::new(
                ...
                matches!(
                    ...
                    repositorytree_state::model::GitLogTagFetchMode::OnRepositoryActivation
                ),
                view_mode,
    ...
    pub(in crate::view) fn set_history_tag_preferences(
        &mut self,
        show_tags: bool,
        tag_fetch_mode: repositorytree_state::model::GitLogTagFetchMode,
        cx: &mut gpui::Context<Self>,
    ) {
    ...
    fn animate_sidebar_render_width_to(&mut self, target: Pixels, cx: &mut gpui::Context<Self>) {
        ...
        cx.spawn(
            async move |view: WeakEntity<RepositoryTreeView>, cx: &mut gpui::AsyncApp| loop {
    ...
    pub(in crate::view) fn close_sidebar_collapsed_popover(
        ...
        cx.spawn(
            async move |view: WeakEntity<RepositoryTreeView>, cx: &mut gpui::AsyncApp| {
                fade_out.await;
                let _ = view.update(cx, |this, cx| {
...
impl Render for RepositoryTreeView {
    fn render(&mut self, window: &mut Window, cx: &mut gpui::Context<Self>) -> impl IntoElement {
        ...
        debug_assert!(matches!(
            self.view_mode,
            RepositoryTreeViewMode::Normal | RepositoryTreeViewMode::FocusedMergetool
        ));
        self.last_window_size = window.viewport_size();
        self.clamp_pane_widths_to_window();
        ...
        if let Some(report) = self.startup_crash_report.clone()
            && self.view_mode == RepositoryTreeViewMode::Normal
        {
