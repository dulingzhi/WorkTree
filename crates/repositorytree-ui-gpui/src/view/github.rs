...
use repositorytree_core::domain::{CommitId, PullRequest, PullRequestChecksState, Remote};
...
mod tests {
    ...
    fn slug_resolves_from_https_and_ssh_github_remotes() {
        let https = github_slug_from_remotes(&[remote("origin", Some("https://github.com/RepositoryTree/RepositoryTree.git"))]);
        assert_eq!(
            https,
            Some(RepoSlug {
                owner: "RepositoryTree".into(),
                repo: "RepositoryTree".into()
            })
        );
