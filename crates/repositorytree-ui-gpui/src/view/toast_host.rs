use super::*;
use repositorytree_state::model::SubmoduleAddProgressState;
...
impl ToastHost {
    pub(super) fn new(theme: AppTheme, root_view: WeakEntity<RepositoryTreeView>) -> Self {
        Self {
            theme,
            root_view,
            toasts: Vec::new(),
    ...
    fn dismiss_toast(
        ...
        match behavior {
            ...
            ToastDismissBehavior::PostponeSurvey {
                ...
                if let Err(err) = repositorytree_state::session::persist_survey_prompt_postponed(
                    &survey_id,
                    postpone_seconds,
                ) {
                    self.push_toast(
    ...
    fn handle_toast_action(&mut self, id: u64, action: ToastAction, cx: &mut gpui::Context<Self>) {
        match action {
            ...
            ToastAction::OpenSurvey {
                ...
                if let Err(err) = repositorytree_state::session::persist_survey_prompt_opened(&survey_id)
                {
                    self.push_toast(
                        components::ToastKind::Error,
                        crate::i18n::t!(
        ...
        let mut children = displayed
            ...
            .map(move |t| {
                ...
                let close = components::Button::new(format!("toast_close_{}", t.id), "")
                    ...
                    .repositorytree_tooltip(theme, crate::i18n::tr("toast.dismiss_notification"))
...
mod tests {
    ...
    use repositorytree_state::model::{CloneProgressMeter, CloneProgressStage};
        ...
        let host = cx.update(|app| {
            app.new(|cx| {
                ...
                host.push_survey_toast(
                    "survey-id",
                    "Survey",
                    "Help shape RepositoryTree by taking a short user survey.",
                    "https://example.com",
                    "Open Survey",
                    "Later",
                    60,
