...
///
/// RepositoryTree currently only exposes structured long-running progress state for
...
impl NetworkFixture {
    pub fn ui_responsiveness_during_fetch(
        ...
        let transport_url = "https://example.invalid/repositorytree/network.git".to_string();
        let transport_dest = std::path::PathBuf::from("/tmp/repositorytree-network-ui-responsiveness");
    ...
    pub fn progress_bar_update_render_cost(
        ...
        let transport_url = "https://example.invalid/repositorytree/network.git".to_string();
        let transport_dest = std::path::PathBuf::from("/tmp/repositorytree-network-progress-bar");
        let baseline = build_network_baseline_state(&transport_url, &transport_dest);
    ...
    pub fn cancel_operation_latency(
        ...
        let transport_url = "https://example.invalid/repositorytree/network.git".to_string();
        let transport_dest = std::path::PathBuf::from("/tmp/repositorytree-network-cancel");
        let baseline = build_network_baseline_state(&transport_url, &transport_dest);
