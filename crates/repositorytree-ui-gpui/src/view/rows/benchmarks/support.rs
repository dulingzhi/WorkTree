...
pub(crate) fn build_bench_file_diff_rebuild_from_text(
    ...
    let rebuild = crate::view::panes::main::diff_cache::build_file_diff_cache_rebuild(
        &file,
        Path::new("/tmp/repositorytree-bench"),
    )
...
pub(crate) fn hash_file_diff_plan(plan: &repositorytree_core::file_diff::FileDiffPlan) -> u64 {
    let mut h = FxHasher::default();
    plan.row_count.hash(&mut h);
    plan.inline_row_count.hash(&mut h);
    match plan.eof_newline {
        Some(repositorytree_core::file_diff::FileDiffEofNewline::MissingInOld) => 1u8,
        Some(repositorytree_core::file_diff::FileDiffEofNewline::MissingInNew) => 2u8,
        None => 0u8,
    }
    .hash(&mut h);
