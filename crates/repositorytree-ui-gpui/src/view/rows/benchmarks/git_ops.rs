            ...
            GitOpsScenario::LogWalkAuthorFilter {
                ...
                let page = repo
                    .log_history_mode_page_filtered(
                        repositorytree_core::domain::HistoryMode::FullReachable,
                        Some(author.as_str()),
                        *requested_commits,
                        None,
                    )
...
fn hash_blame_lines(lines: &[repositorytree_core::services::BlameLine]) -> u64 {
    let mut h = FxHasher::default();
...
fn git_ops_empty_config() -> &'static Path {
    ...
    EMPTY_CONFIG.get_or_init(|| {
        let path = std::env::temp_dir().join("repositorytree-bench-empty.gitconfig");
