...
use repositorytree_core::squash::squash_fold_target as squash_target;
...
fn reword_edit_signatures(entries: &[InteractiveRebaseEntry]) -> Vec<(String, Vec<String>)> {
    entries
        ...
        .map(|(ix, e)| {
            let sources = repositorytree_core::squash::squash_run_message_entries(entries, ix)
...
fn clear_stale_reword_edits(
    ...
    for ix in 0..entries.len() {
        ...
        let new_sources: Vec<String> =
            repositorytree_core::squash::squash_run_message_entries(entries, ix)
                .map(|s| s.commit_id.clone())
                .collect();
...
impl MainPaneView {
    ...
    fn render_irebase_row(
        ...
        let summary = st.entries[ix]
            .new_message
            .as_deref()
            .map(|m| repositorytree_core::squash::split_subject_body(m).0)
            .filter(|s| !s.is_empty())
