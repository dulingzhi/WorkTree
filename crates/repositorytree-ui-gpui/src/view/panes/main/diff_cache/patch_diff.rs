...
pub(in crate::view) struct PagedPatchDiffRows {
    diff: Arc<repositorytree_core::domain::Diff>,
    page_size: usize,
...
impl PagedPatchDiffRows {
    pub(in crate::view) fn new(diff: Arc<repositorytree_core::domain::Diff>, page_size: usize) -> Self {
    ...
    fn advance_state(
        ...
        line: &repositorytree_core::domain::DiffLine,
    ) -> DiffLineNumberState {
        match line.kind {
    ...
    fn build_page(&self, page_ix: usize) -> Option<Arc<[AnnotatedDiffLine]>> {
        ...
        for line in &self.diff.lines[start..end] {
            let (old_line, new_line) = match line.kind {
                repositorytree_core::domain::DiffLineKind::Context => (state.old_line, state.new_line),
                repositorytree_core::domain::DiffLineKind::Remove => (state.old_line, None),
                repositorytree_core::domain::DiffLineKind::Add => (None, state.new_line),
                repositorytree_core::domain::DiffLineKind::Header
                | repositorytree_core::domain::DiffLineKind::Hunk => (None, None),
            };
...
impl repositorytree_core::domain::DiffRowProvider for PagedPatchDiffRows {
...
impl PagedPatchSplitRows {
    ...
    #[cfg(test)]
    fn count_rows(lines: &[repositorytree_core::domain::DiffLine]) -> usize {
    ...
    fn scan_to_block_boundary(
        lines: &[repositorytree_core::domain::DiffLine],
    ...
    fn flush_pending(state: &mut PatchSplitMaterializationState) {
        ...
        for i in 0..pairs {
            ...
            let kind = match (left.is_some(), right.is_some()) {
                (true, true) => repositorytree_core::file_diff::FileDiffRowKind::Modify,
                (true, false) => repositorytree_core::file_diff::FileDiffRowKind::Remove,
                (false, true) => repositorytree_core::file_diff::FileDiffRowKind::Add,
                (false, false) => repositorytree_core::file_diff::FileDiffRowKind::Context,
            };
...
impl repositorytree_core::domain::DiffRowProvider for PagedPatchSplitRows {
...
pub(super) fn should_hide_unified_diff_header_raw(
    ...
    matches!(kind, repositorytree_core::domain::DiffLineKind::Header)
        && (text.starts_with("index ") || text.starts_with("--- ") || text.starts_with("+++ "))
}

pub(super) fn build_patch_split_visible_meta_from_src(
    line_kinds: &[repositorytree_core::domain::DiffLineKind],
    visual_line_kinds: &[repositorytree_core::domain::DiffLineKind],
...
mod tests {
    use super::*;
    use repositorytree_core::domain::{Diff, DiffArea, DiffTarget};
