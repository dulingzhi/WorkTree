...
fn prefixed_inline_text(prefix: char, line: &str) -> repositorytree_core::domain::SharedLineText {
...
pub(super) fn file_diff_text_signature(file: &repositorytree_core::domain::FileDiffText) -> u64 {
...
fn build_file_diff_plan_from_document_sources(
...
) -> repositorytree_core::file_diff::FileDiffPlan {
    ...
    repositorytree_core::file_diff::side_by_side_plan_from_lines(
        old_text.as_ref(),
        new_text.as_ref(),
        old_lines.as_slice(),
        new_lines.as_slice(),
...
fn plan_line_to_inline_row_maps(
    plan: &repositorytree_core::file_diff::FileDiffPlan,
    ...
    for run in &plan.runs {
        match *run {
            repositorytree_core::file_diff::FileDiffPlanRun::Context {
                old_start,
                new_start,
                len,
            } => {
            ...
            repositorytree_core::file_diff::FileDiffPlanRun::Remove { old_start, len } => {
                for offset in 0..len {
                    assign(
                        old_line_to_inline_row.as_mut_slice(),
                        old_start.saturating_add(offset),
            ...
            repositorytree_core::file_diff::FileDiffPlanRun::Add { new_start, len } => {
                for offset in 0..len {
                    assign(
                        new_line_to_inline_row.as_mut_slice(),
                        new_start.saturating_add(offset),
            ...
            repositorytree_core::file_diff::FileDiffPlanRun::Modify {
                old_start,
                new_start,
                len,
            } => {
...
impl IndexedFileDiffSource {
    ...
    fn line_text(&self, line_ix: usize) -> Option<repositorytree_core::file_diff::FileDiffLineText> {
        ...
        match &self.content {
            ...
            IndexedFileDiffContent::Shared(text) => Some(
                repositorytree_core::file_diff::FileDiffLineText::shared_slice(Arc::clone(text), range),
            ),
            IndexedFileDiffContent::File(path) => {
                Some(repositorytree_core::file_diff::FileDiffLineText::file_slice(
                    Arc::clone(path),
                    range,
...
pub(in crate::view) struct InlineFileDiffRowRenderData {
    pub(in crate::view) kind: repositorytree_core::domain::DiffLineKind,
    pub(in crate::view) old_line: Option<u32>,
    pub(in crate::view) new_line: Option<u32>,
    pub(in crate::view) text: repositorytree_core::file_diff::FileDiffLineText,
}

fn file_diff_row_flag(kind: repositorytree_core::file_diff::FileDiffRowKind) -> u8 {
    match kind {
        repositorytree_core::file_diff::FileDiffRowKind::Context => 0,
        repositorytree_core::file_diff::FileDiffRowKind::Add => 1,
        repositorytree_core::file_diff::FileDiffRowKind::Remove => 2,
        repositorytree_core::file_diff::FileDiffRowKind::Modify => 3,
    }
}
...
impl StreamedFileDiffRunStarts {
    fn build(plan: &repositorytree_core::file_diff::FileDiffPlan) -> Self {
...
fn append_file_diff_run_without_whitespace(
    run: &repositorytree_core::file_diff::FileDiffPlanRun,
...
fn visual_kinds_for_file_diff_plan(
    ...
    Box<[repositorytree_core::file_diff::FileDiffRowKind]>,
    Box<[repositorytree_core::domain::DiffLineKind]>,
) {
    use repositorytree_core::domain::DiffLineKind as DK;
    use repositorytree_core::file_diff::{FileDiffPlanRun, FileDiffRowKind as RK};
...
struct StreamedFileDiffSource {
    plan: Arc<repositorytree_core::file_diff::FileDiffPlan>,
    old_source: IndexedFileDiffSource,
    new_source: IndexedFileDiffSource,
    split_visual_kinds: Box<[repositorytree_core::file_diff::FileDiffRowKind]>,
    inline_visual_kinds: Box<[repositorytree_core::domain::DiffLineKind]>,
...
impl StreamedFileDiffSource {
    ...
    fn split_visual_kind(
        &self,
        row_ix: usize,
    ) -> Option<repositorytree_core::file_diff::FileDiffRowKind> {
        self.split_visual_kinds.get(row_ix).copied()
    }

    fn inline_visual_kind(&self, inline_ix: usize) -> Option<repositorytree_core::domain::DiffLineKind> {
        self.inline_visual_kinds.get(inline_ix).copied()
    }

    fn old_line_shared_text(&self, line_ix: usize) -> repositorytree_core::file_diff::FileDiffLineText {
        self.old_source
            .line_text(line_ix)
            .unwrap_or_else(|| repositorytree_core::file_diff::FileDiffLineText::from(""))
    }

    fn new_line_shared_text(&self, line_ix: usize) -> repositorytree_core::file_diff::FileDiffLineText {
        self.new_source
            .line_text(line_ix)
            .unwrap_or_else(|| repositorytree_core::file_diff::FileDiffLineText::from(""))
    }
    ...
    fn split_row(&self, row_ix: usize) -> Option<FileDiffRow> {
        ...
        let mut row = match run {
            repositorytree_core::file_diff::FileDiffPlanRun::Context {
                ...
                FileDiffRow {
                    kind: repositorytree_core::file_diff::FileDiffRowKind::Context,
            ...
            repositorytree_core::file_diff::FileDiffPlanRun::Remove { old_start, .. } => {
                let old_ix = old_start.saturating_add(local_ix);
                FileDiffRow {
                    kind: repositorytree_core::file_diff::FileDiffRowKind::Remove,
                    old_line: line_number(old_ix),
                    new_line: None,
                    old: Some(self.old_line_shared_text(old_ix)),
                    new: None,
            ...
            repositorytree_core::file_diff::FileDiffPlanRun::Add { new_start, .. } => {
                let new_ix = new_start.saturating_add(local_ix);
                FileDiffRow {
                    kind: repositorytree_core::file_diff::FileDiffRowKind::Add,
                    old_line: None,
                    new_line: line_number(new_ix),
                    old: None,
                    new: Some(self.new_line_shared_text(new_ix)),
            ...
            repositorytree_core::file_diff::FileDiffPlanRun::Modify {
                ...
                let new_ix = new_start.saturating_add(local_ix);
                FileDiffRow {
                    kind: repositorytree_core::file_diff::FileDiffRowKind::Modify,
                    old_line: line_number(old_ix),
                    new_line: line_number(new_ix),
                    old: Some(self.old_line_shared_text(old_ix)),
                    new: Some(self.new_line_shared_text(new_ix)),
    ...
    fn split_modify_pair_texts(
        ...
        repositorytree_core::file_diff::FileDiffLineText,
        repositorytree_core::file_diff::FileDiffLineText,
    )> {
        let (run_ix, local_ix) =
            Self::locate_run(self.split_run_starts(), self.plan.row_count, row_ix)?;
        let repositorytree_core::file_diff::FileDiffPlanRun::Modify {
            old_start,
            new_start,
            ..
    ...
    fn inline_modify_pair_texts(
        ...
        repositorytree_core::file_diff::FileDiffLineText,
        repositorytree_core::file_diff::FileDiffLineText,
        repositorytree_core::domain::DiffLineKind,
    )> {
        let (run_ix, local_ix) = Self::locate_run(
            self.inline_run_starts(),
            self.plan.inline_row_count,
        ...
        crate::view::diff_navigation::change_block_entries(len, |row_ix| {
            if inline {
                ...
                self.split_visual_kind(row_ix).is_some_and(|kind| {
                    !matches!(kind, repositorytree_core::file_diff::FileDiffRowKind::Context)
                })
            }
        })
    }
    ...
    fn inline_scrollbar_markers(&self) -> Vec<components::ScrollbarMarker> {
        ...
        while start < self.plan.inline_row_count {
            let flag = match self.inline_visual_kind(start) {
                Some(repositorytree_core::domain::DiffLineKind::Add) => 1,
                Some(repositorytree_core::domain::DiffLineKind::Remove) => 2,
                _ => 0,
            };
            let mut end = start.saturating_add(1);
    ...
    fn build_inline_text(&self) -> SharedString {
        ...
        let mut push_line = |prefix: char, line: repositorytree_core::file_diff::FileDiffLineText| {
            text.push(prefix);
        ...
        for run in self.plan.runs.as_slice() {
            match *run {
                repositorytree_core::file_diff::FileDiffPlanRun::Context { new_start, len, .. } => {
                    for offset in 0..len {
                        push_line(
                            ' ',
...
impl repositorytree_core::domain::DiffRowProvider for PagedFileDiffRows {
...
impl PagedFileDiffInlineRows {
    ...
    pub(in crate::view) fn visual_kind(
        &self,
        inline_ix: usize,
    ) -> Option<repositorytree_core::domain::DiffLineKind> {
...
impl repositorytree_core::domain::DiffRowProvider for PagedFileDiffInlineRows {
...
fn build_file_diff_plan_from_indexed_sources(
    old_source: &IndexedFileDiffSource,
    new_source: &IndexedFileDiffSource,
) -> Result<repositorytree_core::file_diff::FileDiffPlan, String> {
...
fn index_file_diff_side(
    source: Option<&repositorytree_core::domain::FileDiffTextSource>,
...
fn file_diff_plan_from_runs(
    runs: Vec<repositorytree_core::file_diff::FileDiffPlanRun>,
) -> repositorytree_core::file_diff::FileDiffPlan {
...
fn push_file_diff_plan_run(
    runs: &mut Vec<repositorytree_core::file_diff::FileDiffPlanRun>,
    run: repositorytree_core::file_diff::FileDiffPlanRun,
) {
    if run.row_len() == 0 {
        return;
...
fn push_aligned_file_diff_span(
    ...
    if old_len > context_len {
        push_file_diff_plan_run(
            runs,
            repositorytree_core::file_diff::FileDiffPlanRun::Remove {
    ...
    if new_len > context_len {
        push_file_diff_plan_run(
            runs,
            repositorytree_core::file_diff::FileDiffPlanRun::Add {
...
fn build_file_diff_plan_from_patch(
    ...
    while ix < diff.lines.len() {
        ...
        if !matches!(line.kind, repositorytree_core::domain::DiffLineKind::Hunk) {
            ix += 1;
            continue;
        }
        ...
        while ix < diff.lines.len() {
            ...
            if matches!(
                line.kind,
                repositorytree_core::domain::DiffLineKind::Hunk
                    | repositorytree_core::domain::DiffLineKind::Header
            ) {
                break;
            }
...
pub(in crate::view) fn build_file_diff_cache_rebuild(
    file: &repositorytree_core::domain::FileDiffText,
...
pub(in crate::view) fn build_file_diff_cache_rebuild_with_patch(
    file: &repositorytree_core::domain::FileDiffText,
    workdir: &std::path::Path,
    patch_diff: Option<&repositorytree_core::domain::Diff>,
    whitespace_mode: DiffWhitespaceMode,
) -> Result<FileDiffCacheRebuild, String> {
    ...
    let (old_line_to_row, new_line_to_row) = repositorytree_core::file_diff::plan_line_to_row_maps(
        plan.as_ref(),
        old_line_count,
        new_line_count,
    );
...
mod tests {
    ...
    fn file_diff_visual_kinds_ignore_whitespace_only_changes() {
        ...
        assert_eq!(
            split_raw,
            vec![repositorytree_core::file_diff::FileDiffRowKind::Modify]
        );
        assert_eq!(
            inline_raw,
            vec![
        ...
        assert_eq!(
            split_visual,
            vec![repositorytree_core::file_diff::FileDiffRowKind::Context]
        );
        assert_eq!(
            inline_visual,
            vec![
    ...
    fn file_diff_visual_kinds_ignore_line_break_only_changes() {
        ...
        assert!(
            split_visual
                .iter()
                .all(|kind| *kind == repositorytree_core::file_diff::FileDiffRowKind::Context)
        );
        assert!(
            inline_visual
                .iter()
                .all(|kind| *kind == repositorytree_core::domain::DiffLineKind::Context)
        );
        assert!(split.change_visible_indices().is_empty());
        assert!(inline.change_visible_indices().is_empty());
    }
    ...
    fn build_file_diff_cache_rebuild_preserves_real_document_sources() {
        let file = repositorytree_core::domain::FileDiffText::new(
    ...
    fn build_file_diff_cache_rebuild_reports_invalid_source_file() {
        ...
        let file = repositorytree_core::domain::FileDiffText::new_sources(
            PathBuf::from("binary.dat"),
            None,
            Some(repositorytree_core::domain::FileDiffTextSource::with_identity(
                source_path.clone(),
                "invalid-source",
            )),
        );
    ...
    fn build_file_diff_cache_rebuild_source_backed_without_patch_uses_source_content() {
        ...
        assert!(rebuild.rows.iter().any(|row| {
            row.kind != repositorytree_core::file_diff::FileDiffRowKind::Context
