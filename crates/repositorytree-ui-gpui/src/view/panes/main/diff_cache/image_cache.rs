...
const IMAGE_DIFF_CACHE_FILE_PREFIX: &str = "repositorytree-image-diff-";
...
fn file_image_diff_signature(file: &repositorytree_core::domain::FileDiffImage) -> u64 {
...
fn build_file_image_diff_cache_rebuild(
    file: &repositorytree_core::domain::FileDiffImage,
    ...
    pub(in crate::view) fn ensure_file_image_diff_cache(&mut self, cx: &mut gpui::Context<Self>) {
        let Some((repo_id, diff_file_rev, diff_target, workdir, expected_abs_path, file)) =
            (|| {
                ...
                let file: Option<Arc<repositorytree_core::domain::FileDiffImage>> =
...
<rect x="{inset_x}" y="{inset_y}" width="{inner_width}" height="{inner_height}" fill="#00aaff"/>
    ...
    fn file_image_diff_signature_changes_with_payload() {
        let base = repositorytree_core::domain::FileDiffImage {
            path: Path::new("image.png").to_path_buf(),
            old: Some(vec![1, 2, 3]),
            new: Some(vec![4, 5, 6]),
        };
        let changed = repositorytree_core::domain::FileDiffImage {
            path: Path::new("image.png").to_path_buf(),
    ...
    fn build_file_image_diff_cache_rebuild_resolves_absolute_preview_path() {
        ...
        let file = repositorytree_core::domain::FileDiffImage {
...
<rect width="16" height="16" fill="#00aaff"/>
    ...
    fn cleanup_image_diff_cache_dir_removes_stale_prefixed_files() {
        ...
        let stale = write_test_file(
            temp_dir.path(),
            "repositorytree-image-diff-stale.svg",
            b"old-cache",
        );
    ...
    fn cleanup_image_diff_cache_dir_prunes_to_max_total_size() {
        let temp_dir = tempfile::tempdir().expect("tempdir");
        let a = write_test_file(temp_dir.path(), "repositorytree-image-diff-a.svg", b"1234");
        let b = write_test_file(temp_dir.path(), "repositorytree-image-diff-b.svg", b"1234");
        let c = write_test_file(temp_dir.path(), "repositorytree-image-diff-c.svg", b"1234");
