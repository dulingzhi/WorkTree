...
fn conflict_session_plan_projection(
    ...
    for block_index in &session.region_plan_blocks {
        projection.replace_selection(*block_index, repositorytree_core::merge::OrderedSelection::new());
    }
    let projected_plan_blocks = projection.unresolved_blocks.clone();
    let options = repositorytree_core::merge::MergeOptions {
        style: if projection.has_base() {
            repositorytree_core::merge::ConflictStyle::Diff3
        } else {
            repositorytree_core::merge::ConflictStyle::Merge
        },
        ..Default::default()
    };
    Some((
        Arc::from(repositorytree_core::merge::render_merge_plan(&projection, &options).output),
...
fn conflict_file_source_fingerprint(file: &repositorytree_state::model::ConflictFile) -> u64 {
...
impl MainPaneView {
    ...
    pub(super) fn sync_conflict_resolver(&mut self, cx: &mut gpui::Context<Self>) {
        ...
        let conflict_entry = repo
            ...
            .filter(|entry| entry.kind == repositorytree_core::domain::FileStatusKind::Conflicted);
        ...
        if should_load {
            ...
            self.store.dispatch(Msg::LoadConflictFile {
                repo_id,
                path,
                mode: repositorytree_state::model::ConflictFileLoadMode::CurrentOnly,
            });
            return;
        }
        ...
        let shared_path = repositorytree_state::msg::RepoPath::from(path.clone());
        ...
        let current_text = session
            .and_then(|session| match session.current.as_ref() {
                Some(repositorytree_core::conflict_session::ConflictPayload::Text(text)) => {
                    Some(text.clone())
                }
                _ => None,
            })
        ...
        let three_way_aligned =
            if let Some(plan) = session.and_then(|session| session.merge_plan.as_ref()) {
                conflict_resolver::ThreeWayAlignedMap::from_alignment(
                    &repositorytree_core::merge::align_merge_plan(plan),
                )
            ...
            {
                conflict_resolver::ThreeWayAlignedMap::from_alignment(
                    &repositorytree_core::merge::align_two_way(
                        ours_text,
                        theirs_text,
                        repositorytree_core::merge::DiffAlgorithm::Myers,
                    ),
            ...
            && let Some(message) = conflict_resolver::format_open_summary_toast(counts)
        {
            ...
            // The sync runs inside a RepositoryTreeView update; push the
        ...
        {
            let _ = self
                .request_conflict_file_load_mode(repositorytree_state::model::ConflictFileLoadMode::Full);
        }
        mergetool_trace::record_with(|| {
            trace_ctx
                .bootstrap_event(
    ...
    pub(in crate::view) fn request_conflict_file_load_mode(
        &mut self,
        mode: repositorytree_state::model::ConflictFileLoadMode,
    ) -> bool {
        let Some(repo_id) = self.active_repo_id() else {
            return false;
        };
    ...
    pub(in crate::view) fn conflict_resolver_unresolve_active_conflict(
            ...
            )
        {
            self.store.dispatch(Msg::ConflictReplacePlanBlockSelection {
                ...
                path,
                block_id,
                selection: repositorytree_core::merge::OrderedSelection::new(),
            });
            cx.notify();
            return;
        }
    ...
    fn conflict_resolver_apply_block_choice(
        ...
        let dispatch = {
            ...
            if choice.contains(repositorytree_core::conflict_output::ConflictOutputSource::Base)
                && !has_base
            {
                return false;
            }
            let to_merge_source = |source| {
                use repositorytree_core::conflict_output::ConflictOutputSource as Output;
            ...
            if choice == conflict_resolver::ConflictChoice::Both {
                block.choice = choice;
                block.resolved = true;
                Some(Ok(repositorytree_core::merge::OrderedSelection::from_sources(
    ...
    fn conflict_resolver_apply_plan_block_choice(
        &mut self,
        block_id: repositorytree_core::merge::MergeBlockId,
        ...
        if choice == conflict_resolver::ConflictChoice::Both {
            self.store.dispatch(Msg::ConflictReplacePlanBlockSelection {
                ...
                path,
                block_id,
                selection: repositorytree_core::merge::OrderedSelection::from_sources([
                    local_source,
                    remote_source,
                ]),
            });
    ...
    fn conflict_resolver_dispatch_session_resolution_updates(
        ...
        let updates = updates
            .into_iter()
            .map(
                |(region_index, resolution)| repositorytree_state::msg::ConflictRegionResolutionUpdate {
                    region_index,
                    resolution,
                },
            )
    ...
    pub(in crate::view) fn conflict_autosolve_confidence_for_ix(
        &self,
        conflict_ix: usize,
    ) -> Option<repositorytree_core::conflict_session::AutosolveConfidence> {
        let region_ix = self
            .conflict_resolver
            .conflict_region_indices
            .get(conflict_ix)
        ...
        match &session.regions.get(region_ix)?.resolution {
            repositorytree_core::conflict_session::ConflictRegionResolution::AutoResolved {
                confidence,
                ..
            } => Some(*confidence),
            _ => None,
    ...
    fn with_conflict_resolver_session<T: Default>(
        &self,
        read: impl FnOnce(&repositorytree_core::conflict_session::ConflictSession) -> T,
    ) -> T {
    ...
    fn conflict_resolver_session_has_base(&self) -> bool {
        self.with_conflict_resolver_session(|session| {
            session
                .merge_plan
                .as_ref()
                .is_some_and(repositorytree_core::merge::MergePlan::has_base)
        })
    }
    ...
    pub(in crate::view) fn conflict_resolver_choose_everywhere(
        ...
        self.conflict_resolver_dispatch_bulk_choice(
            choice,
            repositorytree_state::msg::ConflictBulkScope::AllDeltas,
            cx,
        );
    }
    ...
    fn conflict_resolver_dispatch_bulk_choice(
        &mut self,
        choice: conflict_resolver::ConflictChoice,
        scope: repositorytree_state::msg::ConflictBulkScope,
        cx: &mut gpui::Context<Self>,
    ) {
        let bulk_choice = if choice == conflict_resolver::ConflictChoice::Base {
            repositorytree_state::msg::ConflictBulkChoice::Base
        } else if choice == conflict_resolver::ConflictChoice::Ours {
            repositorytree_state::msg::ConflictBulkChoice::Ours
        } else if choice == conflict_resolver::ConflictChoice::Theirs {
            repositorytree_state::msg::ConflictBulkChoice::Theirs
        } else if choice == conflict_resolver::ConflictChoice::Both {
            repositorytree_state::msg::ConflictBulkChoice::Both
        } else {
            return;
        };
        let (Some(repo_id), Some(path)) = (
...
mod tests {
    ...
    fn session_with_automatic_delta() -> repositorytree_core::conflict_session::ConflictSession {
    ...
    fn conflict_file_source_fingerprint_is_stable_across_fresh_allocations() {
        let make_file = || repositorytree_state::model::ConflictFile {
