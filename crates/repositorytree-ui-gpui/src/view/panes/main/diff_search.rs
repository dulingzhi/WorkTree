...
use repositorytree_core::domain::Diff;
...
fn collect_file_diff_line_text_literal_stream_match_visible_rows(
    rows: impl IntoIterator<Item = (usize, repositorytree_core::file_diff::FileDiffLineText)>,
...
impl MainPaneView {
    pub(in crate::view) fn active_conflict_target(
        ...
        std::path::PathBuf,
        Option<repositorytree_core::domain::FileConflictKind>,
    )> {
...
mod tests {
    ...
    fn diff_search_file_slice_stream_collector_normalizes_crlf_row_endings() {
        ...
        let rows = [
            (
                10,
                repositorytree_core::file_diff::FileDiffLineText::file_slice(
                    Arc::clone(&path),
                    0..4,
                    true,
                    false,
    ...
    fn diff_search_file_slice_general_collector_does_not_materialize_large_rows() {
        ...
        let materialized = repositorytree_core::file_diff::FileDiffLineText::file_slice(
            Arc::clone(&path),
            0..source.len(),
            false,
            false,
        ...
        let mut matches = Vec::new();
        let streamed = repositorytree_core::file_diff::FileDiffLineText::file_slice(
            Arc::clone(&path),
            0..source.len(),
            false,
            false,
        ...
        let regex_streamed = repositorytree_core::file_diff::FileDiffLineText::file_slice(
            Arc::clone(&path),
            0..source.len(),
            false,
            false,
    ...
    fn diff_search_file_slice_literal_stream_respects_unicode_whole_word_boundaries() {
        ...
        let mut matches = Vec::new();
        let row1_slice = repositorytree_core::file_diff::FileDiffLineText::file_slice(
            Arc::clone(&path),
            0..row1.len(),
            false,
            false,
        ...
        let row2_slice = repositorytree_core::file_diff::FileDiffLineText::file_slice(
            Arc::clone(&path),
