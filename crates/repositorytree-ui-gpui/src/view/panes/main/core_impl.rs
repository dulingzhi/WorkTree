...
fn diff_wrap_byte_ranges_for_optional_file_diff_text(
    text: Option<&repositorytree_core::file_diff::FileDiffLineText>,
    columns: usize,
    reveal_whitespace_chars: bool,
) -> Vec<rows::DiffWrapByteRange> {
    text.map(|text| {
...
/// [`repositorytree_core::services::Repo::blame_worktree_file`]); lines not yet
...
fn blame_path_rev_for_target(
    target: &DiffTarget,
) -> Option<(std::path::PathBuf, repositorytree_core::domain::BlameSource)> {
    ...
    match target {
        ...
        DiffTarget::CommitRange {
            ...
            match to_commit_id {
                ...
                None => BlameSource::WorkingTree(repositorytree_core::domain::DiffArea::Unstaged),
            },
        )),
        _ => None,
    }
...
impl MainPaneView {
    ...
    pub(in crate::view) fn focused_mergetool_save_and_exit(
        ...
        use repositorytree_core::conflict_output::ConflictMarkerLabels;
    ...
    pub(in crate::view) fn active_inline_submodule_diff(
        &self,
    ) -> Option<&repositorytree_state::model::InlineSubmoduleDiffState> {
        self.active_repo()?
            .diff_state
    ...
    pub(in crate::view) fn selected_inline_submodule_diff_entry(
        &self,
    ) -> Option<&repositorytree_state::model::InlineSubmoduleDiffEntry> {
    ...
    pub(in crate::view) fn rendered_patch_diff_loadable(
        &self,
    ) -> Option<&repositorytree_state::model::Loadable<repositorytree_state::model::Shared<Diff>>> {
    ...
    pub(in crate::view) fn rendered_file_diff_loadable(
        &self,
    ) -> Option<&repositorytree_state::model::Loadable<Option<repositorytree_state::model::Shared<FileDiffText>>>>
    {
        if let Some(inline) = self.active_inline_submodule_diff() {
    ...
    pub(in crate::view) fn rendered_file_image_diff_loadable(
        ...
        &repositorytree_state::model::Loadable<Option<repositorytree_state::model::Shared<FileDiffImage>>>,
    > {
        if let Some(inline) = self.active_inline_submodule_diff() {
    ...
    fn patch_src_ixs_for_file_line(
        ...
        for src_ix in 0..self.patch_diff_row_len() {
            ...
            let matched = match line.kind {
                repositorytree_core::domain::DiffLineKind::Add => line.new_line == new_line,
                repositorytree_core::domain::DiffLineKind::Remove
                | repositorytree_core::domain::DiffLineKind::Context => line.old_line == old_line,
                repositorytree_core::domain::DiffLineKind::Header
                | repositorytree_core::domain::DiffLineKind::Hunk => false,
            };
            if matched {
                out.push(src_ix);
            }
...
fn should_request_blame<T>(
    same_target: bool,
    blame: &repositorytree_state::model::Loadable<T>,
    force: bool,
) -> bool {
...
mod tests {
    ...
    fn notify_fingerprint_tracks_cherry_pick_message_readiness() {
        ...
        use repositorytree_state::model::{InteractiveCherryPickSetup, RepoState};
