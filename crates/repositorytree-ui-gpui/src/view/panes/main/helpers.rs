...
pub(super) fn worktree_output_requires_protection(
    ...
    if repositorytree_core::text_utils::text_has_mixed_line_endings(current) {
        return true;
    }

    let marker_ranges = repositorytree_core::conflict_session::parse_conflict_marker_ranges(current);
    if !marker_ranges.iter().any(|segment| {
        matches!(
            segment,
            repositorytree_core::conflict_session::ParsedConflictSegmentRanges::Conflict(_)
        )
    }) {
        return true;
    }
...
fn conflict_document_content_lines<'a>(
    current: &'a str,
    marker_ranges: &'a [repositorytree_core::conflict_session::ParsedConflictSegmentRanges],
) -> impl Iterator<Item = &'a str> {
    use repositorytree_core::conflict_session::ParsedConflictSegmentRanges as Segment;
...
impl FileDiffStyleCacheEpochs {
    ...
    pub(in crate::view) fn inline_epoch(self, kind: repositorytree_core::domain::DiffLineKind) -> u64 {
        match kind {
            repositorytree_core::domain::DiffLineKind::Remove => self.split_left,
            repositorytree_core::domain::DiffLineKind::Add
            | repositorytree_core::domain::DiffLineKind::Context => self.split_right,
            repositorytree_core::domain::DiffLineKind::Header
            | repositorytree_core::domain::DiffLineKind::Hunk => 0,
        }
    }
}
...
pub(super) fn unresolved_subchunk_conflict_ranges_for_block(
    ...
    use repositorytree_core::conflict_session::Subchunk;

    let base = block.base.as_deref()?;
    let subchunks = repositorytree_core::conflict_session::split_conflict_into_subchunks(
        base,
        &block.ours,
        &block.theirs,
    )?;
...
pub(super) fn unresolved_decision_regions_for_block(
    ...
    let plan = repositorytree_core::file_diff::side_by_side_plan(left, right);
    if plan.row_count == 0 {
        return None;
    }
    let regions = repositorytree_core::file_diff::plan_row_region_anchors(&plan).region_anchors;
    if regions.is_empty() {
        return None;
    }
    let (old_prefix, new_prefix) = repositorytree_core::file_diff::plan_emitted_line_prefix_counts(&plan);
        ...
        .filter(|&ix| {
            ...
            match choice {
                conflict_resolver::ConflictChoice::Base => block
                    .choice
                    .contains(repositorytree_core::conflict_output::ConflictOutputSource::Base),
                conflict_resolver::ConflictChoice::Ours => block
                    .choice
                    .contains(repositorytree_core::conflict_output::ConflictOutputSource::Ours),
                conflict_resolver::ConflictChoice::Theirs => block
                    .choice
                    .contains(repositorytree_core::conflict_output::ConflictOutputSource::Theirs),
...
pub(super) fn clear_diff_selection_action(view_mode: RepositoryTreeViewMode) -> ClearDiffSelectionAction {
    match view_mode {
        RepositoryTreeViewMode::Normal => ClearDiffSelectionAction::ClearSelection,
        RepositoryTreeViewMode::FocusedMergetool => ClearDiffSelectionAction::ExitFocusedMergetool,
    }
}
...
pub(super) fn conflict_strategy_needs_full_side_payloads(
    strategy: Option<repositorytree_core::conflict_session::ConflictResolverStrategy>,
) -> bool {
    matches!(
        strategy,
        Some(
            repositorytree_core::conflict_session::ConflictResolverStrategy::BinarySidePick
                | repositorytree_core::conflict_session::ConflictResolverStrategy::TwoWayKeepDelete
                | repositorytree_core::conflict_session::ConflictResolverStrategy::DecisionOnly
        )
    )
}
...
pub(super) fn build_focused_mergetool_save_payload(
    ...
    labels: repositorytree_core::conflict_output::ConflictMarkerLabels<'_>,
) -> FocusedMergetoolSavePayload {
    use repositorytree_core::conflict_output::{GenerateResolvedTextOptions, UnresolvedConflictMode};
    ...
    if let Some(output_text) = materialized_output_text {
        if let Some(updates) = conflict_resolver::derive_region_resolution_updates_from_output(
                ...
                .rev()
            {
                if matches!(
                    resolution,
                    repositorytree_core::conflict_session::ConflictRegionResolution::Unresolved
                ) {
                    let marker_text =
...
pub(in crate::view) type BlameTimeRangeCache = Option<(
    std::sync::Arc<Vec<repositorytree_core::services::BlameLine>>,
    Option<(i64, i64)>,
)>;
...
pub(crate) struct MainPaneView {
    ...
    pub(in crate::view) view_mode: RepositoryTreeViewMode,
    ...
    pub(in crate::view) root_view: WeakEntity<RepositoryTreeView>,
    ...
    pub(in crate::view) diff_line_kind_for_src_ix: Vec<repositorytree_core::domain::DiffLineKind>,
    pub(in crate::view) diff_visual_line_kind_for_src_ix: Vec<repositorytree_core::domain::DiffLineKind>,
...
pub(in crate::view) struct IRebaseViewState {
    ...
    pub(in crate::view) entries: Vec<repositorytree_core::services::InteractiveRebaseEntry>,
    pub(in crate::view) original_entries: Vec<repositorytree_core::services::InteractiveRebaseEntry>,
    ...
    pub(in crate::view) folded:
        FxHashMap<String, Vec<repositorytree_core::services::InteractiveRebaseEntry>>,
...
pub(super) fn conflict_canvas_rows_enabled_from_env() -> bool {
    std::env::var("REPOSITORYTREE_CONFLICT_CANVAS_ROWS")
        .ok()
