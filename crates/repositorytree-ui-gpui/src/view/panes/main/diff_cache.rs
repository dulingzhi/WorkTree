...
use crate::view::rows;
use repositorytree_core::domain::DiffRowProvider;
...
fn patch_diff_content_signature(diff: &repositorytree_core::domain::Diff) -> u64 {
    use std::hash::Hasher;
    ...
    for line in diff.lines.iter() {
        let kind = match line.kind {
            repositorytree_core::domain::DiffLineKind::Header => 0,
            repositorytree_core::domain::DiffLineKind::Hunk => 1,
            repositorytree_core::domain::DiffLineKind::Add => 2,
            repositorytree_core::domain::DiffLineKind::Remove => 3,
            repositorytree_core::domain::DiffLineKind::Context => 4,
        };
...
fn diff_line_content_text(line: &repositorytree_core::domain::DiffLine) -> &str {
    match line.kind {
        ...
        repositorytree_core::domain::DiffLineKind::Header | repositorytree_core::domain::DiffLineKind::Hunk => {
            &line.text
        }
    }
}
...
fn is_patch_diff_whitespace_group_line(line: &repositorytree_core::domain::DiffLine) -> bool {
    matches!(
        line.kind,
        repositorytree_core::domain::DiffLineKind::Remove | repositorytree_core::domain::DiffLineKind::Add
...
) -> Vec<repositorytree_core::domain::DiffLineKind> {
...
fn file_diff_text_is_source_backed(file: &repositorytree_core::domain::FileDiffText) -> bool {
...
impl MainPaneView {
    ...
    pub(in crate::view) fn file_diff_split_visual_kind(
        ...
        self.file_diff_row_provider
            ...
            .unwrap_or(repositorytree_core::file_diff::FileDiffRowKind::Context)
    }
    ...
    pub(in crate::view) fn file_diff_inline_visual_kind(
        ...
        self.file_diff_inline_row_provider
            ...
            .unwrap_or(repositorytree_core::domain::DiffLineKind::Context)
    }
    ...
    pub(in crate::view) fn collapsed_hunk_change_summary(&self, src_ix: usize) -> (bool, bool) {
        ...
        for candidate_ix in src_ix.saturating_add(1)..self.patch_diff_row_len() {
            ...
            match self.patch_visual_line_kind(candidate_ix) {
                repositorytree_core::domain::DiffLineKind::Add => has_additions = true,
                repositorytree_core::domain::DiffLineKind::Remove => has_removals = true,
                repositorytree_core::domain::DiffLineKind::Context
    ...
    pub(in super::super::super) fn ensure_file_diff_cache(&mut self, cx: &mut gpui::Context<Self>) {
        let Some((
            ...
            let file: Option<Arc<repositorytree_core::domain::FileDiffText>> =
            ...
            let patch_diff: Option<Arc<repositorytree_core::domain::Diff>> = match patch_diff_loadable {
    ...
    pub(in super::super::super) fn rebuild_diff_cache(&mut self, cx: &mut gpui::Context<Self>) {
        ...
        self.diff_line_kind_for_src_ix = diff
            ...
            .map(|line| {
                match line.kind {
                    repositorytree_core::domain::DiffLineKind::Remove => pending_split_removes += 1,
                    repositorytree_core::domain::DiffLineKind::Add => pending_split_adds += 1,
                    repositorytree_core::domain::DiffLineKind::Context
                    | repositorytree_core::domain::DiffLineKind::Header
                    | repositorytree_core::domain::DiffLineKind::Hunk => {
        ...
        self.diff_click_kinds = diff
            ...
            .map(|line| {
                ...
                } else if matches!(line.kind, repositorytree_core::domain::DiffLineKind::Header)
        ...
        for (src_ix, line) in diff.lines.iter().enumerate() {
            ...
            let language = match line.kind {
                repositorytree_core::domain::DiffLineKind::Add
                | repositorytree_core::domain::DiffLineKind::Remove
                | repositorytree_core::domain::DiffLineKind::Context => current_language,
                repositorytree_core::domain::DiffLineKind::Header
                | repositorytree_core::domain::DiffLineKind::Hunk => None,
            };
            ...
            DiffViewMode::Inline => {
                scrollbar_markers_from_flags(self.diff_visible_len(), |visible_ix| {
                    ...
                    match self.patch_visual_line_kind(src_ix) {
                        repositorytree_core::domain::DiffLineKind::Add => 1,
                        repositorytree_core::domain::DiffLineKind::Remove => 2,
                        _ => 0,
                    }
                })
            }
...
mod tests {
    ...
    fn file_diff_style_cache_epochs_map_rows_to_matching_side() {
        ...
        assert_eq!(
            epochs.inline_epoch(repositorytree_core::domain::DiffLineKind::Remove),
            11
        );
        assert_eq!(
            epochs.inline_epoch(repositorytree_core::domain::DiffLineKind::Add),
            23
        );
        assert_eq!(
            epochs.inline_epoch(repositorytree_core::domain::DiffLineKind::Context),
            23
        );
        assert_eq!(
            epochs.inline_epoch(repositorytree_core::domain::DiffLineKind::Header),
            0
        );
        assert_eq!(
            epochs.inline_epoch(repositorytree_core::domain::DiffLineKind::Hunk),
            0
        );
    }
    ...
    fn pictures_are_measured_from_their_headers_without_decoding_them() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "repositorytree_ui_test_{}_measure_pictures",
            std::process::id()
        ));
