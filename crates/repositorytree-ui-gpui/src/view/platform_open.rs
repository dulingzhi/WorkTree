...
mod windows_tests {
    ...
    fn windows_shell_path_normalizes_mixed_separators() {
        let mixed = Path::new(r"C:\git\RepositoryTree\crates/repositorytree-ui-gpui/src/smoke_tests.rs");
    ...
    fn windows_shell_path_strips_verbatim_prefix() {
        let prefixed = Path::new(r"\\?\C:\git\RepositoryTree\src\main.rs");
        let normalized = windows_shell_normalized_path(prefixed)
            .display()
            .to_string();
        assert!(!normalized.starts_with(r"\\?\"));
        assert_eq!(normalized, r"C:\git\RepositoryTree\src\main.rs");
    }
}
