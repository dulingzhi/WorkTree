...
pub(crate) struct RepositoryTreeHttpClient {
    agent: ureq::Agent,
    user_agent: String,
}

impl RepositoryTreeHttpClient {
...
impl HttpClient for RepositoryTreeHttpClient {
    fn get(
        &self,
        url: &str,
...
pub(crate) fn client() -> Arc<dyn HttpClient> {
    Arc::new(RepositoryTreeHttpClient::new())
}
...
mod tests {
    ...
    fn user_agent_identifies_the_app_and_platform() {
        let client = RepositoryTreeHttpClient::new();

        assert!(
            client.user_agent.starts_with("RepositoryTree/"),
