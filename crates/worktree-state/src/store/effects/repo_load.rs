    ...
    spawn_detached_with_repo_or_else(
        ...
        move |repo, msg_tx| {
            let result = {
                ...
                let mut on_chunk = |chunk: worktree_core::services::LogChunk| {
                    send_or_log(
                        &msg_tx,
...
pub(super) fn schedule_load_conflict_file(
    ...
    spawn_with_repo(executor, repos, repo_id, msg_tx, move |repo, msg_tx| {
        ...
        let result = if let Some(session) = session_ref {
            ...
            stages.map(|opt| {
                opt.map(|d| {
                    let worktree_core::services::ConflictFileStages {
                        path,
                        base_bytes,
                        ours_bytes,
                        theirs_bytes,
...
pub(super) fn schedule_load_file_browser(
    ...
    spawn_with_repo_or_else(
        ...
        move |repo, msg_tx| {
            let result = match &source {
                worktree_core::domain::FileSource::WorkingDirectory => repo.list_worktree_files(),
                worktree_core::domain::FileSource::Commit(commit_id) => {
                    repo.list_tree_files_at_commit(commit_id)
                }
                worktree_core::domain::FileSource::Branch(_name) => {
                    Err(Error::new(worktree_core::error::ErrorKind::Backend(
...
pub(super) fn schedule_resolve_commit_for_reveal(
    ...
    msg_tx: StoreWorkerSender,
    repo_id: RepoId,
    reference: worktree_core::domain::CommitId,
) {
    spawn_with_repo(executor, repos, repo_id, msg_tx, move |repo, msg_tx| {
        send_or_log(
            &msg_tx,
...
pub(super) fn schedule_load_range_files(
    ...
    msg_tx: StoreWorkerSender,
    repo_id: RepoId,
    from: worktree_core::domain::CommitId,
    to: Option<worktree_core::domain::CommitId>,
    request: u64,
) {
    spawn_with_repo(executor, repos, repo_id, msg_tx, move |repo, msg_tx| {
...
pub(super) struct SquashRebaseSetupRequest {
    pub base: worktree_core::domain::CommitId,
    pub actual_head: worktree_core::domain::CommitId,
    pub selected_ids: Vec<worktree_core::domain::CommitId>,
    pub reword_id: worktree_core::domain::CommitId,
    pub message: String,
    pub count: usize,
}
...
pub(super) fn schedule_open_file_at_commit(
    ...
    spawn_with_repo(executor, repos, repo_id, msg_tx, move |repo, msg_tx| {
        ...
        send_or_log(
            ...
            Msg::OpenFileContent {
                repo_id,
                source: worktree_core::domain::FileSource::Commit(commit_id),
                path: resolved,
            },
        );
    });
            ...
            Ok(details) => {
                if let Some(parent) = details.parent_ids.first() {
                    ...
                    send_or_log(
                        ...
                        Msg::OpenFileContent {
                            repo_id,
                            source: worktree_core::domain::FileSource::Commit(parent.clone()),
                            path: resolved,
                        },
                    );
                }
...
pub(super) fn schedule_load_interactive_cherry_pick_messages(
    ...
    spawn_detached_with_repo_or_else(
        ...
        move |repo, msg_tx| {
            let commit_ids = ids
                .iter()
                .map(|id| worktree_core::domain::CommitId(id.clone().into()))
                .collect::<Vec<_>>();
            let result = repo
...
mod worktree_dirty_tests {
    use super::*;
    use worktree_core::domain::{CommitId, FileStatus, FileStatusKind};
...
mod worktree_scan_handle_tests {
    ...
    impl GitBackend for CountingBackend {
        fn open(&self, workdir: &Path) -> worktree_core::services::Result<Arc<dyn GitRepository>> {
            self.opens
