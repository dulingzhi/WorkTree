...
const GITIGNORE_FILE_NAME: &str = worktree_core::gitignore::FILE_NAME;
...
pub(super) fn schedule_append_gitignore_patterns(
    ...
    schedule_repo_command(
        ...
        move |repo| {
            ...
            let Some(updated) = worktree_core::gitignore::append_patterns(&existing, &patterns)
            else {
                return Ok(CommandOutput {
                    ...
                    .to_string(),
                    stdout: worktree_core::gitignore::NOTHING_TO_ADD.to_string(),
                    stderr: String::new(),
                    exit_code: Some(0),
                });
            };
                ...
                Ok(mergetool_result) => {
                    if mergetool_result.success {
                        ...
                        Err(worktree_core::error::Error::new(
                            worktree_core::error::ErrorKind::Backend(
                                rust_i18n::t!(
