    ...
    pub(super) fn shared_for_store(pool: StoreExecutorPool, threads: usize) -> Self {
        ...
        let tx = match pool {
            StoreExecutorPool::Primary => {
                sender_for(&PRIMARY, "worktree-test-store-primary", threads)
            }
            StoreExecutorPool::RepoLoad => {
                sender_for(&REPO_LOAD, "worktree-test-store-repo-load", threads)
            }
            StoreExecutorPool::Metadata => {
                sender_for(&METADATA, "worktree-test-store-metadata", threads)
            }
            StoreExecutorPool::SessionPersist => sender_for(
                &SESSION_PERSIST,
                "worktree-test-store-session-persist",
                threads,
            ),
        };
