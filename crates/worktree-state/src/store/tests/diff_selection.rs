...
fn select_diff_for_untracked_file_skips_patch_diff_and_loads_file_preview() {
    ...
    assert!(matches!(
        ...
        [Effect::LoadSelectedDiff {
            ...
            load_file_text: false,
            preview_text_side: Some(worktree_core::domain::DiffPreviewTextSide::New),
            load_file_image: false,
            load_submodule_summary: false,
        }]
    ));
...
fn select_diff_for_deleted_file_replaced_by_directory_loads_deleted_preview() {
    ...
    assert!(matches!(
        ...
        [Effect::LoadSelectedDiff {
            ...
            load_patch_diff: false,
            load_file_text: false,
            preview_text_side: Some(worktree_core::domain::DiffPreviewTextSide::Old),
            load_file_image: false,
            load_submodule_summary: false,
        }]
    ));
...
fn select_diff_for_staged_deleted_head_gitlink_loads_submodule_summary() {
    ...
    let target = worktree_core::domain::DiffTarget::WorkingTree {
        path: submodule_path.clone(),
        area: worktree_core::domain::DiffArea::Staged,
    };
...
fn stale_inline_submodule_file_load_is_ignored() {
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::InlineSubmoduleDiffFileLoaded {
            ...
            inline_rev: 0,
            target: target.clone(),
            result: Ok(Some(worktree_core::domain::FileDiffText::new(
...
fn submodule_summary_refresh_reloads_open_inline_diff_when_selected_target_remains() {
    ...
    repo_state.diff_state.inline_submodule_diff = Some(crate::model::InlineSubmoduleDiffState {
        ...
        rev: 1,
        diff_rev: 1,
        diff: Loadable::Ready(Arc::new(worktree_core::domain::Diff {
            target: inline_target.clone(),
            lines: Vec::new(),
        })),
        diff_file_rev: 1,
        diff_file: Loadable::Ready(Some(Arc::new(worktree_core::domain::FileDiffText::new(
            PathBuf::from("src/lib.rs"),
            Some("before\n".to_string()),
            Some("after\n".to_string()),
        )))),
...
fn stage_hunk_command_finished_reloads_current_diff() {
    ...
    assert!(effects.iter().any(|e| {
        matches!(e, Effect::LoadDiff { repo_id: RepoId(1), target: DiffTarget::WorkingTree { path, area: worktree_core::domain::DiffArea::Unstaged } } if path == &PathBuf::from("a.txt"))
    }));
...
fn stage_hunk_command_finished_keeps_loaded_diff_visible_while_reloading() {
    ...
    repo_state.diff_state.diff_target = Some(target.clone());
    repo_state.diff_state.diff = Loadable::Ready(Arc::new(worktree_core::domain::Diff {
        target: target.clone(),
        lines: vec![worktree_core::domain::DiffLine {
    ...
    reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::DiffLoaded {
            repo_id: RepoId(1),
            target: target.clone(),
            result: Ok(worktree_core::domain::Diff {
                target: target.clone(),
                lines: vec![worktree_core::domain::DiffLine {
                    kind: worktree_core::domain::DiffLineKind::Add,
                    text: "+two".into(),
...
fn clear_diff_selection_resets_diff_state() {
    ...
    repo_state.diff_state.diff_target = Some(worktree_core::domain::DiffTarget::WorkingTree {
        path: PathBuf::from("src/lib.rs"),
        area: worktree_core::domain::DiffArea::Unstaged,
    });
    repo_state.diff_state.diff = Loadable::Loading;
...
fn state_with_loaded_diff_and_blame(
    ...
    AppState,
    DiffTarget,
    Arc<Vec<worktree_core::services::BlameLine>>,
) {
    let mut state = AppState::default();
    let mut repo_state = RepoState::new_opening(
        RepoId(1),
    ...
    let blame = Arc::new(vec![worktree_core::services::BlameLine {
    ...
    repo_state.history_state.blame_source = Some(worktree_core::domain::BlameSource::WorkingTree(
...
fn unstaged_diff(line: &str) -> worktree_core::domain::Diff {
...
fn file_diff_text(line: &str) -> worktree_core::domain::FileDiffText {
...
fn state_with_loaded_file_text_and_blame(
    text: worktree_core::domain::FileDiffText,
) -> (
    AppState,
    DiffTarget,
    Arc<Vec<worktree_core::services::BlameLine>>,
    ...
    let (mut state, _, blame) = state_with_loaded_diff_and_blame(worktree_core::domain::Diff {
        target: target.clone(),
        lines: vec![],
    });
...
fn diff_file_loaded_and_image_loaded_cover_success_and_error_paths() {
    ...
    reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::DiffFileImageLoaded {
            repo_id: RepoId(1),
            target: target.clone(),
            result: Ok(Some(worktree_core::domain::FileDiffImage {
                path: PathBuf::from("icon.svg"),
                old: Some(vec![0x01]),
                new: Some(vec![0x02]),
            })),
...
fn selecting_another_worktree_retires_the_previous_worktrees_inline_diff() {
    ...
    let inline_target = worktree_core::domain::DiffTarget::WorkingTree {
        path: PathBuf::from("src/lib.rs"),
        area: worktree_core::domain::DiffArea::Unstaged,
    };
...
fn retiring_a_worktrees_inline_diff_leaves_the_commit_diff_behind_it_intact() {
    ...
    let commit_target = worktree_core::domain::DiffTarget::Commit {
...
fn a_selection_on_a_worktree_that_went_clean_is_dropped() {
    use worktree_core::domain::WorktreeDirtySummary;
    ...
    reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::WorktreeDirtyLoaded {
            ...
            result: Err(worktree_core::error::Error::new(
                worktree_core::error::ErrorKind::Backend("scan failed".to_string()),
            )),
        }),
    );
...
fn a_rescan_keeps_the_worktree_diff_on_the_half_it_was_opened_from() {
    use worktree_core::domain::{DiffArea, FileStatus, WorktreeDirtySummary};
