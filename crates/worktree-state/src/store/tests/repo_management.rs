...
fn open_repo_allows_same_basename_in_different_folders() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "worktree-open-repo-same-basename-test-{}-{}",
        std::process::id(),
    ...
    reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::RefMetadataLoaded {
            ...
            result: Err(worktree_core::error::Error::new(
                worktree_core::error::ErrorKind::Backend("git blew up".to_string()),
            )),
        }),
    );
...
fn unsupported_ref_metadata_latches_instead_of_retrying_forever() {
    ...
    reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::RefMetadataLoaded {
            repo_id,
            result: Err(worktree_core::error::Error::new(
                worktree_core::error::ErrorKind::Unsupported("nope"),
            )),
        }),
    );
...
fn set_active_repo_reloads_cancelled_history_panes_for_existing_selection() {
...

    {
        ...
        repo1_state.history_state.blame_source = Some(
            worktree_core::domain::BlameSource::Revision(Some("HEAD~1".to_string())),
        );
    ...
    assert!(reactivate_effects.iter().any(|effect| matches!(
        effect,
        Effect::LoadBlame { repo_id, path, source: worktree_core::domain::BlameSource::Revision(Some(rev)) }
            if *repo_id == repo1
                && path == &blame_path
                && rev == "HEAD~1"
    )));
...
fn repo_action_finished_reissues_inflight_blame_and_commit_details() {
    ...
    state.repos[0].history_state.blame_source = Some(worktree_core::domain::BlameSource::Revision(
        Some("HEAD".to_string()),
    ));
...
/// ```
fn file_browser_tree_entries() -> Vec<worktree_core::domain::FileEntry> {
    use worktree_core::domain::{FileEntry, FileEntryKind};
