...
impl GitRepository for ReadyOpenRepo {
    ...
    ) -> Result<Vec<worktree_core::domain::Tag>> {
...
impl GitRepository for BlockingDiffRepo {
    ...
    fn diff_file_text(
        ...
        Ok(Some(worktree_core::domain::FileDiffText::new(
...
fn dropping_receiver_then_last_store_suppresses_late_open_result_store_event_failures() {
    ...
    store.dispatch(Msg::OpenRepo(PathBuf::from("/tmp/worktree-blocking-open")));
    started_rx
        .recv_timeout(Duration::from_secs(1))
...
fn selected_diff_results_after_store_drop_do_not_emit_store_event_failures() {
    ...
    let spec = RepoSpec {
        workdir: PathBuf::from("/tmp/worktree-blocking-diff"),
    };
