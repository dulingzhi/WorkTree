...
use super::{ReduceOutcome, begin_local_action, diff_selection};
use crate::model::{
...
pub(super) fn reduce_diff_selection(msg: Msg, state: &mut AppState) -> ReduceOutcome {
    let effects = match msg {
        ...
        Msg::LoadLfsImagePreview { repo_id, target } => {
                ...
                && !matches!(repo_state.diff_state.lfs_image_preview, Loadable::Loading)
            {
                ...
                repo_state.bump_diff_state_rev();
                return ReduceOutcome::Handled(vec![Effect::LoadLfsImagePreview {
                    repo_id,
                    target,
                }]);
            }
