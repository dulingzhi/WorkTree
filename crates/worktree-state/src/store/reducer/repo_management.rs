...
use super::util::{
    self, EffectAccumulator, SelectedConflictTarget, append_auto_background_metadata_effects,
...
use super::{ReduceOutcome, auth_prompt_for_clone, reduce, repo_management};
use crate::model::{
...
fn is_missing_repo_error(error: &Error) -> bool {
    matches!(
        error.kind(),
        worktree_core::error::ErrorKind::Io(std::io::ErrorKind::NotFound)
    )
}
...
fn persist_repo_history_mode_effect(
    repo_id: Option<RepoId>,
    workdir: PathBuf,
    mode: worktree_core::domain::HistoryMode,
) -> Effect {
...
pub(in crate::store::reducer) enum SelectedHistoryReload {
    ...
    CommitDetails(worktree_core::domain::CommitId),
}
...
pub(super) fn reduce_repo_management(
    msg: Msg,
    repos: &mut FxHashMap<RepoId, Arc<dyn GitRepository>>,
    id_alloc: &AtomicU64,
    state: &mut AppState,
