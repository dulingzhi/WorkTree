...
use worktree_test_support::{run_git_capture_command, run_git_command};
...
fn worktree_difftool_cmd(marker: &str, force_exit: Option<i32>) -> String {
...
fn configure_worktree_difftool(repo: &Path) {
    configure_difftool_command(repo, "worktree", &worktree_difftool_cmd("worktree", None));
    configure_difftool_trust_exit_code(repo, true);
    configure_difftool_selection(repo, "worktree", None, None);
}

fn configure_kdiff3_path_override_to_worktree(repo: &Path) {
    let bin = worktree_bin();
...
fn configure_meld_path_override_to_worktree(repo: &Path) {
    let bin = worktree_bin();
    let bin_path = bin.to_string_lossy().to_string();
...
#[test]
fn git_difftool_invokes_worktree_for_basic_diff() {
    ...
    configure_worktree_difftool(repo);
...
fn git_difftool_kdiff3_path_override_invokes_compat_mode() {
    ...
    configure_kdiff3_path_override_to_worktree(repo);
...
fn git_difftool_meld_path_override_invokes_compat_mode() {
    ...
    configure_meld_path_override_to_worktree(repo);
...
fn git_difftool_gui_default_auto_prefers_gui_tool_when_display_set() {
    ...
    configure_difftool_command(repo, "cli", &worktree_difftool_cmd("cli", None));
    configure_difftool_command(repo, "gui", &worktree_difftool_cmd("gui", None));
    configure_difftool_trust_exit_code(repo, true);
...
fn git_difftool_shows_submodule_gitlink_change() {
    ...
    // Verify that WorkTree surfaces both old and new commit pointers.
    let tmp = tempfile::tempdir().unwrap();
...
#[test]
fn git_difftool_tool_help_lists_worktree_tool() {
    if !require_git_shell_for_tool_tests() {
        return;
    }
    let tmp = tempfile::tempdir().unwrap();
