...
#[cfg(target_os = "linux")]
const LINUX_X11_RELAUNCH_ENV: &str = "WORKTREE_SKIP_LINUX_X11_RELAUNCH";
#[cfg(target_os = "linux")]
const LINUX_X11_FALLBACK_DISABLE_ENV: &str = "WORKTREE_DISABLE_LINUX_X11_FALLBACK";
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UnsupportedVersion {
                ...
                f,
                "Wayland global `{interface}` was advertised at version {advertised_version}, but WorkTree requires version {required_version}+"
            ),
        }
    }
}
...
pub(crate) fn maybe_relaunch_with_linux_x11_fallback(mode: &AppMode) -> Option<i32> {
    ...
    eprintln!("Wayland session detected, but it is not usable for WorkTree: {err}");
...
fn relaunch_under_x11_fallback() -> Option<i32> {
    ...
    eprintln!("Relaunching WorkTree under X11 fallback.");
