//! `worktree setup` / `worktree uninstall` support.
//!
...
const BACKUP_ABSENT_SENTINEL: &str = "__worktree_absent__";
...
fn current_exe_path() -> Result<PathBuf, String> {
    std::env::current_exe()
        .map_err(|e| format!("Cannot determine worktree binary path: {e}"))
...
fn executable_path_for_shell(bin_path: &Path) -> Result<String, String> {
    let Some(path_text) = bin_path.to_str() else {
        return Err(format!(
            "Cannot configure worktree setup for non-Unicode executable path: {bin_path:?}"
        ));
    };
...
fn build_config_entries(bin_path: &str) -> Vec<ConfigEntry> {
    ...
    vec![
        ...
        // - `mergetool.worktree.trustExitCode` preserves explicit per-tool
        ...
        ConfigEntry {
            key: "mergetool.worktree.trustExitCode",
            value: "true".into(),
        },
        ConfigEntry {
        ...
        // - `difftool.worktree.trustExitCode` preserves explicit per-tool
        //   behavior even if users override global defaults later.
        ConfigEntry {
        ...
        ConfigEntry {
            key: "difftool.worktree.trustExitCode",
            value: "true".into(),
        },
        ConfigEntry {
            key: "difftool.prompt",
        ...
        ConfigEntry {
            key: "merge.guitool",
            value: "worktree-gui".into(),
        },
        ConfigEntry {
            key: "mergetool.worktree-gui.cmd",
            value: format!(
                "{quoted_bin_path} mergetool --gui --base {base} --local {local} --remote {remote} --merged {merged}"
            ),
        },
        ConfigEntry {
            key: "mergetool.worktree-gui.trustExitCode",
            value: "true".into(),
        },
        ConfigEntry {
            key: "diff.guitool",
        ...
        ConfigEntry {
            key: "difftool.worktree-gui.cmd",
            value: format!(
                "{quoted_bin_path} difftool --gui --local {local} --remote {remote} --path {merged}"
            ),
        },
        ConfigEntry {
            key: "difftool.worktree-gui.trustExitCode",
            value: "true".into(),
        },
        ConfigEntry {
...
fn build_backup_entries() -> Vec<BackupEntry> {
    vec![
        BackupEntry {
            key: "merge.tool",
            expected_setup_value: "worktree",
            backup_key: "worktree.backup.merge-tool",
        },
        BackupEntry {
            key: "diff.tool",
            expected_setup_value: "worktree",
            backup_key: "worktree.backup.diff-tool",
        },
        BackupEntry {
            key: "merge.guitool",
            expected_setup_value: "worktree-gui",
            backup_key: "worktree.backup.merge-guitool",
        },
        BackupEntry {
            key: "diff.guitool",
            expected_setup_value: "worktree-gui",
            backup_key: "worktree.backup.diff-guitool",
        },
        BackupEntry {
            key: "mergetool.trustExitCode",
            expected_setup_value: "true",
            backup_key: "worktree.backup.mergetool-trust-exit-code",
        },
        BackupEntry {
            key: "mergetool.prompt",
            expected_setup_value: "false",
            backup_key: "worktree.backup.mergetool-prompt",
        },
        BackupEntry {
            key: "difftool.trustExitCode",
            expected_setup_value: "true",
            backup_key: "worktree.backup.difftool-trust-exit-code",
        },
        BackupEntry {
            key: "difftool.prompt",
            expected_setup_value: "false",
            backup_key: "worktree.backup.difftool-prompt",
        },
        BackupEntry {
            key: "mergetool.guiDefault",
            expected_setup_value: "auto",
            backup_key: "worktree.backup.mergetool-guidefault",
        },
        BackupEntry {
            key: "difftool.guiDefault",
            expected_setup_value: "auto",
            backup_key: "worktree.backup.difftool-guidefault",
        },
    ]
}
...
fn build_uninstall_entries() -> Vec<UninstallEntry> {
    vec![
        ...
        // WorkTree defaults, so other tools are not disrupted.
        UninstallEntry {
            key: "merge.tool",
            expected_value: Some("worktree"),
            guard: None,
        },
        UninstallEntry {
            key: "diff.tool",
        ...
        UninstallEntry {
            key: "merge.guitool",
            expected_value: Some("worktree-gui"),
            guard: None,
        },
        UninstallEntry {
            key: "diff.guitool",
...
pub fn run_setup(dry_run: bool, local: bool) -> Result<SetupResult, String> {
    ...
    if dry_run {
        ...
        let stdout = format!(
            "# Dry run: the following git config commands would be executed:\n{commands}\n\
             # Setup also stores backup values for {} key(s) under worktree.backup.* when needed.\n",
            backup_entries.len()
        );
    ...
    let stdout = format!(
        "Configured worktree as {scope_label} diff/merge tool.\n\
         Binary: {bin_str}\n\
...
pub fn run_uninstall(dry_run: bool, local: bool) -> Result<UninstallResult, String> {
    ...
    if dry_run {
        ...
        let stdout = format!(
            "# Dry run: the following git config commands may be executed safely:\n{commands}\n\
             # Uninstall also restores backup values from worktree.backup.* when present.\n"
        );
    ...
    let mut stdout = format!(
        "Unconfigured worktree from {scope_label} diff/merge tool.\n\
...
mod tests {
    ...
    fn build_config_entries_contains_all_required_keys() {
        let entries = build_config_entries("/usr/bin/worktree");
        let keys: Vec<&str> = entries.iter().map(|e| e.key).collect();

        // Headless tool
        assert!(keys.contains(&"merge.tool"));
        assert!(keys.contains(&"mergetool.worktree.cmd"));
        assert!(keys.contains(&"mergetool.trustExitCode"));
        assert!(keys.contains(&"mergetool.worktree.trustExitCode"));
        assert!(keys.contains(&"mergetool.prompt"));
        assert!(keys.contains(&"diff.tool"));
        assert!(keys.contains(&"difftool.worktree.cmd"));
        assert!(keys.contains(&"difftool.trustExitCode"));
        assert!(keys.contains(&"difftool.worktree.trustExitCode"));
        assert!(keys.contains(&"difftool.prompt"));

        // GUI tool variant
        assert!(keys.contains(&"merge.guitool"));
        assert!(keys.contains(&"diff.guitool"));
        assert!(keys.contains(&"mergetool.worktree-gui.cmd"));
        assert!(keys.contains(&"mergetool.worktree-gui.trustExitCode"));
        assert!(keys.contains(&"difftool.worktree-gui.cmd"));
        assert!(keys.contains(&"difftool.worktree-gui.trustExitCode"));
    ...
    fn gui_tool_uses_separate_tool_name() {
        ...
        assert_eq!(merge_guitool.value, "worktree-gui");
        assert_eq!(diff_guitool.value, "worktree-gui");
    }
    ...
    fn gui_tool_cmd_includes_gui_flag() {
        ...
        let merge_gui_cmd = entries
            .iter()
            .find(|e| e.key == "mergetool.worktree-gui.cmd")
            .unwrap();
        let diff_gui_cmd = entries
            .iter()
            .find(|e| e.key == "difftool.worktree-gui.cmd")
            .unwrap();
    ...
    fn headless_tool_cmd_omits_gui_flag() {
        ...
        let merge_cmd = entries
            .iter()
            .find(|e| e.key == "mergetool.worktree.cmd")
            .unwrap();
        let diff_cmd = entries
            .iter()
            .find(|e| e.key == "difftool.worktree.cmd")
            .unwrap();
    ...
    fn mergetool_cmd_escapes_single_quote_in_binary_path() {
        let entries = build_config_entries("/tmp/it's/worktree");
        let cmd = entries
            .iter()
            .find(|e| e.key == "mergetool.worktree.cmd")
            .unwrap();
        ...
        assert!(
            cmd.value.starts_with("'/tmp/it'\"'\"'s/worktree'"),
    ...
    fn format_commands_global_scope() {
        ...
        assert!(output.contains("git config --global difftool.worktree.cmd"));
        assert!(output.contains("git config --global difftool.trustExitCode"));
        assert!(
            output.contains("git config --global difftool.worktree.trustExitCode"),
        ...
        assert!(output.contains("git config --global mergetool.worktree-gui.cmd"));
        assert!(output.contains("git config --global mergetool.worktree-gui.trustExitCode"));
        assert!(output.contains("git config --global diff.guitool"));
        assert!(output.contains("git config --global difftool.worktree-gui.cmd"));
        assert!(output.contains("git config --global difftool.worktree-gui.trustExitCode"));
    ...
    fn apply_config_to_local_repo() {
        ...
        let entries = build_config_entries("/test/worktree");
    ...
    fn uninstall_dry_run_lists_unset_commands_and_conditions() {
        ...
        assert!(
            result
                .stdout
                .contains("git config --local --unset-all mergetool.worktree.cmd")
        );
        assert!(
            result.stdout.contains(
                "git config --local --unset-all merge.tool  # only if value is 'worktree'"
            )
        );
        assert!(
            result.stdout.contains("and merge.tool is 'worktree'"),
    ...
    fn uninstall_plan_unsets_matching_setup_values() {
        ...
        snapshot.insert("mergetool.worktree.cmd", vec!["custom-cmd".to_string()]);
        snapshot.insert("merge.tool", vec!["worktree".to_string()]);
        snapshot.insert("mergetool.prompt", vec!["false".to_string()]);

        let plan = plan_uninstall(&entries, &snapshot);

        assert_eq!(
            decision_for(&plan, "mergetool.worktree.cmd"),
            &UninstallDecision::Unset
        );
    ...
    #[test]
    fn uninstall_plan_preserves_non_worktree_generic_settings() {
        let entries = build_uninstall_entries();
        let mut snapshot: FxHashMap<&'static str, Vec<String>> = FxHashMap::default();
        snapshot.insert("mergetool.worktree.cmd", vec!["custom-cmd".to_string()]);
    ...
    fn git_config_helpers_cover_missing_and_error_paths() {
        ...
        let missing = read_git_config_values(&scope, "worktree.coverage.missing").unwrap();
        assert!(missing.is_empty());

        let err = read_git_config_values("--not-a-valid-scope", "worktree.coverage")
    ...
    fn maybe_capture_backup_skips_when_current_value_matches_setup_default() {
        let (_dir, scope, _) = temp_file_scope();
        set_single_config_value(&scope, "merge.tool", "worktree").unwrap();
