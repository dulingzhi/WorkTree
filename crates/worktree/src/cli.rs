//! CLI argument parsing for worktree.
//!
//! Supports six modes:
...
//! - `uninstall`: remove worktree difftool/mergetool integration
...
#[derive(Parser, Debug)]
#[command(name = "worktree", about = "Git GUI built with GPUI", version)]
pub struct Cli {
    #[command(subcommand)]
...
pub enum Command {
    ...
    /// Configure git to use worktree as the global diff/merge tool.
    Setup(SetupArgs),
    /// Remove worktree diff/merge tool config entries.
...
pub enum AppMode {
    ...
    /// Remove worktree-specific difftool/mergetool integration.
