...
// `worktree.exe` from Explorer or Start Menu shortcuts does not create a
...
fn main() {
    ...
    match mode {
        AppMode::Difftool(config) => {
            ...
            if config.gui {
                eprintln!(
                    "GUI difftool mode is unavailable in this build. Rebuild with `-p worktree --features ui-gpui`."
                );
                std::process::exit(exit_code::ERROR);
            }
                ...
                && should_launch_focused_diff_gui(&config, result)
            {
                ...
                let gui_config = worktree_ui_gpui::FocusedDiffConfig {
                    label_left,
                    label_right,
                    display_path: config.display_path.clone(),
                    diff_text: result.stdout.clone(),
                };
                let code = worktree_ui_gpui::run_focused_diff(gui_config);
                std::process::exit(code);
            }
        ...
        AppMode::Browser { path } => {
            #[cfg(feature = "ui-gpui-runtime")]
            {
                ...
                if let Err(err) = crashlog::begin_session() {
                    eprintln!(
                        "Failed to create WorkTree crash-recovery marker before UI launch: {err}"
                    );
                }
                let backend = build_backend();
                let startup_report =
                    startup_crash_report.map(|report| worktree_ui_gpui::StartupCrashReport {
                        issue_url: report.issue_url,
                        summary: report.summary,
                        crash_log_path: report.crash_log_path,
                    });
                let run_result =
                    worktree_ui_gpui::run_with_startup_crash_report_and_shutdown_callback(
                        backend,
                        path.clone(),
                        startup_report,
                        Some(|| {
                ...
                match run_result {
                    ...
                    Err(err) => {
                        if let Err(record_err) = crashlog::record_session_failure(
                            ...
                            eprintln!(
                                "Failed to preserve WorkTree UI launch failure data: {record_err}"
                            );
                        }
            ...
            {
                let _ = path;
                eprintln!("WorkTree UI is disabled. Build with `-p worktree --features ui-gpui`.");
                std::process::exit(exit_code::ERROR);
            }
        }
        AppMode::Mergetool(config) => {
            ...
            if config.gui {
                eprintln!(
                    "GUI mergetool mode is unavailable in this build. Rebuild with `-p worktree --features ui-gpui`."
                );
                std::process::exit(exit_code::ERROR);
            }
            ...
            if config.gui {
                ...
                let backend = build_backend();
                let code = worktree_ui_gpui::run_focused_mergetool(backend, gui_config);
                std::process::exit(code);
            }
...
#[cfg(all(target_os = "macos", feature = "ui-gpui-runtime"))]
const MACOS_BUNDLE_RELAUNCH_ENV: &str = "WORKTREE_SKIP_APP_BUNDLE_RELAUNCH";
#[cfg(all(target_os = "macos", feature = "ui-gpui-runtime"))]
const MACOS_APP_ICON_PNG: &[u8] = include_bytes!("../../../assets/worktree-512.png");
...
fn macos_user_app_bundle_path_with_home(home: Option<&std::path::Path>) -> std::path::PathBuf {
    let base_dir = home
        .map(|value| value.join("Library/Application Support/WorkTree"))
        .unwrap_or_else(|| std::env::temp_dir().join("WorkTree"));
    base_dir.join("WorkTree.app")
}
...
fn candidate_macos_app_bundle_paths(resolved_exe: &std::path::Path) -> Vec<std::path::PathBuf> {
    ...
    if let Some(bin_dir) = resolved_exe.parent() {
        candidates.push(bin_dir.join("WorkTree.app"));
    }
...
fn ensure_macos_dev_app_bundle(
    ...
    let app_exe = macos.join("worktree");
    std::fs::copy(current_exe, &app_exe)
        .map_err(|e| format!("failed to copy executable into bundle: {e}"))?;

    let icon_png = resources.join("WorkTree.png");
    let icon_icns = resources.join("WorkTree.icns");
...
<dict>
  ...
  <string>ai.autoexplore.worktree.dev</string>
  <key>CFBundleIconFile</key>
  <string>WorkTree.icns</string>
...
fn print_startup_crash_report_hint(report: &crashlog::StartupCrashReport) {
    eprintln!("WorkTree detected a crash from a previous run.");
    eprintln!(
...
fn build_backend() -> std::sync::Arc<dyn worktree_core::services::GitBackend> {
    if cfg!(feature = "gix") {
        #[cfg(feature = "gix")]
        {
            std::sync::Arc::new(worktree_git_gix::GixBackend)
        }
...
fn build_focused_mergetool_gui_config(
...
) -> Result<worktree_ui_gpui::FocusedMergetoolConfig, String> {
    ...
    Ok(worktree_ui_gpui::FocusedMergetoolConfig {
        repo_path,
...
mod tests {
    ...
    fn build_focused_mergetool_gui_config_errors_without_repo_root() {
        let tmp = tempfile::Builder::new()
            .prefix("worktree-mergetool-no-repo-")
            .tempdir()
    ...
    fn detects_macos_app_bundle_executable_paths() {
        assert!(is_macos_app_bundle_executable(std::path::Path::new(
            "/tmp/WorkTree.app/Contents/MacOS/worktree"
        )));
        assert!(!is_macos_app_bundle_executable(std::path::Path::new(
            "/opt/homebrew/bin/worktree"
        )));
    }
    ...
    fn canonicalized_symlink_resolves_to_macos_app_bundle_executable() {
        ...
        let app_exe = temp.path().join("WorkTree.app/Contents/MacOS/worktree");
        fs::create_dir_all(app_exe.parent().unwrap()).unwrap();
        fs::write(&app_exe, b"#!/bin/sh\n").unwrap();

        let bin_dir = temp.path().join("bin");
        fs::create_dir_all(&bin_dir).unwrap();
        let symlink_path = bin_dir.join("worktree");
    ...
    fn macos_user_app_bundle_path_uses_home_directory_when_available() {
        ...
        assert_eq!(
            ...
            std::path::PathBuf::from(
                "/Users/example/Library/Application Support/WorkTree/WorkTree.app"
            )
        );
    }
    ...
    fn candidate_macos_app_bundle_paths_include_executable_dir_and_fallback() {
        let exe = std::path::Path::new("/opt/homebrew/bin/worktree");
        let candidates = candidate_macos_app_bundle_paths(exe);

        assert_eq!(
            candidates.first(),
            Some(&std::path::PathBuf::from("/opt/homebrew/bin/WorkTree.app"))
        );
    ...
    fn ensure_macos_dev_app_bundle_replaces_stale_signature_artifacts() {
        let temp = tempfile::tempdir().unwrap();
        let app_bundle = temp.path().join("WorkTree.app");
        ...
        assert_eq!(
            app_exe,
            app_bundle.join(std::path::Path::new("Contents/MacOS/worktree"))
        );
