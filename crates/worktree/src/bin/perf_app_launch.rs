...
const EXPECT_READY_REPOS_ENV: &str = "WORKTREE_PERF_STARTUP_EXPECT_READY_REPOS";
...
fn run_harness(args: &CliArgs) -> Result<(), String> {
    ...
    command
        .arg("--child")
        .env("WORKTREE_PERF_STARTUP_PROBE", "1")
        .env("WORKTREE_PERF_STARTUP_AUTO_EXIT", "1")
        .env(
            EXPECT_READY_REPOS_ENV,
            fixture.expected_ready_repos.to_string(),
        )
        .env("WORKTREE_SESSION_FILE", &fixture.session_file)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::piped());
    if fixture.disable_auto_restore {
        command.env("WORKTREE_PERF_STARTUP_DISABLE_AUTO_RESTORE", "1");
    }
...
fn run_child() -> Result<(), String> {
    ...
    worktree_ui_gpui::run(build_backend()).map_err(|err| format!("child launch failed: {err}"))
}
...
fn build_backend() -> Arc<dyn GitBackend> {
    if cfg!(feature = "gix") {
        #[cfg(feature = "gix")]
        {
            Arc::new(worktree_git_gix::GixBackend)
        }
...
fn empty_git_config() -> &'static Path {
    ...
    EMPTY_CONFIG.get_or_init(|| {
        let path = std::env::temp_dir().join("worktree-perf-empty.gitconfig");
...
mod tests {
    ...
    fn parse_probe_event_line_reads_json_payload() {
        let event = parse_probe_event_line(
            "WORKTREE_PERF_STARTUP_EVENT {\"name\":\"first_interactive\",\"rss_kib\":98304}",
        )
        .expect("event");
    ...
    fn parse_probe_event_line_reads_repo_progress_payload() {
        let event = parse_probe_event_line(
            "WORKTREE_PERF_STARTUP_EVENT {\"name\":\"repos_loaded\",\"repos_loaded\":5,\"repos_total\":5}",
        )
    ...
    fn parse_probe_event_line_reads_allocation_payload() {
        let event = parse_probe_event_line(
            "WORKTREE_PERF_STARTUP_EVENT {\"name\":\"first_paint\",\"alloc_ops\":12,\"dealloc_ops\":4,\"realloc_ops\":1,\"alloc_bytes\":4096,\"dealloc_bytes\":1024,\"realloc_bytes_delta\":512,\"net_alloc_bytes\":3072}",
        )
