...
fn compat_log_arg_text(arg: &std::ffi::OsStr) -> String {
    ...
    #[cfg(unix)]
    {
        ...
        format!("worktree-argv-bytes:{}", hex_encode(arg.as_bytes()))
    }
    ...
    #[cfg(windows)]
    {
        ...
        format!("worktree-argv-utf16le:{}", hex_encode(&bytes))
    }
...
fn compat_argv_log_dir() -> PathBuf {
    std::env::temp_dir().join("worktree-compat-argv")
}

fn resolve_compat_argv_log_path(env: &dyn EnvLookup) -> Option<PathBuf> {
    let requested = PathBuf::from(env.var_os("WORKTREE_COMPAT_ARGV_LOG")?);
