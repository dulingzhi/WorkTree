...
use repositorytree_core::services::{CancellationToken, ConflictFileStages, Result};
...
fn decode_utf8_bytes(bytes: Vec<u8>) -> Result<String> {
    ...
    Ok(repositorytree_core::encoding::text_bytes_to_utf8_string(bytes))
}
...
fn rewrite_temp_file_as_utf8(tmp_file: &mut tempfile::NamedTempFile) -> Result<()> {
    ...
    let converted = repositorytree_core::encoding::text_bytes_to_utf8(raw.as_slice());
...
fn worktree_git_cache_path(logical_path: &Path, identity: &str) -> std::path::PathBuf {
    let suffix = logical_path
        ...
        .map(|ext| format!(".{ext}"))
        .unwrap_or_default();
    std::env::temp_dir().join(format!("repositorytree-diff-worktree-{identity}{suffix}"))
}
...
fn preview_blob_cache_path(
    ...
    std::env::temp_dir().join(format!("repositorytree-diff-preview2-{hash:016x}{suffix}"))
}
...
mod tests {
    ...
    use repositorytree_core::error::ErrorKind;
    use std::process::Command;
    ...
    fn persist_worktree_git_cache_file_replaces_existing_cache_file() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let cache_path = tmp.path().join("repositorytree-diff-worktree-test.txt");
