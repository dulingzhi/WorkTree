...
//!
//! This module mirrors those `merge_cmd` definitions so RepositoryTree launches the
...
pub(super) fn builtin_merge_command(tool: &str, files: &MergetoolFiles<'_>) -> BuiltinMergeCommand {
    ...
    let args = match key {
        ...
        // BASE file RepositoryTree materialises serves the same purpose here.
            ...
            if !base_present {
                return BuiltinMergeCommand::Unsupported(format!(
                    "Merge tool '{tool}' cannot be used without a merge base. \
                     Resolve this conflict in RepositoryTree or configure a different merge.tool."
                ));
            }
            argv![
                prefixed("-base:", base),
...
mod tests {
    ...
    fn program_lookup_finds_executables_on_the_search_path() {
        ...
        let program = dir.path().join("repositorytree-fake-mergetool");
        ...
        assert!(program_exists_in(search_path, "repositorytree-fake-mergetool"));
        assert!(!program_exists_in(
            search_path,
            "repositorytree-missing-mergetool"
        ));
    }
    ...
    fn program_lookup_ignores_non_executable_files() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("repositorytree-not-executable"), b"data").unwrap();

        assert!(!program_exists_in(
            dir.path().as_os_str(),
            "repositorytree-not-executable"
        ));
    }
}
