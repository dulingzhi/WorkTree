...
use repositorytree_core::services::{CancellationToken, Result};
    ...
    pub(super) fn status_cancellable_impl(
        ...
        let (staged, index_stamp_after_write) = if let Some(cached_staged) = cached_staged {
            ...
            let platform = repo
                ...
                // RepositoryTree supplements gitlink/submodule status separately to match
...
fn maybe_persist_status_outcome_changes(
    ...
    //
    // The reducer relies on this: `finish_status_lane_replay` (repositorytree-state) now *always*
...
mod tests {
    ...
    use repositorytree_core::domain::{FileConflictKind, FileStatus, FileStatusKind};
    use std::fs;
    use std::path::{Path, PathBuf};
