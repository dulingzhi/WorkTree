...
impl GixRepo {
    ...
    ///    explicitly trusted via a RepositoryTree global consent key.
...
fn run_custom_mergetool_command(
    ...
    #[cfg(windows)]
    {
        let script_dir = tempfile::Builder::new()
            .prefix("repositorytree-mergetool-shell-")
            .tempdir()
...
fn repo_local_mergetool_command_consent_key(workdir: &Path, tool_name: &str) -> String {
    ...
    format!("repositorytree.mergetool.allowrepolocalcmd-{repo_tool_fingerprint}")
}
...
fn build_stage_paths(
    ...
    if write_to_temp {
        let tmp_dir = tempfile::Builder::new()
            .prefix("repositorytree-mergetool-")
            .tempdir()
            .map_err(|e| Error::new(ErrorKind::Io(e.kind())))?;
...
mod tests {
    ...
    fn test_shell_command_windows_uses_cmd_percent_expansion() {
        let output = shell_command("echo %REPOSITORYTREE_MERGETOOL_TEST_TOKEN%")
            .env("REPOSITORYTREE_MERGETOOL_TEST_TOKEN", "from-cmd")
            .output()
            .unwrap();
    ...
    fn test_shell_command_unix_uses_sh_dollar_expansion() {
        let output = shell_command("echo \"$REPOSITORYTREE_MERGETOOL_TEST_TOKEN\"")
            .env("REPOSITORYTREE_MERGETOOL_TEST_TOKEN", "from-sh")
            .output()
            .unwrap();
        assert!(output.status.success());
        let stdout = String::from_utf8(output.stdout).expect("stdout should be utf-8");
