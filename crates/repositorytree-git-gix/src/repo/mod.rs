...
impl GitRepository for GixRepo {
    ...
    fn worktree_status(&self) -> Result<Vec<repositorytree_core::domain::FileStatus>> {
    ...
    fn worktree_status_cancellable(
        &self,
        cancellation: &CancellationToken,
    ) -> Result<Vec<repositorytree_core::domain::FileStatus>> {
        let _scope = git_ops_trace::scope(GitOpTraceKind::Status);
        self.worktree_status_cancellable_impl(cancellation)
    }

    fn staged_status(&self) -> Result<Vec<repositorytree_core::domain::FileStatus>> {
        let _scope = git_ops_trace::scope(GitOpTraceKind::Status);
