...
const PERSISTED_REWORD_DIR: &str = "repositorytree-reword";
const PERSISTED_REWORD_STAGING_DIR: &str = "repositorytree-reword.staging";
const PERSISTED_PLAN_NAME: &str = "planned-todo";
const PERSISTED_CHERRY_PICK_MAINLINE: &str = "repositorytree-cherry-pick-mainline";
const PERSISTED_CHERRY_PICK_MAINLINE_STAGING: &str = "repositorytree-cherry-pick-mainline.staging";
...
const MSG_EDITOR_NAME: &str = "repositorytree-msg-editor.sh";
...
const CHERRY_PICK_ALREADY_APPLIED_SENTINEL: &str = "REPOSITORYTREE_CHERRY_PICK_ALREADY_APPLIED";
...
impl GixRepo {
    ...
    pub(super) fn squash_message_preview_impl(
        ...
        messages.reverse();
        Ok(repositorytree_core::squash::build_squash_message(&messages))
    }
    ...
    pub(super) fn rebase_continue_with_output_impl(&self) -> Result<CommandOutput> {
        ...
        match persisted_reword_state(repo.path()) {
            PersistedReword::Ready {
                ...
                // RepositoryTree would silently keep its old message.
                if rebase_unplanned_message_edit(repo.path(), &plan) {
                    return Err(Error::new(ErrorKind::Backend(
                        "this rebase has pending reword/squash steps that RepositoryTree did not \
                ...
                cmd.env("REPOSITORYTREE_MSGS_DIR", messages);
                cmd.env("REPOSITORYTREE_GIT_DIR", repo.path());
            }
            PersistedReword::Damaged => {
                return Err(Error::new(ErrorKind::Backend(
                    "RepositoryTree's reword data for this rebase is incomplete; continue it in a \
    ...
    /// that progressed but left no conflict behind: RepositoryTree plans only
    ...
    fn cherry_pick_stopped_became_empty(&self) -> Result<bool> {
        ...
        // RepositoryTree persists the chosen parent for merge picks it starts; an
    ...
    fn run_planned_rebase(
        ...
        if let Some(ref msg_editor) = scripts.msg_editor_path {
            ...
            cmd.env("REPOSITORYTREE_MSGS_DIR", &scripts.msgs_dir);
            let repo = self._repo.to_thread_local();
            cmd.env("REPOSITORYTREE_GIT_DIR", repo.path());
        }
        cmd.env("REPOSITORYTREE_TODO_FILE", &scripts.todo_path);
    ...
    pub(super) fn interactive_cherry_pick_with_output_impl(
        ...
        // can neither continue nor abort. RepositoryTree plans no mainline
...
impl RebaseScripts {
    fn create(entries: &[InteractiveRebaseEntry]) -> std::io::Result<Self> {
        ...
        let seq_editor_path = dir.path().join("repositorytree-seq-editor.sh");
        ...
        if needs_msg_editor {
            ...
            for (ix, entry) in entries.iter().enumerate() {
                    ...
                    && let Some(ref msg) = entry.new_message
                {
                    ...
                    let key_entry = repositorytree_core::squash::squash_run_final_entry(entries, ix)
...
enum PersistedReword {
    /// No RepositoryTree reword plan exists for this rebase.
    Absent,
    Ready {
        editor: PathBuf,
        messages: PathBuf,
...
/// plan never had — a todo edited outside RepositoryTree (e.g. `git rebase
...
fn rebase_unplanned_message_edit(git_dir: &Path, plan: &Path) -> bool {
    use repositorytree_core::squash::{TodoLineRole, todo_line_commit_word, todo_line_role};
...
/// classification lives in core ([`repositorytree_core::squash::todo_line_role`])
/// so these rules cannot drift from the planner's.
fn rebase_pending_message_edit(git_dir: &Path) -> bool {
    use repositorytree_core::squash::{TodoLineRole, todo_line_role};
    let rebase_dir = git_dir.join("rebase-merge");
...
fn seq_editor_script_contents() -> &'static [u8] {
    br#"#!/bin/sh
cp "$(printf '%s' "$REPOSITORYTREE_TODO_FILE" | tr '\\' /)" "$1"
"#
}

#[cfg(not(windows))]
fn seq_editor_script_contents() -> &'static [u8] {
    b"#!/bin/sh\ncp \"$REPOSITORYTREE_TODO_FILE\" \"$1\"\n"
}
...
if [ -n "$REPOSITORYTREE_GIT_DIR" ]; then
  git_dir=$(winpath "$REPOSITORYTREE_GIT_DIR")
else
  git_dir=$(dirname "$message_file")
fi
msgs_dir=$(winpath "$REPOSITORYTREE_MSGS_DIR")
...
sha=
message_file=$1
git_dir=${REPOSITORYTREE_GIT_DIR:-$(dirname "$message_file")}
if [ -f "$git_dir/REBASE_HEAD" ]; then
...
for msg_file in "$REPOSITORYTREE_MSGS_DIR/$sha"*; do
  if [ -f "$msg_file" ]; then
    cp "$msg_file" "$message_file" || exit 1
  fi
  break
