...
fn git_range_status_changes(
    ...
    while let Some(header) = next_non_empty_nul_field(&mut fields) {
        ...
        let kind = match status_code {
            b'A' | b'C' => repositorytree_core::domain::FileStatusKind::Added,
            b'D' => repositorytree_core::domain::FileStatusKind::Deleted,
            b'R' => repositorytree_core::domain::FileStatusKind::Renamed,
            b'U' => repositorytree_core::domain::FileStatusKind::Conflicted,
            _ => repositorytree_core::domain::FileStatusKind::Modified,
        };
    ...
    let clone_only_state = match failed_submodule_add_left_clone_only_state(workdir, path) {
        ...
        Err(probe_err) => {
            return append_failed_submodule_add_note(
                err,
                &format!("RepositoryTree could not inspect failed submodule add state: {probe_err}"),
            );
        }
    };
...
fn submodule_file_transport_consent_key(trust_root: &Path, source_path: &Path) -> String {
    ...
    format!(
        "repositorytree.submodule.allowfiletransport-{:016x}",
        fnv1a_64(&bytes)
    )
}
...
mod tests {
    ...
    use repositorytree_core::domain::{CommitId, DiffArea, DiffTarget, SubmoduleDiffRangeKind};
    use repositorytree_core::error::{Error, ErrorKind, GitFailure, GitFailureId};
    use repositorytree_core::services::CancellationToken;
    use std::cell::Cell;
    use std::ffi::OsStr;
    use std::path::Path;
    use std::process::Command;
    ...
    fn git_config_failure(stderr: &str) -> Error {
        Error::new(ErrorKind::Git(GitFailure::new(
            "git config --global repositorytree.submodule.allowfiletransport-example true",
    ...
    fn git_config_contention_retry_does_not_retry_unrelated_failures() {
        ...
        assert_eq!(
            err.to_string(),
            "git config --global repositorytree.submodule.allowfiletransport-example true failed"
        );
    }
