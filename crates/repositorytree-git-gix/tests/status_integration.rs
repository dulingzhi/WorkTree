use repositorytree_core::conflict_session::{ConflictPayload, ConflictResolverStrategy};
...
use repositorytree_core::error::{Error, ErrorKind, GitFailureId};
use repositorytree_core::services::GitBackend;
use repositorytree_core::services::{ConflictSide, InteractiveRebaseAction, InteractiveRebaseEntry};
...
fn ensure_isolated_git_test_env() -> &'static TestGitEnv {
    ...
    ENV.get_or_init(|| {
        ...
        repositorytree_git_gix::install_test_git_command_environment(
            global_config.clone(),
            home_dir.clone(),
...
fn allow_repo_local_mergetool_cmd(repo: &Path, tool_name: &str) {
    ...
    repositorytree_git_gix::allow_test_repo_local_mergetool_command(repo, tool_name);
}
...
fn set_fixed_mtime(path: &Path) {
    let status = Command::new("powershell")
        .args([
            "-NoProfile",
            "-Command",
            "(Get-Item -LiteralPath $env:REPOSITORYTREE_TARGET).LastWriteTimeUtc=[DateTimeOffset]::FromUnixTimeSeconds(1700000000).UtcDateTime",
        ])
        .env("REPOSITORYTREE_TARGET", path)
        .status()
...
fn status_ignores_nested_target_directories_with_target_slash_pattern() {
    ...
    write(
        repo,
        "crates/repositorytree-ui-gpui/target/criterion/report/index.html",
        "ignored\n",
    );
    ...
    assert!(
        status.unstaged.iter().all(|entry| !entry
            .path
            .starts_with(Path::new("crates/repositorytree-ui-gpui/target"))),
...
fn diff_file_text_reports_old_and_new_for_working_tree_and_commits() {
    ...
    let commit = opened
        .diff_file_text(&DiffTarget::Commit {
            commit_id: repositorytree_core::domain::CommitId(head.into()),
...
fn diff_commit_with_unknown_revision_and_outside_conflict_path_are_handled() {
    ...
    let unknown_target = DiffTarget::Commit {
        commit_id: repositorytree_core::domain::CommitId("not-a-real-revision".into()),
        path: Some(PathBuf::from("a.txt")),
    };
...
fn launch_mergetool_write_to_temp_true_uses_temp_stage_paths() {
    ...
    for var in vars {
        ...
        assert!(
            normalized_var.contains("repositorytree-mergetool-"),
...
fn reset_soft_moves_head_and_leaves_changes_staged() {
    ...
    opened
        .reset_with_output("HEAD~1", repositorytree_core::services::ResetMode::Soft)
        .unwrap();
...
fn reset_mixed_moves_head_and_leaves_changes_unstaged() {
    ...
    opened
        .reset_with_output("HEAD~1", repositorytree_core::services::ResetMode::Mixed)
        .unwrap();
...
fn reset_hard_moves_head_and_discards_changes() {
    ...
    opened
        .reset_with_output("HEAD~1", repositorytree_core::services::ResetMode::Hard)
        .unwrap();
...
fn revert_commit_creates_new_commit_and_reverts_content() {
    ...
    opened
        .revert(&repositorytree_core::domain::CommitId(c2.clone().into()))
        .unwrap();
...
fn create_rename_and_delete_local_branch() {
    ...
    opened
        .create_branch("feature", &repositorytree_core::domain::CommitId(head.into()))
        .unwrap();
    run_git(
        repo,
...
fn create_branch_from_detached_head_using_head_revision() {
    ...
    let opened = backend.open(repo).unwrap();
    opened
        .checkout_commit(&repositorytree_core::domain::CommitId(
            first_commit.clone().into(),
        ))
        .unwrap();
    opened
        .create_branch("rescue", &repositorytree_core::domain::CommitId("HEAD".into()))
        .unwrap();
...
fn create_branch_from_annotated_tag_peels_to_commit() {
    ...
    let opened = backend.open(repo).unwrap();
    opened
        .create_branch("feature", &repositorytree_core::domain::CommitId("v1".into()))
        .unwrap();
...
fn create_branch_from_blob_target_returns_structured_git_error() {
    ...
    let err = opened
        .create_branch(
            "feature",
            &repositorytree_core::domain::CommitId(blob.clone().into()),
        )
...
fn create_branch_target_branch_created_after_backend_open() {
    ...
    opened
        .create_branch("feature", &repositorytree_core::domain::CommitId("source".into()))
        .unwrap();
...
fn cherry_pick_applies_commit_onto_current_branch() {
    ...
    let opened = backend.open(repo).unwrap();
    opened
        .cherry_pick(&repositorytree_core::domain::CommitId(feature_sha.into()))
        .unwrap();
...
fn create_tag_respects_tag_gpgsign_config() {
    ...
    run_git(
        repo,
        &["config", "gpg.program", "repositorytree-missing-gpg-program"],
    );
    ...
    match err.kind() {
        ErrorKind::Git(failure) => {
            ...
            assert!(
                msg.contains("repositorytree-missing-gpg-program") || lower.contains("sign"),
...
fn pull_with_output_fast_forwards_from_remote() {
    ...
    opened_b
        .pull_with_output(repositorytree_core::services::PullMode::FastForwardOnly)
        .unwrap();
...
fn pull_with_output_fast_forwards_when_possible_even_if_pull_ff_is_disabled() {
    ...
    let opened_b = backend.open(&repo_b).unwrap();
    opened_b
        .pull_with_output(repositorytree_core::services::PullMode::Merge)
        .unwrap();
...
fn checkout_commit_detaches_head_at_target() {
    ...
    let opened = backend.open(repo).unwrap();
    opened
        .checkout_commit(&repositorytree_core::domain::CommitId(sha.clone().into()))
        .unwrap();
...
fn resolve_conflict_write_and_stage_clears_conflict() {
    ...
    let marker_projection = repositorytree_core::merge::render_merge_plan(
        plan,
        &repositorytree_core::merge::MergeOptions {
            style: repositorytree_core::merge::ConflictStyle::Diff3,
            ..Default::default()
        },
    )
    .output;
    ...
    let validation = repositorytree_core::services::validate_conflict_resolution_text(&worktree_content);
    assert!(
    ...
    let resolved_validation =
        repositorytree_core::services::validate_conflict_resolution_text(resolved_content);
    assert!(
...
fn resolve_both_added_conflict_write_and_stage_clears_conflict() {
    ...
    let validation = repositorytree_core::services::validate_conflict_resolution_text(resolved);
...
fn validate_conflict_resolution_detects_partial_resolution() {
    ...
    use repositorytree_core::services::validate_conflict_resolution_text;
