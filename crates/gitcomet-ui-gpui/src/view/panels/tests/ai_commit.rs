...
fn ai_generate_fetches_context_then_runs_the_provider_request(cx: &mut gpui::TestAppContext) {
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            ...
            repo.ai_commit_context = gitcomet_state::model::Loadable::Ready(Arc::new(
                gitcomet_state::model::AiCommitContext {
...
fn switching_repos_abandons_an_in_flight_generation(cx: &mut gpui::TestAppContext) {
    ...
    let (view, cx) =
        cx.add_window_view(|window, cx| GitCometView::new(store, events, None, window, cx));
    let repo_a = gitcomet_state::model::RepoId(63);
    let repo_b = gitcomet_state::model::RepoId(64);
    ...
    cx.update(|_window, app| {
        view.update(app, |this, cx| {
            let mut state = gitcomet_state::model::AppState::default();
