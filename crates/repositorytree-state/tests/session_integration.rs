...
use repositorytree_state::model::{AppState, RepoId, RepoState};
use repositorytree_state::session::{self, ExternalCodeEditorSetting, UiSession, UiSettings};
use serde_json::json;
...
fn unique_temp_dir(label: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!(
        "repositorytree-state-session-integration-{label}-{}-{}",
        std::process::id(),
        SystemTime::now()
...
fn wrapper_apis_return_defaults_without_session_path() {
    ...
    let repo = Path::new("/tmp/repositorytree-session-wrapper");
...
fn wrapper_apis_use_session_file_env_when_set() {
    ...
    let repo = Path::new("/tmp/repositorytree-session-env-repo");
...
fn load_from_path_parses_encoded_paths_and_filters_invalid_active_repo() {
    ...
    write_session_json(
        ...
        json!({
            ...
            "open_repos": [
                ...
                "/tmp/alpha",
                "/tmp/alpha",
                "repositorytree-path-bytes:2F746D702F4A",
                "repositorytree-path-bytes:abc",
                "repositorytree-path-bytes:zz"
            ],
    ...
    #[cfg(not(unix))]
    expected_open_repos.push(PathBuf::from("repositorytree-path-bytes:2F746D702F4A"));
    expected_open_repos.push(PathBuf::from("repositorytree-path-bytes:abc"));
    expected_open_repos.push(PathBuf::from("repositorytree-path-bytes:zz"));
...
fn persist_ui_settings_to_path_updates_optional_fields_and_requires_both_window_dims() {
    ...
    session::persist_ui_settings_to_path(
        UiSettings {
            ...
            history_tag_fetch_mode: Some(repositorytree_state::model::GitLogTagFetchMode::Disabled),
    ...
    assert_eq!(
        ...
        Some(repositorytree_state::model::GitLogTagFetchMode::Disabled)
    );
