...
fn record_monitor_failure(
    ...
    eprintln!(
        "repositorytree-state: repo monitor failure ({kind:?}) in {context}: {detail}; total_failures={count}"
    );
}
...
fn spawn_monitor_join(repo_id: RepoId, join: thread::JoinHandle<()>, context: &'static str) {
    let thread_name = format!("repositorytree-repo-monitor-join-{}", repo_id.0);
...
fn setup_workdir_watch_with_limit(
    ...
    if subdir_count > max_dirs {
        ...
        eprintln!(
            "repositorytree-state: repo monitor is not watching the {subdir_count} worktree folders of \
    ...
    if failed > 0 {
        ...
        eprintln!(
            "repositorytree-state: repo monitor could not watch {failed}/{subdir_count} worktree \
...
mod tests {
    ...
    fn resolve_git_dir_handles_dot_git_directory() {
        let dir = unique_temp_dir("repositorytree-monitor-test");
    ...
    fn gitignore_rules_match_git_semantics_for_nested_negation_and_anchoring() {
        ...
        assert!(rules.is_ignored_rel(Path::new("foo.repositorytree-log"), Some(false)));
        assert!(!rules.is_ignored_rel(Path::new("keep.repositorytree-log"), Some(false)));
        ...
        assert!(rules.is_ignored_rel(Path::new("info-excluded.repositorytree"), Some(false)));
    ...
    fn setup_workdir_watch_delivers_tracked_events_and_skips_ignored_dirs() {
        ...
        let dir = unique_temp_dir("repositorytree-monitor-watch");
        let workdir = dir.path().join("repo");
        init_repo_for_ignore_tests(&workdir);
        fs::write(workdir.join(".gitignore"), "target/\n").expect("write .gitignore");
    ...
    fn watcher_does_not_deliver_events_for_reads_of_watched_files() {
        ...
        let dir = unique_temp_dir("repositorytree-monitor-read-noise");
        let workdir = dir.path().join("repo");
    ...
    fn ignore_aware_watch_suppresses_build_churn_that_used_to_drown_real_edits() {
        ...
        // both.
        let dir = unique_temp_dir("repositorytree-monitor-churn");
        let workdir = dir.path().join("repo");
        init_repo_for_ignore_tests(&workdir);
        fs::write(workdir.join(".gitignore"), "target/\n").expect("write .gitignore");
        fs::create_dir_all(workdir.join("src")).expect("create src");
    ...
    fn gitignore_change_reinit_unwatches_newly_ignored_dir() {
        ...
        let dir = unique_temp_dir("repositorytree-monitor-reinit");
        let workdir = dir.path().join("repo");
        init_repo_for_ignore_tests(&workdir);
    ...
    fn watch_budget_count_does_not_inflate_on_rewatch() {
        ...
        let dir = unique_temp_dir("repositorytree-monitor-rewatch");
        let workdir = dir.path().join("repo");
        init_repo_for_ignore_tests(&workdir);
        fs::create_dir_all(workdir.join("src").join("sub")).expect("create src/sub");
        let git_dir = resolve_git_dir(&workdir);
    ...
    fn over_budget_worktree_skips_source_folder_watches() {
        ...
        let dir = unique_temp_dir("repositorytree-monitor-budget");
        let workdir = dir.path().join("repo");
        init_repo_for_ignore_tests(&workdir);
        fs::create_dir_all(workdir.join("src").join("sub")).expect("create src/sub");
        let git_dir = resolve_git_dir(&workdir);
    ...
    fn git_dir_watch_excludes_objects_but_sees_root_and_refs() {
        ...
        let dir = unique_temp_dir("repositorytree-monitor-gitdir");
        let workdir = dir.path().join("repo");
        init_repo_for_ignore_tests(&workdir);
