...
use repositorytree_core::services::{CancellationToken, ConflictFileStages, GitBackend, GitRepository};
    ...
    spawn_detached_with_repo_or_else(
        ...
        move |repo, msg_tx| {
            let result = {
                ...
                let mut on_chunk = |chunk: repositorytree_core::services::LogChunk| {
                    send_or_log(
                        &msg_tx,
...
pub(super) fn schedule_load_conflict_file(
    ...
    spawn_with_repo(executor, repos, repo_id, msg_tx, move |repo, msg_tx| {
        ...
        let result = if let Some(session) = session_ref {
            ...
            stages.map(|opt| {
                opt.map(|d| {
                    let repositorytree_core::services::ConflictFileStages {
                        path,
                        base_bytes,
                        ours_bytes,
                        theirs_bytes,
...
pub(super) fn schedule_load_file_browser(
    ...
    msg_tx: StoreWorkerSender,
    repo_id: RepoId,
    source: repositorytree_core::domain::FileSource,
    ...
    spawn_with_repo_or_else(
        ...
        move |repo, msg_tx| {
            let result = match &source {
                repositorytree_core::domain::FileSource::WorkingDirectory => repo.list_worktree_files(),
                repositorytree_core::domain::FileSource::Commit(commit_id) => {
                    repo.list_tree_files_at_commit(commit_id)
                }
                repositorytree_core::domain::FileSource::Branch(_name) => {
...
pub(super) fn schedule_resolve_commit_for_reveal(
    ...
    msg_tx: StoreWorkerSender,
    repo_id: RepoId,
    reference: repositorytree_core::domain::CommitId,
) {
    spawn_with_repo(executor, repos, repo_id, msg_tx, move |repo, msg_tx| {
        send_or_log(
            &msg_tx,
...
pub(super) fn schedule_load_range_files(
    ...
    msg_tx: StoreWorkerSender,
    repo_id: RepoId,
    from: repositorytree_core::domain::CommitId,
    to: Option<repositorytree_core::domain::CommitId>,
    request: u64,
) {
    spawn_with_repo(executor, repos, repo_id, msg_tx, move |repo, msg_tx| {
...
pub(super) fn schedule_load_squash_message_preview(
    ...
    head: repositorytree_core::domain::CommitId,
) {
    spawn_with_repo(executor, repos, repo_id, msg_tx, move |repo, msg_tx| {
        send_or_log(
            &msg_tx,
...
pub(super) struct SquashRebaseSetupRequest {
    pub base: repositorytree_core::domain::CommitId,
    pub actual_head: repositorytree_core::domain::CommitId,
    pub selected_ids: Vec<repositorytree_core::domain::CommitId>,
    pub reword_id: repositorytree_core::domain::CommitId,
    pub message: String,
    pub count: usize,
}
...
pub(super) fn schedule_open_file_at_commit(
    ...
    spawn_with_repo(executor, repos, repo_id, msg_tx, move |repo, msg_tx| {
        ...
        send_or_log(
            ...
            Msg::OpenFileContent {
                repo_id,
                source: repositorytree_core::domain::FileSource::Commit(commit_id),
                path: resolved,
            },
        );
    });
            ...
            Ok(details) => {
                if let Some(parent) = details.parent_ids.first() {
                    ...
                    send_or_log(
                        ...
                        Msg::OpenFileContent {
                            repo_id,
                            source: repositorytree_core::domain::FileSource::Commit(parent.clone()),
                            path: resolved,
                        },
                    );
                }
...
pub(super) fn schedule_load_interactive_cherry_pick_messages(
    ...
    spawn_detached_with_repo_or_else(
        ...
        move |repo, msg_tx| {
            let commit_ids = ids
                .iter()
                .map(|id| repositorytree_core::domain::CommitId(id.clone().into()))
                .collect::<Vec<_>>();
            let result = repo
...
mod worktree_dirty_tests {
    use super::*;
    use repositorytree_core::domain::{CommitId, FileStatus, FileStatusKind};
...
mod worktree_scan_handle_tests {
    ...
    impl GitBackend for CountingBackend {
        fn open(&self, workdir: &Path) -> repositorytree_core::services::Result<Arc<dyn GitRepository>> {
            self.opens
