    ...
    pub(super) fn shared_for_store(pool: StoreExecutorPool, threads: usize) -> Self {
        ...
        let tx = match pool {
            StoreExecutorPool::Primary => {
                sender_for(&PRIMARY, "repositorytree-test-store-primary", threads)
            }
            StoreExecutorPool::RepoLoad => {
                sender_for(&REPO_LOAD, "repositorytree-test-store-repo-load", threads)
            }
            StoreExecutorPool::Metadata => {
                sender_for(&METADATA, "repositorytree-test-store-metadata", threads)
            }
            StoreExecutorPool::SessionPersist => sender_for(
                &SESSION_PERSIST,
                "repositorytree-test-store-session-persist",
                threads,
            ),
        };
