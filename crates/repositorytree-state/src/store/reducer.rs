...
use repositorytree_core::services::{GitRepository, SafePushAfterCommitContext};
...
fn retry_msg_for_repo_command(repo_id: RepoId, command: RepoCommandKind) -> Option<Msg> {
    Some(match command {
        ...
        // A signing failure mid-rebase leaves git's state (and RepositoryTree's
...
pub(crate) fn fill_select_diff_inline(
    state: &mut AppState,
    repo_id: RepoId,
    target: repositorytree_core::domain::DiffTarget,
...
fn reduce_inner(
    ...
    match msg {
        ...
        Msg::Internal(crate::msg::InternalMsg::SubmoduleAddTrustChecked {
            ...
            match result {
                Ok(repositorytree_core::services::SubmoduleTrustDecision::Proceed) => {
                    begin_local_action(state, repo_id);
                    start_submodule_add_progress(state, repo_id, &url, &path);
                    actions_emit_effects::add_submodule(
                        repo_id,
                ...
                Ok(repositorytree_core::services::SubmoduleTrustDecision::Prompt { sources }) => {
...
mod comparison_tests {
    ...
    fn loaded_details(id: &str) -> repositorytree_core::domain::CommitDetails {
