...
const TRACE_FILE_ENV: &str = "REPOSITORYTREE_REPO_LOAD_TRACE";
const TRACE_STDERR_ENV: &str = "REPOSITORYTREE_TRACE_REPO_LOADS";
...
static TARGET: LazyLock<TraceTarget> = LazyLock::new(|| {
        ...
        .map(PathBuf::from)
    {
        match OpenOptions::new().create(true).append(true).open(&path) {
            ...
            Err(error) => {
                eprintln!(
                    "repositorytree-state: failed to open repo load trace file {}: {error}",
                    path.display()
                );
            }
        }
