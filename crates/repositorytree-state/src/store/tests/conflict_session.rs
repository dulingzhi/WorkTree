...
use repositorytree_core::conflict_session::{ConflictPayload, ConflictResolverStrategy, ConflictSession};
use repositorytree_core::domain::{FileConflictKind, FileStatus, FileStatusKind, RepoStatus};
...
fn current_only_session_preserves_first_paint_pick_on_full_upgrade() {
    ...
    reduce(
        ...
        Msg::ConflictToggleRegionSource {
            ...
            path: PathBuf::from("file.txt").into(),
            region_index: 0,
            source: repositorytree_core::merge::MergeSource::A,
        },
    );
    reduce(
        &mut repos,
    ...
    assert_eq!(
        ...
        ConflictRegionResolution::Sources(repositorytree_core::merge::MergeSource::B.into()),
...
";
...

    {
        ...
        session.regions[0].resolution =
            repositorytree_core::conflict_session::ConflictRegionResolution::PickTheirs;
    }
    let before_rev = state
        .repos
        .iter()
    ...
    assert_eq!(
        session.regions[0].resolution,
        repositorytree_core::conflict_session::ConflictRegionResolution::Sources(
            repositorytree_core::merge::MergeSource::B.into()
        )
    );
    assert_eq!(
...
";
    ...
    reduce(
        ...
        Msg::ConflictSyncRegionResolutions {
            ...
            updates: vec![crate::msg::ConflictRegionResolutionUpdate {
                region_index: 0,
                resolution: repositorytree_core::conflict_session::ConflictRegionResolution::Unresolved,
            }],
        },
    );
...
";
    ...
    assert!(session.regions.iter().all(|region| matches!(
        region.resolution,
        repositorytree_core::conflict_session::ConflictRegionResolution::PickBase
    )));
    assert_eq!(repo_state.conflict_state.conflict_rev, before_rev + 1);
}
...
fn conflict_split_geometry_survives_same_path_reload() {
    use repositorytree_core::conflict_session::ConflictRegionSplitBoundaries;
...
fn conflict_reload_via_stash_keeps_ordered_resolution() {
    use repositorytree_core::conflict_session::ConflictRegionResolution;
    use repositorytree_core::domain::{DiffArea, DiffTarget};
...
fn conflict_manual_alignment_replans_in_memory_and_clears_back() {
    use repositorytree_core::merge::ManualAlignment;
