...
fn test_force_push_lease() -> repositorytree_core::services::ForcePushLease {
    repositorytree_core::services::ForcePushLease {
...
fn test_recent_commit_message() -> repositorytree_core::domain::RecentCommitMessage {
...
fn test_recent_commit_message_with_summary(
    id: &str,
    summary: &str,
) -> repositorytree_core::domain::RecentCommitMessage {
    repositorytree_core::domain::RecentCommitMessage {
        id: CommitId(id.into()),
...
fn reset_emits_effect() {
    ...
    let effects = reduce(
        ...
        Msg::Reset {
            ...
            mode: repositorytree_core::services::ResetMode::Hard,
        },
    );

    assert!(matches!(
        effects.as_slice(),
        [Effect::Reset { repo_id: RepoId(1), target, mode: repositorytree_core::services::ResetMode::Hard }]
            if target == "HEAD~1"
    ));
}
...
fn selected_submodule_command_reloads_selected_summary() {
    ...
    ) -> (AppState, repositorytree_core::domain::DiffTarget) {
        let mut state = AppState::default();
...
fn commit_finished_clears_commit_state_and_requests_primary_refreshes() {
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::CommitFinished {
            repo_id,
            result: Ok(repositorytree_core::services::CommitOperationOutcome::default()),
        }),
    );
...
fn ready_working_tree_blame() -> Loadable<std::sync::Arc<Vec<repositorytree_core::services::BlameLine>>> {
...
fn repo_command_finished_stage_hunk_invalidates_loaded_blame() {
    ...
    state.repos[0].history_state.blame_source = Some(
        repositorytree_core::domain::BlameSource::WorkingTree(DiffArea::Unstaged),
    );
...
fn commit_finished_invalidates_loaded_blame() {
    ...
    state.repos[0].history_state.blame_path = Some(PathBuf::from("src/lib.rs"));
    state.repos[0].history_state.blame_source = Some(
        repositorytree_core::domain::BlameSource::WorkingTree(DiffArea::Staged),
    );
    state.repos[0].history_state.blame = ready_working_tree_blame();
...
fn local_submodule_add_trust_prompt_confirms_into_add_effect() {
    ...
    let source = repositorytree_core::services::SubmoduleTrustTarget {
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::SubmoduleAddTrustChecked {
            ...
            name: None,
            force: false,
            result: Ok(repositorytree_core::services::SubmoduleTrustDecision::Prompt {
...
fn submodule_trust_check_pending_marks_and_clears_around_the_check() {
    ...
    let _ = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::SubmoduleAddTrustChecked {
            ...
            force: false,
            result: Ok(repositorytree_core::services::SubmoduleTrustDecision::Prompt {
                sources: vec![repositorytree_core::services::SubmoduleTrustTarget {
                    submodule_path: PathBuf::from("mods/sub"),
                    display_source: "../local-sub".to_string(),
                    local_source_path: PathBuf::from("/tmp/local-sub"),
                }],
...
fn submodule_add_progress_starts_when_trust_check_proceeds() {
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::SubmoduleAddTrustChecked {
            ...
            force: true,
            result: Ok(repositorytree_core::services::SubmoduleTrustDecision::Proceed),
        }),
    );
...
fn submodule_add_progress_clears_on_failed_add_command() {
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::RepoCommandFinished {
            ...
            result: Err(repositorytree_core::error::Error::new(
                repositorytree_core::error::ErrorKind::Backend("submodule add failed".to_string()),
            )),
        }),
    );
...
fn commit_finished_push_after_commit_enqueues_safe_push_only_on_success() {
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::CommitFinished {
            repo_id,
            result: Ok(repositorytree_core::services::CommitOperationOutcome {
...
fn safe_push_after_commit_decision_push_enqueues_checked_push() {
    ...
    let mut state = AppState::default();
    let repo_id = RepoId(1);
    let target = repositorytree_core::services::SafePushAfterCommitTarget {
        remote: "origin".to_string(),
        branch: "main".to_string(),
        local_branch: "main".to_string(),
        local_head: CommitId("2222222222222222222222222222222222222222".into()),
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::SafePushAfterCommitFinished {
            ...
            result: Ok(repositorytree_core::services::SafePushAfterCommitDecision::Push {
                target: target.clone(),
            }),
        }),
    );
...
fn safe_push_after_commit_decision_push_set_upstream_enqueues_checked_push() {
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::SafePushAfterCommitFinished {
            ...
            auth: None,
            result: Ok(
                repositorytree_core::services::SafePushAfterCommitDecision::PushSetUpstream {
                    target: target.clone(),
                },
            ),
        }),
...
fn safe_push_after_commit_published_amend_block_stores_lease_offer() {
    ...
    let lease = repositorytree_core::services::ForcePushLease {
        remote: "origin".to_string(),
        branch: "main".to_string(),
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::SafePushAfterCommitFinished {
            ...
            auth: None,
            result: Ok(
                repositorytree_core::services::SafePushAfterCommitDecision::Blocked {
    ...
    reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::RepoCommandFinished {
            ...
            command: RepoCommandKind::InteractiveCherryPick {
                entries: vec![repositorytree_core::services::InteractiveRebaseEntry {
                    action: repositorytree_core::services::InteractiveRebaseAction::Pick,
...
fn hunk_staging_is_logged_without_announcing_success() {
    ...
    reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::RepoCommandFinished {
            ...
            command: RepoCommandKind::StageHunk,
            result: Err(repositorytree_core::error::Error::new(
                repositorytree_core::error::ErrorKind::Backend("patch does not apply".into()),
            )),
        }),
    );
    let repo_state = state.repos.iter().find(|r| r.id == repo_id).unwrap();
...
fn repo_state_with_tags_loaded(repo_id: RepoId) -> RepoState {
    ...
    repo_state.local_actions_in_flight = 1;
    repo_state.set_tags(Loadable::Ready(vec![repositorytree_core::domain::Tag {
        name: "v1.0.0".to_string(),
...
fn create_tag_failed_does_not_reload_tags() {
    ...
    reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::RepoCommandFinished {
            ...
            result: Err(repositorytree_core::error::Error::new(
                repositorytree_core::error::ErrorKind::Backend("tag already exists".to_string()),
            )),
        }),
    );
...
fn cherry_pick_setup_applies_repository_topological_order() {
    ...
    let entry = |commit_id: &str, summary: &str| repositorytree_core::services::InteractiveRebaseEntry {
        action: repositorytree_core::services::InteractiveRebaseAction::Pick,
...
fn cherry_pick_setup_never_enables_rewording_after_partial_or_stale_message_load() {
    ...
    let entry = |commit_id: &str| repositorytree_core::services::InteractiveRebaseEntry {
        action: repositorytree_core::services::InteractiveRebaseAction::Pick,
        commit_id: commit_id.to_string(),
        summary: "subject".to_string(),
        message: "subject".to_string(),
...
fn ai_commit_context_load_error_is_recorded_not_swallowed() {
    ...
    reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::AiCommitContextLoaded {
            ...
            request_rev,
            result: Err(repositorytree_core::error::Error::new(
                repositorytree_core::error::ErrorKind::Backend("diff failed".to_string()),
            )),
        }),
    );
    assert!(matches!(
