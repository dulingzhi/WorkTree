...
fn commit_details_for(id: &CommitId, message: &str) -> repositorytree_core::domain::CommitDetails {
...
fn external_git_state_change_refreshes_history_and_selected_diff() {
    ...
    reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::RebaseStateLoaded {
            repo_id: RepoId(1),
            result: Ok(repositorytree_core::services::SequencerState::None),
        }),
    );
    reduce(
        &mut repos,
...
fn external_worktree_refresh_coalesces_status_while_status_is_in_flight() {
    ...
    reduce(
        ...
        Msg::SelectDiff {
            repo_id: RepoId(1),
            target: DiffTarget::WorkingTree {
                path: PathBuf::from("crates/repositorytree-ui-gpui/src/smoke_tests.rs"),
                area: DiffArea::Unstaged,
            },
        },
    );
...
fn reload_repo_clears_stale_navigation_history() {
    ...
    state.repos[0]
        ...
        .record(crate::model::ViewHistoryEntry {
            source: repositorytree_core::domain::FileSource::Commit(commit_a.clone()),
            path: PathBuf::from("src/lib.rs"),
        });
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::CommitRevealResolved {
            ...
            result: Err(repositorytree_core::error::Error::new(
                repositorytree_core::error::ErrorKind::Backend("unknown revision".to_string()),
            )),
        }),
    );
...
fn external_tags_change_reloads_tags() {
    ...
    state.active_repo = Some(repo_id);
    state.repos[0].set_tags(Loadable::Ready(vec![repositorytree_core::domain::Tag {
...
fn cancelled_log_reply_is_not_reported_as_an_error() {
    ...
    let effects = reduce(
        ...
        Msg::Internal(crate::msg::InternalMsg::LogLoaded {
            ...
            cursor: None,
            result: Err(repositorytree_core::error::Error::new(
                repositorytree_core::error::ErrorKind::Cancelled,
            )),
        }),
    );
