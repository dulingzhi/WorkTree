...
fn unavailable_git_effect_emits_synthetic_repo_command_error() {
    ...
    let state = AppState {
        git_runtime: repositorytree_core::process::GitRuntimeState {
            preference: repositorytree_core::process::GitExecutablePreference::Custom(PathBuf::new()),
            availability: repositorytree_core::process::GitExecutableAvailability::Unavailable {
...
fn safe_push_after_commit_effect_carries_auth_to_finished_message() {
    ...
    let auth = repositorytree_core::auth::StagedGitAuth {
        kind: repositorytree_core::auth::GitAuthKind::UsernamePassword,
...
fn clone_repo_effect_clones_local_repo_and_emits_finished_and_open_repo() {
    ...
    let base = std::env::temp_dir().join(format!(
        "repositorytree-clone-effect-test-{}-{}",
        std::process::id(),
        SystemTime::now()
...
fn load_conflict_file_effect_reads_worktree_and_emits_loaded() {
    ...
    struct Repo {
        spec: RepoSpec,
        diff: repositorytree_core::domain::FileDiffText,
    }
    ...
    let base = std::env::temp_dir().join(format!(
        "repositorytree-conflict-load-test-{}-{}",
        std::process::id(),
        SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap_or_default()
    ...
    let repo: Arc<dyn GitRepository> = Arc::new(Repo {
        ...
        diff: repositorytree_core::domain::FileDiffText::new(
            rel.clone(),
            Some("ours\n".to_string()),
            Some("theirs\n".to_string()),
        ),
...
fn load_conflict_file_effect_reuses_conflict_session_payloads_without_stage_fetch() {
    ...
    use repositorytree_core::domain::FileConflictKind;
    use repositorytree_core::services::ConflictFileStages;
    ...
    let base = std::env::temp_dir().join(format!(
        "repositorytree-conflict-load-session-reuse-test-{}-{}",
        std::process::id(),
        SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap_or_default()
...
fn load_conflict_file_effect_preserves_binary_payloads_when_reusing_session() {
    ...
    let base = std::env::temp_dir().join(format!(
        "repositorytree-conflict-load-session-reuse-binary-test-{}-{}",
        std::process::id(),
        SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap_or_default()
...
fn load_conflict_file_effect_reuses_absent_current_payload_without_rereading_worktree() {
    ...
    let base = std::env::temp_dir().join(format!(
        "repositorytree-conflict-load-session-reuse-absent-current-test-{}-{}",
        std::process::id(),
        SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap_or_default()
...
fn load_conflict_file_effect_records_trace_stages_and_sizes() {
    ...
    let _trace = mergetool_trace::capture();
    let base = std::env::temp_dir().join(format!(
        "repositorytree-conflict-load-trace-test-{}-{}",
        std::process::id(),
        SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap_or_default()
...
fn save_worktree_file_effect_writes_and_can_stage() {
    ...
    let base = std::env::temp_dir().join(format!(
        "repositorytree-save-worktree-file-test-{}-{}",
        std::process::id(),
        SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap_or_default()
    ...
    let escaped_name = format!(
        "repositorytree-save-worktree-file-escape-{}-{}.txt",
        std::process::id(),
        SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap_or_default()
...
fn append_gitignore_patterns_effect_creates_appends_and_dedupes() {
    ...
    assert_eq!(
        stdout.trim(),
        repositorytree_core::gitignore::NOTHING_TO_ADD,
...
fn load_stashes_effect_truncates_results_to_limit() {
    ...
    let base = std::env::temp_dir().join(format!(
        "repositorytree-stash-load-test-{}-{}",
        std::process::id(),
        SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap_or_default()
...
impl GitRepository for MetadataSchedulingRepo {
    ...
    fn list_tags(&self) -> Result<Vec<repositorytree_core::domain::Tag>> {
        match self.mode {
    ...
    fn list_remote_tags(&self) -> Result<Vec<repositorytree_core::domain::RemoteTag>> {
        match self.mode {
...
impl GitRepository for SelectedDiffSchedulingRepo {
    ...
    fn diff_parsed_cancellable(
    ...
    ) -> Result<repositorytree_core::domain::Diff> {
        ...
        Ok(repositorytree_core::domain::Diff::from_unified(
            target.clone(),
...
fn checkout_branch_effect_requests_branch_and_worktree_reload_on_success() {
    ...
    let repo: Arc<dyn GitRepository> = Arc::new(RecordingCheckoutRepo {
        spec: RepoSpec {
            workdir: unique_temp_path("repositorytree-checkout-branch-effect"),
        },
        calls: Arc::clone(&calls),
    });
    let repos: FxHashMap<RepoId, Arc<dyn GitRepository>> = {
...
fn checkout_remote_branch_effect_requests_branch_and_worktree_reload_on_success() {
    ...
    let repo: Arc<dyn GitRepository> = Arc::new(RecordingCheckoutRepo {
        spec: RepoSpec {
            workdir: unique_temp_path("repositorytree-checkout-remote-branch-effect"),
        },
        calls: Arc::clone(&calls),
    });
    let repos: FxHashMap<RepoId, Arc<dyn GitRepository>> = {
...
fn checkout_commit_effect_requests_worktree_reload_on_success() {
    ...
    let repo: Arc<dyn GitRepository> = Arc::new(RecordingCheckoutRepo {
        spec: RepoSpec {
            workdir: unique_temp_path("repositorytree-checkout-commit-effect"),
        },
        calls: Arc::clone(&calls),
    });
    let repos: FxHashMap<RepoId, Arc<dyn GitRepository>> = {
...
fn create_branch_and_checkout_effect_requests_branch_and_worktree_reload_on_success() {
    ...
    let repo: Arc<dyn GitRepository> = Arc::new(RecordingCheckoutRepo {
        spec: RepoSpec {
            workdir: unique_temp_path("repositorytree-create-branch-and-checkout-effect"),
        },
        calls: Arc::clone(&calls),
    });
    let repos: FxHashMap<RepoId, Arc<dyn GitRepository>> = {
...
fn open_repo_effect_emits_repo_opened_ok() {
    ...
    let workdir = unique_temp_path("repositorytree-open-repo-ok");
...
fn open_repo_effect_emits_repo_opened_err() {
    ...
    let workdir = unique_temp_path("repositorytree-open-repo-err");
    let backend: Arc<dyn GitBackend> = Arc::new(Backend);
    let executor = super::executor::TaskExecutor::new(1);
    let repos: FxHashMap<RepoId, Arc<dyn GitRepository>> = FxHashMap::default();
    let (msg_tx, msg_rx) = std::sync::mpsc::channel::<Msg>();
...
fn open_repo_effect_suppresses_result_after_cancellation() {
    ...
    let workdir = unique_temp_path("repositorytree-open-repo-cancelled");
    let repo: Arc<dyn GitRepository> = Arc::new(UnsupportedRepo {
        spec: RepoSpec {
            workdir: workdir.clone(),
        },
...
fn open_repo_effects_are_bounded_by_repo_load_executor() {
    ...
    let workdir_a = unique_temp_path("repositorytree-open-repo-bounded-a");
    let workdir_b = unique_temp_path("repositorytree-open-repo-bounded-b");
...
fn load_log_effect_uses_history_mode_api() {
    ...
    let repo: Arc<dyn GitRepository> = Arc::new(RecordingLogRepo {
        spec: RepoSpec {
            workdir: unique_temp_path("repositorytree-load-log-history-mode-effect"),
        },
        calls: Arc::clone(&calls),
    });
    let repos: FxHashMap<RepoId, Arc<dyn GitRepository>> = {
...
fn activation_load_effect_is_not_blocked_by_main_executor_queue() {
    ...
    let repo: Arc<dyn GitRepository> = Arc::new(UnsupportedRepo {
        spec: RepoSpec {
            workdir: unique_temp_path("repositorytree-foreground-load-effect"),
        },
    });
    let repos: FxHashMap<RepoId, Arc<dyn GitRepository>> = {
        let mut repos = FxHashMap::default();
...
fn remote_tag_load_for_one_repo_does_not_block_other_repo_metadata_refresh() {
    ...
    let repos: FxHashMap<RepoId, Arc<dyn GitRepository>> = {
        ...
        repos.insert(
            ...
            Arc::new(MetadataSchedulingRepo {
                spec: RepoSpec {
                    workdir: unique_temp_path("repositorytree-metadata-blocking-remote-tags"),
                },
        ...
        repos.insert(
            ...
            Arc::new(MetadataSchedulingRepo {
                spec: RepoSpec {
                    workdir: unique_temp_path("repositorytree-metadata-ready-tags"),
                },
    ...
    state.repos.push(RepoState::new_opening(
        repo_a,
        RepoSpec {
            workdir: unique_temp_path("repositorytree-metadata-state-a"),
        },
    ));
    state.repos.push(RepoState::new_opening(
        repo_b,
        RepoSpec {
            workdir: unique_temp_path("repositorytree-metadata-state-b"),
        },
    ));
    let thread_state = Arc::new(std::sync::RwLock::new(Arc::new(state)));
    let mut repo_task_tokens = FxHashMap::default();
...
fn cancelled_selected_diff_does_not_keep_executor_busy_for_next_repo() {
    ...
    let repos: FxHashMap<RepoId, Arc<dyn GitRepository>> = {
        ...
        repos.insert(
            ...
            Arc::new(SelectedDiffSchedulingRepo {
                spec: RepoSpec {
                    workdir: unique_temp_path("repositorytree-selected-diff-blocking-a"),
                },
        ...
        repos.insert(
            ...
            Arc::new(SelectedDiffSchedulingRepo {
                spec: RepoSpec {
                    workdir: unique_temp_path("repositorytree-selected-diff-ready-b"),
                },
    ...
    let mut repo_state_a = RepoState::new_opening(
        repo_a,
        RepoSpec {
            workdir: unique_temp_path("repositorytree-selected-diff-state-a"),
        },
    );
    ...
    let mut repo_state_b = RepoState::new_opening(
        repo_b,
        RepoSpec {
            workdir: unique_temp_path("repositorytree-selected-diff-state-b"),
        },
    );
...
fn schedule_effect_dispatches_many_variants_with_repo_present() {
    ...
    let workdir = unique_temp_path("repositorytree-effects-dispatch");
    ...
    let effect_specs: Vec<(Effect, usize)> = vec![
        ...
        (
            Effect::LoadBlame {
                repo_id,
                path: PathBuf::from("tracked.txt"),
                source: repositorytree_core::domain::BlameSource::Revision(Some("HEAD".to_string())),
            },
            1,
        ),
        ...
        (
            Effect::Reset {
                ...
                mode: repositorytree_core::services::ResetMode::Mixed,
            },
            1,
        ),
        (
        ...
        (
            Effect::SetRemoteUrl {
                ...
                name: "origin".to_string(),
                url: "https://example.com/repo.git".to_string(),
                kind: repositorytree_core::services::RemoteUrlKind::Fetch,
            },
            1,
        ),
        (
