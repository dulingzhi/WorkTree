...
#[cfg(test)]
use repositorytree_core::auth::stage_git_auth;
use repositorytree_core::auth::{GitAuthKind, StagedGitAuth, clear_staged_git_auth};
use repositorytree_core::domain::{DiffArea, DiffTarget, FileStatusKind};
use repositorytree_core::error::{Error, ErrorKind, GitFailure};
...
pub(super) struct SelectedDiffLoadPlan {
    ...
    pub preview_text_side: Option<repositorytree_core::domain::DiffPreviewTextSide>,
...
fn diff_target_preview_text_side(
    ...
    target: &DiffTarget,
) -> Option<repositorytree_core::domain::DiffPreviewTextSide> {
    match target {
        DiffTarget::WorkingTree { path, area } => {
        ...
        DiffTarget::Commit {
            ...
            details.files.iter().find_map(|file| {
                (file.path == *path && !file.is_submodule).then_some(match file.kind {
                    FileStatusKind::Added => Some(repositorytree_core::domain::DiffPreviewTextSide::New),
                    FileStatusKind::Deleted => {
                        Some(repositorytree_core::domain::DiffPreviewTextSide::Old)
                    }
                    FileStatusKind::Modified
    ...
    let preview_text_side = if supports_file && (!preview.wants_image || preview.is_svg) {
        if content_preview {
            ...
            matches!(target, DiffTarget::Commit { .. })
                .then_some(repositorytree_core::domain::DiffPreviewTextSide::New)
        } else {
...
fn summarize_command(
    ...
    use repositorytree_core::services::ConflictSide;
    ...
    let summary = match command {
        ...
        RepoCommandKind::AppendGitignorePatterns { patterns } => {
            if output.stdout.trim() == repositorytree_core::gitignore::NOTHING_TO_ADD {
        ...
        RepoCommandKind::Reset { mode, target } => {
            let mode = match mode {
                repositorytree_core::services::ResetMode::Soft => "soft",
                repositorytree_core::services::ResetMode::Mixed => "mixed",
                repositorytree_core::services::ResetMode::Hard => "hard",
            };
        ...
        RepoCommandKind::CherryPick {
            ...
            if output
                .stdout
                .contains("REPOSITORYTREE_CHERRY_PICK_ALREADY_APPLIED")
            {
        ...
        RepoCommandKind::SetRemoteUrl { name, kind, .. } => {
            let kind = match kind {
                repositorytree_core::services::RemoteUrlKind::Fetch => "fetch",
                repositorytree_core::services::RemoteUrlKind::Push => "push",
            };
...
mod tests {
    ...
    use repositorytree_core::domain::{CommitId, DiffArea, DiffTarget, RepoSpec};
    use repositorytree_core::error::{GitFailure, GitFailureId};
    use repositorytree_core::services::{PullMode, RemoteUrlKind, ResetMode};
    use std::path::Path;
    ...
    fn repo_state(id: u64) -> RepoState {
        RepoState::new_opening(
            RepoId(id),
            RepoSpec {
                workdir: PathBuf::from("/tmp/repositorytree-state-util-tests"),
            },
        )
    }
    ...
    fn preview_only_svg_still_loads_file_text_for_the_code_view() {
        ...
        use repositorytree_core::domain::{FileStatus, RepoStatus};
    ...
    fn summarize_command_success_covers_status_variants() {
        ...
        let (_, cherry_pick_already_applied_summary) = summarize_command(
            ...
            &command_output(
                ...
                "REPOSITORYTREE_CHERRY_PICK_ALREADY_APPLIED",
                "",
            ),
            true,
            None,
    ...
    fn stage_git_auth_env_stages_and_clears_shared_auth_slot() {
        ...
        repositorytree_core::auth::clear_staged_git_auth();
        stage_git_auth_env(
        ...
        let staged = repositorytree_core::auth::take_staged_git_auth().expect("staged auth to exist");
        assert_eq!(staged.username.as_deref(), Some("alice"));
        assert_eq!(staged.secret, "secret-token");
        assert_eq!(
            staged.kind,
            repositorytree_core::auth::GitAuthKind::UsernamePassword
        );
        ...
        let staged =
            repositorytree_core::auth::take_staged_git_auth().expect("staged passphrase to exist");
        assert_eq!(staged.kind, repositorytree_core::auth::GitAuthKind::Passphrase);
        ...
        let staged =
            repositorytree_core::auth::take_staged_git_auth().expect("staged host verification to exist");
        assert_eq!(
            staged.kind,
            repositorytree_core::auth::GitAuthKind::HostVerification
        );
        assert_eq!(staged.secret, "yes");

        clear_staged_git_auth_env();
        assert!(repositorytree_core::auth::take_staged_git_auth().is_none());
    }
}
