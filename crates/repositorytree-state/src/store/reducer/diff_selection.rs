...
fn content_view_target(
    ...
    match source {
        repositorytree_core::domain::FileSource::WorkingDirectory => Some(DiffTarget::WorkingTree {
            path,
            area: DiffArea::Unstaged,
        }),
        repositorytree_core::domain::FileSource::Commit(commit_id) => Some(DiffTarget::Commit {
            commit_id,
            path: Some(path),
        }),
        // Branch file listing is not wired, so this is unreachable from the UI.
        repositorytree_core::domain::FileSource::Branch(_) => None,
    }
}
...
fn view_history_entry_for_target(target: &DiffTarget) -> Option<ViewHistoryEntry> {
    match target {
        DiffTarget::Commit {
            ...
            source: repositorytree_core::domain::FileSource::Commit(commit_id.clone()),
            path: path.clone(),
        }),
        DiffTarget::WorkingTree {
            path,
        ...
        && repo_state.diff_state.diff_target.as_ref() == Some(&target)
    {
        match result {
            Ok(v) => {
                ...
                repo_state.diff_state.lfs_image_preview =
