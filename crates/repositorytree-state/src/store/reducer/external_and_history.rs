...
fn file_browser_refresh_for_external_change(
    ...
    if repo_state.file_browser.source != repositorytree_core::domain::FileSource::WorkingDirectory {
        return None;
    }
...
pub(super) fn log_loaded(
    ...
    if let Some(repo_state) = state.repos.iter_mut().find(|r| r.id == repo_id) {
        ...
        match result {
            ...
            Err(e) if matches!(e.kind(), repositorytree_core::error::ErrorKind::Cancelled) => {
                if is_load_more {
                    repo_state.set_log_loading_more(false);
                }
            }
            ...
            && let Loadable::Ready(page) = &repo_state.log
        {
            ...
            let survives = |id: &repositorytree_core::domain::CommitId| {
