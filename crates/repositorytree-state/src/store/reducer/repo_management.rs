...
use repositorytree_core::services::{CommandOutput, GitRepository};
...
fn is_missing_repo_error(error: &Error) -> bool {
    matches!(
        error.kind(),
        repositorytree_core::error::ErrorKind::Io(std::io::ErrorKind::NotFound)
    )
}
...
fn persist_repo_history_mode_effect(
    repo_id: Option<RepoId>,
    workdir: PathBuf,
    mode: repositorytree_core::domain::HistoryMode,
) -> Effect {
...
pub(in crate::store::reducer) enum SelectedHistoryReload {
    ...
    CommitDetails(repositorytree_core::domain::CommitId),
}
...
pub(super) fn repo_opened_err(
    ...
    if matches!(
        error.kind(),
        repositorytree_core::error::ErrorKind::NotARepository
    ) {
        let mut effects = Vec::new();
        clear_banner_error_for_repo(state, repo_id);
        push_notification(
