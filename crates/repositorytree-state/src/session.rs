...
use repositorytree_core::domain::{HistoryMode, LogScope};
...
#[cfg(unix)]
const SESSION_PATH_BYTES_PREFIX: &str = "repositorytree-path-bytes:";
#[cfg(windows)]
const SESSION_PATH_WIDE_PREFIX: &str = "repositorytree-path-utf16le:";

const SESSION_FILE_ENV: &str = "REPOSITORYTREE_SESSION_FILE";
const DISABLE_SESSION_PERSIST_ENV: &str = "REPOSITORYTREE_DISABLE_SESSION_PERSIST";
...
/// `repositorytree_state::store::canonicalize_path`). The repo picker relies on plain
...
fn recent_repo_storage_key(workdir: &Path) -> String {
    path_storage_key(&repositorytree_core::path_utils::canonicalize_or_original(
        workdir.to_path_buf(),
    ))
}
...
// RepositoryTree before; it does not need to prove the repository still exists on disk.
...
fn app_data_dir() -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        Some(home.join("Library/Application Support/repositorytree"))
    }

    #[cfg(target_os = "windows")]
    {
        let appdata = env::var_os("LOCALAPPDATA").or_else(|| env::var_os("APPDATA"));
        Some(non_empty_path(appdata.as_deref())?.join("repositorytree"))
    }

    #[cfg(not(any(target_os = "linux", target_os = "macos", target_os = "windows")))]
    {
        non_empty_path(env::var_os("HOME").as_deref()).map(|home| home.join(".repositorytree"))
    }
}
...
fn app_data_dir_linux(xdg_data_home: Option<&OsStr>, home: Option<&OsStr>) -> Option<PathBuf> {
    if let Some(data_home) = non_empty_path(xdg_data_home) {
        return Some(data_home.join("repositorytree"));
    }
    let home = non_empty_path(home)?;
    Some(home.join(".local/share/repositorytree"))
}
...
fn app_state_dir() -> Option<PathBuf> {
    ...
    #[cfg(target_os = "linux")]
    {
        if let Some(state_home) = non_empty_path(env::var_os("XDG_STATE_HOME").as_deref()) {
            return Some(state_home.join("repositorytree"));
        }
        let home = non_empty_path(env::var_os("HOME").as_deref())?;
        Some(home.join(".local/state/repositorytree"))
    }
...
mod tests {
    ...
    use repositorytree_core::domain::{HistoryMode, LogScope, RepoSpec};
    ...
    fn unique_session_test_dir(label: &str) -> PathBuf {
        let dir = env::temp_dir().join(format!(
            "repositorytree-session-unit-test-{label}-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
    ...
    fn session_file_round_trips() {
        let dir = env::temp_dir().join(format!("repositorytree-session-test-{}", std::process::id()));
        let _ = fs::create_dir_all(&dir);
    ...
    fn path_storage_key_keeps_utf8_plain_text() {
        let path = Path::new("/tmp/repositorytree-repo");
        let key = path_storage_key(path);
        assert_eq!(key, "/tmp/repositorytree-repo");
    ...
    fn path_storage_key_round_trips_non_utf8_unix_bytes() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/repositorytree-\xff"));
        let key = path_storage_key(path);
    ...
    fn persist_ui_settings_round_trips_ai_commit_settings() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-session-ai-commit-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn persist_ui_settings_round_trips_sidebar_collapsed() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-session-sidebar-collapsed-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn survey_prompt_requires_recorded_repository() {
        const SURVEY_ID: &str = "repositorytree_user_survey_2026_04";
    ...
    fn survey_prompt_respects_id_opened_and_postponed_state() {
        const SURVEY_ID: &str = "repositorytree_user_survey_2026_04";
        const NEXT_SURVEY_ID: &str = "repositorytree_user_survey_2026_05";
    ...
    fn detects_test_harness_executable_paths() {
        ...
        assert!(looks_like_test_binary(Path::new(
            "/tmp/target/debug/repositorytree_ui_gpui-3ad1b0fd3f0c0d3e"
        )));
        assert!(!looks_like_test_binary(Path::new(
            "/tmp/target/debug/repositorytree"
        )));
    }
    ...
    fn app_data_dir_prefers_xdg_data_home() {
        assert_eq!(
            app_data_dir_linux(
                Some(OsStr::new("/tmp/repositorytree-data")),
                Some(OsStr::new("/home/alice"))
            ),
            Some(PathBuf::from("/tmp/repositorytree-data/repositorytree"))
        );
    }
    ...
    fn app_data_dir_falls_back_to_local_share() {
        assert_eq!(
            ...
            Some(PathBuf::from("/home/alice/.local/share/repositorytree"))
        );
    }
    ...
    fn load_from_path_migrates_v2_scaled_dimensions_to_design_units() {
        ...
        {
            let dir = env::temp_dir().join(format!(
                "repositorytree-session-v2-migration-test-{}-{}-{percent}",
                std::process::id(),
                std::time::SystemTime::now()
                    .duration_since(std::time::SystemTime::UNIX_EPOCH)
                    .unwrap_or_default()
    ...
    fn load_from_path_migrates_v2_scaled_dimensions_without_saved_zoom_as_100_percent() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-session-v2-migration-default-scale-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn persist_recent_repo_round_trips_dedup_and_reorders() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-recent-repos-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
        ...
        let canonical = |path: &std::path::Path| {
            repositorytree_core::path_utils::canonicalize_or_original(path.to_path_buf())
        };
        let loaded = load_from_path(&path);
        assert_eq!(
            loaded.recent_repos,
    ...
    fn history_highlight_commit_chain_round_trips_and_defaults_to_unset() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-highlight-chain-setting-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn remove_recent_repo_drops_matching_entry() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-remove-recent-repo-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn persist_pinned_repo_appends_in_pin_order_and_dedupes() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-pinned-repos-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn persist_recent_repo_collapses_a_padded_duplicate() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-recent-padded-dupe-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn pinned_repos_survive_the_recent_repository_cap() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-pinned-repo-cap-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn promote_recent_repo_matches_the_file_at_the_cap() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-promote-recent-cap-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn persist_ui_settings_round_trips_repo_picker_collapsed_sections() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-picker-collapse-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn remove_recent_repo_drops_entries_written_uncanonicalized() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-remove-recent-legacy-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
        ...
        let canonical = repositorytree_core::path_utils::canonicalize_or_original(repo.clone());
    ...
    fn remove_recent_repo_matches_an_entry_spelled_through_a_symlink() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-remove-recent-normalized-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn remove_recent_repo_normalizes_an_encoded_entry_through_its_decoder() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-remove-recent-encoded-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn remove_recent_repo_leaves_relative_entries_alone() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-remove-recent-relative-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn persist_recent_repo_truncates_to_max_entries_and_skips_blank_values() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-recent-repo-truncate-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn load_from_path_filters_blank_and_duplicate_recent_repos() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-recent-repo-load-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn persist_ui_settings_round_trips_repo_sidebar_collapsed_items() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-ui-settings-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn persist_repo_history_scope_round_trips() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-repo-history-scope-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn persist_repo_history_scope_skips_rewriting_unchanged_value() {
        let dir = env::temp_dir().join(format!(
            "repositorytree-repo-history-scope-noop-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
