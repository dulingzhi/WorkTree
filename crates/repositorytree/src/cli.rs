//! CLI argument parsing for repositorytree.
//!
//! Supports six modes:
...
//! - `uninstall`: remove repositorytree difftool/mergetool integration
...
#[derive(Parser, Debug)]
#[command(name = "repositorytree", about = "Git GUI built with GPUI", version)]
pub struct Cli {
    #[command(subcommand)]
...
pub enum Command {
    ...
    /// Configure git to use repositorytree as the global diff/merge tool.
    Setup(SetupArgs),
    /// Remove repositorytree diff/merge tool config entries.
...
pub enum AppMode {
    ...
    /// Remove repositorytree-specific difftool/mergetool integration.
