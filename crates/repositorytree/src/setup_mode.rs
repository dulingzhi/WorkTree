//! `repositorytree setup` / `repositorytree uninstall` support.
//!
//! Setup writes the recommended global (or local) git config entries so that
//! `git difftool` and `git mergetool` invoke repositorytree automatically.
//! Uninstall removes those entries while preserving unrelated tool settings.

use repositorytree_core::path_utils::strip_windows_verbatim_prefix;
use repositorytree_core::process::git_command as process_git_command;
...
const BACKUP_ABSENT_SENTINEL: &str = "__repositorytree_absent__";
...
fn current_exe_path() -> Result<PathBuf, String> {
    std::env::current_exe()
        .map_err(|e| format!("Cannot determine repositorytree binary path: {e}"))
...
fn executable_path_for_shell(bin_path: &Path) -> Result<String, String> {
    let Some(path_text) = bin_path.to_str() else {
        return Err(format!(
            "Cannot configure repositorytree setup for non-Unicode executable path: {bin_path:?}"
        ));
    };
...
fn build_config_entries(bin_path: &str) -> Vec<ConfigEntry> {
    ...
    vec![
        ...
        ConfigEntry {
            key: "merge.tool",
            value: "repositorytree".into(),
        },
        ConfigEntry {
            key: "mergetool.repositorytree.cmd",
            value: format!(
        ...
        // - `mergetool.repositorytree.trustExitCode` preserves explicit per-tool
        ...
        ConfigEntry {
            key: "mergetool.repositorytree.trustExitCode",
            value: "true".into(),
        },
        ConfigEntry {
        ...
        ConfigEntry {
            key: "difftool.repositorytree.cmd",
            value: format!(
        ...
        // - `difftool.repositorytree.trustExitCode` preserves explicit per-tool
        //   behavior even if users override global defaults later.
        ConfigEntry {
        ...
        ConfigEntry {
            key: "difftool.repositorytree.trustExitCode",
            value: "true".into(),
        },
        ConfigEntry {
            key: "difftool.prompt",
        ...
        ConfigEntry {
            key: "merge.guitool",
            value: "repositorytree-gui".into(),
        },
        ConfigEntry {
            key: "mergetool.repositorytree-gui.cmd",
            value: format!(
                "{quoted_bin_path} mergetool --gui --base {base} --local {local} --remote {remote} --merged {merged}"
            ),
        },
        ConfigEntry {
            key: "mergetool.repositorytree-gui.trustExitCode",
            value: "true".into(),
        },
        ConfigEntry {
            key: "diff.guitool",
        ...
        ConfigEntry {
            key: "difftool.repositorytree-gui.cmd",
            value: format!(
                "{quoted_bin_path} difftool --gui --local {local} --remote {remote} --path {merged}"
            ),
        },
        ConfigEntry {
            key: "difftool.repositorytree-gui.trustExitCode",
            value: "true".into(),
        },
        ConfigEntry {
...
fn build_backup_entries() -> Vec<BackupEntry> {
    vec![
        BackupEntry {
            key: "merge.tool",
            expected_setup_value: "repositorytree",
            backup_key: "repositorytree.backup.merge-tool",
        },
        BackupEntry {
            key: "diff.tool",
            expected_setup_value: "repositorytree",
            backup_key: "repositorytree.backup.diff-tool",
        },
        BackupEntry {
            key: "merge.guitool",
            expected_setup_value: "repositorytree-gui",
            backup_key: "repositorytree.backup.merge-guitool",
        },
        BackupEntry {
            key: "diff.guitool",
            expected_setup_value: "repositorytree-gui",
            backup_key: "repositorytree.backup.diff-guitool",
        },
        BackupEntry {
            key: "mergetool.trustExitCode",
            expected_setup_value: "true",
            backup_key: "repositorytree.backup.mergetool-trust-exit-code",
        },
        BackupEntry {
            key: "mergetool.prompt",
            expected_setup_value: "false",
            backup_key: "repositorytree.backup.mergetool-prompt",
        },
        BackupEntry {
            key: "difftool.trustExitCode",
            expected_setup_value: "true",
            backup_key: "repositorytree.backup.difftool-trust-exit-code",
        },
        BackupEntry {
            key: "difftool.prompt",
            expected_setup_value: "false",
            backup_key: "repositorytree.backup.difftool-prompt",
        },
        BackupEntry {
            key: "mergetool.guiDefault",
            expected_setup_value: "auto",
            backup_key: "repositorytree.backup.mergetool-guidefault",
        },
        BackupEntry {
            key: "difftool.guiDefault",
            expected_setup_value: "auto",
            backup_key: "repositorytree.backup.difftool-guidefault",
        },
    ]
}
...
fn build_uninstall_entries() -> Vec<UninstallEntry> {
    vec![
        ...
        // RepositoryTree defaults, so other tools are not disrupted.
        UninstallEntry {
            key: "merge.tool",
            expected_value: Some("repositorytree"),
            guard: None,
        },
        UninstallEntry {
            key: "diff.tool",
        ...
        UninstallEntry {
            key: "merge.guitool",
            expected_value: Some("repositorytree-gui"),
            guard: None,
        },
        UninstallEntry {
            key: "diff.guitool",
        ...
        // targets RepositoryTree.
        UninstallEntry {
            key: "mergetool.trustExitCode",
            expected_value: Some("true"),
            guard: Some(UninstallGuard {
                key: "merge.tool",
                expected_value: "repositorytree",
            }),
        },
        UninstallEntry {
            key: "mergetool.prompt",
        ...
        UninstallEntry {
            ...
            guard: Some(UninstallGuard {
                key: "merge.guitool",
                expected_value: "repositorytree-gui",
            }),
        },
        UninstallEntry {
            key: "difftool.guiDefault",
...
pub fn run_setup(dry_run: bool, local: bool) -> Result<SetupResult, String> {
    ...
    if dry_run {
        ...
        let stdout = format!(
            "# Dry run: the following git config commands would be executed:\n{commands}\n\
             # Setup also stores backup values for {} key(s) under repositorytree.backup.* when needed.\n",
            backup_entries.len()
        );
    ...
    let stdout = format!(
        "Configured repositorytree as {scope_label} diff/merge tool.\n\
         Binary: {bin_str}\n\
...
pub fn run_uninstall(dry_run: bool, local: bool) -> Result<UninstallResult, String> {
    ...
    if dry_run {
        ...
        let stdout = format!(
            "# Dry run: the following git config commands may be executed safely:\n{commands}\n\
             # Uninstall also restores backup values from repositorytree.backup.* when present.\n"
        );
    ...
    let mut stdout = format!(
        "Unconfigured repositorytree from {scope_label} diff/merge tool.\n\
...
mod tests {
    ...
    fn build_config_entries_contains_all_required_keys() {
        let entries = build_config_entries("/usr/bin/repositorytree");
        let keys: Vec<&str> = entries.iter().map(|e| e.key).collect();

        // Headless tool
        assert!(keys.contains(&"merge.tool"));
        assert!(keys.contains(&"mergetool.repositorytree.cmd"));
        assert!(keys.contains(&"mergetool.trustExitCode"));
        assert!(keys.contains(&"mergetool.repositorytree.trustExitCode"));
        assert!(keys.contains(&"mergetool.prompt"));
        assert!(keys.contains(&"diff.tool"));
        assert!(keys.contains(&"difftool.repositorytree.cmd"));
        assert!(keys.contains(&"difftool.trustExitCode"));
        assert!(keys.contains(&"difftool.repositorytree.trustExitCode"));
        assert!(keys.contains(&"difftool.prompt"));

        // GUI tool variant
        assert!(keys.contains(&"merge.guitool"));
        assert!(keys.contains(&"diff.guitool"));
        assert!(keys.contains(&"mergetool.repositorytree-gui.cmd"));
        assert!(keys.contains(&"mergetool.repositorytree-gui.trustExitCode"));
        assert!(keys.contains(&"difftool.repositorytree-gui.cmd"));
        assert!(keys.contains(&"difftool.repositorytree-gui.trustExitCode"));
    ...
    fn gui_tool_uses_separate_tool_name() {
        ...
        assert_eq!(merge_guitool.value, "repositorytree-gui");
        assert_eq!(diff_guitool.value, "repositorytree-gui");
    }
    ...
    fn gui_tool_cmd_includes_gui_flag() {
        ...
        let merge_gui_cmd = entries
            .iter()
            .find(|e| e.key == "mergetool.repositorytree-gui.cmd")
            .unwrap();
        let diff_gui_cmd = entries
            .iter()
            .find(|e| e.key == "difftool.repositorytree-gui.cmd")
            .unwrap();
    ...
    fn headless_tool_cmd_omits_gui_flag() {
        ...
        let merge_cmd = entries
            .iter()
            .find(|e| e.key == "mergetool.repositorytree.cmd")
            .unwrap();
        let diff_cmd = entries
            .iter()
            .find(|e| e.key == "difftool.repositorytree.cmd")
            .unwrap();
    ...
    fn mergetool_cmd_escapes_single_quote_in_binary_path() {
        let entries = build_config_entries("/tmp/it's/repositorytree");
        let cmd = entries
            .iter()
            .find(|e| e.key == "mergetool.repositorytree.cmd")
            .unwrap();
        ...
        assert!(
            cmd.value.starts_with("'/tmp/it'\"'\"'s/repositorytree'"),
    ...
    fn format_commands_global_scope() {
        let entries = build_config_entries("/bin/repositorytree");
        let output = format_commands(&entries, "--global");

        // Headless mergetool entries
        assert!(output.contains("git config --global merge.tool"));
        assert!(output.contains("git config --global mergetool.repositorytree.cmd"));
        assert!(output.contains("git config --global mergetool.trustExitCode"));
        assert!(output.contains("git config --global mergetool.repositorytree.trustExitCode"));
        ...
        assert!(output.contains("git config --global difftool.repositorytree.cmd"));
        assert!(output.contains("git config --global difftool.trustExitCode"));
        assert!(
            output.contains("git config --global difftool.repositorytree.trustExitCode"),
        ...
        assert!(output.contains("git config --global mergetool.repositorytree-gui.cmd"));
        assert!(output.contains("git config --global mergetool.repositorytree-gui.trustExitCode"));
        assert!(output.contains("git config --global diff.guitool"));
        assert!(output.contains("git config --global difftool.repositorytree-gui.cmd"));
        assert!(output.contains("git config --global difftool.repositorytree-gui.trustExitCode"));
        ...
        assert!(
            !output.contains("''/bin/repositorytree'"),
    ...
    fn apply_config_to_local_repo() {
        ...
        let entries = build_config_entries("/test/repositorytree");
    ...
    fn uninstall_dry_run_lists_unset_commands_and_conditions() {
        ...
        assert!(
            result
                .stdout
                .contains("git config --local --unset-all mergetool.repositorytree.cmd")
        );
        assert!(
            result.stdout.contains(
                "git config --local --unset-all merge.tool  # only if value is 'repositorytree'"
            )
        );
        assert!(
            result.stdout.contains("and merge.tool is 'repositorytree'"),
    ...
    fn uninstall_plan_unsets_matching_setup_values() {
        ...
        snapshot.insert("mergetool.repositorytree.cmd", vec!["custom-cmd".to_string()]);
        snapshot.insert("merge.tool", vec!["repositorytree".to_string()]);
        snapshot.insert("mergetool.prompt", vec!["false".to_string()]);

        let plan = plan_uninstall(&entries, &snapshot);

        assert_eq!(
            decision_for(&plan, "mergetool.repositorytree.cmd"),
            &UninstallDecision::Unset
        );
    ...
    #[test]
    fn uninstall_plan_preserves_non_repositorytree_generic_settings() {
        let entries = build_uninstall_entries();
        let mut snapshot: FxHashMap<&'static str, Vec<String>> = FxHashMap::default();
        snapshot.insert("mergetool.repositorytree.cmd", vec!["custom-cmd".to_string()]);
    ...
    fn git_config_helpers_cover_missing_and_error_paths() {
        ...
        let missing = read_git_config_values(&scope, "repositorytree.coverage.missing").unwrap();
        assert!(missing.is_empty());

        let err = read_git_config_values("--not-a-valid-scope", "repositorytree.coverage")
    ...
    fn maybe_capture_backup_skips_when_current_value_matches_setup_default() {
        let (_dir, scope, _) = temp_file_scope();
        set_single_config_value(&scope, "merge.tool", "repositorytree").unwrap();
