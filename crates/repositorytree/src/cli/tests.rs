...
fn install_linux_script_does_not_use_invalid_debug_flag() {
    ...
    assert!(
        !script.contains("cargo build -p repositorytree --${mode}"),
...
fn clap_parses_difftool_subcommand() {
    let cli = Cli::try_parse_from([
        "repositorytree", "difftool", "--local", "/tmp/a", "--remote", "/tmp/b", "--path", "foo.txt",
    ])
    .unwrap();
...
fn clap_parses_setup_subcommand() {
    let cli = Cli::try_parse_from(["repositorytree", "setup", "--dry-run", "--local"]).unwrap();
...
fn clap_parses_uninstall_subcommand() {
    let cli = Cli::try_parse_from(["repositorytree", "uninstall", "--dry-run", "--local"]).unwrap();
...
fn clap_parses_no_subcommand_as_browser() {
    let cli = Cli::try_parse_from(["repositorytree"]).unwrap();
...
fn clap_parses_path_argument() {
    let cli = Cli::try_parse_from(["repositorytree", "/some/repo"]).unwrap();
    assert!(cli.command.is_none());
    assert_eq!(
        cli.path.as_deref(),
