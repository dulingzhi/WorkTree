...
fn compat_log_arg_text(arg: &std::ffi::OsStr) -> String {
    ...
    #[cfg(unix)]
    {
        ...
        format!("repositorytree-argv-bytes:{}", hex_encode(arg.as_bytes()))
    }
    ...
    #[cfg(windows)]
    {
        ...
        format!("repositorytree-argv-utf16le:{}", hex_encode(&bytes))
    }
...
fn compat_argv_log_dir() -> PathBuf {
    std::env::temp_dir().join("repositorytree-compat-argv")
}

fn resolve_compat_argv_log_path(env: &dyn EnvLookup) -> Option<PathBuf> {
    let requested = PathBuf::from(env.var_os("REPOSITORYTREE_COMPAT_ARGV_LOG")?);
