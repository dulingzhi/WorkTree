...
#[cfg(windows)]
const PENDING_REPORT_PATH_WIDE_PREFIX: &str = "repositorytree-crashlog-utf16le:";
...
pub fn install() {
    ...
    if let Err(err) = install_terminal_interrupt_handler() {
        eprintln!("Failed to install RepositoryTree terminal interrupt handler: {err}");
    }

    #[cfg(windows)]
    if !install_terminal_interrupt_handler() {
        eprintln!("Failed to install RepositoryTree terminal interrupt handler");
    }
...
fn should_record_runtime_error(target: &str, message: &str) -> bool {
    ...
    // that RepositoryTree itself failed and can otherwise obscure a later exit's
...
fn install_terminal_interrupt_handler() -> std::io::Result<()> {
    ...
    std::thread::Builder::new()
        .name("repositorytree-sigint".to_string())
        .spawn(move || {
...
fn install_terminal_interrupt_handler() -> bool {
    repositorytree_win32_window_utils::install_console_ctrl_handler(handle_console_ctrl_event)
}
...
fn handle_console_ctrl_event(event: u32) -> bool {
    ...
    let _ = std::thread::Builder::new()
        .name("repositorytree-ctrl-c-watchdog".to_string())
        .spawn(|| {
            std::thread::sleep(CONSOLE_CTRL_CLEANUP_GRACE);
            repositorytree_win32_window_utils::terminate_current_process(
                repositorytree_win32_window_utils::CONTROL_C_EXIT_CODE,
            );
        });

    retire_active_session();
    // Declining the event here would hand termination to the console's default
    // handler, which calls `ExitProcess`. That deadlocks in a running RepositoryTree
...
fn console_ctrl_event_ends_process(event: u32) -> bool {
    use repositorytree_win32_window_utils::{
...
fn write_runtime_error_log_in_dir(
    ...
    writeln!(file, "=== RepositoryTree runtime error ===")?;
...
fn begin_session_in_dir(dir: &Path) -> std::io::Result<()> {
    ...
    writeln!(file, "=== RepositoryTree abnormal exit candidate ===")?;
    ...
    writeln!(file, "message=RepositoryTree did not exit cleanly")?;
    writeln!(
        file,
...
fn record_session_failure_in_dir_with_diagnostics(
    ...
    writeln!(
        file,
        "info=RepositoryTree could not complete its GPUI launch or event loop."
    )?;
...
fn take_startup_report_from_crash_dir_with_process_check(
    ...
    let mut report_log = match std::fs::read_to_string(&report_path) {
        ...
        Err(err) => {
            eprintln!(
                "Failed to read pending RepositoryTree crash report {}: {err}",
                report_path.display()
            );
            return None;
        }
    ...
    let (panic_log, pending_panic_read_failed) = match read_pending_panic_log(&pending_path) {
        ...
        Err(err) => {
            eprintln!(
                "Failed to read RepositoryTree pending panic report {}: {err}",
    ...
    let report_path = match write_startup_report_snapshot(dir, &report_log) {
        ...
        Err(err) => {
            eprintln!("Failed to persist RepositoryTree startup crash report: {err}");
            return None;
        }
    };
    for session_log in &session_logs {
    ...
    if let Some(panic_log) = &panic_log {
        ...
        if let Err(err) = remove_file_if_exists(&panic_log.path) {
            eprintln!(
                "Failed to clear snapshotted RepositoryTree panic log {}: {err}",
...
fn stale_abnormal_exit_logs(
    ...
    let entries = match std::fs::read_dir(dir) {
        ...
        Err(err) => {
            eprintln!(
                "Failed to enumerate RepositoryTree session markers in {}: {err}",
                dir.display()
            );
            return Vec::new();
        }
...
fn read_abnormal_exit_log(
    ...
    let mut session_log = match std::fs::read_to_string(marker) {
        ...
        Err(err) => {
            eprintln!(
                "Failed to read RepositoryTree session marker {}: {err}",
                marker.display()
            );
            return None;
        }
    ...
    match std::fs::read_to_string(last_operation_path) {
        Ok(last_operation) => {
            let _ = writeln!(session_log, "\n=== RepositoryTree operation context ===");
            let _ = write!(session_log, "{last_operation}");
        }
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {}
        Err(err) => {
            eprintln!(
                "Failed to read RepositoryTree last-operation diagnostics {}: {err}",
    ...
    match std::fs::read_to_string(runtime_error_path) {
        ...
        Err(err) => {
            eprintln!(
                "Failed to read RepositoryTree runtime-error diagnostics {}: {err}",
...
fn write_panic_log(info: &std::panic::PanicHookInfo<'_>) {
    ...
    let _ = writeln!(file, "=== RepositoryTree crash (panic) ===");
...
fn crash_dir() -> Option<PathBuf> {
    crash_dir_base().map(|base| base.join("repositorytree").join("crashes"))
}

fn crash_directory_unavailable_error() -> std::io::Error {
    std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "RepositoryTree crash state directory is unavailable because XDG_STATE_HOME and HOME are unset",
    )
}
...
fn parse_crash_log(crash_log: &str) -> ParsedCrashLog {
    ...
    for raw_line in crash_log.lines() {
        ...
        if line == "=== RepositoryTree operation context ===" {
            in_backtrace = false;
            continue;
        }

        if line.starts_with("=== RepositoryTree ") && line.ends_with(" ===") {
...
fn build_issue_body(parsed: &ParsedCrashLog, crash_log_path: &Path) -> String {
    ...
    let _ = writeln!(body, "RepositoryTree ended unexpectedly.");
    let _ = writeln!(body);

    let _ = writeln!(body, "## Environment");
    let _ = writeln!(body);
    let _ = writeln!(body, "- RepositoryTree crate: `{crate_name}`");
    let _ = writeln!(body, "- RepositoryTree version: `{crate_version}`");
...
mod tests {
    ...
    fn parse_crash_log_extracts_fields() {
        let log = r#"=== RepositoryTree crash (panic) ===
timestamp_unix_ms=123
crate=repositorytree version=0.1.0
thread=main
...
"#;
        ...
        assert_eq!(parsed.crate_name.as_deref(), Some("repositorytree"));
...
"#;
    ...
    fn previous_session_report_is_snapshotted_before_marker_is_removed() {
        ...
        std::fs::write(
            &marker,
            "timestamp_unix_ms=123\ncrate=repositorytree version=0.1.0\n\
             thread=main\nmessage=RepositoryTree did not exit cleanly\n\
    ...
    fn runtime_error_supplies_failure_location_message_and_backtrace() {
        ...
        std::fs::write(
            &marker,
            "=== RepositoryTree abnormal exit candidate ===\n\
...
"#;
    ...
    fn begin_session_starts_with_fresh_diagnostics() {
        ...
        assert!(contents.contains("=== RepositoryTree abnormal exit candidate ==="));
        assert!(
    ...
    fn panic_report_takes_precedence_over_previous_session_marker() {
        ...
        write_pending_report_path(&pending_report_path(dir.path()), &crash_log_path)
            .expect("write pending marker");
        let marker = session_marker_path(dir.path());
        std::fs::write(&marker, "message=RepositoryTree did not exit cleanly\n")
            .expect("write session marker");
        ...
        let persisted =
            std::fs::read_to_string(startup_report_path(dir.path())).expect("read startup report");
        assert!(
            persisted.contains("message=RepositoryTree did not exit cleanly"),
    ...
    fn startup_recovery_ignores_live_process_markers() {
        ...
        std::fs::write(&live_marker, "message=live RepositoryTree session\n").expect("write live marker");
        ...
        std::fs::write(&stale_marker, "message=stale RepositoryTree session\n")
        ...
        let persisted =
            std::fs::read_to_string(report.crash_log_path).expect("read startup report");
        assert!(persisted.contains("stale RepositoryTree session"));
        assert!(persisted.contains("copy_source=stale-instance"));
        assert!(!persisted.contains("live RepositoryTree session"));
    ...
    fn session_diagnostic_paths_include_the_owning_pid() {
        let dir = Path::new("/tmp/repositorytree-crash-path-test");
        assert_eq!(
    ...
    fn terminal_interrupt_clears_active_session_state() {
        const CHILD_PROCESS: &str = "REPOSITORYTREE_SIGINT_CLEANUP_TEST_CHILD";
        ...
        let dir = state_root.path().join("repositorytree").join("crashes");
    ...
    fn terminal_interrupt_ends_process_and_clears_active_session_state() {
        ...
        const CHILD_PROCESS: &str = "REPOSITORYTREE_CTRL_BREAK_CLEANUP_TEST_CHILD";
        ...
        if !repositorytree_win32_window_utils::send_console_ctrl_break(child_pid) {
        ...
        assert_eq!(
            status.code(),
            Some(repositorytree_win32_window_utils::CONTROL_C_EXIT_CODE as i32)
        );
        assert!(!marker.exists());
        assert!(!last_operation_path_for_pid(&dir, child_pid).exists());
        assert!(!runtime_error_path_for_pid(&dir, child_pid).exists());
