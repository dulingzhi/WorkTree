...
fn repositorytree_difftool_cmd(marker: &str, force_exit: Option<i32>) -> String {
...
fn configure_repositorytree_difftool(repo: &Path) {
    configure_difftool_command(repo, "repositorytree", &repositorytree_difftool_cmd("repositorytree", None));
    configure_difftool_trust_exit_code(repo, true);
    configure_difftool_selection(repo, "repositorytree", None, None);
}

fn configure_kdiff3_path_override_to_repositorytree(repo: &Path) {
    let bin = repositorytree_bin();
...
fn configure_meld_path_override_to_repositorytree(repo: &Path) {
    let bin = repositorytree_bin();
    let bin_path = bin.to_string_lossy().to_string();
...
#[test]
fn git_difftool_invokes_repositorytree_for_basic_diff() {
    ...
    configure_repositorytree_difftool(repo);
...
fn git_difftool_kdiff3_path_override_invokes_compat_mode() {
    ...
    configure_kdiff3_path_override_to_repositorytree(repo);
...
fn git_difftool_meld_path_override_invokes_compat_mode() {
    ...
    configure_meld_path_override_to_repositorytree(repo);
...
fn git_difftool_gui_default_auto_prefers_gui_tool_when_display_set() {
    ...
    configure_difftool_command(repo, "cli", &repositorytree_difftool_cmd("cli", None));
    configure_difftool_command(repo, "gui", &repositorytree_difftool_cmd("gui", None));
    configure_difftool_trust_exit_code(repo, true);
...
fn git_difftool_shows_submodule_gitlink_change() {
    ...
    // Verify that RepositoryTree surfaces both old and new commit pointers.
    let tmp = tempfile::tempdir().unwrap();
...
#[test]
fn git_difftool_tool_help_lists_repositorytree_tool() {
    if !require_git_shell_for_tool_tests() {
        return;
    }
    let tmp = tempfile::tempdir().unwrap();
