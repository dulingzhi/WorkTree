...
fn repositorytree_bin() -> PathBuf {
    for env_key in ["CARGO_BIN_EXE_repositorytree"] {
    ...
    if let Some(path) = repositorytree_bin_from_current_exe() {
        return path;
    }

    panic!(
        "repositorytree binary path was not found. Tried CARGO_BIN_EXE_repositorytree and a fallback relative to current test executable"
    );
}

fn repositorytree_bin_from_current_exe() -> Option<PathBuf> {
...
fn run_repositorytree<I, S>(args: I) -> Output
where
    I: IntoIterator<Item = S>,
    S: AsRef<OsStr>,
{
    no_window_command(repositorytree_bin())
        .args(args)
        .output()
        .expect("repositorytree command to run")
}

fn run_repositorytree_in_dir<I, S>(dir: &Path, args: I) -> Output
where
    I: IntoIterator<Item = S>,
    S: AsRef<OsStr>,
{
...
fn run_repositorytree_in_dir_with_global_env<I, S>(
    dir: &Path,
    args: I,
...
where
    ...
    S: AsRef<OsStr>,
{
    let mut command = no_window_command(repositorytree_bin());
...
fn setup_dry_run_prints_commands_without_writing() {
    let output = run_repositorytree([OsString::from("setup"), OsString::from("--dry-run")]);
    ...
    assert!(
        stdout.contains("mergetool.repositorytree.cmd"),
        "expected mergetool cmd\n{text}"
    );
    assert!(
        stdout.contains("difftool.repositorytree.cmd"),
    ...
    assert!(
        stdout.contains("mergetool.repositorytree.trustExitCode"),
        "expected mergetool.repositorytree.trustExitCode\n{text}"
    );
    assert!(
    ...
    // GUI tool entries
    assert!(
        stdout.contains("mergetool.repositorytree-gui.cmd"),
        "expected GUI mergetool cmd\n{text}"
    );
    assert!(
        stdout.contains("difftool.repositorytree-gui.cmd"),
        "expected GUI difftool cmd\n{text}"
    );
    assert!(
        stdout.contains("mergetool.repositorytree-gui.trustExitCode"),
        "expected GUI mergetool trustExitCode\n{text}"
    );
    assert!(
        stdout.contains("difftool.repositorytree-gui.trustExitCode"),
...
fn setup_dry_run_commands_execute_verbatim_in_shell() {
    ...
    let output = run_repositorytree_in_dir(
        dir.path(),
        [
            OsString::from("setup"),
            OsString::from("--dry-run"),
    ...
    let merge_cmd = git_config_get(dir.path(), "mergetool.repositorytree.cmd")
    ...
    let diff_cmd = git_config_get(dir.path(), "difftool.repositorytree.cmd")
    ...
    let gui_merge_cmd = git_config_get(dir.path(), "mergetool.repositorytree-gui.cmd")
    ...
    let gui_diff_cmd = git_config_get(dir.path(), "difftool.repositorytree-gui.cmd")
    ...
    assert_eq!(
        ...
        Some("repositorytree-gui"),
        "merge.guitool should reference repositorytree-gui"
    );
    assert_eq!(
        git_config_get(dir.path(), "diff.guitool").as_deref(),
        Some("repositorytree-gui"),
        "diff.guitool should reference repositorytree-gui"
    );
}
...
fn setup_local_writes_config_to_repo() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Configured repositorytree as local diff/merge tool"),
        "{text}"
    );
    ...
    assert_eq!(
        git_config_get(dir.path(), "mergetool.repositorytree.trustExitCode").as_deref(),
        Some("true"),
        "mergetool.repositorytree.trustExitCode not set"
    );
    assert_eq!(
    ...
    assert_eq!(
        git_config_get(dir.path(), "merge.guitool").as_deref(),
        Some("repositorytree-gui"),
        "merge.guitool not set to repositorytree-gui"
    );
    assert_eq!(
        git_config_get(dir.path(), "diff.guitool").as_deref(),
        Some("repositorytree-gui"),
        "diff.guitool not set to repositorytree-gui"
    );
    assert_eq!(
    ...
    let merge_cmd =
        git_config_get(dir.path(), "mergetool.repositorytree.cmd").expect("mergetool cmd should be set");
    assert_placeholder_is_quoted(&merge_cmd, "BASE");
    assert_placeholder_is_quoted(&merge_cmd, "LOCAL");
    assert_placeholder_is_quoted(&merge_cmd, "REMOTE");
    assert_placeholder_is_quoted(&merge_cmd, "MERGED");
    ...
    let diff_cmd =
        git_config_get(dir.path(), "difftool.repositorytree.cmd").expect("difftool cmd should be set");
    assert_placeholder_is_quoted(&diff_cmd, "LOCAL");
    assert_placeholder_is_quoted(&diff_cmd, "REMOTE");
    assert_placeholder_is_quoted(&diff_cmd, "MERGED");
    assert!(
    ...
    assert_placeholder_is_quoted(&gui_merge_cmd, "MERGED");
    assert_eq!(
        git_config_get(dir.path(), "mergetool.repositorytree-gui.trustExitCode").as_deref(),
        Some("true"),
    ...
    assert_placeholder_is_quoted(&gui_diff_cmd, "MERGED");
    assert_eq!(
        git_config_get(dir.path(), "difftool.repositorytree-gui.trustExitCode").as_deref(),
        Some("true"),
...
fn setup_local_mergetool_tool_help_lists_headless_and_gui_entries() {
    ...
    let setup = run_repositorytree_in_dir(repo, ["setup", "--local"]);
    ...
    assert!(
        text.contains("repositorytree.cmd"),
        "expected headless repositorytree tool in mergetool --tool-help output\n{text}"
    );
    assert!(
        text.contains("repositorytree-gui.cmd"),
        "expected gui repositorytree-gui tool in mergetool --tool-help output\n{text}"
    );
    assert!(
...
fn setup_local_difftool_tool_help_lists_headless_and_gui_entries() {
    ...
    assert!(
        text.contains("repositorytree.cmd"),
        "expected headless repositorytree tool in difftool --tool-help output\n{text}"
    );
    assert!(
        text.contains("repositorytree-gui.cmd"),
        "expected gui repositorytree-gui tool in difftool --tool-help output\n{text}"
    );
    assert!(
...
fn uninstall_dry_run_local_after_setup_lists_unset_commands() {
    ...
    let uninstall = run_repositorytree_in_dir(repo, ["uninstall", "--dry-run", "--local"]);
    ...
    assert!(
        stdout.contains("git config --local --unset-all mergetool.repositorytree.cmd"),
...
fn uninstall_local_removes_setup_keys_from_repo() {
    ...
    let uninstall = run_repositorytree_in_dir(repo, ["uninstall", "--local"]);
    let text = output_text(&uninstall);
    assert_eq!(uninstall.status.code(), Some(0), "expected exit 0\n{text}");
    assert!(
        String::from_utf8_lossy(&uninstall.stdout)
            .contains("Unconfigured repositorytree from local diff/merge tool"),
    ...
    assert_eq!(git_config_get_local(repo, "mergetool.repositorytree.cmd"), None);
    assert_eq!(git_config_get_local(repo, "difftool.repositorytree.cmd"), None);
    assert_eq!(
        git_config_get_local(repo, "mergetool.repositorytree-gui.cmd"),
        None
    );
    assert_eq!(
        git_config_get_local(repo, "difftool.repositorytree-gui.cmd"),
        None
    );

    // Generic behavior keys written by setup should be removed when selectors
    // still point to RepositoryTree.
...
#[test]
fn uninstall_local_keeps_non_repositorytree_generic_settings() {
    let dir = tempfile::tempdir().unwrap();
    let repo = dir.path();
    ...
    git_config_set_local(repo, "mergetool.repositorytree.cmd", "echo custom");

    let uninstall = run_repositorytree_in_dir(repo, ["uninstall", "--local"]);
    let text = output_text(&uninstall);
    assert_eq!(uninstall.status.code(), Some(0), "expected exit 0\n{text}");

    // Keep non-RepositoryTree generic behavior.
    assert_eq!(
    ...
    // Remove RepositoryTree tool-specific key regardless of command content.
    assert_eq!(git_config_get_local(repo, "mergetool.repositorytree.cmd"), None);
}
...
fn setup_then_uninstall_local_restores_preexisting_user_settings() {
    ...
    assert_eq!(
        git_config_get_local(repo, "repositorytree.backup.merge-tool"),
        None
    );
    assert_eq!(
        git_config_get_local(repo, "repositorytree.backup.diff-tool"),
        None
    );
}
...
fn setup_local_is_idempotent_and_preserves_original_backup_state() {
    ...
    let setup1 = run_repositorytree_in_dir(repo, ["setup", "--local"]);
    ...
    let setup2 = run_repositorytree_in_dir(repo, ["setup", "--local"]);
...
fn uninstall_local_preserves_user_changes_made_after_setup() {
    ...
    assert_eq!(
        git_config_get_local(repo, "repositorytree.backup.difftool-prompt"),
        None
    );
}
...
fn uninstall_local_is_idempotent() {
    ...
    let uninstall1 = run_repositorytree_in_dir(repo, ["uninstall", "--local"]);
    ...
    let uninstall2 = run_repositorytree_in_dir(repo, ["uninstall", "--local"]);
...
//
// These tests verify that `repositorytree setup --local` produces config that
...
/// After `repositorytree setup --local`, `git mergetool` should invoke
/// repositorytree's built-in 3-way merge for conflicted files.
///
/// For a true content conflict, repositorytree exits 1 and git mergetool
/// restores the original file (expected behavior with trustExitCode=true).
/// We verify the tool was invoked by checking for repositorytree's specific
...
fn setup_local_enables_git_mergetool_end_to_end() {
    ...
    setup_e2e_init(repo);
    let setup = run_repositorytree_in_dir(repo, [OsString::from("setup"), OsString::from("--local")]);
    ...
    // 3. Run `git mergetool` — should invoke repositorytree via setup config.
    ...
    // 4. Verify repositorytree was invoked by checking for its specific stderr
    //    messages.  "conflict(s) remain" is emitted by repositorytree's mergetool
    ...
    assert!(
        ...
        "expected repositorytree's conflict message in mergetool stderr\n\
         stdout: {}\nstderr: {}",
        String::from_utf8_lossy(&mt.stdout),
        mt_stderr,
    );

    // Also verify repositorytree's "CONFLICT (content)" message is present.
    assert!(
        mt_stderr.contains("CONFLICT (content)"),
        "expected CONFLICT marker from repositorytree\nstderr: {}",
        mt_stderr,
    );
}

/// After `repositorytree setup --local`, `git difftool` should invoke
/// repositorytree's built-in diff and produce unified diff output.
#[test]
fn setup_local_enables_git_difftool_end_to_end() {
    ...
    // 3. Run `git difftool` — should invoke repositorytree difftool.
    //    DISPLAY is removed so guiDefault=auto selects the headless tool.
    ...
    // repositorytree difftool produces unified diff output.
...
/// After `repositorytree setup --local`, quoted stage variables in the generated
...
fn setup_local_mergetool_handles_spaced_unicode_path_end_to_end() {
    ...
    assert!(
        mt_stderr.contains("CONFLICT (content)"),
        "expected repositorytree conflict output\n{mt_text}"
    );
    assert!(
...
/// `repositorytree setup` (global scope) should configure an isolated global
...
fn setup_global_enables_git_mergetool_end_to_end_with_isolated_global_config() {
    ...
    let setup = run_repositorytree_in_dir_with_global_env(&repo, [OsString::from("setup")], &env);
    let setup_text = output_text(&setup);
    assert_eq!(setup.status.code(), Some(0), "setup failed\n{setup_text}");
    assert!(
        String::from_utf8_lossy(&setup.stdout).contains("Configured repositorytree as global"),
    ...
    assert!(
        mt_stderr.contains("conflict(s) remain"),
        "expected repositorytree conflict message in mergetool stderr\n{}",
        output_text(&mt)
    );
    assert!(
        mt_stderr.contains("CONFLICT (content)"),
...
/// `repositorytree setup` (global scope) should make both headless and GUI
...
fn help_flag_exits_zero() {
    let out = run_repositorytree(["--help"]);
    assert_eq!(
        out.status.code(),
        Some(0),
    ...
    assert!(
        stdout.contains("repositorytree"),
...
fn version_flag_exits_zero() {
    let out = run_repositorytree(["--version"]);
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn subcommand_help_exits_zero() {
    for subcmd in ["difftool", "mergetool", "setup", "uninstall"] {
        let out = run_repositorytree([subcmd, "--help"]);
