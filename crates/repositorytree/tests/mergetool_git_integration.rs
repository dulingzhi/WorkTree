...
fn configure_repositorytree_mergetool(repo: &Path) {
    let bin = repositorytree_bin();
    let bin_q = shell_quote(&bin.to_string_lossy());
    let cmd = format!(
        "{bin_q} mergetool --base \"$BASE\" --local \"$LOCAL\" --remote \"$REMOTE\" --merged \"$MERGED\""
    );

    run_git(repo, &["config", "merge.tool", "repositorytree"]);
    run_git(repo, &["config", "mergetool.repositorytree.cmd", &cmd]);
    run_git(
        repo,
        &["config", "mergetool.repositorytree.trustExitCode", "true"],
    );
...
fn configure_repositorytree_mergetool_with_alias_flags(repo: &Path) {
    let bin = repositorytree_bin();
    let bin_q = shell_quote(&bin.to_string_lossy());
    let cmd = format!(
...
fn configure_kdiff3_path_override_to_repositorytree(repo: &Path, trust_exit_code: bool) {
    let bin = repositorytree_bin();
...
fn configure_meld_path_override_to_repositorytree(repo: &Path, trust_exit_code: bool) {
    let bin = repositorytree_bin();
    let bin_path = bin.to_string_lossy().to_string();
...
/// running mergetool, but can be auto-resolved by repositorytree with `--auto`.
...
fn git_mergetool_resolves_overlapping_conflict() {
    ...
    configure_repositorytree_mergetool(repo);
...
fn git_mergetool_accepts_kdiff3_alias_flags_in_cmd() {
    ...
    setup_overlapping_conflict(repo);
    configure_repositorytree_mergetool_with_alias_flags(repo);
    ...
    assert!(
        ...
        "expected alias flags to be accepted by repositorytree mergetool\noutput:\n{text}"
    );
    assert!(
        text.contains("Auto-merging file.txt"),
        "expected repositorytree mergetool to run\noutput:\n{text}"
    );
    assert!(
...
fn git_mergetool_kdiff3_path_override_invokes_compat_mode() {
    ...
    configure_kdiff3_path_override_to_repositorytree(repo, true);

    let output = run_git_capture(repo, &["mergetool", "--no-prompt"]);
    let text = output_text(&output);
    assert!(
        output.status.success(),
        "expected kdiff3 path-override invocation to succeed with repositorytree compatibility parsing\n{text}"
    );
    assert!(
...
fn git_mergetool_kdiff3_path_override_records_real_argv_shape() {
    ...
    let argv_log = std::env::temp_dir()
        .join("repositorytree-compat-argv")
        .join(&argv_log_name);
    let _ = fs::remove_file(&argv_log);
    let output = run_git_capture_with_env(
        repo,
        &["mergetool", "--no-prompt"],
        &[("REPOSITORYTREE_COMPAT_ARGV_LOG", argv_log_name.as_str())],
    );
    let text = output_text(&output);
...
fn git_mergetool_meld_path_override_invokes_compat_mode() {
    ...
    setup_whitespace_only_conflict(repo);
    configure_meld_path_override_to_repositorytree(repo, true);

    let output = run_git_capture(repo, &["mergetool", "--no-prompt"]);
    let text = output_text(&output);
    assert!(
        output.status.success(),
        "expected meld path-override invocation to succeed with repositorytree compatibility parsing\n{text}"
    );
    assert!(
...
fn git_mergetool_trust_exit_code_conflict_preserves_unmerged_state() {
    ...
    // contract between repositorytree and git mergetool.
    let tmp = tempfile::tempdir().unwrap();
    let repo = tmp.path();
...
#[test]
fn git_mergetool_tool_help_lists_repositorytree_tool() {
    if !require_git_shell_for_tool_tests() {
        return;
    }
    let tmp = tempfile::tempdir().unwrap();
    ...
    assert!(
        text.contains("repositorytree"),
        "expected repositorytree tool name in --tool-help output\n{text}"
    );
}
...
fn git_mergetool_delete_delete_conflict_handling() {
    ...
    configure_mergetool_command(repo, "repositorytree", &mergetool_marker_cmd("repositorytree"));
    configure_mergetool_trust_exit_code(repo, "repositorytree", true);
    configure_mergetool_selection(repo, "repositorytree", None, None);
...
//
// Conflictstyle assertions only need the MERGED content written by repositorytree.
// Run repositorytree and then force a zero exit to avoid Git-version-dependent
// behavior around trustExitCode=false with non-zero tool exits.
fn configure_repositorytree_mergetool_preserve_output(repo: &Path) {
    let bin = repositorytree_bin();
    let bin_q = shell_quote(&bin.to_string_lossy());
    let cmd = format!(
...
fn git_mergetool_respects_merge_conflictstyle_zdiff3_from_git_config() {
    ...
    configure_repositorytree_mergetool_preserve_output(&repo);
...
#[test]
fn repositorytree_mergetool_reads_conflictstyle_from_repo_when_cwd_is_outside_repo() {
    let tmp = tempfile::tempdir().unwrap();
    let repo = tmp.path().to_path_buf();
    init_repo(&repo);
    run_git(&repo, &["config", "merge.conflictstyle", "diff3"]);
    ...
    let mut cmd = no_window_command(repositorytree_bin());
    apply_isolated_git_config_env(&mut cmd);
    let output = cmd
        ...
        .arg(&merged)
        .output()
        .expect("repositorytree mergetool command to run");
