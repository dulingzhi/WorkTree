...
Prints extra Cargo CLI args for WorkTree macOS builds, one argument per line.

Environment:
  WORKTREE_MACOS_X86_RELEASE_LTO
...
lto_mode="${WORKTREE_MACOS_X86_RELEASE_LTO:-thin}"
...
case "$lto_mode" in
  ...
  false|off)
    ...
    ;;
  *)
    echo "Unsupported WORKTREE_MACOS_X86_RELEASE_LTO value: $lto_mode" >&2
    exit 2
    ;;
esac
