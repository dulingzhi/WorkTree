...
usage() {
  cat <<'EOF'
Usage: scripts/profile-worktree-process-tree.sh [options] REPO_PATH [-- extra_worktree_args...]

Launch WorkTree under a process-tree profiling wrapper that captures:
...
Options:
  --binary PATH             WorkTree binary to launch.
                            Default: ./target/release-with-debug/worktree
  --out-dir PATH            Output directory for captured artifacts.
                            Default: tmp/worktree-profiles/<utc-timestamp>
...
Examples:
  scripts/profile-worktree-process-tree.sh /home/sampo/chromium/src
  scripts/profile-worktree-process-tree.sh --timeout 60 /home/sampo/chromium/src
  scripts/profile-worktree-process-tree.sh \
...
write_summary() {
  {
    printf 'WorkTree process-tree profile bundle\n'
    printf 'Repository: %s\n' "$repo_path"
    printf 'Binary: %s\n' "$binary_path"
    printf 'Output: %s\n' "$out_dir"
    printf 'Root PID: %s\n' "$root_pid"
...
binary_path="${repo_root}/target/release-with-debug/worktree"
if [[ -n "$binary_raw" ]]; then
  binary_path="$(resolve_path "$binary_raw")"
fi

timestamp="$(date -u +%Y%m%d-%H%M%SZ)"
out_dir="${repo_root}/tmp/worktree-profiles/${timestamp}"
...
if ((${#extra_worktree_args[@]} > 0)); then
  profile_command+=("${extra_worktree_args[@]}")
fi
...
if [[ ! -x "$binary_path" ]]; then
  die "WorkTree binary not found or not executable at ${binary_path}. Build it with: bash scripts/build_release_debug.sh"
fi
verify_strace_usable
