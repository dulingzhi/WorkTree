...
usage() {
  cat <<'EOF'
Usage: scripts/profile-repositorytree-process-tree.sh [options] REPO_PATH [-- extra_repositorytree_args...]

Launch RepositoryTree under a process-tree profiling wrapper that captures:
...
Options:
  --binary PATH             RepositoryTree binary to launch.
                            Default: ./target/release-with-debug/repositorytree
  --out-dir PATH            Output directory for captured artifacts.
                            Default: tmp/repositorytree-profiles/<utc-timestamp>
...
Examples:
  scripts/profile-repositorytree-process-tree.sh /home/sampo/chromium/src
  scripts/profile-repositorytree-process-tree.sh --timeout 60 /home/sampo/chromium/src
  scripts/profile-repositorytree-process-tree.sh \
...
write_summary() {
  {
    printf 'RepositoryTree process-tree profile bundle\n'
    printf 'Repository: %s\n' "$repo_path"
    printf 'Binary: %s\n' "$binary_path"
    printf 'Output: %s\n' "$out_dir"
    printf 'Root PID: %s\n' "$root_pid"
...
binary_path="${repo_root}/target/release-with-debug/repositorytree"
if [[ -n "$binary_raw" ]]; then
  binary_path="$(resolve_path "$binary_raw")"
fi

timestamp="$(date -u +%Y%m%d-%H%M%SZ)"
out_dir="${repo_root}/tmp/repositorytree-profiles/${timestamp}"
...
if ((${#extra_repositorytree_args[@]} > 0)); then
  profile_command+=("${extra_repositorytree_args[@]}")
fi
...
if [[ ! -x "$binary_path" ]]; then
  die "RepositoryTree binary not found or not executable at ${binary_path}. Build it with: bash scripts/build_release_debug.sh"
fi
verify_strace_usable
