...
WorkTree's performance is measured against a **fixed, version-pinned real-world
target** so the numbers below are reproducible and comparable release to release —
not a moving synthetic fixture.

**Fixed target (iteration 06):** [rust-lang/rust](https://github.com/rust-lang/rust)
@ `main`, pinned at commit `a8a1e6fd9df2e094d6f09c0d57991508680acc1c` —
**340,056 commits**, 163 tags, **1.4 GB** on disk. The snapshot and its metrics are
generated by `scripts/generate-perf-target-manifest.sh` and recorded in
`benches/performance/real_repo_target.json`; re-run that script after a release to
refresh the target.

| Scenario | Measures | Measured (mean) |
| --- | --- | --- |
| `monorepo_open_and_history_load` | Open the repo + load the first 10k history pages | 8.87 s |
| `deep_history_open_and_scroll` | Open + scroll 50k commits of history | 532 ms |
| `mid_merge_conflict_list_and_open` | List + open conflicts of a mid-merge | 15.30 s |
| `large_file_diff_open` | Open a large single-file diff (`Cargo.lock`) | 587 ms |

The numbers come from the `real_repo` Criterion benchmark
(`cargo bench -p worktree-ui-gpui --features benchmarks --bench performance real_repo`)
run against the pinned snapshot. See
`docs/superpowers/plans/2026-09-14-iteration06-performance.md` for the methodology,
the budget framework, and the four-leg verification baseline.
