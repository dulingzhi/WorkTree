...
2. **增量 status** `M/L` ✅（2026-08-28 A+B 两切片）
   ...
   Slice A（`StatusForPaths` 管线）：core 新类型 `StatusForPaths::{Lists{unstaged,staged}, NeedsFullScan}` + trait `status_for_paths(&[PathBuf])`；gix = 单次 `git status --porcelain=v2 -z --untracked-files=all --ignore-submodules=none -- <paths>`（`--no-optional-locks`），完整 v2 解析（`1` 双列分道、`u` 冲突带 FileConflictKind 入 unstaged、`?` 未跟踪；**`2` rename/copy 记录 → NeedsFullScan**——增量合并不复刻配对，全量是诚实答案）；state = `Effect::LoadStatusForPaths` → `schedule_load_status_for_paths`（repo_load 同款 spawn/取消/缺仓回退）→ `InternalMsg::StatusForPathsLoaded` → `patch_status_for_paths` 合并（只并在 Ready 快照上：覆盖路径两道整体替换、未覆盖路径原样保留、排序镜像 gix 的 path-then-kind-priority——合并结果与全量逐字节同形；内容不变不 bump rev；NotLoaded/Error/NeedsFullScan/Err 一律回落 `LoadStatus`，全量不会再路由回来故无重试环）。测试：gix +3（与全量在覆盖路径逐项等价 + 目录 pathspec 递归 + 未触碰路径不出现 / `2` 记录 NeedsFullScan / `u` 记录冲突种类入 unstaged 且 staged 清空）、state +2（补丁替换/追加/去抖 rev、不可合并三态回落全量）；基线 core 529 / state 765。Slice B（激活，同日）：`Msg::RepoExternallyChanged` 增 `worktree_paths: Option<Arc<[PathBuf]>>`（monitor flush 两个闭包经 `relativize_paths` 剥 workdir 前缀 + 排序去重后携带；激活全刷/reducer 其余派发点皆 None）；分类器 `ClassifiedEvent` 增 `worktree_paths`（收集非 git、非 ignored 的事件路径——纯 worktree 事件恒非空，.git/need-rescan/gitignore 变更类粗事件恒空）；合并器 `DebouncedChange` 增路径累积（**首个事件初始化、任一空侧即整体作废、超 `INCREMENTAL_STATUS_PATH_CAP=64` 作废**；作废后同窗再累积仅是 advisory——flags 已带粗车道，消费者路由不采纳）；消费者 `repo_externally_changed`：纯 worktree + 路径非空 + `status Ready` → `Effect::LoadStatusForPaths`（占同一 `WORKTREE_STATUS` in-flight 位），否则原样 `LoadWorktreeStatus`；落地补 `finish_status_lane_replay`（飞行中折叠的新变更按**粗粒度**重放——折叠批次的路径此处不可知）。测试：monitor +2（分类收集/粗事件清空、累积-去重/粗事件作废/超 cap 作废）、state +1（带路径 → 定向效果且不叠粗扫；无快照 → 粗扫）；基线 state 768。顺延：`.git/index` 事件也可走 index-lane 定向（现 index 一律全量双道）；watcher 事件 pathspec 对大小写不敏感文件系统（macOS 默认）的路径形态对齐。
3. **建 PR/MR 轻闭环（GitHub + GitLab 同等）** `S` ✅（2026-08-28，双 forge）
...
4. **桌面小件包** `S/M`（合计）✅（2026-08-28 五件全齐）
